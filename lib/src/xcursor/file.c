...
unsigned long
_XcursorReturnLong(unsigned long code)
{
...
unsigned
_XcursorReturnUint(unsigned code)
{
...
XcursorImageCreate(int width, int height)
{
    ...
    else if (width > XCURSOR_IMAGE_MAX_SIZE || height > XCURSOR_IMAGE_MAX_SIZE)
    {
        /* EMPTY */;
    }
    else
...
XcursorImagesCreate(int size)
{
    ...
    images =
        malloc(sizeof(XcursorImages) + (size_t)size * sizeof(XcursorImage *));
    if (images)
    {
        images->nimage = 0;
...
{
    if (!file || !bytes ||
        (*file->read)(file, (unsigned char *)bytes, length) != length)
        return XcursorFalse;
    return XcursorTrue;
}
...
{
    if (!file || !bytes ||
        (*file->write)(file, (unsigned char *)bytes, length) != length)
        return XcursorFalse;
    return XcursorTrue;
}
...
_XcursorFileHeaderCreate(XcursorUInt ntoc)
{
    ...
    fileHeader =
        malloc(sizeof(XcursorFileHeader) + ntoc * sizeof(XcursorFileToc));
...
_XcursorReadFileHeader(XcursorFile *file)
{
    ...
    if (head.header < XCURSOR_FILE_HEADER_LEN) return NULL;
                     ...
                     int               *nsizesp)
{
    ...
    if (!fileHeader || !nsizesp) returnUint(0);
    ...
    for (n = 0; n < fileHeader->ntoc; n++)
    {
        ...
        else if (thisSize == bestSize) nsizes++;
    }
    *nsizesp = nsizes;
    returnUint(bestSize);
}
...
_XcursorFindImageToc(XcursorFileHeader *fileHeader, XcursorDim size, int count)
{
    ...
    if (toc == fileHeader->ntoc) returnCode(-1);
    returnCode((int)toc);
}
...
_XcursorReadImage(XcursorFile *file, XcursorFileHeader *fileHeader, int toc)
{
    ...
    if (!file || !fileHeader) returnAddr(NULL);

    if (!_XcursorFileReadChunkHeader(file, fileHeader, toc, &chunkHeader))
        returnAddr(NULL);
    if (!_XcursorReadUInt(file, &head.width)) returnAddr(NULL);
    if (!_XcursorReadUInt(file, &head.height)) returnAddr(NULL);
    if (!_XcursorReadUInt(file, &head.xhot)) returnAddr(NULL);
    if (!_XcursorReadUInt(file, &head.yhot)) returnAddr(NULL);
    if (!_XcursorReadUInt(file, &head.delay)) returnAddr(NULL);
    /* sanity check data */
    if (head.width > XCURSOR_IMAGE_MAX_SIZE ||
        head.height > XCURSOR_IMAGE_MAX_SIZE)
        returnAddr(NULL);
    if (head.width == 0 || head.height == 0) returnAddr(NULL);
    if (head.xhot > head.width || head.yhot > head.height) returnAddr(NULL);

    /* Create the image and initialize it */
    image = XcursorImageCreate((int)head.width, (int)head.height);
    if (image == NULL) returnAddr(NULL);
                   ...
                   XcursorImage      *image)
{
    ...
    if (!file || !fileHeader || !image) returnCode(XcursorFalse);

    /* sanity check data */
    if (image->width > XCURSOR_IMAGE_MAX_SIZE ||
        image->height > XCURSOR_IMAGE_MAX_SIZE)
        returnCode(XcursorFalse);
    if (image->width == 0 || image->height == 0) returnCode(XcursorFalse);
    ...
    if (!_XcursorWriteUInt(file, image->width)) returnCode(XcursorFalse);
    if (!_XcursorWriteUInt(file, image->height)) returnCode(XcursorFalse);
    if (!_XcursorWriteUInt(file, image->xhot)) returnCode(XcursorFalse);
    if (!_XcursorWriteUInt(file, image->yhot)) returnCode(XcursorFalse);
    if (!_XcursorWriteUInt(file, image->delay)) returnCode(XcursorFalse);
    ...
    while (n--)
    {
        if (!_XcursorWriteUInt(file, *p)) returnCode(XcursorFalse);
        p++;
    }
...
_XcursorReadComment(XcursorFile *file, XcursorFileHeader *fileHeader, int toc)
{
    ...
    if (!_XcursorReadUInt(file, &length)) returnAddr(NULL);
    comment = XcursorCommentCreate(chunkHeader.subtype, (int)length);
    if (!comment) returnAddr(NULL);
...
_XcursorXcFileLoadImage(XcursorFile *file, int size, XcursorBool resize)
{
    ...
    if (size < 0) returnAddr(NULL);
    fileHeader = _XcursorReadFileHeader(file);
    if (!fileHeader) returnAddr(NULL);
    bestSize = _XcursorFindBestSize(fileHeader, (XcursorDim)size, &nsize);
    if (!bestSize) returnAddr(NULL);
    toc = _XcursorFindImageToc(fileHeader, bestSize, 0);
    if (toc < 0) returnAddr(NULL);
...
{
    enterFunc(
        (T_CALLED(XcursorXcFileLoadImage) "(%p, %d)\n", (void *)file, size));
...
_XcursorXcFileLoadImages(XcursorFile *file, int size, XcursorBool resize)
{
    ...
    if (!file || size < 0) returnAddr(NULL);
    fileHeader = _XcursorReadFileHeader(file);
    if (!fileHeader) returnAddr(NULL);
    bestSize = _XcursorFindBestSize(fileHeader, (XcursorDim)size, &nsize);
    if (!bestSize)
...
{
    enterFunc(
        (T_CALLED(XcursorXcFileLoadImages) "(%p, %d)\n", (void *)file, size));
                  ...
                  const XcursorImages   *images)
{
    ...
    if (!file || !comments || !images) returnCode(XcursorFalse);
    ...
    if (!fileHeader) returnCode(XcursorFalse);
...
XcursorFileLoadImage(FILE *file, int size)
{
    ...
    enterFunc(
        (T_CALLED(XcursorFileLoadImage) "(%p, %d)\n", (void *)file, size));
...
XcursorFileLoadImages(FILE *file, int size)
{
    ...
    enterFunc(
        (T_CALLED(XcursorFileLoadImages) "(%p, %d)\n", (void *)file, size));
                ...
                XcursorImages   **imagesp)
{
    ...
    if (!file || !commentsp || !imagesp) returnCode(XcursorFalse);
...
XcursorFileSaveImages(FILE *file, const XcursorImages *images)
{
    ...
    if (!file || !images) returnCode(0);
    if ((comments = XcursorCommentsCreate(0)) == NULL) returnCode(0);
    _XcursorStdioFileInitialize(file, &f);
...
XcursorFilenameLoadAllImages(const char *file)
{
    ...
    enterFunc(
        (T_CALLED(XcursorFilenameLoadAllImages) "(\"%s\")\n", NonNull(file)));
                    ...
                    XcursorImages   **imagesp)
{
    ...
    if (!file) returnCode(XcursorFalse);
...
XcursorFilenameSaveImages(const char *file, const XcursorImages *images)
{
    ...
    if (!file || !images) returnCode(XcursorFalse);
