...
#ifndef XCURSORPATH
#  define XCURSORPATH                                              \
      "~/.local/share/icons:~/.icons:/usr/share/icons:/usr/share/" \
      "pixmaps:" ICONDIR
#endif
...
_XcursorBuildThemeDir(const char *dir, const char *theme)
{
    ...
    if (!dir || !theme) return NULL;

    colon = strchr(dir, ':');
    if (!colon) colon = dir + strlen(dir);

    dirlen = (int)(colon - dir);

    tcolon = strchr(theme, ':');
    if (!tcolon) tcolon = theme + strlen(theme);
    ...
    if (home) _XcursorAddPathElt(full, home, -1);
...
_XcursorBuildFullname(const char *dir, const char *subdir, const char *file)
{
    ...
    if (!dir || !subdir || !file) return NULL;
...
XcursorScanTheme(const char *theme, const char *name)
{
	 ...
	 * Scan this theme
	 */
        for (path = XcursorLibraryPath(); path && f == NULL;
        ...
        while (d > 0 && inherits[d].theme == NULL)
        {
            ...
            if (--d == 0) inherits[d].theme = NULL;
            else inherits[d].theme = _XcursorNextPath(inherits[d].theme);
        }
...
XcursorLibraryLoadImage(const char *file, const char *theme, int size)
{
    ...
    if (theme) f = XcursorScanTheme(theme, file);
    if (!f) f = XcursorScanTheme("default", file);
...
_XcursorLibraryLoadImages(Display *dpy, const char *file)
{
    ...
    if (f != NULL && f != XCURSOR_SCAN_CORE)
    {
        ...
        if (images) XcursorImagesSetName(images, file);
        fclose(f);
    }
    return images;
}
...
XcursorLibraryLoadCursor(Display *dpy, const char *file)
{
    ...
    if (!images)
    {
        ...
        if (id >= 0) cursor = _XcursorCreateFontCursor(dpy, (unsigned)id);
    }
    else
    {
...
XcursorLibraryLoadCursors(Display *dpy, const char *file)
{
    ...
    if (!images)
    {
        ...
        if (id >= 0)
        {
            ...
            {
                cursors->cursors[0] =
                    _XcursorCreateFontCursor(dpy, (unsigned)id);
                if (cursors->cursors[0] == None)
                {
                    XcursorCursorsDestroy(cursors);
                    cursors = NULL;
                }
                else cursors->ncursor = 1;
            }
        }
        else cursors = NULL;
    }
...
static const char _XcursorStandardNames[] = "X_cursor\0"
                                            "arrow\0"
                                            "based_arrow_down\0"
                                            "based_arrow_up\0"
                                            "boat\0"
...
static const unsigned short _XcursorStandardNameOffsets[] = {
    0,   9,   15,  32,  47,  52,  61,  80,  100, 112, 123, 134, 145,
    152, 158, 169, 175, 189, 199, 213, 217, 224, 237, 249, 261, 272,
    281, 287, 295, 301, 307, 313, 319, 324, 335, 344, 354, 363, 374,
    383, 392, 396, 409, 415, 422, 429, 434, 449, 459, 470, 480, 492,
    501, 510, 524, 542, 556, 571, 583, 601, 609, 616, 623, 632, 637,
    644, 651, 666, 682, 699, 708, 716, 721, 730, 739, 748, 754
};

#define NUM_STANDARD_NAMES \
    (sizeof _XcursorStandardNameOffsets / sizeof _XcursorStandardNameOffsets[0])
...
_XcursorShapeLoadImages(Display *dpy, unsigned int shape)
{
    ...
    enterFunc(
        (T_CALLED(_XcursorShapeLoadImages) "(%p, %u)\n", (void *)dpy, shape));
...
XcursorShapeLoadCursor(Display *dpy, unsigned int shape)
{
    ...
    enterFunc(
        (T_CALLED(XcursorShapeLoadCursor) "(%p, %u)\n", (void *)dpy, shape));
...
XcursorShapeLoadCursors(Display *dpy, unsigned int shape)
{
    ...
    enterFunc(
        (T_CALLED(XcursorShapeLoadCursors) "(%p, %u)\n", (void *)dpy, shape));
...
XcursorLibraryShape(const char *library)
{
    ...
    while (low < high - 1)
    {
        ...
        if (c == 0) returnCode(mid << 1);
        if (c > 0) low = mid;
        else high = mid;
    }
    while (low <= high)
    {
        if (!strcmp(library, STANDARD_NAME(low))) return (low << 1);
        low++;
    }
    returnCode(-1);
}
