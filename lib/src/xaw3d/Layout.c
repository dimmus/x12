...
#  define DBUG_ENTER(s)                                         \
      {                                                         \
          DBUG_lastproc    = DBUG_currentproc;                  \
          DBUG_currentproc = s;                                 \
...
#  define DBUG_VOID_RETURN                                                     \
      {                                                                        \
          ...
          DBUG_currentproc = DBUG_lastproc;                                    \
          return;                                                              \
      }
                ...
                XtWidgetGeometry *reply)
{
    ...
    if (changed) LayoutLayout(w, TRUE);
...
/* ARGSUSED */
static Boolean
SetValues(Widget    gold,
          Widget    greq,
          Widget    gnew,
          ArgList   args,
          Cardinal *num_args)
{
    LayoutWidget old = (LayoutWidget)gold, new = (LayoutWidget)gnew;

    if (old->layout.layout != new->layout.layout) LayoutLayout(new, TRUE);
    return FALSE;
} /* SetValues */

static XtGeometryResult
QueryGeometry(Widget            gw,
              XtWidgetGeometry *request,
              XtWidgetGeometry *prefered_return)
{
    ...
    else
    {
        if (request->request_mode & CWWidth)
        {
            ...
            {
                if (prefered_size.width == w->core.width) result = XtGeometryNo;
...
PrintBox(BoxPtr box, int level)
{
    ...
    switch (box->type)
    {
        case BoxBox:
            ...
            (void)printf("%s\n", "");
            for (child = box->u.box.firstChild; child;
...
LookupVariable(BoxPtr child, XrmQuark quark)
{
    ...
    {
        for (box = parent->u.box.firstChild; box != child;
             box = box->nextSibling)
        {
...
ComputeNaturalSizes(LayoutWidget l, BoxPtr box, LayoutDirection dir)
{
    ...
    switch (box->type)
    {
        case WidgetBox:
            ...
            {
                info = SubInfo(w);
                box->natural[LayoutHorizontal] =
                    info->naturalSize[LayoutHorizontal];
                box->natural[LayoutVertical] =
                    info->naturalSize[LayoutVertical];
            }
            DoStretch(l, box, dir);
            DoShrink(l, box, dir);
            DoStretch(l, box, !dir);
	     ...
	     */
                if (box->natural[!thisDir] >= child->natural[!thisDir])
                {
                    if (child->params.stretch[!thisDir].order < minShrinkOrder)
                    {
                        ...
                        if (child->params.shrink[!thisDir].order <
                            minShrinkOrder)
                            minShrinkOrder =
                                child->params.shrink[!thisDir].order;
                    }
                }
                else
                {
                    if (child->params.shrink[!thisDir].order <= minStretchOrder)
                    {
                        ...
                        if (child->params.stretch[!thisDir].order <
                            minStretchOrder)
                            minStretchOrder =
                                child->params.stretch[!thisDir].order;
                    }
                }
            ...
            if (box->params.shrink[!thisDir].order <= 0)
            {
                ...
                box->params.shrink[!thisDir].value =
                    box->natural[!thisDir] - largestMinSize;
...
#define GluePart(a, b, dist) \
    ((a) ? ((int)(((a) * (dist)) / (b) + ((dist >= 0) ? 0.5 : -0.5))) : 0)
    ...
    for (child = box->u.box.firstChild; child; child = child->nextSibling)
    {
        ...
        for (i = 0; i < maxGlue; i++)
        {
            if (glue->order == totalGlue[i].order)
            {
                ...
                if (remainingGlue <= 0) change = remainingChange;
                else
        ...
        if (child->type == BoxBox)
            if (!ComputeSizes(child)) happy = False;
    }
    ...
    switch (box->type)
    {
        ...
        case BoxBox:
                 ...
                 child = child->nextSibling)
            {
                ...
                else y += child->size[LayoutVertical];
            }
            break;
        case GlueBox:
        case VariableBox:
