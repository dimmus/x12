...
static void
XimFreeDefaultTree(DefTreeBase *b)
{
    if (!b) return;
...
char *
_XimLocalGetIMValues(XIM xim, XIMArg *values)
{
    Xim            im = (Xim)xim;
...
char *
_XimLocalSetIMValues(XIM xim, XIMArg *values)
{
    Xim            im = (Xim)xim;
    XimDefIMValues im_values;
    char          *name = (char *)NULL;
...
static Bool
_XimReadCachedDefaultTree(int           fd_cache,
                          const char   *name,
                          const char   *encoding,
                          DTStructIndex size)
{
    ...
    if (m == NULL || m == MAP_FAILED) return False;
...
static unsigned int
strToHash(const char *name)
{
    unsigned int hash = 0;
    while (*name)
        hash = hash * XIM_HASH_PRIME_1 + *(const unsigned char *)name++;
...
static int
_XimCachedFileName(const char *dir,
                   const char *name,
                   const char *intname,
                   const char *encoding,
                   uid_t       uid,
                   ...
                   off_t      *size)
{
    ...
    *res  = Xmalloc(len + 1 + 27 + 1); /* Max VERSION 9999 */
    ...
    /* fprintf (stderr, "-> %s\n", *res); */
    if ((fd = _XOpenFile(*res, O_RDONLY)) == -1) return -1;
...
static Bool
_XimLoadCache(int         fd,
              const char *name,
              const char *encoding,
              off_t       size,
              Xim         im)
{
    if (_XimCache_mmap || _XimReadCachedDefaultTree(fd, name, encoding, size))
    {
        _XimCachedDefaultTreeRefcount++;
        memcpy(&im->private.local.base,
...
static void
_XimWriteCachedDefaultTree(const char *name,
                           const char *encoding,
                           ...
                           Xim         im)
{
    ...
    FILE                   *fp;
    struct _XimCacheStruct *m;
    int msize = (sizeof(struct _XimCacheStruct) + strlen(name) +
                 strlen(encoding) + 2 + XIM_CACHE_TREE_ALIGNMENT - 1) &
                -XIM_CACHE_TREE_ALIGNMENT;
    DefTreeBase *b = &im->private.local.base;

    if (!b->tree && !(b->tree = Xcalloc(1, sizeof(DefTree)))) return;
    if (!b->mb && !(b->mb = Xmalloc(1))) return;
    if (!b->wc && !(b->wc = Xmalloc(sizeof(wchar_t)))) return;
    if (!b->utf8 && !(b->utf8 = Xmalloc(1))) return;
    ...
    if ((fd = _XOpenFileMode(cachename, O_WRONLY | O_CREAT | O_EXCL, 0600)) < 0)
    {
        Xfree(m);
        return;
    }
    ...
    if (fclose(fp) != 0) unlink(cachename);
    _XimCache_mmap = m;
    memcpy(&_XimCachedDefaultTreeBase,
           &im->private.local.base,
           sizeof(_XimCachedDefaultTreeBase));
...
_XimCreateDefaultTree(Xim im)
{
    ...
    if (!cachedir)
    {
        cachefd = _XimCachedFileName(XIM_GLOBAL_CACHE_DIR,
                                     name,
                                     intname,
                                     encoding,
                                     0,
    ...
    if (getuid() == euid && getgid() == egid && euid != 0 && home)
    {
        if (!cachedir)
        {
            tmpcachedir = cachedir =
                Xmalloc(hl + strlen(XIM_HOME_CACHE_DIR) + 1);
            strcpy(cachedir, home);
            strcat(cachedir, XIM_HOME_CACHE_DIR);
        }
        cachefd = _XimCachedFileName(cachedir,
                                     name,
                                     intname,
                                     encoding,
                                     euid,
