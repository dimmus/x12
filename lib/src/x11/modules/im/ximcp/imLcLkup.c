...
int
_XimLocalMbLookupString(XIC        xic,
                        XKeyEvent *ev,
                        char      *buffer,
                        int        bytes,
                        KeySym    *keysym,
                        Status    *status)
{
    ...
    if (ev->keycode == 0 && ((ic->private.local.composed != 0) ||
                             (ic->private.local.brl_committed != 0)))
    {
        if (ic->private.local.brl_committed != 0)
        { /* Braille Event */
...
int
_XimLocalWcLookupString(XIC        xic,
                        XKeyEvent *ev,
                        wchar_t   *buffer,
                        int        wlen,
                        KeySym    *keysym,
                        Status    *status)
{
    ...
    if (ev->keycode == 0)
    {
        ...
        { /* Composed Event */
            ...
            memcpy((char *)buffer,
                   (char *)&wc[b[ic->private.local.composed].wc],
                   ret * sizeof(wchar_t));
            if (keysym) *keysym = b[ic->private.local.composed].ks;
            if (ret > 0)
            {
...
int
_XimLocalUtf8LookupString(XIC        xic,
                          XKeyEvent *ev,
                          char      *buffer,
                          int        bytes,
                          KeySym    *keysym,
