...
typedef struct _XimValueOffsetInfo
{
    ...
    XrmQuark       quark;
    unsigned int   offset;
    Bool (*defaults)(struct _XimValueOffsetInfo *,
                     XPointer,
                     XPointer,
                     unsigned long);
    Bool (*encode)(struct _XimValueOffsetInfo *, XPointer, XPointer);
    Bool (*decode)(struct _XimValueOffsetInfo *, XPointer, XPointer);
...
char *str;
{
    if (!strcmp(str, "True") || !strcmp(str, "true") || !strcmp(str, "Yes") ||
        !strcmp(str, "yes") || !strcmp(str, "ON") || !strcmp(str, "on"))
        return True;
    return False;
}

void _XimSetProtoResource(im) Xim im;
{
    ...
    if (!preedit_style) preedit_style = XIMPreeditNothing;
    ...
    if (XrmGetResource(im->core.rdb, res_name, res_class, &str_type, &value))
    {
        if (!strcmp(value.addr, "XIMStatusArea")) status_style = XIMStatusArea;
    ...
    if (!status_style) status_style = XIMStatusNothing;
...
static const unsigned short supported_local_im_values_list[] = {
    OFFSET_XNQUERYINPUTSTYLE,   OFFSET_XNRESOURCENAME,
    OFFSET_XNRESOURCECLASS,     OFFSET_XNDESTROYCALLBACK,
    OFFSET_XNQUERYIMVALUESLIST, OFFSET_XNQUERYICVALUESLIST,
...
static const XIMStyle supported_local_styles[] = {
...
static Bool
_XimDefaultStyles(XimValueOffsetInfo info,
                  XPointer           top,
...
static Bool
_XimDefaultIMValues(XimValueOffsetInfo info,
                    XPointer           top,
                    XPointer           parm,   /* unused */
                    unsigned long      mode)   /* unused */
{
...
static Bool
_XimDefaultICValues(XimValueOffsetInfo info,
                    XPointer           top,
                    XPointer           parm,   /* unused */
                    unsigned long      mode)   /* unused */
{
...
static Bool
_XimDefaultVisiblePos(XimValueOffsetInfo info,
                      XPointer           top,
                      XPointer           parm,   /* unused */
                      unsigned long      mode)   /* unused */
{
...
static Bool
_XimDefaultFocusWindow(XimValueOffsetInfo info,
                       XPointer           top,
                       XPointer           parm,
                       unsigned long      mode)
{
...
static Bool
_XimDefaultResName(XimValueOffsetInfo info,
                   XPointer           top,
                   XPointer           parm,
                   unsigned long      mode)
{
...
static Bool
_XimDefaultResClass(XimValueOffsetInfo info,
                    XPointer           top,
                    XPointer           parm,
                    unsigned long      mode)
{
...
static Bool
_XimDefaultDestroyCB(XimValueOffsetInfo info,
                     XPointer           top,
                     XPointer           parm,
                     unsigned long      mode)
{
...
static Bool
_XimDefaultResetState(XimValueOffsetInfo info,
                      XPointer           top,
                      XPointer           parm,
                      unsigned long      mode)
{
...
static Bool
_XimDefaultHotKeyState(XimValueOffsetInfo info,
                       XPointer           top,
                       XPointer           parm,
                       unsigned long      mode)
{
...
static Bool
_XimDefaultArea(XimValueOffsetInfo info,
                XPointer           top,
                XPointer           parm,
                unsigned long      mode)
{
    ...
    if (XGetGeometry(im->core.display,
                     (Drawable)ic->core.focus_window,
                     &root_return,
                     &x_return,
                     &y_return,
                     &width_return,
...
static Bool
_XimDefaultColormap(XimValueOffsetInfo info,
                    XPointer           top,
                    XPointer           parm,
                    unsigned long      mode)
{
    ...
    if (XGetWindowAttributes(im->core.display,
...
static Bool
_XimDefaultStdColormap(XimValueOffsetInfo info,
                       XPointer           top,
                       XPointer           parm,
                       unsigned long      mode)
{
...
static Bool
_XimDefaultFg(XimValueOffsetInfo info,
              XPointer           top,
              XPointer           parm,
              unsigned long      mode)
{
...
static Bool
_XimDefaultBg(XimValueOffsetInfo info,
              XPointer           top,
              XPointer           parm,
              unsigned long      mode)
{
...
static Bool
_XimDefaultBgPixmap(XimValueOffsetInfo info,
                    XPointer           top,
                    XPointer           parm,
                    unsigned long      mode)
{
...
static Bool
_XimDefaultFontSet(XimValueOffsetInfo info,
                   XPointer           top,
                   XPointer           parm,
                   unsigned long      mode)
{
...
static Bool
_XimDefaultLineSpace(XimValueOffsetInfo info,
                     XPointer           top,
                     XPointer           parm,
                     unsigned long      mode)
{
...
static Bool
_XimDefaultCursor(XimValueOffsetInfo info,
                  XPointer           top,
                  XPointer           parm,
                  unsigned long      mode)
{
...
static Bool
_XimDefaultPreeditState(XimValueOffsetInfo info,
                        XPointer           top,
                        XPointer           parm,
                        unsigned long      mode)
{
...
static Bool
_XimDefaultNest(XimValueOffsetInfo info,
                XPointer           top,
                XPointer           parm,
                unsigned long      mode)
{
...
static Bool
_XimEncodeCallback(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XIMCallback *out;
...
static Bool
_XimEncodeString(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    char  *string;
    char **out;
...
static Bool
_XimEncodeStyle(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XIMStyle *out;
...
static Bool
_XimEncodeWindow(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    Window *out;
...
static Bool
_XimEncodeStringConv(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    /*
     * Not yet
     */
...
static Bool
_XimEncodeResetState(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XIMResetState *out;
...
static Bool
_XimEncodeHotKey(XimValueOffsetInfo info, XPointer top, XPointer val)
{
...
static Bool
_XimEncodeHotKetState(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XIMHotKeyState *out;
...
static Bool
_XimEncodeRectangle(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XRectangle *out;
...
static Bool
_XimEncodeSpot(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XPoint *out;
...
static Bool
_XimEncodeColormap(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    Colormap *out;
...
static Bool
_XimEncodeStdColormap(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    Atom *out;
...
static Bool
_XimEncodeLong(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    unsigned long *out;
...
static Bool
_XimEncodeBgPixmap(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    Pixmap *out;
...
static Bool
_XimEncodeFontSet(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XFontSet *out;
...
static Bool
_XimEncodeLineSpace(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    int *out;
...
static Bool
_XimEncodeCursor(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    Cursor *out;
...
static Bool
_XimEncodePreeditState(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XIMPreeditState *out;
...
static Bool
_XimEncodeNest(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    return True;
}

static Bool
_XimDecodeStyles(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XIMStyles   *styles;
    XIMStyles   *out;
    register int i;
...
static Bool
_XimDecodeValues(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XIMValuesList *values_list;
    XIMValuesList *out;
    register int   i;
...
static Bool
_XimDecodeCallback(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XIMCallback *in;
...
static Bool
_XimDecodeString(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    char *in;
    char *string;
...
static Bool
_XimDecodeBool(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    Bool *in;
...
static Bool
_XimDecodeStyle(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XIMStyle *in;
...
static Bool
_XimDecodeWindow(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    Window *in;
...
static Bool
_XimDecodeStringConv(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    /*
     * Not yet
     */
...
static Bool
_XimDecodeResetState(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XIMResetState *in;
...
static Bool
_XimDecodeHotKey(XimValueOffsetInfo info, XPointer top, XPointer val)
{
...
static Bool
_XimDecodeHotKetState(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XIMHotKeyState *in;
...
static Bool
_XimDecodeRectangle(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XRectangle *in;
    XRectangle *rect;
...
static Bool
_XimDecodeSpot(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XPoint *in;
    XPoint *spot;
...
static Bool
_XimDecodeColormap(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    Colormap *in;
...
static Bool
_XimDecodeStdColormap(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    Atom *in;
...
static Bool
_XimDecodeLong(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    unsigned long *in;
...
static Bool
_XimDecodeBgPixmap(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    Pixmap *in;
...
static Bool
_XimDecodeFontSet(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XFontSet *in;
...
static Bool
_XimDecodeLineSpace(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    int *in;
...
static Bool
_XimDecodeCursor(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    Cursor *in;
...
static Bool
_XimDecodePreeditState(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    XIMPreeditState *in;
...
static Bool
_XimDecodeNest(XimValueOffsetInfo info, XPointer top, XPointer val)
{
    return True;
}
...
static XimValueOffsetInfoRec im_attr_info[] = {
    { OFFSET_XNQUERYINPUTSTYLE,
     0, XOffsetOf(XimDefIMValues, styles),
     _XimDefaultStyles,     NULL,
     _XimDecodeStyles   },

    { OFFSET_XNDESTROYCALLBACK,
     0, XOffsetOf(XimDefIMValues, destroy_callback),
     NULL,                  _XimEncodeCallback,
     _XimDecodeCallback },

    { OFFSET_XNRESOURCENAME,
     0, XOffsetOf(XimDefIMValues, res_name),
     NULL,                  _XimEncodeString,
     _XimDecodeString   },

    { OFFSET_XNRESOURCECLASS,
     0, XOffsetOf(XimDefIMValues, res_class),
     NULL,                  _XimEncodeString,
     _XimDecodeString   },

    { OFFSET_XNQUERYIMVALUESLIST,
     0, XOffsetOf(XimDefIMValues, im_values_list),
     _XimDefaultIMValues,   NULL,
     _XimDecodeValues   },

    { OFFSET_XNQUERYICVALUESLIST,
     0, XOffsetOf(XimDefIMValues, ic_values_list),
     _XimDefaultICValues,   NULL,
     _XimDecodeValues   },

    { OFFSET_XNVISIBLEPOSITION,
     0, XOffsetOf(XimDefIMValues, visible_position),
     _XimDefaultVisiblePos, NULL,
     _XimDecodeBool     }
};

static XimValueOffsetInfoRec ic_attr_info[] = {
    { OFFSET_XNINPUTSTYLE,
     0, XOffsetOf(XimDefICValues, input_style),
     NULL,                   _XimEncodeStyle,
     _XimDecodeStyle       },

    { OFFSET_XNCLIENTWINDOW,
     0, XOffsetOf(XimDefICValues, client_window),
     NULL,                   _XimEncodeWindow,
     _XimDecodeWindow      },

    { OFFSET_XNFOCUSWINDOW,
     0, XOffsetOf(XimDefICValues, focus_window),
     _XimDefaultFocusWindow, _XimEncodeWindow,
     _XimDecodeWindow      },

    { OFFSET_XNRESOURCENAME,
     0, XOffsetOf(XimDefICValues, res_name),
     _XimDefaultResName,     _XimEncodeString,
     _XimDecodeString      },

    { OFFSET_XNRESOURCECLASS,
     0, XOffsetOf(XimDefICValues, res_class),
     _XimDefaultResClass,    _XimEncodeString,
     _XimDecodeString      },

    { OFFSET_XNGEOMETRYCALLBACK,
     0, XOffsetOf(XimDefICValues, geometry_callback),
     NULL,                   _XimEncodeCallback,
     _XimDecodeCallback    },

    { OFFSET_XNFILTEREVENTS,
     0, XOffsetOf(XimDefICValues, filter_events),
     NULL,                   NULL,
     _XimDecodeLong        },

    { OFFSET_XNDESTROYCALLBACK,
     0, XOffsetOf(XimDefICValues, destroy_callback),
     _XimDefaultDestroyCB,   _XimEncodeCallback,
     _XimDecodeCallback    },

    { OFFSET_XNSTRINGCONVERSIONCALLBACK,
     0, XOffsetOf(XimDefICValues, string_conversion_callback),
     NULL,                   _XimEncodeCallback,
     _XimDecodeCallback    },

    { OFFSET_XNSTRINGCONVERSION,
     0, XOffsetOf(XimDefICValues, string_conversion),
     NULL,                   _XimEncodeStringConv,
     _XimDecodeStringConv  },

    { OFFSET_XNRESETSTATE,
     0, XOffsetOf(XimDefICValues, reset_state),
     _XimDefaultResetState,  _XimEncodeResetState,
     _XimDecodeResetState  },

    { OFFSET_XNHOTKEY,
     0, XOffsetOf(XimDefICValues, hotkey),
     NULL,                   _XimEncodeHotKey,
     _XimDecodeHotKey      },

    { OFFSET_XNHOTKEYSTATE,
     0, XOffsetOf(XimDefICValues, hotkey_state),
     _XimDefaultHotKeyState, _XimEncodeHotKetState,
     _XimDecodeHotKetState },

    { OFFSET_XNPREEDITATTRIBUTES,
     0, XOffsetOf(XimDefICValues, preedit_attr),
     _XimDefaultNest,        _XimEncodeNest,
     _XimDecodeNest        },

    { OFFSET_XNSTATUSATTRIBUTES,
     0, XOffsetOf(XimDefICValues, status_attr),
     _XimDefaultNest,        _XimEncodeNest,
     _XimDecodeNest        },
};
...
static XimValueOffsetInfoRec ic_pre_attr_info[] = {
    { OFFSET_XNAREA,
     0, XOffsetOf(ICPreeditAttributes, area),
     _XimDefaultArea,         _XimEncodeRectangle,
     _XimDecodeRectangle    },

    { OFFSET_XNAREANEEDED,
     0, XOffsetOf(ICPreeditAttributes, area_needed),
     NULL,                    _XimEncodeRectangle,
     _XimDecodeRectangle    },

    { OFFSET_XNSPOTLOCATION,
     0, XOffsetOf(ICPreeditAttributes, spot_location),
     NULL,                    _XimEncodeSpot,
     _XimDecodeSpot         },

    { OFFSET_XNCOLORMAP,
     0, XOffsetOf(ICPreeditAttributes, colormap),
     _XimDefaultColormap,     _XimEncodeColormap,
     _XimDecodeColormap     },

    { OFFSET_XNSTDCOLORMAP,
     0, XOffsetOf(ICPreeditAttributes, std_colormap),
     _XimDefaultStdColormap,  _XimEncodeStdColormap,
     _XimDecodeStdColormap  },

    { OFFSET_XNFOREGROUND,
     0, XOffsetOf(ICPreeditAttributes, foreground),
     _XimDefaultFg,           _XimEncodeLong,
     _XimDecodeLong         },

    { OFFSET_XNBACKGROUND,
     0, XOffsetOf(ICPreeditAttributes, background),
     _XimDefaultBg,           _XimEncodeLong,
     _XimDecodeLong         },

    { OFFSET_XNBACKGROUNDPIXMAP,
     0, XOffsetOf(ICPreeditAttributes, background_pixmap),
     _XimDefaultBgPixmap,     _XimEncodeBgPixmap,
     _XimDecodeBgPixmap     },

    { OFFSET_XNFONTSET,
     0, XOffsetOf(ICPreeditAttributes, fontset),
     _XimDefaultFontSet,      _XimEncodeFontSet,
     _XimDecodeFontSet      },

    { OFFSET_XNLINESPACE,
     0, XOffsetOf(ICPreeditAttributes, line_spacing),
     _XimDefaultLineSpace,    _XimEncodeLineSpace,
     _XimDecodeLineSpace    },

    { OFFSET_XNCURSOR,
     0, XOffsetOf(ICPreeditAttributes, cursor),
     _XimDefaultCursor,       _XimEncodeCursor,
     _XimDecodeCursor       },

    { OFFSET_XNPREEDITSTARTCALLBACK,
     0, XOffsetOf(ICPreeditAttributes, start_callback),
     NULL,                    _XimEncodeCallback,
     _XimDecodeCallback     },

    { OFFSET_XNPREEDITDONECALLBACK,
     0, XOffsetOf(ICPreeditAttributes, done_callback),
     NULL,                    _XimEncodeCallback,
     _XimDecodeCallback     },

    { OFFSET_XNPREEDITDRAWCALLBACK,
     0, XOffsetOf(ICPreeditAttributes, draw_callback),
     NULL,                    _XimEncodeCallback,
     _XimDecodeCallback     },

    { OFFSET_XNPREEDITCARETCALLBACK,
     0, XOffsetOf(ICPreeditAttributes, caret_callback),
     NULL,                    _XimEncodeCallback,
     _XimDecodeCallback     },

    { OFFSET_XNPREEDITSTATE,
     0, XOffsetOf(ICPreeditAttributes, preedit_state),
     _XimDefaultPreeditState, _XimEncodePreeditState,
     _XimDecodePreeditState },

    { OFFSET_XNPREEDITSTATENOTIFYCALLBACK,
     0, XOffsetOf(ICPreeditAttributes, state_notify_callback),
     NULL,                    _XimEncodeCallback,
     _XimDecodeCallback     },
};
...
static XimValueOffsetInfoRec ic_sts_attr_info[] = {
    { OFFSET_XNAREA,
     0, XOffsetOf(ICStatusAttributes, area),
     _XimDefaultArea,        _XimEncodeRectangle,
     _XimDecodeRectangle   },

    { OFFSET_XNAREANEEDED,
     0, XOffsetOf(ICStatusAttributes, area_needed),
     NULL,                   _XimEncodeRectangle,
     _XimDecodeRectangle   },

    { OFFSET_XNCOLORMAP,
     0, XOffsetOf(ICStatusAttributes, colormap),
     _XimDefaultColormap,    _XimEncodeColormap,
     _XimDecodeColormap    },

    { OFFSET_XNSTDCOLORMAP,
     0, XOffsetOf(ICStatusAttributes, std_colormap),
     _XimDefaultStdColormap, _XimEncodeStdColormap,
     _XimDecodeStdColormap },

    { OFFSET_XNFOREGROUND,
     0, XOffsetOf(ICStatusAttributes, foreground),
     _XimDefaultFg,          _XimEncodeLong,
     _XimDecodeLong        },

    { OFFSET_XNBACKGROUND,
     0, XOffsetOf(ICStatusAttributes, background),
     _XimDefaultBg,          _XimEncodeLong,
     _XimDecodeLong        },

    { OFFSET_XNBACKGROUNDPIXMAP,
     0, XOffsetOf(ICStatusAttributes, background_pixmap),
     _XimDefaultBgPixmap,    _XimEncodeBgPixmap,
     _XimDecodeBgPixmap    },
    ...
    { OFFSET_XNLINESPACE,
     0, XOffsetOf(ICStatusAttributes, line_spacing),
     _XimDefaultLineSpace,   _XimEncodeLineSpace,
     _XimDecodeLineSpace   },

    { OFFSET_XNCURSOR,
     0, XOffsetOf(ICStatusAttributes, cursor),
     _XimDefaultCursor,      _XimEncodeCursor,
     _XimDecodeCursor      },

    { OFFSET_XNSTATUSSTARTCALLBACK,
     0, XOffsetOf(ICStatusAttributes, start_callback),
     NULL,                   _XimEncodeCallback,
     _XimDecodeCallback    },

    { OFFSET_XNSTATUSDONECALLBACK,
     0, XOffsetOf(ICStatusAttributes, done_callback),
     NULL,                   _XimEncodeCallback,
     _XimDecodeCallback    },

    { OFFSET_XNSTATUSDRAWCALLBACK,
     0, XOffsetOf(ICStatusAttributes, draw_callback),
     NULL,                   _XimEncodeCallback,
     _XimDecodeCallback    }
};
...
static const XimIMMode im_mode[] = {
    { OFFSET_XNQUERYINPUTSTYLE,   (XIM_MODE_IM_DEFAULT | XIM_MODE_IM_GET) },
    { OFFSET_XNDESTROYCALLBACK,
    ...
    { OFFSET_XNRESOURCECLASS,
     (XIM_MODE_IM_DEFAULT | XIM_MODE_IM_SET | XIM_MODE_IM_GET)            },
    { OFFSET_XNQUERYIMVALUESLIST, (XIM_MODE_IM_DEFAULT | XIM_MODE_IM_GET) },
...
static const XimICMode ic_mode[] = {
    ...
    { OFFSET_XNCLIENTWINDOW,
     ...
     (XIM_MODE_PRE_ONCE | XIM_MODE_PRE_GET),
     (XIM_MODE_PRE_ONCE | XIM_MODE_PRE_GET),
     0,                                                                                                                                                                       (XIM_MODE_STS_ONCE | XIM_MODE_STS_GET),
    ...
    { OFFSET_XNAREANEEDED,
     ...
     (XIM_MODE_PRE_SET | XIM_MODE_PRE_GET),
     0,                                                                                                                                                                    0,
     0,                                                                                                                                                                                                                                     (XIM_MODE_STS_SET | XIM_MODE_STS_GET),
     0,                                                                                                                                                                                                                                                                                                                     0 },
    { OFFSET_XNSPOTLOCATION,
     (XIM_MODE_PRE_SET | XIM_MODE_PRE_GET),
    ...
    { OFFSET_XNSTATUSSTARTCALLBACK,
     0,                                                            0,
     0,                                                                                                                             0,
     0,                                                                                                                                                                       (XIM_MODE_STS_CREATE | XIM_MODE_STS_SET | XIM_MODE_STS_GET),
     0,                                                                                                                                                                                                                                                                                                   0,
     0                                                                                                                                                                                                                                                                                                                        },
    { OFFSET_XNSTATUSDONECALLBACK,
     0,                                                            0,
...
static Bool
_XimSetResourceList(XIMResourceList *res_list,
...
Bool
_XimSetIMResourceList(XIMResourceList *res_list, unsigned int *list_num)
{
    return _XimSetResourceList(res_list,
                               list_num,
                               im_resources,
                               XIMNumber(im_resources),
                               100);
}

Bool
_XimSetICResourceList(XIMResourceList *res_list, unsigned int *list_num)
{
    return _XimSetResourceList(res_list,
                               list_num,
                               ic_resources,
                               XIMNumber(ic_resources),
                               200);
}

Bool
_XimSetInnerIMResourceList(XIMResourceList *res_list, unsigned int *list_num)
{
    return _XimSetResourceList(res_list,
                               list_num,
                               im_inner_resources,
                               XIMNumber(im_inner_resources),
                               100);
}

Bool
_XimSetInnerICResourceList(XIMResourceList *res_list, unsigned int *list_num)
{
    return _XimSetResourceList(res_list,
                               list_num,
                               ic_inner_resources,
                               XIMNumber(ic_inner_resources),
                               200);
}

static XIMResourceList
_XimGetResourceListRecByMode(XIMResourceList res_list,
                             unsigned int    list_num,
                             unsigned short  mode)
{
    register int i;
...
Bool
_XimCheckCreateICValues(XIMResourceList res_list, unsigned int list_num)
{
...
XIMResourceList
_XimGetResourceListRecByQuark(XIMResourceList res_list,
                              unsigned int    list_num,
                              XrmQuark        quark)
{
    register int i;
...
XIMResourceList
_XimGetResourceListRec(XIMResourceList res_list,
                       unsigned int    list_num,
                       const char     *name)
{
...
void
_XimSetIMMode(XIMResourceList res_list, unsigned int list_num)
{
    XIMResourceList res;
...
static int
_XimCheckSetIMDefaultsMode(XIMResourceList res)
{
...
static int
_XimCheckSetIMValuesMode(XIMResourceList res)
{
...
static int
_XimCheckGetIMValuesMode(XIMResourceList res)
{
...
int
_XimCheckIMMode(XIMResourceList res, unsigned long mode)
{
    if (res->mode == 0)
    {
        return XIM_CHECK_INVALID;
...
static int
_XimCheckSetICDefaultsMode(XIMResourceList res, unsigned long mode)
{
    if (mode & XIM_PREEDIT_ATTR)
    {
...
static int
_XimCheckCreateICMode(XIMResourceList res, unsigned long mode)
{
    if (mode & XIM_PREEDIT_ATTR)
    {
        if (!(res->mode & XIM_MODE_PRE_MASK))
...
static int
_XimCheckSetICValuesMode(XIMResourceList res, unsigned long mode)
{
    if (mode & XIM_PREEDIT_ATTR)
    {
        if (!(res->mode & XIM_MODE_PRE_MASK))
...
static int
_XimCheckGetICValuesMode(XIMResourceList res, unsigned long mode)
{
    if (mode & XIM_PREEDIT_ATTR)
    {
        if (!(res->mode & XIM_MODE_PRE_MASK))
...
int
_XimCheckICMode(XIMResourceList res, unsigned long mode)
{
...
static Bool
_XimEncodeAttr(XimValueOffsetInfo info,
               unsigned int       num,
               XIMResourceList    res,
               XPointer           top,
               XPointer           val)
...
Bool
_XimEncodeLocalIMAttr(XIMResourceList res, XPointer top, XPointer val)
{
    return _XimEncodeAttr(im_attr_info, XIMNumber(im_attr_info), res, top, val);
}

Bool
_XimEncodeLocalICAttr(Xic             ic,
                      XIMResourceList res,
                      XPointer        top,
                      XIMArg         *arg,
                      unsigned long   mode)
...
static Bool
_XimEncodeLocalTopValue(Xic ic, XIMResourceList res, XPointer val, Bool flag)
{
    XIMArg *p = (XIMArg *)val;
    ...
    else if (res->xrm_name == XrmStringToQuark(XNFocusWindow))
    {
        ...
        else ic->core.focus_window = (Window)p->value;
    }
    return True;
}

static Bool
_XimEncodeLocalPreeditValue(Xic ic, XIMResourceList res, XPointer val)
{
    XIMArg *p = (XIMArg *)val;
...
static Bool
_XimEncodeLocalStatusValue(Xic ic, XIMResourceList res, XPointer val)
{
    XIMArg *p = (XIMArg *)val;
...
static Bool
_XimCheckInputStyle(XIMStyles *styles, XIMStyle style)
{
...
Bool
_XimCheckLocalInputStyle(Xic             ic,
                         XPointer        top,
                         XIMArg         *values,
                         XIMStyles      *styles,
                         XIMResourceList res_list,
...
static Bool
_XimDecodeAttr(XimValueOffsetInfo info,
               unsigned int       num,
               XIMResourceList    res,
               XPointer           top,
               XPointer           val)
...
Bool
_XimDecodeLocalIMAttr(XIMResourceList res, XPointer top, XPointer val)
{
    return _XimDecodeAttr(im_attr_info, XIMNumber(im_attr_info), res, top, val);
}

Bool
_XimDecodeLocalICAttr(XIMResourceList res,
                      XPointer        top,
                      XPointer        val,
                      unsigned long   mode)
{
...
_XimGetCurrentICValues(Xic ic, XimDefICValues *ic_values)
{
    ...
    ic_values->string_conversion_callback = ic->core.string_conversion_callback;
...
void
_XimSetCurrentICValues(Xic ic, XimDefICValues *ic_values)
{
    ...
    ic->core.string_conversion_callback = ic_values->string_conversion_callback;
...
_XimInitialICOffsetInfo(void)
{
    ...
    for (i = 0; i < n; i++)
    {
        ic_pre_attr_info[i].quark =
            XrmStringToQuark(GET_NAME(ic_pre_attr_info[i]));
    }

    n = XIMNumber(ic_sts_attr_info);
    for (i = 0; i < n; i++)
    {
        ic_sts_attr_info[i].quark =
            XrmStringToQuark(GET_NAME(ic_sts_attr_info[i]));
    }
}
