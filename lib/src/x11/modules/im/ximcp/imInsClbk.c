                         ...
                         XPointer client_data)
{
    ...
    for (ii = 0; ii < nitems; ii++)
    {
        if (XGetSelectionOwner(display, atoms[ii]))
        {
            for (icb = callback_list; icb; icb = icb->next)
            {
                ...
                {
                    xim = (*icb->lcd->methods->open_im)(icb->lcd,
                                                        display,
                                                        icb->rdb,
                                                        icb->res_name,
                                                        icb->res_class);
    ...
    for (icb = callback_list, picb = NULL; icb;)
    {
        if (icb->destroy)
        {
            if (picb) picb->next = icb->next;
            else callback_list = icb->next;
            tmp = icb;
            icb = icb->next;
            XFree(tmp);
        }
...
Bool
_XimRegisterIMInstantiateCallback(XLCd        lcd,
                                  Display    *display,
                                  XrmDatabase rdb,
                                  char       *res_name,
                                  char       *res_class,
                                  ...
                                  XPointer    client_data)
{
    ...
    if (!callback_list) callback_list = icb;
    else
    {
    ...
    if (icb == callback_list)
    {
        ...
        _XRegisterFilterByType(display,
                               ...
                               (XPointer)NULL);
        XSelectInput(display, root, attr.your_event_mask | PropertyChangeMask);
    }
...
Bool
_XimUnRegisterIMInstantiateCallback(XLCd        lcd,
                                    Display    *display,
                                    XrmDatabase rdb,
                                    char       *res_name,
                                    char       *res_class,
                                    ...
                                    XPointer    client_data)
{
    ...
    for (icb = callback_list, picb = NULL; icb; picb = icb, icb = icb->next)
    {
        if (!strcmp(locale, icb->name) &&
            ...
            !icb->destroy)
        {
            if (lock) icb->destroy = True;
            else
            {
                if (!picb)
                {
                    callback_list = icb->next;
                    _XUnregisterFilter(display,
                                       RootWindow(display, 0),
                                       _XimFilterPropertyNotify,
                                       (XPointer)NULL);
                }
                else picb->next = icb->next;
                _XCloseLC(icb->lcd);
                XFree(icb->modifiers);
                XFree(icb);
            }
