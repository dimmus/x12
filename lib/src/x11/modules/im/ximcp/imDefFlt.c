...
static long
_XimTriggerCheck(Xim im, XKeyEvent *ev, INT32 len, CARD32 *keylist)
{
    register long i;
    KeySym        keysym;
...
static long
_XimTriggerOnCheck(Xim im, XKeyEvent *ev)
{
    return _XimTriggerCheck(im,
                            ev,
                            (INT32)im->private.proto.im_onkeylist[0],
                            &im->private.proto.im_onkeylist[1]);
}

static long
_XimTriggerOffCheck(Xim im, XKeyEvent *ev)
{
    return _XimTriggerCheck(im,
                            ev,
                            (INT32)im->private.proto.im_offkeylist[0],
                            &im->private.proto.im_offkeylist[1]);
}

static Bool
_XimOnKeysCheck(Xic ic, XKeyEvent *ev)
{
    Xim  im = (Xim)ic->core.im;
    long idx;

    if (IS_DYNAMIC_EVENT_FLOW(ic->core.im) && im->private.proto.im_onkeylist &&
...
static Bool
_XimOffKeysCheck(Xic ic, XKeyEvent *ev)
{
    Xim  im = (Xim)ic->core.im;
    long idx;

    if (IS_DYNAMIC_EVENT_FLOW(ic->core.im) && im->private.proto.im_offkeylist &&
...
static Bool
_XimProtoKeypressFilter(Xic ic, XKeyEvent *ev, Window w)
{
    Xim im = (Xim)ic->core.im;
    ...
    if (!w) return NOTFILTERD;

    if (IS_NEGLECT_EVENT(ic, KeyPressMask)) return FILTERD;
...
#else
    if (!IS_IC_CONNECTED(ic)) return NOTFILTERD;
#endif /* XIM_CONNECTABLE */

    if (!IS_FORWARD_EVENT(ic, KeyPressMask))
    {
        if (_XimOnKeysCheck(ic, ev)) return FILTERD;
        return NOTFILTERD;
    }
    if (_XimOffKeysCheck(ic, ev)) return FILTERD;

    if (_XimForwardEvent(ic,
                         (XEvent *)ev,
...
static Bool
_XimFilterKeypress(Display *d, Window w, XEvent *ev, XPointer client_data)
{
    return _XimProtoKeypressFilter((Xic)client_data, (XKeyEvent *)ev, w);
}

static Bool
_XimProtoKeyreleaseFilter(Xic ic, XKeyEvent *ev, Window w)
{
    Xim im = (Xim)ic->core.im;
    ...
    if (IS_NEGLECT_EVENT(ic, KeyReleaseMask)) return FILTERD;
...
static Bool
_XimFilterKeyrelease(Display *d, Window w, XEvent *ev, XPointer client_data)
{
    return _XimProtoKeyreleaseFilter((Xic)client_data, (XKeyEvent *)ev, w);
}

static void
_XimRegisterKeyPressFilter(Xic ic)
{
...
static void
_XimRegisterKeyReleaseFilter(Xic ic)
{
    if (ic->core.focus_window)
    {
...
static void
_XimUnregisterKeyPressFilter(Xic ic)
{
    if (ic->core.focus_window)
    {
        if (ic->private.proto.registed_filter_event & KEYPRESS_MASK)
...
static void
_XimUnregisterKeyReleaseFilter(Xic ic)
{
    if (ic->core.focus_window)
    {
        if (ic->private.proto.registed_filter_event & KEYRELEASE_MASK)
...
void
_XimRegisterFilter(Xic ic)
{
    _XimRegisterKeyPressFilter(ic);
    if (IS_FORWARD_EVENT(ic, KeyReleaseMask)) _XimRegisterKeyReleaseFilter(ic);
    return;
}

void
_XimUnregisterFilter(Xic ic)
{
...
void
_XimReregisterFilter(Xic ic)
{
    if (IS_FORWARD_EVENT(ic, KeyReleaseMask)) _XimRegisterKeyReleaseFilter(ic);
    else _XimUnregisterKeyReleaseFilter(ic);

    return;
}

static Bool
_XimFilterServerDestroy(Display *d, Window w, XEvent *ev, XPointer client_data)
{
...
void
_XimRegisterServerFilter(Xim im)
{
...
void
_XimUnregisterServerFilter(Xim im)
{
    if (im->private.proto.im_window)
    {
        if (im->private.proto.registed_filter_event & DESTROYNOTIFY_MASK)
