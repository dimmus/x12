...
static Bool
_XimCheckInnerIMAttributes(Xim im, XIMArg *arg, unsigned long mode)
{
    XIMResourceList res;
    int             check;
    ...
    if (check == XIM_CHECK_INVALID) return True;
    else if (check == XIM_CHECK_ERROR) return False;

    return True;
}

char *
_XimMakeIMAttrIDList(Xim             im,
                     ...
                     unsigned long   mode)
{
    ...
    *len = 0;
    if (!arg) return (char *)NULL;

    for (p = arg; p->name; p++)
    {
        if (!(res = _XimGetResourceListRec(res_list, res_num, p->name)))
        {
            if (_XimCheckInnerIMAttributes(im, p, mode)) continue;
            return p->name;
        }

        check = _XimCheckIMMode(res, mode);
        if (check == XIM_CHECK_INVALID) continue;
        else if (check == XIM_CHECK_ERROR) return p->name;
...
static Bool
_XimCheckInnerICAttributes(Xic ic, XIMArg *arg, unsigned long mode)
{
    XIMResourceList res;
    int             check;
                     ...
                     unsigned long   mode)
{
    ...
    *len = 0;
    ...
    for (p = arg; p && p->name; p++)
    {
        if (!(res = _XimGetResourceListRec(res_list, res_num, p->name)))
        {
            if (_XimCheckInnerICAttributes(ic, p, mode)) continue;
            *len = -1;
            return p->name;
        }
        ...
        *len += sizeof(CARD16);
        ...
        if (res->resource_size == XimType_NEST)
        {
            ...
            {
                if ((name = _XimMakeICAttrIDList(ic,
                                                 res_list,
                                                 res_num,
                                                 (XIMArg *)p->value,
                                                 buf,
        ...
        case XimType_STRING8:
            {
                ...
                if (!(str = Xmalloc(data_len + 1))) return False;
        ...
        case XimType_XIMStyles:
            {
                ...
                if (num > (USHRT_MAX / sizeof(XIMStyle))) return False;
                if ((2 * sizeof(CARD16) + (num * sizeof(CARD32))) > data_len)
                    return False;
                alloc_len = sizeof(XIMStyles) + sizeof(XIMStyle) * num;
                if (alloc_len < sizeof(XIMStyles)) return False;
                if (!(p = Xmalloc(alloc_len))) return False;
        ...
        case XimType_XRectangle:
            {
                ...
                if (!(rep = Xmalloc(sizeof(XRectangle)))) return False;
        ...
        case XimType_XPoint:
            {
                ...
                if (!(rep = Xmalloc(sizeof(XPoint)))) return False;
        ...
        case XimType_XFontSet:
            {
                ...
                if (!(base_name = Xmalloc(len + 1))) return False;
        ...
        case XimType_XIMHotKeyTriggers:
            {
                ...
                if (num > (UINT_MAX / sizeof(XIMHotKeyTrigger))) return False;
                if ((2 * sizeof(CARD16) + (num * 3 * sizeof(CARD32))) >
                    data_len)
                    return False;
                alloc_len =
                    sizeof(XIMHotKeyTriggers) + sizeof(XIMHotKeyTrigger) * num;
                if (alloc_len < sizeof(XIMHotKeyTriggers)) return False;
                if (!(p = Xmalloc(alloc_len))) return False;
...
static Bool
_XimDecodeInnerIMATTRIBUTE(Xim im, XIMArg *arg)
{
    XIMResourceList res;
                      ...
                      BITMASK32       mode)
{
    ...
    for (p = arg; p->name; p++)
    {
        if (!(res = _XimGetResourceListRec(res_list, res_num, p->name)))
        {
            if (_XimDecodeInnerIMATTRIBUTE(im, p)) continue;
            return p->name;
        }
        ...
        if (total < min_len) return p->name;
...
static Bool
_XimDecodeInnerICATTRIBUTE(Xic ic, XIMArg *arg, unsigned long mode)
{
    XIMResourceList res;
                      ...
                      BITMASK32       mode)
{
    ...
    for (p = arg; p->name; p++)
    {
        if (!(res = _XimGetResourceListRec(res_list, res_num, p->name)))
        {
            if (_XimDecodeInnerICATTRIBUTE(ic, p, mode)) continue;
            return p->name;
        }
        ...
        if (res->resource_size == XimType_NEST)
        {
            if (res->xrm_name == pre_quark)
            {
                if ((name = _XimDecodeICATTRIBUTE(ic,
                                                  res_list,
                                                  res_num,
                                                  &buf[2],
                                                  buf[1],
...
static Bool
_XimValueToAttribute(XIMResourceList res,
                     XPointer        buf,
                     int             buf_size,
                     XPointer        value,
                     int            *len,
...
static Bool
_XimSetInnerIMAttributes(Xim im, XPointer top, XIMArg *arg, unsigned long mode)
{
    XIMResourceList res;
    int             check;
                      ...
                      unsigned long   mode)
{
    ...
    *ret_len = 0;
    for (p = arg; p->name; p++)
    {
        if (!(res = _XimGetResourceListRec(res_list, res_num, p->name)))
        {
            if (_XimSetInnerIMAttributes(im, top, p, mode)) continue;
            return p->name;
        }
        ...
        if (!(_XimEncodeLocalIMAttr(res, top, p->value))) return p->name;

        buf_s = (CARD16 *)buf;
        if (!(_XimValueToAttribute(res,
                                   (XPointer)&buf_s[2],
                                   (size - min_len),
                                   p->value,
                                   &len,
...
Bool
_XimEncodeSavedIMATTRIBUTE(Xim             im,
                           XIMResourceList res_list,
                           unsigned int    res_num,
                           int            *idx,
                           char           *buf,
                           ...
                           unsigned long   mode)
{
    ...
    *ret_len = 0;
    for (i = *idx; i < num; i++)
    {
        ...
        if (!_XimDecodeLocalIMAttr(res, top, value)) return False;
...
static Bool
_XimEncodeTopValue(Xic ic, XIMResourceList res, XIMArg *p)
{
...
static Bool
_XimEncodePreeditValue(Xic ic, XIMResourceList res, XIMArg *p)
{
    ...
    else if (res->xrm_name == XrmStringToQuark(XNFontSet))
    {
        ...
        list_ret =
            XFontsOfFontSet((XFontSet)p->value, &struct_list, &name_list);
...
static Bool
_XimEncodeStatusValue(Xic ic, XIMResourceList res, XIMArg *p)
{
    if (res->xrm_name == XrmStringToQuark(XNStdColormap))
    {
...
static Bool
_XimSetInnerICAttributes(Xic ic, XPointer top, XIMArg *arg, unsigned long mode)
{
    XIMResourceList res;
    int             check;
                      ...
                      unsigned long   mode)
{
    ...
    *ret_len = 0;
    for (p = arg; p && p->name; p++)
    {
        ...
        if (!(res = _XimGetResourceListRec(res_list, res_num, p->name)))
        {
            if (_XimSetInnerICAttributes(ic, top, p, mode)) continue;
            return p->name;
        }
        ...
        if (mode & XIM_PREEDIT_ATTR)
        {
            if (!(_XimEncodePreeditValue(ic, res, p))) return p->name;
        }
        else if (mode & XIM_STATUS_ATTR)
        {
            if (!(_XimEncodeStatusValue(ic, res, p))) return p->name;
        }
        else
        {
            if (!(_XimEncodeTopValue(ic, res, p))) return p->name;
        }
        ...
        {
#ifdef EXT_MOVE
            if (flag) *flag |= _XimExtenArgCheck(p);
#endif
            if (!(_XimEncodeLocalICAttr(ic, res, top, p, mode))) return p->name;
...
#ifdef XIM_CONNECTABLE
static Bool
_XimEncodeSavedPreeditValue(Xic ic, XIMResourceList res, XPointer value)
{
    int           list_ret;
    XFontStruct **struct_list;
    char        **name_list;
    ...
    if (res->xrm_name == XrmStringToQuark(XNFontSet))
    {
        ...
        list_ret = XFontsOfFontSet((XFontSet)value, &struct_list, &name_list);
        for (i = 0, len = 0; i < list_ret; i++)
        {
            len += (strlen(name_list[i]) + sizeof(char));
        }
...
static Bool
_XimEncodeSavedStatusValue(Xic ic, XIMResourceList res, XPointer value)
{
    int           list_ret;
    XFontStruct **struct_list;
    char        **name_list;
...
Bool
_XimEncodeSavedICATTRIBUTE(Xic             ic,
                           XIMResourceList res_list,
                           unsigned int    res_num,
                           int            *idx,
                           char           *buf,
...
static unsigned int
_XimCountNumberOfAttr(CARD16 total, CARD16 *attr, unsigned int *names_len)
{
    unsigned int n;
    CARD16       len;
...
Bool
_XimGetAttributeID(Xim im, CARD16 *buf)
{
    ...
    if (!(n = _XimCountNumberOfAttr(buf[0], &buf[1], &names_len))) return False;

    if (!(res = Xcalloc(n, sizeof(XIMResource)))) return False;
    ...
    if (!(n = _XimCountNumberOfAttr(buf[0], &buf[2], &names_len))) return False;
