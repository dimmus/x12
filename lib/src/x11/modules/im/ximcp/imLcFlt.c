...
_XimLocalFilter(Display *d, Window w, XEvent *ev, XPointer client_data)
{
    ...
    if (((Xim)ic->core.im)->private.local.top == 0) goto emit_braille;
    ...
    {
        if (prevcode != ev->xkey.keycode) return False;
    ...
    {
        if (IsModifierKey(b[t].keysym)) anymodifier = True;
    ...
    { /* Unmatched */
        ...
        if ((ic->private.local.context ==
             ((Xim)ic->core.im)->private.local.top) ||
