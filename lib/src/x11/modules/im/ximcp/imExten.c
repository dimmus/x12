...
static XIM_QueryExtRec extensions[] = {
    { False, "XIM_EXT_SET_EVENT_MASK", 0, 0, 0, XIM_EXT_SET_EVENT_MASK_IDX },
#ifdef EXT_FORWARD
    { False,
     "XIM_EXT_FORWARD_KEYEVENT",       0,
     0,                                      0,
...
_XimIsSupportExt(int idx)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            if (extensions[i].is_support) return i;
            else break;
        }
    }
    return -1;
...
static Bool
_XimProcExtSetEventMask(Xim im, Xic ic, XPointer buf)
{
    ...
    if (!(_XimProcSyncReply(im, ic))) return False;
    return True;
}

static Bool
_XimExtSetEventMaskCallback(Xim      xim,
                            INT16    len,
                            XPointer data,
                            XPointer call_data)
{
...
#ifdef EXT_FORWARD
static Bool
_XimProcExtForwardKeyEvent(Xim im, Xic ic, XPointer buf)
{
...
static Bool
_XimExtForwardKeyEventCallback(Xim      xim,
                               INT16    len,
                               XPointer data,
                               XPointer call_data)
{
...
static Bool
_XimExtForwardKeyEventCheck(Xim im, INT16 len, XPointer data, XPointer arg)
{
    Xic     ic           = (Xic)arg;
    CARD16 *buf_s        = (CARD16 *)((CARD8 *)data + XIM_HEADER_SIZE);
...
Bool
_XimExtForwardKeyEvent(Xic ic, XKeyEvent *ev, Bool sync)
{
...
static void
_XimSetExtensionList(CARD8 *buf)
{
    register int i;
    int          len;
    int          n = XIMNumber(extensions) - 1;
...
static unsigned int
_XimCountNumberOfExtension(INT16 total, CARD8 *ext)
{
    unsigned int n;
    INT16        len;
    INT16        min_len = sizeof(CARD8) + sizeof(CARD8) + sizeof(INT16);
...
static Bool
_XimParseExtensionList(Xim im, CARD16 *data)
{
...
static Bool
_XimQueryExtensionCheck(Xim im, INT16 len, XPointer data, XPointer arg)
{
    CARD16 *buf_s        = (CARD16 *)((CARD8 *)data + XIM_HEADER_SIZE);
    CARD8   major_opcode = *((CARD8 *)data);
    CARD8   minor_opcode = *((CARD8 *)data + 1);
    ...
    if ((major_opcode == XIM_QUERY_EXTENSION_REPLY) && (minor_opcode == 0) &&
...
_XimExtension(Xim im)
{
    ...
    if (!(len = _XimCheckExtensionListSize())) return True;

    buf_len =
        XIM_HEADER_SIZE + sizeof(CARD16) + sizeof(INT16) + len + XIM_PAD(len);

    if (!(buf = Xmalloc(buf_len))) return False;
...
static Bool
_XimExtMove(Xim im, Xic ic, CARD16 x, CARD16 y)
{
    CARD32  buf32[BUFSIZE / 4];
    CARD8  *buf   = (CARD8 *)buf32;
    ...
    if ((idx = _XimIsSupportExt(XIM_EXT_MOVE_IDX)) < 0) return False;
...
BITMASK32
_XimExtenArgCheck(XIMArg *arg)
{
    CARD32 flag = 0L;
    if (!strcmp(arg->name, XNSpotLocation)) SET_EXT_XNSPOTLOCATION(flag);
    return flag;
}
