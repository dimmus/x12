...
modmask(char *name)
{
    ...
    for (i = 0; i < num_entries; i++)
        if (!strcmp(name, tbl[i].name)) return tbl[i].mask;
...
TransFileName(Xim im, char *name)
{
    ...
    while (*i)
    {
        if (*i == '%')
        {
            ...
            switch (*i)
            {
                ...
                case 'L':
                    ...
                    break;
                case 'S':
                    if (dir[0] == '\0') xlocaledir(dir, XLC_BUFSIZE);
                    if (dir[0])
                    {
        ...
        i++;
        if (l > PATH_MAX) /* your expanded path length is ridiculous */
            goto end;
    }
...
static int
parseline(FILE *fp, Xim im, char *tokenbuf, int depth)
{
    int          token;
    ...
    do
    {
        if ((token == KEY) && (strcmp("include", tokenbuf) == 0))
        {
            ...
            FILE *infp;
            token = nexttoken(fp, tokenbuf, &lastch);
            if (token != KEY && token != STRING) goto error;
            if (++depth > 100) goto error;
            if ((filename = TransFileName(im, tokenbuf)) == NULL) goto error;
            infp = _XFopenFile(filename, "r");
            Xfree(filename);
            if (infp == NULL) goto error;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        else
        {
		   ...
		   (and crashes on architectures that track pointer bounds). */
                if (old && top >= (DTIndex *)old &&
                    top < (DTIndex *)&old[oldsize])
...
void
_XimParseStringFile(FILE *fp, Xim im)
{
    parsestringfile(fp, im, 0);
}

static void
parsestringfile(FILE *fp, Xim im, int depth)
{
    char        tb[8192];
    char       *tbp;
    struct stat st;
