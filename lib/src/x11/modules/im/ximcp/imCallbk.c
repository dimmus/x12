...
typedef XimCbStatus (*XimCb)(Xim, Xic, char *, int);
...
#define PACKET_TO_ICID(p) \
    (*(XICID *)((CARD8 *)(p) + XIM_HEADER_SIZE + sz_XIMID))
...
_XimProcessPendingCallbacks(Xic ic)
{
    ...
    while (
        ((pcbq = ic->private.proto.pend_cb_que) != (XimPendingCallback)NULL) &&
...
Bool
_XimCbDispatch(Xim xim, INT16 len, XPointer data, XPointer call_data)
{
	 ...
	 */
        ...
        {
            if (proto_len > 0) memcpy(proto_buf, proto, proto_len);
...
static XimCbStatus
_XimGeometryCallback(Xim im, Xic ic, char *proto, int len)
{
...
static XimCbStatus
_XimStrConversionCallback(Xim im, Xic ic, char *proto, int len)
{
    ...
    if (cb && cb->callback)
    {
        ...
        cbrec.position = (XIMStringConversionPosition) * (CARD32 *)&proto[p];
        p += sz_CARD32;
        cbrec.direction = (XIMCaretDirection) * (CARD32 *)&proto[p];
        p += sz_CARD32;
        cbrec.operation = (XIMStringConversionOperation) * (CARD32 *)&proto[p];
        p += sz_CARD32;
        cbrec.factor = (unsigned short)*(CARD32 *)&proto[p];
	 ...
	 */
        {
            ...
            blen = XIM_HEADER_SIZE + sz_CARD16 + 2 + length_in_bytes +
                   XIM_PAD(2 + length_in_bytes) + 2 + 2 +
                   sz_CARD32 * cbrec.text->length;
            buf     = Xmalloc(blen);
            buf_len = (INT16)blen;
        }
        ...
        p                  = XIM_HEADER_SIZE;
        *(CARD16 *)&buf[p] = (CARD16)im->private.proto.imid;
        p += sz_CARD16;
        *(CARD16 *)&buf[p] = (CARD16)ic->private.proto.icid;
        p += sz_CARD16;
        *(CARD16 *)&buf[p] = (CARD16)cbrec.text->length;
        p += sz_CARD16;
...
static XimCbStatus
_XimPreeditStartCallback(Xim im, Xic ic, char *proto, int len)
{
...
static XimCbStatus
_XimPreeditDoneCallback(Xim im, Xic ic, char *proto, int len)
{
...
static void
_read_text_from_packet(Xim im, char *buf, XIMText **text_ptr)
{
    int      status;
    XIMText *text;
    int      tmp_len;
    ...
    *text_ptr = text = Xmalloc(sizeof(XIMText));
    ...
    if ((tmp_buf = Xmalloc(tmp_len + 1)))
    {
        ...
        if (s != XLookupNone)
        {
            ...
            if ((text->string.multi_byte = Xmalloc(
...
static XimCbStatus
_XimPreeditDrawCallback(Xim im, Xic ic, char *proto, int len)
{
    ...
    if (cb && cb->callback)
    {
        cbs.caret = (int)*(INT32 *)proto;
        proto += sz_INT32;
        cbs.chg_first = (int)*(INT32 *)proto;
        proto += sz_INT32;
        cbs.chg_length = (int)*(INT32 *)proto;
        proto += sz_INT32;
...
static XimCbStatus
_XimPreeditCaretCallback(Xim im, Xic ic, char *proto, int len)
{
    ...
    if (cb && cb->callback)
    {
        cbs.position = (int)*(INT32 *)proto;
        proto += sz_INT32;
        cbs.direction = (XIMCaretDirection) * (CARD32 *)proto;
        proto += sz_CARD32;
        cbs.style = (XIMCaretStyle) * (CARD32 *)proto;
        proto += sz_CARD32;
...
static XimCbStatus
_XimStatusStartCallback(Xim im, Xic ic, char *proto, int len)
{
...
static XimCbStatus
_XimStatusDoneCallback(Xim im, Xic ic, char *proto, int len)
{
...
static XimCbStatus
_XimStatusDrawCallback(Xim im, Xic ic, char *proto, int len)
{
    ...
    if (cb && cb->callback)
    {
        cbs.type = (XIMStatusDataType) * (CARD32 *)proto;
        proto += sz_CARD32;
