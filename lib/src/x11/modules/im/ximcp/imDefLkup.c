...
Xic
_XimICOfXICID(Xim im, XICID icid)
{
    Xic pic;

    for (pic = (Xic)im->core.ic_chain; pic; pic = (Xic)pic->core.next)
    {
        if (pic->private.proto.icid == icid) return pic;
    }
    return (Xic)0;
}

static void
_XimProcIMSetEventMask(Xim im, XPointer buf)
{
...
static void
_XimProcICSetEventMask(Xic ic, XPointer buf)
{
    EVENTMASK *buf_l = (EVENTMASK *)buf;
...
Bool
_XimSetEventMaskCallback(Xim xim, INT16 len, XPointer data, XPointer call_data)
{
    ...
    if (imid == im->private.proto.imid)
    {
        if (icid)
        {
            if (!(ic = _XimICOfXICID(im, icid))) return False;
...
static Bool
_XimSyncCheck(Xim im, INT16 len, XPointer data, XPointer arg)
{
    Xic     ic           = (Xic)arg;
    CARD16 *buf_s        = (CARD16 *)((CARD8 *)data + XIM_HEADER_SIZE);
    ...
    if ((major_opcode == XIM_SYNC_REPLY) && (minor_opcode == 0) &&
...
Bool
_XimProcSyncReply(Xim im, Xic ic)
{
    CARD32  buf32[BUFSIZE / 4];
    CARD8  *buf   = (CARD8 *)buf32;
    CARD16 *buf_s = (CARD16 *)&buf[XIM_HEADER_SIZE];
...
Bool
_XimSyncCallback(Xim xim, INT16 len, XPointer data, XPointer call_data)
{
    CARD16 *buf_s = (CARD16 *)((CARD8 *)data + XIM_HEADER_SIZE);
    XIMID   imid  = buf_s[0];
    XICID   icid  = buf_s[1];
    ...
    if ((imid == im->private.proto.imid) && (ic = _XimICOfXICID(im, icid)))
    {
...
static INT16
_XimSetEventToWire(XEvent *ev, xEvent *event)
{
    if (!(_XimProtoEventToWire(ev, event, False))) return 0;
...
static Bool
_XimForwardEventCore(Xic ic, XEvent *ev, Bool sync)
{
    ...
    bzero(buf32,
          sizeof(buf32)); /* valgrind noticed uninitialized memory use! */
...
Bool
_XimForwardEvent(Xic ic, XEvent *ev, Bool sync)
{
#ifdef EXT_FORWARD
    if (((ev->type == KeyPress) || (ev->type == KeyRelease)))
        if (_XimExtForwardKeyEvent(ic, (XKeyEvent *)ev, sync)) return True;
#endif
    return _XimForwardEventCore(ic, ev, sync);
}

Bool
_XimFabricateSerial(Xim im, XKeyEvent *event)
{
    ...
    {
        fprintf(stderr,
                "%s,%d: The key event is already fabricated.\n",
                __FILE__,
                __LINE__);
        return False;
    }
    if (im->private.proto.fabricated_serial)
        fprintf(stderr,
                "%s,%d: Tried to fabricate a wrong key event.\n",
                __FILE__,
                __LINE__);
...
Bool
_XimUnfabricateSerial(Xim im, Xic ic, XKeyEvent *event)
{
    ...
    {
        fprintf(stderr,
                "%s,%d: The key event is already unfabricated.\n",
                __FILE__,
                __LINE__);
        return False;
    }
    if (event->serial != im->private.proto.fabricated_serial)
        fprintf(stderr,
                "%s,%d: Tried to unfabricate a wrong key event.\n",
                __FILE__,
                __LINE__);
...
Bool
_XimIsFabricatedSerial(Xim im, XKeyEvent *event)
{
    /* GTK2 XIM module sets serial=0. */
    if (!event->serial || !im->private.proto.enable_fabricated_order)
        return IS_FABRICATED(im) ? True : False;
    if (event->serial == im->private.proto.fabricated_serial) return True;
    if (!im->private.proto.fabricated_serial) return False;
    /* Rotate time */
    if (event->time < im->private.proto.fabricated_time)
    {
        if (event->time >= 1000) im->private.proto.fabricated_time = 0;
    }
...
static void
_XimProcEvent(Display *d, Xic ic, XEvent *ev, CARD16 *buf)
{
    INT16   serial = buf[0];
...
static Bool
_XimForwardEventRecv(Xim im, Xic ic, XPointer buf)
{
...
Bool
_XimForwardEventCallback(Xim xim, INT16 len, XPointer data, XPointer call_data)
{
    CARD16 *buf_s = (CARD16 *)((CARD8 *)data + XIM_HEADER_SIZE);
    XIMID   imid  = buf_s[0];
    XICID   icid  = buf_s[1];
...
static Bool
_XimRegisterTriggerkey(Xim im, XPointer buf)
{
...
Bool
_XimRegisterTriggerKeysCallback(Xim      xim,
                                INT16    len,
                                XPointer data,
                                XPointer call_data)
{
...
static Bool
_XimTriggerNotifyCheck(Xim im, INT16 len, XPointer data, XPointer arg)
{
    Xic     ic           = (Xic)arg;
    CARD16 *buf_s        = (CARD16 *)((CARD8 *)data + XIM_HEADER_SIZE);
    CARD8   major_opcode = *((CARD8 *)data);
    ...
    if ((major_opcode == XIM_TRIGGER_NOTIFY_REPLY) && (minor_opcode == 0) &&
        (imid == im->private.proto.imid) && (icid == ic->private.proto.icid))
        return True;
    if ((major_opcode == XIM_ERROR) && (minor_opcode == 0) &&
        (buf_s[2] & XIM_IMID_VALID) && (imid == im->private.proto.imid) &&
                  ...
                  int     keysym_len)
{
    ...
    if (!(info = Xmalloc(sizeof(XimCommitInfoRec)))) return False;
    info->string                  = string;
...
static void
_XimUnregRealCommitInfo(Xic ic, Bool reverse)
{
    XimCommitInfo info;
    ...
    Xfree(info->string);
    Xfree(info->keysym);
    if (prev_info) prev_info->next = info->next;
    else ic->private.proto.commit_info = info->next;
    Xfree(info);
    return;
}

static void
_XimUnregCommitInfo(Xic ic)
{
    _XimUnregRealCommitInfo(ic, False);
}

static void
_XimUnregFirstCommitInfo(Xic ic)
{
    _XimUnregRealCommitInfo(ic, True);
}

void
_XimFreeCommitInfo(Xic ic)
{
...
static Bool
_XimProcKeySym(Xic ic, CARD32 sym, KeySym **xim_keysym, int *xim_keysym_len)
{
    Xim im = (Xim)ic->core.im;
...
static Bool
_XimCommitRecv(Xim im, Xic ic, XPointer buf)
{
    CARD16   *buf_s = (CARD16 *)buf;
    ...
    bzero(
        &ev,
        sizeof(
            ev)); /* uninitialized : found when running kterm under valgrind */
...
Bool
_XimCommitCallback(Xim xim, INT16 len, XPointer data, XPointer call_data)
{
    CARD16 *buf_s = (CARD16 *)((CARD8 *)data + XIM_HEADER_SIZE);
    XIMID   imid  = buf_s[0];
    XICID   icid  = buf_s[1];
...
void
_XimProcError(Xim im, Xic ic, XPointer data)
{
    return;
}

Bool
_XimErrorCallback(Xim xim, INT16 len, XPointer data, XPointer call_data)
{
    CARD16   *buf_s = (CARD16 *)((CARD8 *)data + XIM_HEADER_SIZE);
    ...
    {
        imid = buf_s[0];
        if (imid != im->private.proto.imid) return False;
    }
...
int
_XimProtoMbLookupString(XIC        xic,
                        XKeyEvent *ev,
                        char      *buffer,
                        int        bytes,
                        KeySym    *keysym,
                        Status    *state)
{
    ...
    if (!IS_SERVER_CONNECTED(im)) return 0;
    ...
    { /* Filter function */
        ...
        ret = im->methods->ctstombs((XIM)im,
                                    info->string,
                                    info->string_len,
                                    buffer,
                                    bytes,
                                    state);
        if (*state == XBufferOverflow) return ret;
        if (keysym && (info->keysym && *(info->keysym)))
        {
            *keysym = *(info->keysym);
            if (*state == XLookupChars) *state = XLookupBoth;
            else *state = XLookupKeySym;
        }
    ...
    else if (ev->type == KeyPress)
    {
        ...
        if (ret > 0)
        {
            if (ret > bytes) *state = XBufferOverflow;
            else if (keysym && *keysym != NoSymbol) *state = XLookupBoth;
            else *state = XLookupChars;
        }
        else
        {
            if (keysym && *keysym != NoSymbol) *state = XLookupKeySym;
            else *state = XLookupNone;
        }
    }
    else
    {
...
int
_XimProtoWcLookupString(XIC        xic,
                        XKeyEvent *ev,
                        wchar_t   *buffer,
                        int        bytes,
                        KeySym    *keysym,
                        Status    *state)
{
    ...
    { /* Filter function */
        ...
        ret = im->methods->ctstowcs((XIM)im,
                                    info->string,
                                    info->string_len,
                                    buffer,
                                    bytes,
...
int
_XimProtoUtf8LookupString(XIC        xic,
                          XKeyEvent *ev,
                          char      *buffer,
                          int        bytes,
                          KeySym    *keysym,
                          Status    *state)
{
    ...
    { /* Filter function */
        ...
        ret = im->methods->ctstoutf8((XIM)im,
                                     info->string,
                                     info->string_len,
                                     buffer,
                                     bytes,
