                         ...
                         XPointer call_data)
{
    ...
    if (!(rec = Xmalloc(sizeof(XimProtoIntrRec)))) return False;
    rec->func                   = proc;
...
static Bool
_XimTransportIntr(Xim im, INT16 len, XPointer data, XPointer call_data)
{
    ...
    for (; rec; rec = rec->next)
    {
        if ((major_opcode == (CARD8)rec->major_code) &&
            ...
            if ((*rec->func)(call_im, len, data, rec->call_data)) return True;
    }
    return False;
}
