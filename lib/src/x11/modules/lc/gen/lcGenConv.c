...
static int mbstocts(XlcConv   conv,
                    XPointer *from,
                    int      *from_left,
                    XPointer *to,
                    int      *to_left,
                    XPointer *args,
                    int       num_args);
static int ctstombs(XlcConv   conv,
                    XPointer *from,
                    int      *from_left,
                    XPointer *to,
                    int      *to_left,
                    XPointer *args,
                    int       num_args);
static int cstombs(XlcConv   conv,
                   XPointer *from,
                   int      *from_left,
                   XPointer *to,
                   int      *to_left,
...
static int
compare(const char *src, const char *encoding, int length)
{
    const char *start = src;

    while (length-- > 0)
    {
        if (*src++ != *encoding++) return 0;
        if (*encoding == '\0') return src - start;
    }
...
gi_to_mb(unsigned long glyph_index, CodeSet codeset)
{
    ...
    if (codeset->mbconv) return (conv_to_source(codeset->mbconv, glyph_index));
...
static CodeSet
mb_parse_codeset(State state, int num, const char **inbufptr, int *from_left)
{
    int         len;
    ...
    for (--num; (parse_info = mb_parse_list[num]) != NULL; num++)
    {
        ...
        if (len > 0)
        {
            ...
            if (parse_info->type == E_LSL) state->GL_codeset = codeset;
            else if (parse_info->type == E_LSR) state->GR_codeset = codeset;
            --len;
            *inbufptr += len;
            *from_left -= len;
            return codeset;
...
static CodeSet
byteM_parse_codeset(XLCd lcd, const char *inbufptr)
{
    unsigned char   ch;
    CodeSet         codeset;
    ByteInfoList    byteM;
    ...
    for (i = 0; i < codeset_num; i++)
    {
        codeset = codeset_list[i];
        byteM   = codeset->byteM;
        if (codeset->side != XlcNONE || byteM == NULL) continue;
...
#define GLGR_parse_codeset(ch) \
    (isrightside(ch) ? (state->GR_codeset) : (state->GL_codeset))
...
gi_parse_charset(unsigned long glyph_index, CodeSet codeset)
{
    ...
    for (i = 0; i < num_charsets; i++)
    {
        ...
        if (*charset->ct_sequence != '\0') break;
    }
    if (i >= num_charsets) return (NULL);

    if (charset->source != CSsrcStd) return (charset);

    if (!ctextseg) return (charset);
    ...
    for (i = 0; i < area_num; i++)
    {
        if (area[i].start <= glyph_index && glyph_index <= area[i].end)
        {
            ...
            if (*charset->ct_sequence == '\0') return (NULL);
...
static Bool
ct_parse_csi(const char *inbufptr, int *ctr_seq_len)
{
    int i;
    ...
    for (i = 0; i < num; i++)
    {
        if (!(*ctr_seq_len = strlen(directionality_data[i].encoding))) continue;

        if (strncmp(inbufptr, directionality_data[i].encoding, *ctr_seq_len) ==
            0)
            return (True);
    }
...
static int
cmp_esc_sequence(const char *inbufptr, XlcCharSet charset)
{
    ...
    if (((byte_m - 128) * 128 + (byte_l - 128) - 1) < name_len) return (0);
...
segment_conversion(XLCd lcd, XlcCharSet *charset, unsigned long *glyph_index)
{
    ...
    if (!segment_conv) return (True);

    for (i = 0; i < segment_conv_num; i++)
    {
        if (segment_conv[i].source == *charset) break;
    }

    if (i >= segment_conv_num) return (True);

    range = segment_conv[i].range;
    if (*glyph_index < range.start || range.end < *glyph_index) return (True);
...
static CodeSet
_XlcGetCodeSetFromName(XLCd lcd, const char *name)
{
    int         i, j;
    XlcCharSet  charset;
    int         num_charsets;
    ...
    for (i = 0; i < codeset_num; i++)
    {
        ...
        for (j = 0; j < num_charsets; j++)
        {
            ...
            if (!strlen(charset->name)) continue;
            if (strcmp(charset->name, name) == 0) return (codeset);
        }
    }
...
static Bool
_XlcGetCodeSetFromCharSet(XLCd           lcd,
                          XlcCharSet     charset,
                          CodeSet       *codeset,
...
static void
output_ulong_value(char         *outbufptr,
                   unsigned long code,
                   int           length,
                   XlcSide       side)
{
             ...
             int       num_args)
{
    ...
    while (*from_left && *to_left)
    {
        ...
        /* same mb char data */
        if (len_left) goto output_one_wc;
        ...
        if ((codeset = GLGR_parse_codeset(ch))) goto next_mb_char;
              ...
              int       num_args)
{
    ...
    unconv_num1 = stdc_wcstombs(conv,
                                ...
                                args,
                                num_args);
    if (unconv_num1 < 0) goto ret;

    buf_left2 = buf_ptr1 - buf_ptr2;

    unconv_num2 =
        mbstocts(conv, &buf_ptr2, &buf_left2, to, to_left, args, num_args);
    if (unconv_num2 < 0) goto ret;
         ...
         int       num_args)
{
    ...
    while (*from_left && *to_left)
    {
        ...
        if (gi_len_left) goto output_one_wc;

        /* control sequence ? */
        if (ch == CSI)
        {
            if (!ct_parse_csi(inbufptr - 1, &ctr_seq_len)) goto skip_the_seg;
        ...
        if (ch == ESC)
        {
            ...
            if (state->charset->side == XlcC0 || state->charset->side == XlcGL)
            {
        ...
        /* check current state */
        if (isleftside(ch)) state->charset = state->GL_charset;
        else state->charset = state->GR_charset;
...
output_one_wc:
        ...
        else glyph_index = (glyph_index << 8) | ch;
        ...
        if (!gi_len_left)
        {
            ...
            /* get codeset */
            if (!_XlcGetCodeSetFromCharSet(lcd,
                                           charset_tmp,
                                           &codeset,
                                           &glyph_index))
            {
              ...
              int       num_args)
{
    ...
    unconv_num1 =
        ctstombs(conv, from, from_left, &buf_ptr1, &buf_left1, args, num_args);
    if (unconv_num1 < 0) goto ret;

    buf_left2 = buf_ptr1 - buf_ptr2;

    unconv_num2 =
        stdc_mbstowcs(conv, &buf_ptr2, &buf_left2, to, to_left, args, num_args);
    if (unconv_num2 < 0) goto ret;
             ...
             int       num_args)
{
    ...
    unconv_num1 =
        cstombs(conv, from, from_left, &buf_ptr1, &buf_left1, args, num_args);
    if (unconv_num1 < 0) goto ret;
         ...
         int       num_args)
{
    ...
    unconv_num2 +=
        wcstocts(conv, &buf_ptr2, &buf_left2, to, to_left, args, num_args);
    if (unconv_num2 < 0) goto ret;
         ...
         int       num_args)
{
    ...
    while (*from_left && *to_left)
    {
        ...
        /* same mb char data */
        if (len_left) goto output_one_mb;
       ...
       int       num_args)
{
    ...
    while (*from_left && *to_left)
    {
        ...
        {
            unconv_num = 1;
            if (len_left) unconv_num += (length - len_left);
            break;
        }

        /* same mb char data */
        if (len_left) goto output;
...
output:
    ...
    *to   = (XPointer)outbufptr;
    ...
    if (num_args > 0) *((XlcCharSet *)args[0]) = charset;

    return 0;
}

static int
mbstocs(XlcConv   conv,
        XPointer *from,
        int      *from_left,
        XPointer *to,
        int      *to_left,
        ...
        int       num_args)
{
    ...
    if (num_args > 0) *((XlcCharSet *)args[0]) = charset_old;
              ...
              int       num_args)
{
    ...
    unconv_num2 =
        mbstostr(conv, &buf_ptr2, &buf_left2, to, to_left, args, num_args);
    if (unconv_num2 < 0) goto ret;
         ...
         int       num_args)
{
    ...
    unconv_num1 =
        ctstowcs(conv, from, from_left, &buf_ptr1, &buf_left1, args, num_args);
    if (unconv_num1 < 0) goto ret;

    buf_left2 = (buf_ptr1 - buf_ptr2) / sizeof(wchar_t);

    unconv_num2 +=
        wcstombs_org(conv, &buf_ptr2, &buf_left2, to, to_left, args, num_args);
    if (unconv_num2 < 0) goto ret;
        ...
        int       num_args)
{
    ...
    unconv_num1 =
        cstowcs(conv, from, from_left, &buf_ptr1, &buf_left1, args, num_args);
    if (unconv_num1 < 0) goto ret;
              ...
              int       num_args)
{
    ...
    unconv_num1 =
        strtombs(conv, from, from_left, &buf_ptr1, &buf_left1, args, num_args);
    if (unconv_num1 < 0) goto ret;
...
static XlcConvMethodsRec mbstocts_methods = { close_converter, mbstocts, NULL };

static XlcConv
open_mbstocts(XLCd        from_lcd,
...
static XlcConvMethodsRec mbstostr_methods = { close_converter, mbstostr, NULL };

static XlcConv
open_mbstostr(XLCd        from_lcd,
              const char *from_type,
              XLCd        to_lcd,
              const char *to_type)
{
...
static XlcConvMethodsRec mbstocs_methods = { close_converter, mbstocs, NULL };

static XlcConv
open_mbstocs(XLCd        from_lcd,
             const char *from_type,
             XLCd        to_lcd,
             const char *to_type)
{
...
static XlcConvMethodsRec mbtocs_methods = { close_converter, mbtocs, NULL };

static XlcConv
open_mbtocs(XLCd        from_lcd,
            const char *from_type,
            XLCd        to_lcd,
            const char *to_type)
{
...
static XlcConvMethodsRec ctstombs_methods = { close_converter, ctstombs, NULL };

static XlcConv
open_ctstombs(XLCd        from_lcd,
              const char *from_type,
              XLCd        to_lcd,
              const char *to_type)
{
...
static XlcConvMethodsRec cstombs_methods = { close_converter, cstombs, NULL };

static XlcConv
open_cstombs(XLCd        from_lcd,
             const char *from_type,
             XLCd        to_lcd,
             const char *to_type)
{
...
static XlcConvMethodsRec strtombs_methods = { close_converter, strtombs, NULL };

static XlcConv
open_strtombs(XLCd        from_lcd,
              const char *from_type,
              XLCd        to_lcd,
              const char *to_type)
{
...
static XlcConvMethodsRec stdc_mbstowcs_methods = { close_converter,
                                                   stdc_mbstowcs,
                                                   NULL };

static XlcConv
open_stdc_mbstowcs(XLCd        from_lcd,
                   const char *from_type,
                   XLCd        to_lcd,
                   const char *to_type)
{
...
static XlcConvMethodsRec stdc_wcstombs_methods = { close_converter,
                                                   stdc_wcstombs,
                                                   NULL };

static XlcConv
open_stdc_wcstombs(XLCd        from_lcd,
                   const char *from_type,
                   XLCd        to_lcd,
                   const char *to_type)
{
...
static XlcConvMethodsRec stdc_wcstocts_methods = { close_converter,
                                                   stdc_wcstocts,
                                                   NULL };

static XlcConv
open_stdc_wcstocts(XLCd        from_lcd,
                   const char *from_type,
                   XLCd        to_lcd,
                   const char *to_type)
{
...
static XlcConvMethodsRec stdc_wcstostr_methods = { close_converter,
                                                   stdc_wcstostr,
                                                   NULL };

static XlcConv
open_stdc_wcstostr(XLCd        from_lcd,
                   const char *from_type,
                   XLCd        to_lcd,
                   const char *to_type)
{
...
static XlcConvMethodsRec stdc_wcstocs_methods = { close_converter,
                                                  stdc_wcstocs,
                                                  NULL };

static XlcConv
open_stdc_wcstocs(XLCd        from_lcd,
                  const char *from_type,
                  XLCd        to_lcd,
                  const char *to_type)
{
...
static XlcConvMethodsRec stdc_wctocs_methods = { close_converter,
                                                 stdc_wctocs,
                                                 NULL };

static XlcConv
open_stdc_wctocs(XLCd        from_lcd,
                 const char *from_type,
                 XLCd        to_lcd,
                 const char *to_type)
{
...
static XlcConvMethodsRec stdc_ctstowcs_methods = { close_converter,
                                                   stdc_ctstowcs,
                                                   NULL };

static XlcConv
open_stdc_ctstowcs(XLCd        from_lcd,
                   const char *from_type,
                   XLCd        to_lcd,
                   const char *to_type)
{
...
static XlcConvMethodsRec stdc_cstowcs_methods = { close_converter,
                                                  stdc_cstowcs,
                                                  NULL };

static XlcConv
open_stdc_cstowcs(XLCd        from_lcd,
                  const char *from_type,
                  XLCd        to_lcd,
                  const char *to_type)
{
...
static XlcConvMethodsRec stdc_strtowcs_methods = { close_converter,
                                                   stdc_strtowcs,
                                                   NULL };

static XlcConv
open_stdc_strtowcs(XLCd        from_lcd,
                   const char *from_type,
                   XLCd        to_lcd,
                   const char *to_type)
{
...
static XlcConvMethodsRec mbstowcs_methods = { close_converter,
                                              mbstowcs_org,
                                              NULL };

static XlcConv
open_mbstowcs(XLCd        from_lcd,
              const char *from_type,
              XLCd        to_lcd,
              const char *to_type)
{
...
static XlcConvMethodsRec wcstombs_methods = { close_converter,
                                              wcstombs_org,
                                              NULL };

static XlcConv
open_wcstombs(XLCd        from_lcd,
              const char *from_type,
              XLCd        to_lcd,
              const char *to_type)
{
...
static XlcConvMethodsRec wcstocts_methods = { close_converter, wcstocts, NULL };

static XlcConv
open_wcstocts(XLCd        from_lcd,
              const char *from_type,
              XLCd        to_lcd,
              const char *to_type)
{
...
static XlcConvMethodsRec wcstocs_methods = { close_converter, wcstocs, NULL };
...
static XlcConvMethodsRec wctocs_methods = { close_converter, wctocs, NULL };

static XlcConv
open_wctocs(XLCd        from_lcd,
            const char *from_type,
            XLCd        to_lcd,
            const char *to_type)
{
...
static XlcConvMethodsRec ctstowcs_methods = { close_converter, ctstowcs, NULL };

static XlcConv
open_ctstowcs(XLCd        from_lcd,
              const char *from_type,
              XLCd        to_lcd,
              const char *to_type)
{
...
static XlcConvMethodsRec cstowcs_methods = { close_converter, cstowcs, NULL };
...
XLCd
_XlcGenericLoader(const char *name)
{
    XLCd lcd;
#ifdef STDCVT
    XLCdGenericPart *gen;
