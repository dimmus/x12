...
static Bool
MBtoWCdef(State state, const char *ch, wchar_t *wc)
{
    wchar_t wc_encoding;
...
#ifdef STDCVT
static Bool
MBtoWCstd(State state, const char *ch, wchar_t *wc)
{
...
static Bool
WCtoMBdef(State state, wchar_t wc, char *ch)
{
...
#ifdef STDCVT
static Bool
WCtoMBstd(State state, wchar_t wc, char *ch)
{
...
static XlcCharSet
get_charset(State state, char side)
{
    ...
    if (codeset)
    {
        ...
        for (i = 0; i < codeset->num_charsets; i++)
        {
            ...
            if (*charset->ct_sequence != '\0') return charset;
        }
        ...
        int       num_args)
{
    ...
    while (*from_left)
    {
        if ((found = state->WCtoMB(state, *src, ch))) break;
        unconv++;
        src++;
        (*from_left)--;
    }
...
#ifdef STDCVT
    if (XLC_GENERIC(lcd, use_stdc_env) == True) state->MBtoWC = &MBtoWCstd;
    else
#endif
        state->MBtoWC = &MBtoWCdef;

#ifdef STDCVT
    if (XLC_GENERIC(lcd, use_stdc_env) == True) state->WCtoMB = &WCtoMBstd;
    else
#endif
...
XLCd
_XlcDefaultLoader(const char *name)
{
    XLCd lcd;
