...
/* for VW/UDC start */
static Bool
ismatch_scopes(FontData fontdata, unsigned long *value, Bool is_shift)
{
    register int scopes_num = fontdata->scopes_num;
    FontScope    scopes     = fontdata->scopes;
    if (!scopes_num) return False;

    if (fontdata->font == NULL) return False;
...
static int
check_vertical_fonttype(char *name)
{
    char *ptr;
    int   type = 0;

    if (name == (char *)NULL || (int)strlen(name) <= 0) return False;

    /* Obtains the pointer of CHARSET_ENCODING_FIELD. */
    if ((ptr = strchr(name, '-')) == (char *)NULL) return False;
    ptr++;

    /* Obtains the pointer of vertical_map font type. */
    if ((ptr = strchr(ptr, '.')) == (char *)NULL) return False;
    ptr++;
...
FontData
_XomGetFontDataFromFontSet(FontSet        fs,
                           unsigned char *str,
                           int            len,
                           int           *len_ret,
                           int            is2b,
        ...
        for (i = 0; i < num; i++)
        {
            ...
            else if (type == VMAP)
            {
                if (fontdata[i].font)
                {
                    ...
                    {
                        if (fontdata[i].scopes_num <= 0) break;
...
static FontSet
_XomGetFontSetFromCharSet(XOC oc, XlcCharSet charset)
{
    ...
    for (; num-- > 0; font_set++)
    {
        ...
        for (; charset_count-- > 0; charset_list++)
            if (*charset_list == charset) return font_set;
    }
...
static void
shift_to_gl(register char *text, register int length)
{
    while (length-- > 0)
        *text++ &= 0x7f;
}
...
static void
shift_to_gr(register char *text, register int length)
{
    while (length-- > 0)
        *text++ |= 0x80;
}
...
{
    font_set->font =
        XLoadQueryFont(oc->core.om->core.display,
            ...
            int       num_args)
{
    ...
    if (font_set == NULL) return -1;

    if (font_set->font == NULL && load_font(oc, font_set) == False) return -1;
    ...
    if (font_set->is_xchar2b) length >>= 1;
    *to = cs;
    *to_left -= length;
...
XlcConv
_XomInitConverter(XOC oc, XOMTextType type)
{
