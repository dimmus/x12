...
init_fontdata(FontData font_data, int font_data_count)
{
    ...
    if (fd == (FontData)NULL) return False;
...
static VRotate
init_vrotate(FontData  font_data,
             int       font_data_count,
             int       type,
             ...
             int       code_range_num)
{
    ...
    if (vrotate == (VRotate)NULL) return False;
...
init_fontset(XOC oc)
{
    ...
    for (; count-- > 0; data++, font_set++)
    {
        ...
        if ((font_set->font_data =
                 init_fontdata(data->font_data, data->font_data_count)) == NULL)
            goto err;
        font_set->font_data_count = data->font_data_count;
        if ((font_set->substitute =
                 init_fontdata(data->substitute, data->substitute_num)) == NULL)
            goto err;
        font_set->substitute_num = data->substitute_num;
        if ((font_set->vmap = init_fontdata(data->vmap, data->vmap_num)) ==
            NULL)
            goto err;
...
get_prop_name(Display *dpy, XFontStruct *fs)
{
    ...
    if (XGetFontProperty(fs, XA_FONT, &fp)) return XGetAtomName(dpy, fp);
...
load_font(XOC oc)
{
    ...
    for (; num-- > 0; font_set++)
    {
        if (font_set->font_name == NULL) continue;

        if (load_fontset_data(oc, font_set) != True) return False;
#ifndef TESTVERSION
        if (load_fontdata(oc, font_set->font_data, font_set->font_data_count) !=
            True)
            return False;

        if (load_fontdata(oc, font_set->substitute, font_set->substitute_num) !=
            True)
            return False;
#endif
...
/* Add 1996.05.20 */
        ...
        else font_set->is_xchar2b = False;
    }
...
load_font_info(XOC oc)
{
    ...
    for (; num-- > 0; font_set++)
    {
        ...
        {
            fn_list = XListFontsWithInfo(dpy,
                                         font_set->font_name,
                                         1,
                                         &fn_num,
                                         &font_set->info);
            if (font_set->info == NULL) return False;
...
static void
check_fontset_extents(XCharStruct *overall,
                      int         *logical_ascent,
                      int         *logical_descent,
                      XFontStruct *font)
{
...
init_core_part(XOC oc)
{
    ...
    if (font_struct_list == NULL) return False;

    font_name_list = Xmalloc(sizeof(char *) * count);
    if (font_name_list == NULL) goto err;

    font_name_buf = Xmalloc(length);
    if (font_name_buf == NULL) goto err;
    ...
    for (count = 0; font_set_num-- > 0; font_set++)
    {
        ...
        font_set->id = count;
        if (font_set->font) *font_struct_list++ = font_set->font;
        else *font_struct_list++ = font_set->info;
...
get_rotate_fontname(char *font_name)
{
    ...
    if (field_num < CHARSET_ENCODING_FIELD) goto free_pattern;
    ...
    len = 0;
    for (field_num = 0; field_num < CHARSET_ENCODING_FIELD && fields[field_num];
         field_num++)
    {
    ...
    if (len > XLFD_MAX_LEN) goto free_pattern;

    rotate_font_ptr = Xmalloc(len + 1);
    if (!rotate_font_ptr) goto free_pattern;
...
is_match_charset(FontData font_data, char *font_name)
{
    ...
    if (length > name_len) return False;

    if (_XlcCompareISOLatin1(last - length, font_data->name) == 0) return True;
...
{
#ifdef OLDCODE
    if (is_match_charset(font_data, pattern) != True) return False;

    font_data->xlfd_name = strdup(pattern);
    if (font_data->xlfd_name == NULL) return (-1);
	 ...
	 */
        if ((fn_list = XListFontsWithInfo(dpy,
                                          pattern,
                                          MAXFONTS,
                                          &list_num,
                                          &fs_list)) == NULL)
...
parse_omit_name(XOC oc, FontData font_data, char *pattern)
{
    ...
    */
    ...
    if (length > XLFD_MAX_LEN - 1) return -1;
    ...
    */
    ...
    if ((last - buf) + strlen(font_data->name) > XLFD_MAX_LEN) return -1;

    strcpy(last, font_data->name);
    if ((font_data->xlfd_name = get_font_name(oc, buf)) != NULL) return True;

    /* This may not be needed anymore as XListFonts() takes care of this */
    if (num_fields < 12)
    {
        if ((last - buf) > (XLFD_MAX_LEN - 2)) return -1;
        *last       = '*';
        *(last + 1) = '-';
               ...
               FontDataRec *font_data_return)
{
	...
	*/
		  ...
		  */
            ...
            case C_VROTATE:
                if (is_found == True)
                {
                    ...
                    if ((rotate_name =
                             get_rotate_fontname(font_data->xlfd_name)) != NULL)
                    {
    ...
    if (class == C_PRIMARY && found_num >= 1) return True;
...
parse_vw(XOC oc, FontSet font_set, char **name_list, int count)
{
    ...
    if (vrotate_num > 0)
    {
        ...
        else if (ret == False)
        {
            ...
            {
                if (vrotate[i].xlfd_name) Xfree(vrotate[i].xlfd_name);
            }
            Xfree(vrotate);
            ...
            if (sub_num > 0)
            {
                vrotate = font_set->vrotate =
                    Xcalloc(sub_num, sizeof(VRotateRec));
                if (font_set->vrotate == (VRotate)NULL) return (-1);
...
parse_fontname(XOC oc)
{
    ...
    */
    for (; font_set_num-- > 0; font_set++)
    {
        if (font_set->font_name) continue;
	    ...
	    */
		 ...
		 */
                ...
                if (font_set->font_name == (char *)NULL) goto err;
                ...
                if (parse_vw(oc, font_set, name_list, count) == -1) goto err;
                found_num++;
            }
        }
...
create_fontset(XOC oc)
{
    ...
    if (init_fontset(oc) == False) return False;

    found_num = parse_fontname(oc);
    if (found_num <= 0)
    {
        if (found_num == 0) set_missing_list(oc);
        return False;
    }

    if (gen->on_demand_loading == True)
    {
        if (load_font_info(oc) == False) return False;
    }
    else
    {
        if (load_font(oc) == False) return False;
    }

    if (init_core_part(oc) == False) return False;

    if (set_missing_list(oc) == False) return False;
...
destroy_fontdata(XOCGenericPart *gen, Display *dpy)
{
    ...
    if (gen->font_set)
    {
        ...
        for (; font_set_num--; font_set++)
        {
            if (font_set->font)
            {
                if (font_set->font->fid) XFreeFont(dpy, font_set->font);
                else XFreeFontInfo(NULL, font_set->font, 1);
                font_set->font = NULL;
            }
            if (font_set->font_data)
            {
                if (font_set->info) XFreeFontInfo(NULL, font_set->info, 1);
                free_fontdataOC(dpy,
                                font_set->font_data,
                                font_set->font_data_count);
                Xfree(font_set->font_data);
            ...
            if (font_set->vmap)
            {
                free_fontdataOC(dpy, font_set->vmap, font_set->vmap_num);
                Xfree(font_set->vmap);
                font_set->vmap = NULL;
            }
            if (font_set->vrotate)
...
destroy_oc(XOC oc)
{
    ...
    if (gen->mbs_to_cs) _XlcCloseConverter(gen->mbs_to_cs);

    if (gen->wcs_to_cs) _XlcCloseConverter(gen->wcs_to_cs);

    if (gen->utf8_to_cs) _XlcCloseConverter(gen->utf8_to_cs);
...
set_oc_values(XOC oc, XlcArgList args, int num_args)
{
    ...
    if (oc->core.resources == NULL) return NULL;

    ret = _XlcSetValues((XPointer)oc,
                        oc->core.resources,
                        oc->core.num_resources,
                        args,
                        num_args,
                        XlcSetMask);
    if (ret != NULL)
    ...
    else
    {
        ...
        {
            if (font_set->font_name == NULL) continue;
            if (font_set->vpart_initialize != 0) continue;
            if (oc->core.orientation == XOMOrientation_TTB_RTL ||
                oc->core.orientation == XOMOrientation_TTB_LTR)
            {
                load_fontdata(oc, font_set->vmap, font_set->vmap_num);
...
get_oc_values(XOC oc, XlcArgList args, int num_args)
{
    ...
    return _XlcGetValues((XPointer)oc,
                         oc->core.resources,
                         oc->core.num_resources,
                         args,
                         num_args,
...
static XOCMethodsRec oc_default_methods = { destroy_oc,
                                            set_oc_values,
                                            get_oc_values,
...
static XOCMethodsRec oc_generic_methods = { destroy_oc,
                                            set_oc_values,
                                            get_oc_values,
...
static XlcResource oc_resources[] = {
    { XNBaseFontName,
     ...
     XOffsetOf(XOCRec, core.base_name_list),
     XlcCreateMask | XlcGetMask               },
    { XNOMAutomatic,
     NULLQUARK, sizeof(Bool),
     XOffsetOf(XOCRec, core.om_automatic),
     XlcGetMask                               },
    { XNMissingCharSet,
     NULLQUARK, sizeof(XOMCharSetList),
     XOffsetOf(XOCRec, core.missing_list),
     XlcGetMask                               },
    { XNDefaultString,
     NULLQUARK, sizeof(char *),
     ...
     XlcGetMask                               },
    { XNOrientation,
     NULLQUARK, sizeof(XOrientation),
     XOffsetOf(XOCRec, core.orientation),
     XlcDefaultMask | XlcSetMask | XlcGetMask },
    { XNResourceName,
     NULLQUARK, sizeof(char *),
     XOffsetOf(XOCRec, core.res_name),
     XlcSetMask | XlcGetMask                  },
    { XNResourceClass,
     NULLQUARK, sizeof(char *),
     XOffsetOf(XOCRec, core.res_class),
     XlcSetMask | XlcGetMask                  },
    { XNFontInfo,
     NULLQUARK, sizeof(XOMFontInfo),
     XOffsetOf(XOCRec, core.font_info),
     XlcGetMask                               }
};
...
create_oc(XOM om, XlcArgList args, int num_args)
{
    ...
    if (_XlcSetValues((XPointer)oc,
                      oc_resources,
                      XlcNumber(oc_resources),
                      args,
                      num_args,
                      XlcCreateMask | XlcDefaultMask))
        goto err;

    if (oc->core.base_name_list == NULL) goto err;

    oc->core.resources     = oc_resources;
    oc->core.num_resources = XlcNumber(oc_resources);

    if (create_fontset(oc) == False) goto err;
...
{
    if (om->core.resources == NULL) return NULL;

    return _XlcSetValues((XPointer)om,
                         om->core.resources,
                         om->core.num_resources,
                         args,
                         num_args,
                         XlcSetMask);
}
...
get_om_values(XOM om, XlcArgList args, int num_args)
{
    ...
    return _XlcGetValues((XPointer)om,
                         om->core.resources,
                         om->core.num_resources,
                         args,
                         num_args,
...
static XOMMethodsRec methods = { close_om,
                                 set_om_values,
                                 get_om_values,
                                 create_oc };
...
static XlcResource om_resources[] = {
    { XNRequiredCharSet,
     NULLQUARK, sizeof(XOMCharSetList),
     XOffsetOf(XOMRec, core.required_charset),
     XlcGetMask },
    { XNQueryOrientation,
     NULLQUARK, sizeof(XOMOrientation),
     XOffsetOf(XOMRec, core.orientation_list),
     XlcGetMask },
    { XNDirectionalDependentDrawing,
     NULLQUARK, sizeof(Bool),
     XOffsetOf(XOMRec, core.directional_dependent),
     XlcGetMask },
    { XNContextualDrawing,
     NULLQUARK, sizeof(Bool),
     XOffsetOf(XOMRec, core.contextual_drawing),
     XlcGetMask }
};
          ...
          _Xconst char *res_class)
{
    ...
    if (res_name)
    {
        ...
        if (om->core.res_name == NULL) goto err;
    }
    if (res_class)
    {
        om->core.res_class = strdup(res_class);
        if (om->core.res_class == NULL) goto err;
    }
...
add_data(XOM om)
{
    ...
    else new = Xmalloc(sizeof(OMDataRec));
...
FontData
read_EncodingInfo(int count, char **value)
{
    FontData font_data, ret;
    char    *buf, *bufptr, *scp;
    int      len, i;
    font_data = Xcalloc(count, sizeof(FontDataRec));
    if (font_data == NULL) return NULL;
...
*/
        ...
        else font_data->side = XlcGLGR;

        if (bufptr && (scp = strchr(bufptr, '[')))
        {
            font_data->scopes =
                _XlcParse_scopemaps(scp, &(font_data->scopes_num));
        }
    }
    return (ret);
}
...
static CodeRange
read_vrotate(int count, char **value, int *type, int *vrotate_num)
{
    CodeRange range;
...
read_vw(XLCd lcd, OMData font_set, int num)
{
    ...
    {
        _XlcDbg_printValue(buf, value, count);
        font_set->vrotate = read_vrotate(count,
                                         value,
                                         &(font_set->vrotate_type),
                                         &(font_set->vrotate_num));
    }
}
...
init_om(XOM om)
{
    ...
    if (count > 0)
    {
        ...
        if (gen->object_name == NULL) return False;
    }
    ...
    for (num = 0;; num++)
    {
        ...
        if (charset_list == NULL) return False;
    ...
    if (required_list == NULL) return False;
