...
static int
is_rotate(XOC oc, XFontStruct *font)
{
    ...
    for (; font_set_count--; font_set++)
    {
        if ((font_set->vrotate_num > 0) && (font_set->vrotate != NULL))
        {
            ...
            for (; vrotate_num--; vrotate++)
                if (vrotate->font == font) return True;
        }
    }
    return False;
}
...
static int
is_codemap(XOC oc, XFontStruct *font)
{
    XOCGenericPart *gen = XOC_GENERIC(oc);
    FontSet         font_set;
    FontData        vmap;
                    ...
                    int          length)
{
    ...
    if (is_xchar2b)
    {
        for (i = 0, buf2b = (XChar2b *)text; i < length; i++, buf2b++)
        {
            ...
            {
                escapement +=
                    (int)(font->max_bounds.ascent + font->max_bounds.descent);
            }
        }
    }
    else
...
static int
TextWidthWithFontSet(FontSet font_set, XOC oc, XPointer text, int length)
{
    FontData       fd;
    XFontStruct   *font;
    ...
    if (font_set == (FontSet)NULL) return escapement;

    is_xchar2b = font_set->is_xchar2b;

    while (length > 0)
    {
        fd = _XomGetFontDataFromFontSet(font_set,
                                        ptr,
                                        length,
                                        &ptr_len,
                                        is_xchar2b,
	 ...
	 */
        ...
        switch (oc->core.orientation)
        {
            ...
            case XOMOrientation_TTB_RTL:
                ...
                if (is_xchar2b) char_len = ptr_len / sizeof(XChar2b);
                else char_len = ptr_len;
                escapement += escapement_vertical(oc,
                                                  font,
                                                  is_xchar2b,
                                                  (XPointer)ptr,
                                                  char_len);
...
static int
_XomGenericTextEscapement(XOC oc, XOMTextType type, XPointer text, int length)
{
    XlcConv      conv;
    XFontStruct *font;
    Bool         is_xchar2b;
...
/* VW/UDC */
    ...
    if (conv == NULL) return escapement;
...
/* VW/UDC */
        escapement +=
            TextWidthWithFontSet(font_set, oc, (XPointer)xchar2b_buf, buf_len);
/* VW/UDC */
    }
...
{
    return _XomGenericTextEscapement(oc, XOMUtf8String, (XPointer)text, length);
}
