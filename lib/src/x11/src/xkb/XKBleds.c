...
XkbGetIndicatorState(Display *dpy, unsigned deviceSpec, unsigned *pStateRtrn)
{
    ...
    if (ok && (pStateRtrn != NULL)) *pStateRtrn = rep.state;
    UnlockDisplay(dpy);
    SyncHandle();
        ...
        if (rep->which)
        {
                 ...
                 i++, bit <<= 1)
            {
                if (left & bit)
                {
                    ...
                    wire = (xkbIndicatorMapWireDesc *)_XkbGetReadBufferPtr(
                        &buf,
...
XkbSetIndicatorMap(Display *dpy, unsigned long which, XkbDescPtr xkb)
{
    ...
    if ((dpy->flags & XlibDisplayNoXkb) ||
        (!dpy->xkb_info && !XkbUseExtension(dpy, NULL, NULL)))
        return False;
    if ((!xkb) || (!which) || (!xkb->indicators)) return False;
    LockDisplay(dpy);
    xkbi = dpy->xkb_info;
    ...
    BufAlloc(xkbIndicatorMapWireDesc *,
             wire,
                           ...
                           Bool              *pRealRtrn)
{
    ...
    UnlockDisplay(dpy);
    SyncHandle();
    if ((!rep.found) || (!rep.supported)) return False;
    if (pNdxRtrn != NULL) *pNdxRtrn = rep.ndx;
    if (pStateRtrn != NULL) *pStateRtrn = rep.on;
    if (pMapRtrn != NULL)
    {
    ...
    if (pRealRtrn != NULL) *pRealRtrn = rep.realIndicator;
    return True;
}
...
XkbGetNamedIndicator(Display           *dpy,
                     ...
                     Bool              *pRealRtrn)
{
    return XkbGetNamedDeviceIndicator(dpy,
                                      XkbUseCoreKbd,
                                      XkbDfltXIClass,
                                      XkbDfltXIId,
                                      name,
...
XkbSetNamedIndicator(Display           *dpy,
                     ...
                     XkbIndicatorMapPtr pMap)
{
    return XkbSetNamedDeviceIndicator(dpy,
                                      XkbUseCoreKbd,
                                      XkbDfltXIClass,
                                      XkbDfltXIId,
                                      name,
