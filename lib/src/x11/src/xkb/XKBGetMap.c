...
_XkbReadKeyTypes(XkbReadBufferPtr buf, XkbDescPtr xkb, xkbGetMapReply *rep)
{
    ...
    if (rep->nTypes > 0)
    {
        ...
        if (xkb->map->num_types >= n) n = xkb->map->num_types;
        ...
        for (i = 0; i < (int)rep->nTypes; i++, type++)
        {
            ...
            ndx = i + rep->firstType;
            if (ndx >= xkb->map->num_types) xkb->map->num_types = ndx + 1;

            desc = (xkbKeyTypeWireDesc *)_XkbGetReadBufferPtr(
                buf,
                SIZEOF(xkbKeyTypeWireDesc));
            if (desc == NULL) return BadLength;
            ...
            if (desc->nMapEntries > 0)
            {
                ...
                if (desc->preserve)
                {
                    ...
                    if (pwire == NULL) return BadLength;
...
_XkbReadKeySyms(XkbReadBufferPtr buf, XkbDescPtr xkb, xkbGetMapReply *rep)
{
    ...
    if (map->key_sym_map == NULL)
    {
        ...
        {
            newMap = (xkbSymMapWireDesc *)_XkbGetReadBufferPtr(
                buf,
                SIZEOF(xkbSymMapWireDesc));
            if (newMap == NULL) return BadLength;
            ...
            if (newMap->nSyms > 0)
            {
                if (_XkbReadBufferCopyKeySyms(buf,
                                              (KeySym *)&map->syms[offset],
                                              newMap->nSyms) == 0)
                    return BadLength;
                offset += newMap->nSyms;
            }
    ...
    else
    {
        ...
        for (i = 0; i < (int)rep->nKeySyms; i++, oldMap++)
        {
            ...
            if (newMap->nSyms > 0) tmp = newMap->nSyms;
            else tmp = 0;
...
_XkbReadKeyActions(XkbReadBufferPtr buf, XkbDescPtr info, xkbGetMapReply *rep)
{
    ...
    if ((nKeyActs = rep->nKeyActs) > 0)
    {
        ...
        else numDesc = Xmallocarray(nKeyActs, sizeof(CARD8));
        ...
        for (i = 0; i < (int)rep->nKeyActs; i++, symMap++)
        {
            ...
            else
            {
                ...
                newActs =
                    XkbResizeKeyActions(info, i + rep->firstKeyAct, numDesc[i]);
                if (newActs == NULL)
                {
                    ret = BadAlloc;
                    goto done;
                }
                if (!_XkbCopyFromReadBuffer(
                        buf,
                        (char *)newActs,
...
_XkbReadKeyBehaviors(XkbReadBufferPtr buf, XkbDescPtr xkb, xkbGetMapReply *rep)
{
    ...
    if (rep->totalKeyBehaviors > 0)
    {
        ...
        if (xkb->server->behaviors == NULL)
        {
            ...
            if (xkb->server->behaviors == NULL) return BadAlloc;
        }
        else
        {
        ...
        for (i = 0; i < rep->totalKeyBehaviors; i++)
        {
            ...
            wire = (xkbBehaviorWireDesc *)_XkbGetReadBufferPtr(
                buf,
                SIZEOF(xkbBehaviorWireDesc));
            if (wire == NULL || wire->key >= size) return BadLength;
...
_XkbReadVirtualMods(XkbReadBufferPtr buf, XkbDescPtr xkb, xkbGetMapReply *rep)
{
    if (rep->virtualMods)
    {
        ...
        {
            if (rep->virtualMods & bit) nVMods++;
        }
        data = _XkbGetReadBufferPtr(buf, XkbPaddedSize(nVMods));
        if (data == NULL) return BadLength;
                           ...
                           xkbGetMapReply  *rep)
{
    ...
    if (rep->totalKeyExplicit > 0)
    {
        ...
        if (xkb->server->explicit == NULL)
        {
            ...
            if (xkb->server->explicit == NULL) return BadAlloc;
        }
        else
        {
                    ...
                    int            *nread_rtrn)
{
    ...
    if (rep->maxKeyCode < rep->minKeyCode) return BadImplementation;
    ...
    if (rep->length)
    {
        ...
        if (_XkbInitReadBuffer(dpy, &buf, extraData))
        {
            ...
            if (nread_rtrn != NULL) *nread_rtrn = extraData;
            if (status == Success) status = _XkbReadKeyTypes(&buf, xkb, rep);
            if (status == Success) status = _XkbReadKeySyms(&buf, xkb, rep);
            if (status == Success) status = _XkbReadKeyActions(&buf, xkb, rep);
            if (status == Success)
                status = _XkbReadKeyBehaviors(&buf, xkb, rep);
            if (status == Success) status = _XkbReadVirtualMods(&buf, xkb, rep);
            if (status == Success)
                status = _XkbReadExplicitComponents(&buf, xkb, rep);
            if (status == Success) status = _XkbReadModifierMap(&buf, xkb, rep);
            if (status == Success)
...
XkbGetKeyTypes(Display *dpy, unsigned first, unsigned num, XkbDescPtr xkb)
{
    ...
    if ((dpy->flags & XlibDisplayNoXkb) ||
        (!dpy->xkb_info && !XkbUseExtension(dpy, NULL, NULL)))
        return BadAccess;
    if ((num < 1) || (num > XkbMaxKeyTypes)) return BadValue;
...
XkbGetKeyActions(Display *dpy, unsigned first, unsigned num, XkbDescPtr xkb)
{
    ...
    if ((num < 1) || (num > XkbMaxKeyCount)) return BadValue;
                            ...
                            XkbDescPtr xkb)
{
    ...
    if (xkb) status = _XkbHandleGetMapReply(dpy, xkb);
    else status = BadMatch;
...
Status
XkbGetKeyVirtualModMap(Display   *dpy,
                       unsigned   first,
                       unsigned   num,
                       XkbDescPtr xkb)
{
