...
XkbComputeShapeBounds(XkbShapePtr shape)
{
    ...
    if ((!shape) || (shape->num_outlines < 1)) return False;
...
XkbComputeRowBounds(XkbGeometryPtr geom, XkbSectionPtr section, XkbRowPtr row)
{
    ...
    if ((!geom) || (!section) || (!row)) return False;
    bounds = &row->bounds;
...
XkbComputeSectionBounds(XkbGeometryPtr geom, XkbSectionPtr section)
{
    ...
    if ((!geom) || (!section)) return False;
    bounds = &section->bounds;
    bzero(bounds, sizeof(XkbBoundsRec));
    for (i = 0, row = section->rows; i < section->num_rows; i++, row++)
    {
        if (!XkbComputeRowBounds(geom, section, row)) return False;
        rbounds = &row->bounds;
        _XkbCheckBounds(bounds,
                        row->left + rbounds->x1,
                        row->top + rbounds->y1);
        _XkbCheckBounds(bounds,
                        row->left + rbounds->x2,
...
char *
XkbFindOverlayForKey(XkbGeometryPtr geom,
                     ...
                     _Xconst char  *under)
{
    ...
    for (s = 0; s < geom->num_sections; s++, section++)
    {
        ...
        if (section->num_overlays < 1) continue;
                       ...
                       xkbGetGeometryReply *rep)
{
    ...
    if (rep->nProperties < 1) return Success;
                       ...
                       xkbGetGeometryReply *rep)
{
    ...
    if (rep->nKeyAliases < 1) return Success;
    ...
    {                      /* alloc failed, just skip the aliases */
                   ...
                   xkbGetGeometryReply *rep)
{
    ...
    if (rep->nColors < 1) return Success;
    if ((rtrn = XkbAllocGeomColors(geom, rep->nColors)) == Success)
    {
        ...
        {
            char *spec = NULL;
            if (!_XkbGetReadBufferCountedString(buf, &spec)) rtrn = BadLength;
            else if (XkbAddGeomColor(geom, spec, geom->num_colors) == NULL)
                rtrn = BadAlloc;

            _XkbFree(spec);
            if (rtrn != Success) return rtrn;
        }
        return Success;
    }
    return rtrn;
                   ...
                   xkbGetGeometryReply *rep)
{
    ...
    if (rep->nShapes < 1) return Success;
    if ((rtrn = XkbAllocGeomShapes(geom, rep->nShapes)) != Success) return rtrn;
    for (i = 0; i < rep->nShapes; i++)
    {
        ...
        shapeWire =
            (xkbShapeWireDesc *)_XkbGetReadBufferPtr(buf,
                                                     SIZEOF(xkbShapeWireDesc));
        if (!shapeWire) return BadLength;
        shape = XkbAddGeomShape(geom, shapeWire->name, shapeWire->nOutlines);
        if (!shape) return BadAlloc;
        for (o = 0; o < shapeWire->nOutlines; o++)
        {
            ...
            olWire = (xkbOutlineWireDesc *)_XkbGetReadBufferPtr(
                buf,
                SIZEOF(xkbOutlineWireDesc));
            if (!olWire) return BadLength;
            ol = XkbAddGeomOutline(shape, olWire->nPoints);
            if (!ol) return BadAlloc;
            ol->corner_radius = olWire->cornerRadius;
            for (p = 0, pt = ol->points; p < olWire->nPoints; p++, pt++)
            {
                xkbPointWireDesc *ptWire;

                ptWire = (xkbPointWireDesc *)_XkbGetReadBufferPtr(
                    buf,
                    SIZEOF(xkbPointWireDesc));
                if (!ptWire) return BadLength;
                pt->x = ptWire->x;
                pt->y = ptWire->y;
            }
                   ...
                   XkbSectionPtr    section)
{
    ...
    doodadWire =
        (xkbDoodadWireDesc *)_XkbGetReadBufferPtr(buf,
                                                  SIZEOF(xkbDoodadWireDesc));
    if (!doodadWire) return BadLength;
    doodad = XkbAddGeomDoodad(geom, section, doodadWire->any.name);
    if (!doodad) return BadAlloc;
                    ...
                    XkbSectionPtr    section)
{
    ...
    olWire =
        (xkbOverlayWireDesc *)_XkbGetReadBufferPtr(buf,
                                                   SIZEOF(xkbOverlayWireDesc));
    if (olWire == NULL) return BadLength;
    ol = XkbAddGeomOverlay(section, olWire->name, olWire->nRows);
    if (ol == NULL) return BadLength;
    for (r = 0; r < olWire->nRows; r++)
    {
        ...
        rowWire = (xkbOverlayRowWireDesc *)_XkbGetReadBufferPtr(
            buf,
            SIZEOF(xkbOverlayRowWireDesc));
        if (rowWire == NULL) return BadLength;
        row = XkbAddGeomOverlayRow(ol, rowWire->rowUnder, rowWire->nKeys);
        if (!row) return BadAlloc;
        row->row_under = rowWire->rowUnder;
        if (rowWire->nKeys < 1) continue;
        keyWire = (xkbOverlayKeyWireDesc *)_XkbGetReadBufferPtr(
            buf,
            SIZEOF(xkbOverlayKeyWireDesc) * rowWire->nKeys);
        if (keyWire == NULL) return BadLength;
        for (k = 0; k < rowWire->nKeys; k++, keyWire++, row->num_keys++)
        {
            memcpy(row->keys[row->num_keys].over.name,
                   keyWire->over,
                   XkbKeyNameLength);
            memcpy(row->keys[row->num_keys].under.name,
                   keyWire->under,
                   XkbKeyNameLength);
        }
    }
                     ...
                     xkbGetGeometryReply *rep)
{
    ...
    if (rep->nSections < 1) return Success;
    if ((rtrn = XkbAllocGeomSections(geom, rep->nSections)) != Success)
        return rtrn;
    for (s = 0; s < rep->nSections; s++)
    {
        sectionWire = (xkbSectionWireDesc *)_XkbGetReadBufferPtr(
            buf,
            SIZEOF(xkbSectionWireDesc));
        if (!sectionWire) return BadLength;
        section = XkbAddGeomSection(geom,
                                    sectionWire->name,
                                    sectionWire->nRows,
                                    sectionWire->nDoodads,
                                    sectionWire->nOverlays);
        if (!section) return BadAlloc;
        ...
        if (sectionWire->nRows > 0)
        {
            ...
            for (r = 0; r < sectionWire->nRows; r++)
            {
                ...
                rowWire = (xkbRowWireDesc *)_XkbGetReadBufferPtr(
                    buf,
                    SIZEOF(xkbRowWireDesc));
                if (!rowWire) return BadLength;
                ...
                if (rowWire->nKeys > 0)
                {
                    ...
                    for (k = 0; k < rowWire->nKeys; k++)
                    {
                        ...
                        keyWire = (xkbKeyWireDesc *)_XkbGetReadBufferPtr(
                            buf,
                            SIZEOF(xkbKeyWireDesc));
                        if (!keyWire) return BadLength;
                        key = XkbAddGeomKey(row);
                        if (!key) return BadAlloc;
                    ...
                    xkbGetGeometryReply *rep)
{
    ...
    if (rep->nDoodads < 1) return Success;
                         ...
                         int                 *nread_rtrn)
{
    ...
    if (!geom) return BadAlloc;
    if (xkb->geom) XkbFreeGeometry(xkb->geom, XkbGeomAllMask, True);
    xkb->geom = geom;
    ...
    if (rep->length)
    {
        ...
        if (_XkbInitReadBuffer(dpy, &buf, (int)rep->length * 4))
        {
            ...
            if (status == Success) status = _XkbReadGeomColors(&buf, geom, rep);
            if (status == Success) status = _XkbReadGeomShapes(&buf, geom, rep);
            if (status == Success)
...
XkbGetGeometry(Display *dpy, XkbDescPtr xkb)
{
    ...
    req->name       = None;
    if (!_XReply(dpy, (xReply *)&rep, 0, xFalse)) status = BadImplementation;
    else if (!rep.found) status = BadName;
    else status = _XkbReadGetGeometryReply(dpy, &rep, xkb, NULL);
    UnlockDisplay(dpy);
    SyncHandle();
    return status;
}
