...
static Status
_XkbReadAtoms(XkbReadBufferPtr buf, Atom *atoms, int maxAtoms, CARD32 present)
{
    register int i, bit;
                      ...
                      int              *nread_rtrn)
{
    ...
    if ((xkb->names == NULL) &&
        (XkbAllocNames(xkb, rep->which, rep->nRadioGroups, rep->nKeyAliases) !=
         Success))
    {
        return BadAlloc;
    }
    names = xkb->names;
    if (rep->length == 0) return Success;
    ...
    if (rep->which & XkbKeyTypeNamesMask)
    {
        ...
        if ((len > 0) && (!_XkbSkipReadBufferData(&buf, len))) goto BAILOUT;
    }
    ...
    if (rep->which & XkbRGNamesMask)
    {
        if (rep->nRadioGroups > 0)
        {
            ...
            {
                _XkbResizeArray(names->radio_groups,
                                names->num_rg,
                                rep->nRadioGroups,
                                Atom);
            }
...
_XkbCopyAtoms(Display *dpy, Atom *atoms, CARD32 mask, int maxAtoms)
{
    ...
    {
        if (mask & bit) Data32(dpy, &atoms[i], 4);
    }
    return;
}
            ...
            XkbDescPtr   xkb)
{
    ...
    if ((dpy->flags & XlibDisplayNoXkb) ||
        (!dpy->xkb_info && !XkbUseExtension(dpy, NULL, NULL)))
        return False;
    if ((!xkb) || (!xkb->names)) return False;
    firstLvlType = firstType;
    nLvlTypes    = nTypes;
    if (nTypes < 1) which &= ~(XkbKTLevelNamesMask | XkbKeyTypeNamesMask);
    else if (firstType <= XkbLastRequiredType)
    {
        ...
        firstType += adjust;
        nTypes -= adjust;
        if (nTypes < 1) which &= ~XkbKeyTypeNamesMask;
    }
    names = xkb->names;
    if (which & (XkbKTLevelNamesMask | XkbKeyTypeNamesMask))
    {
        ...
        if (which & XkbKTLevelNamesMask)
        {
            ...
            {
                if (type->level_names != NULL) nLvlNames += type->num_levels;
            }
        }
    }
    ...
    if (which & XkbKeycodesNameMask) nAtoms++;
    if (which & XkbGeometryNameMask) nAtoms++;
    if (which & XkbSymbolsNameMask) nAtoms++;
    if (which & XkbPhysSymbolsNameMask) nAtoms++;
    if (which & XkbTypesNameMask) nAtoms++;
    if (which & XkbCompatNameMask) nAtoms++;
    if (which & XkbKeyTypeNamesMask) nAtoms += nTypes;
    if (which & XkbKTLevelNamesMask)
    {
    ...
    {
        vmods = req->virtualMods =
            (CARD16)_XkbCountAtoms(names->vmods, XkbNumVirtualMods, &nVMods);
        if (nVMods > 0) nAtoms += nVMods;
    ...
    if (which & XkbGroupNamesMask)
    {
        groups = req->groupNames =
            (CARD8)_XkbCountAtoms(names->groups, XkbNumKbdGroups, &nGroups);
        if (nGroups > 0) nAtoms += nGroups;
    ...
    if (which & XkbKeycodesNameMask) Data32(dpy, (long *)&names->keycodes, 4);
    if (which & XkbGeometryNameMask) Data32(dpy, (long *)&names->geometry, 4);
    if (which & XkbSymbolsNameMask) Data32(dpy, (long *)&names->symbols, 4);
    if (which & XkbPhysSymbolsNameMask)
        Data32(dpy, (long *)&names->phys_symbols, 4);
    if (which & XkbTypesNameMask) Data32(dpy, (long *)&names->types, 4);
    if (which & XkbCompatNameMask) Data32(dpy, (long *)&names->compat, 4);
...
XkbChangeNames(Display *dpy, XkbDescPtr xkb, XkbNameChangesPtr changes)
{
    ...
    if ((dpy->flags & XlibDisplayNoXkb) ||
        (!dpy->xkb_info && !XkbUseExtension(dpy, NULL, NULL)))
        return False;
    if ((!xkb) || (!xkb->names) || (!changes)) return False;
    ...
    if (which & XkbKTLevelNamesMask)
    {
        if (nLvlTypes < 1) which &= ~XkbKTLevelNamesMask;
    }
    ...
    if (changes->num_keys < 1) which &= ~XkbKeyNamesMask;
    ...
    if ((which & XkbVirtualModNamesMask) == 0) changes->changed_vmods = 0;
    else if (changes->changed_vmods == 0) which &= ~XkbVirtualModNamesMask;

    if ((which & XkbIndicatorNamesMask) == 0) changes->changed_indicators = 0;
    else if (changes->changed_indicators == 0) which &= ~XkbIndicatorNamesMask;

    if ((which & XkbGroupNamesMask) == 0) changes->changed_groups = 0;
    else if (changes->changed_groups == 0) which &= ~XkbGroupNamesMask;
    ...
    {
        nKA = ((names->key_aliases != NULL) ? names->num_key_aliases : 0);
        if (nKA > 0) nAtoms += nKA * 2; /* not atoms, but 8 bytes on the wire */
                   ...
                   unsigned int wanted)
{
    ...
    if (wanted & XkbKeyTypeNamesMask)
    {
        if (old->changed & XkbKeyTypeNamesMask)
        {
            ...
            if (new->first_type < old->first_type) first = new->first_type;
            else first = old->first_type;

            if (old_last > new_last) last = old_last;
            else last = new_last;
    ...
    if (wanted & XkbKTLevelNamesMask)
    {
        if (old->changed & XkbKTLevelNamesMask)
        {
            ...
            if (new->first_lvl < old->first_lvl) first = new->first_lvl;
    ...
    if (wanted & XkbKeyNamesMask)
    {
        if (old->changed & XkbKeyNamesMask)
        {
            ...
            if (new->first_key < old->first_key) first = new->first_key;
            if (old_last > new_last) new_last = old_last;
    ...
    if (wanted & XkbRGNamesMask) old->num_rg = new->num_radio_groups;
    if (wanted & XkbKeyAliasesMask) old->num_aliases = new->num_aliases;
    old->changed |= wanted;
    return;
}
