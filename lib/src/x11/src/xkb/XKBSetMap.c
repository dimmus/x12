...
_XkbSizeKeyTypes(XkbDescPtr xkb, xkbSetMapReq *req)
{
    ...
    for (i = 0; i < req->nTypes; i++, map++)
    {
        ...
        if (map->preserve) len += map->map_count * SIZEOF(xkbModsWireDesc);
    }
    return len;
}
...
_XkbWriteKeyTypes(Display *dpy, XkbDescPtr xkb, xkbSetMapReq *req)
{
    ...
    if ((req->present & XkbKeyTypesMask) == 0) return;
    ...
    for (i = 0; i < req->nTypes; i++, type++)
    {
        ...
        if (type->preserve) sz += type->map_count * SIZEOF(xkbModsWireDesc);
...
_XkbWriteKeySyms(Display *dpy, XkbDescPtr xkb, xkbSetMapReq *req)
{
    ...
    if ((req->present & XkbKeySymsMask) == 0) return;
...
_XkbWriteKeyActions(Display *dpy, XkbDescPtr xkb, xkbSetMapReq *req)
{
    ...
    if ((req->present & XkbKeyActionsMask) == 0) return;
    ...
    {
        if (xkb->server->key_acts[i + req->firstKeyAct] == 0) numDesc[i] = 0;
    ...
    for (i = 0; i < req->nKeyActs; i++)
    {
        if (xkb->server->key_acts[i + req->firstKeyAct] != 0)
        {
            ...
            memcpy(actDesc,
                   XkbKeyActionsPtr(xkb, (i + req->firstKeyAct)),
...
_XkbSizeKeyBehaviors(XkbDescPtr xkb, xkbSetMapReq *req)
{
    ...
    {
        if (xkb->server->behaviors[i].type != XkbKB_Default) nFound++;
    }
...
_XkbWriteKeyBehaviors(Display *dpy, XkbDescPtr xkb, xkbSetMapReq *req)
{
    ...
    if ((req->present & XkbKeyBehaviorsMask) == 0) return;
    first = req->firstKeyBehavior;
    last  = first + req->nKeyBehaviors - 1;
                     ...
                     unsigned      size)
{
    ...
    if (((req->present & XkbVirtualModsMask) == 0) || (size < 1)) return;
    BufAlloc(CARD8 *, vmods, size);
    for (i = 0, bit = 1; i < XkbNumVirtualMods; i++, bit <<= 1)
    {
        if (req->virtualMods & bit) *vmods++ = xkb->server->vmods[i];
    }
    return;
}
...
_XkbSizeKeyExplicit(XkbDescPtr xkb, xkbSetMapReq *req)
{
    ...
    for (i = first, nFound = 0; i <= last; i++)
    {
        if (xkb->server->explicit[i] != 0) nFound++;
    }
...
_XkbWriteKeyExplicit(Display *dpy, XkbDescPtr xkb, xkbSetMapReq *req)
{
    ...
    if ((req->present & XkbExplicitComponentsMask) == 0) return;
    first = req->firstKeyExplicit;
    last  = first + req->nKeyExplicit - 1;
...
_XkbWriteModifierMap(Display *dpy, XkbDescPtr xkb, xkbSetMapReq *req)
{
    ...
    if ((req->present & XkbModifierMapMask) == 0) return;
    first = req->firstModMapKey;
    last  = first + req->nModMapKeys - 1;
...
_XkbSizeVirtualModMap(XkbDescPtr xkb, xkbSetMapReq *req)
{
    ...
    for (i = first, nFound = 0; i <= last; i++)
    {
        if (xkb->server->vmodmap[i] != 0) nFound++;
    }
...
_XkbWriteVirtualModMap(Display *dpy, XkbDescPtr xkb, xkbSetMapReq *req)
{
    ...
    if ((req->present & XkbVirtualModMapMask) == 0) return;
    first = req->firstVModMapKey;
    last  = first + req->nVModMapKeys - 1;
...
SendSetMap(Display *dpy, XkbDescPtr xkb, xkbSetMapReq *req)
{
    ...
    tmp = *req;
    if (tmp.nTypes > 0) _XkbWriteKeyTypes(dpy, xkb, &tmp);
    if (tmp.nKeySyms > 0) _XkbWriteKeySyms(dpy, xkb, &tmp);
    if (tmp.nKeyActs) _XkbWriteKeyActions(dpy, xkb, &tmp);
    if (tmp.totalKeyBehaviors > 0) _XkbWriteKeyBehaviors(dpy, xkb, &tmp);
    if (tmp.virtualMods) _XkbWriteVirtualMods(dpy, xkb, &tmp, szMods);
    if (tmp.totalKeyExplicit > 0) _XkbWriteKeyExplicit(dpy, xkb, &tmp);
    if (tmp.totalModMapKeys > 0) _XkbWriteModifierMap(dpy, xkb, &tmp);
    if (tmp.totalVModMapKeys > 0) _XkbWriteVirtualModMap(dpy, xkb, &tmp);
    return;
}
...
XkbSetMap(Display *dpy, unsigned which, XkbDescPtr xkb)
{
    ...
    if (which & XkbVirtualModsMask) req->virtualMods = ~0;
...
XkbChangeMap(Display *dpy, XkbDescPtr xkb, XkbMapChangesPtr changes)
{
    ...
    if (((changes->changed & XkbKeyTypesMask) && ((!map) || (!map->types))) ||
        ...
        ((changes->changed & XkbExplicitComponentsMask) &&
         ((!srv) || (!srv->explicit))) ||
        ((changes->changed & XkbModifierMapMask) &&
         ((!map) || (!map->modmap))) ||
