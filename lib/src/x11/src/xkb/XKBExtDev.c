...
XkbNoteDeviceChanges(XkbDeviceChangesPtr old,
                     ...
                     unsigned int wanted)
{
    if ((!old) || (!new) || (!wanted) || ((new->reason &wanted) == 0)) return;
    if ((wanted & new->reason) & XkbXI_ButtonActionsMask)
    {
        if (old->changed & XkbXI_ButtonActionsMask)
        {
            ...
            if (new->first_btn < old->first_btn) first = new->first_btn;
                      ...
                      XkbDeviceInfoPtr devi)
{
    ...
    if (!wireli) return BadLength;
    devli = XkbAddDeviceLedInfo(devi, wireli->ledClass, wireli->ledID);
    if (!devli) return BadAlloc;
    devli->phys_indicators = wireli->physIndicators;

    if (present & XkbXI_IndicatorStateMask) devli->state = wireli->state;
                           ...
                           XkbDeviceInfoPtr       devi)
{
    ...
    _XkbFree(devi->name);
    if (!_XkbGetReadBufferCountedString(&buf, &devi->name)) goto BAILOUT;
    if (rep->nBtnsRtrn > 0)
    {
        int size;
        ...
        if (!_XkbCopyFromReadBuffer(&buf, (char *)act, size)) goto BAILOUT;
    }
    ...
    if (tmp || buf.error) return BadLength;
    return Success;
BAILOUT:
                        ...
                        XkbDeviceChangesPtr changes)
{
    ...
    if ((dpy->flags & XlibDisplayNoXkb) ||
        (!dpy->xkb_info && !XkbUseExtension(dpy, NULL, NULL)))
        return BadMatch;
    if ((changes->changed & XkbXI_AllDeviceFeaturesMask) == 0) return Success;
...
_InitLedStuff(SetLedStuff *stuff, unsigned wanted, XkbDeviceInfoPtr devi)
{
    ...
    if ((devi->num_leds < 1) || ((wanted & XkbXI_IndicatorsMask) == 0)) return;
    stuff->info = _XkbTypedCalloc(devi->num_leds, LedInfoStuff);
    if (!stuff->info) return;
...
{
    if (stuff->num_info > 0) _XkbFree(stuff->info);
    bzero(stuff, sizeof(SetLedStuff));
    return;
}
...
_XkbSizeLedInfo(unsigned changed, XkbDeviceLedInfoPtr devli)
{
    ...
    if (changed & XkbXI_IndicatorNamesMask) namesNeeded = devli->names_present;
    if (changed & XkbXI_IndicatorMapsMask) mapsNeeded = devli->maps_present;
    if ((namesNeeded) || (mapsNeeded))
    {
        for (i = 0, bit = 1; i < XkbNumIndicators; i++, bit <<= 1)
        {
            ...
            if (mapsNeeded & bit) size += SIZEOF(xkbIndicatorMapWireDesc);
        }
    }
    return size;
}
             ...
             int                   *nleds_rtrn)
{
    ...
    id     = changes->led_id;
    if (class == XkbDfltXIClass) class = stuff->dflt_class;
                      ...
                      int                *num_leds_rtrn)
{
    *sz_rtrn = 0;
    ...
    {
        if (!XkbXI_LegalDevBtn(devi,
                               (changes->first_btn + changes->num_btns - 1)))
            return BadMatch;
