...
_WriteCountedString(char *wire, char *str)
{
    ...
    pLen  = (CARD16 *)wire;
    *pLen = len;
    if (len && str) memcpy(&wire[2], str, len);
...
_SizeGeomDoodads(int num_doodads, XkbDoodadPtr doodad)
{
    ...
    for (i = size = 0; i < num_doodads; i++, doodad++)
    {
        ...
        {
            size =
                (int)((unsigned)size + _SizeCountedString(doodad->text.text));
            size =
                (int)((unsigned)size + _SizeCountedString(doodad->text.font));
        }
        else if (doodad->any.type == XkbLogoDoodad)
        {
            size = (int)((unsigned)size +
                         _SizeCountedString(doodad->logo.logo_name));
        }
    }
    return size;
}
    ...
    for (i = 0; i < num_doodads; i++, doodad++)
    {
        ...
        switch (doodad->any.type)
        {
            ...
            case XkbIndicatorDoodad:
                ...
                doodadWire->indicator.onColorNdx =
...
_WriteGeomSections(char *wire, XkbGeometryPtr geom)
{
         ...
         i++, section++)
    {
        ...
        {
            wire =
                _WriteGeomDoodads(wire, section->num_doodads, section->doodads);
        }
        if (section->overlays)
        {
            register int o;
...
_SendSetGeometry(Display *dpy, XkbGeometryPtr geom, xkbSetGeometryReq *req)
{
    ...
    else
    {
        ...
        if (!tbuf) return BadAlloc;
        wire = tbuf;
    }
    wire = _WriteCountedString(wire, geom->label_font);
    if (geom->num_properties > 0) wire = _WriteGeomProperties(wire, geom);
    if (geom->num_colors > 0) wire = _WriteGeomColors(wire, geom);
    if (geom->num_shapes > 0) wire = _WriteGeomShapes(wire, geom);
    if (geom->num_sections > 0) wire = _WriteGeomSections(wire, geom);
    if (geom->num_doodads > 0)
        wire = _WriteGeomDoodads(wire, geom->num_doodads, geom->doodads);
    if (geom->num_key_aliases > 0) wire = _WriteGeomKeyAliases(wire, geom);
    if (tbuf != NULL)
    {
        Data(dpy, tbuf, sz);
