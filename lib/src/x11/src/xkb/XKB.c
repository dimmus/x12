...
XkbQueryExtension(Display *dpy,
                  ...
                  int     *minorReturn)
{
    if (!XkbUseExtension(dpy, majorReturn, minorReturn)) return False;
    if (opcodeReturn) *opcodeReturn = dpy->xkb_info->codes->major_opcode;
    if (eventBaseReturn) *eventBaseReturn = dpy->xkb_info->codes->first_event;
    if (errorBaseReturn) *errorBaseReturn = dpy->xkb_info->codes->first_error;
    if (majorReturn) *majorReturn = dpy->xkb_info->srv_major;
    if (minorReturn) *minorReturn = dpy->xkb_info->srv_minor;
    return True;
}
                ...
                unsigned int selectAll)
{
    ...
    if (affect & XkbMapNotifyMask)
    {
        ...
        if (selectAll & XkbMapNotifyMask) req->map = XkbAllMapEventsMask;
...
{
    if (!dpy->xkb_info) XkbUseExtension(dpy, NULL, NULL);
                     ...
                     unsigned int *rtrn_ctrls)
{
    ...
    if (rtrn_flags) *rtrn_flags = rep.currentFlags;
    if (rtrn_ctrls) *rtrn_ctrls = rep.currentCtrls;
    UnlockDisplay(dpy);
    SyncHandle();
    return True;
}
                       ...
                       unsigned char *map_rtrn)
{
    ...
    if ((!xkb) || (!type) || (!xkb->server)) return False;

    if (type->mods.vmods != 0)
    {
        if (!XkbVirtualModsToReal(xkb, type->mods.vmods, &tmp)) return False;
...
XkbSetDetectableAutoRepeat(Display *dpy, Bool detectable, Bool *supported)
{
    ...
    if (detectable) req->value = XkbPCF_DetectableAutoRepeatMask;
    else req->value = 0;
                        ...
                        unsigned *auto_ctrl_values)
{
    ...
    UnlockDisplay(dpy);
    SyncHandle();
    if (auto_ctrls) *auto_ctrls = rep.autoCtrls;
    if (auto_ctrl_values) *auto_ctrl_values = rep.autoCtrlValues;
    return ((rep.value & XkbPCF_AutoResetControlsMask) != 0);
}
...
XkbSetPerClientControls(Display *dpy, unsigned change, unsigned *values)
{
    ...
    if ((dpy->flags & XlibDisplayNoXkb) ||
        (!dpy->xkb_info && !XkbUseExtension(dpy, NULL, NULL)) ||
        (change &
         ~(XkbPCF_GrabsUseXKBStateMask | XkbPCF_LookupStateWhenGrabbed |
               ...
               int          *reason)
{
    ...
    if ((major_rtrn != NULL) && (minor_rtrn != NULL))
    {
        ...
        {
            if (reason != NULL) *reason = XkbOD_BadLibraryVersion;
            return NULL;
        }
    }
    else
    ...
    if (dpy == NULL)
    {
        if (reason != NULL) *reason = XkbOD_ConnectionRefused;
        return NULL;
    }
    ...
    if (reason != NULL) *reason = XkbOD_Success;
    return dpy;
}
