...
XkbInitCanonicalKeyTypes(XkbDescPtr xkb, unsigned which, int keypadVMod)
{
    ...
    map = xkb->map;
    if ((which & XkbAllRequiredTypes) == 0) return Success;
    rtrn = Success;
    from = canonicalTypes;
    to   = map->types;
                          ...
                          KeySym *xkb_syms_rtrn)
{
    ...
    for (i = 0; i < XkbNumKbdGroups; i++)
    {
        if ((protected & (1 << i)) && (types_inout[i] < xkb->map->num_types))
        {
            ...
            if (nSyms[i] > groupsWidth) groupsWidth = nSyms[i];
        }
        else
        {
    ...
    if (nSyms[XkbGroup1Index] < 2) nSyms[XkbGroup1Index] = 2;
    if (nSyms[XkbGroup2Index] < 2) nSyms[XkbGroup2Index] = 2;
    ...
    if ((tmp >= map_width) && ((protected & (XkbExplicitKeyType3Mask |
                                             XkbExplicitKeyType4Mask)) == 0))
    {
    ...
    if (empty)
    {
        ...
        {
            if (((empty & (1 << i)) == 0) || (protected & (1 << i))) break;
            nGroups--;
        }
    }
    ...
    if (nGroups > 1)
    {
        ...
        {
            sameType =
                (sameType && (types_inout[i] == types_inout[XkbGroup1Index]));
            if (allOneLevel)
                allOneLevel = (xkb->map->types[types_inout[i]].num_levels == 1);
        }
        if ((sameType) && (!(protected & (XkbExplicitKeyTypesMask &
                                          ~XkbExplicitKeyType1Mask))))
        {
            ...
            for (i = 1, identical = True; identical && (i < nGroups); i++)
            {
                ...
                {
                    if (syms[s] != xkb_syms_rtrn[s]) identical = False;
                }
            }
            if (identical) nGroups = 1;
        }
...
XkbApplyCompatMapToKey(XkbDescPtr xkb, KeyCode key, XkbChangesPtr changes)
{
    ...
    if ((!xkb) || (!xkb->map) || (!xkb->map->key_sym_map) || (!xkb->compat) ||
        (!xkb->compat->sym_interpret) || (key < xkb->min_key_code) ||
    ...
    for (n = 0; n < nSyms; n++)
    {
        ...
        if (syms[n] != NoSymbol)
        {
            ...
            if (interps[n] && interps[n]->act.type != XkbSA_NoAction) found++;
            else interps[n] = NULL;
        }
    }
    ...
    else
    {
        ...
        if (!pActs)
        {
            if (nSyms > IBUF_SIZE) Xfree(interps);
            return False;
        }
        new_vmodmask = 0;
        for (n = 0; n < nSyms; n++)
    ...
    if (changes)
    {
        ...
        {
            _XkbAddKeyChange(&mc->first_key_behavior,
        ...
        if (tmp & XkbVirtualModMapMask)
            _XkbAddKeyChange(&mc->first_vmodmap_key,
                             &mc->num_vmodmap_keys,
                             key);
    ...
    if (interps != ibuf) _XkbFree(interps);
    return True;
}
                     ...
                     XkbChangesPtr changes)
{
    ...
    for (key = first_key; key <= last_key; key++, syms += map_width)
    {
        ...
        nG                    = XkbKeyTypesForCoreSymbols(xkb,
                                       map_width,
                                       syms,
                                       explicit,
                                       types,
                                       tsyms);
        if (changes) mc = &changes->map;
        else mc = NULL;
        XkbChangeTypesOfKey(xkb, key, nG, XkbAllGroupsMask, types, mc);
        memcpy((char *)XkbKeySymsPtr(xkb, key),
               (char *)tsyms,
        ...
        (changes->map.changed & (XkbVirtualModMapMask | XkbModifierMapMask)))
    {
        ...
        {
            if (xkb->server->vmodmap[key] == 0) continue;
                    ...
                    XkbMapChangesPtr changes)
{
    ...
    {
        if (groups & (1 << i)) newTypes[i] = newTypesIn[i];
        else if (i < nOldGroups) newTypes[i] = XkbKeyKeyTypeIndex(xkb, key, i);
        else if (nOldGroups > 0)
            newTypes[i] = XkbKeyKeyTypeIndex(xkb, key, XkbGroup1Index);
        else newTypes[i] = XkbTwoLevelIndex;
        if (newTypes[i] > xkb->map->num_types) return BadMatch;
        pNewType = &xkb->map->types[newTypes[i]];
        if (pNewType->num_levels > width) width = pNewType->num_levels;
    }
    ...
    if ((width != oldWidth) || (nGroups != nOldGroups))
    {
        ...
        memcpy(oldSyms,
               pSyms,
               (size_t)XkbKeyNumSyms(xkb, key) * sizeof(KeySym));
        pSyms = XkbResizeKeySyms(xkb, key, width * nGroups);
        if (pSyms == NULL) return BadAlloc;
        ...
        if (XkbKeyHasActions(xkb, key))
        {
            ...
            memcpy(oldActs,
                   pActs,
                   (size_t)XkbKeyNumSyms(xkb, key) * sizeof(XkbAction));
            pActs = XkbResizeKeyActions(xkb, key, width * nGroups);
            if (pActs == NULL) return BadAlloc;
    ...
    if (changes != NULL)
    {
        ...
        {
            _XkbAddKeyChange(&changes->first_key_sym,
...
XkbVirtualModsToReal(XkbDescPtr xkb, unsigned virtual_mask, unsigned *mask_rtrn)
{
    ...
    if (xkb == NULL) return False;
    if (virtual_mask == 0)
    {
        *mask_rtrn = 0;
        return True;
    }
    if (xkb->server == NULL) return False;
    for (i = mask = 0, bit = 1; i < XkbNumVirtualMods; i++, bit <<= 1)
    {
        if (virtual_mask & bit) mask |= xkb->server->vmods[i];
    }
    *mask_rtrn = mask;
    return True;
}
                            ...
                            XkbChangesPtr changes)
{
    ...
    if (changes)
    {
        ...
        if ((type_ndx < 0) || (type_ndx > xkb->map->num_types)) return;
                          ...
                          XkbChangesPtr changes)
{
    ...
    if ((!xkb) || (!xkb->map) || (changed == 0)) return False;
    for (i = 0; i < xkb->map->num_types; i++)
    {
        if (xkb->map->types[i].mods.vmods & changed)
            XkbUpdateKeyTypeVirtualMods(xkb,
                                        &xkb->map->types[i],
                                        changed,
                                        changes);
    }
    ...
    if (xkb->map && xkb->server)
    {
        ...
        { /* something changed */
            if (changes->map.changed & XkbKeyActionsMask)
            {
                ...
                if (last > highChange) highChange = last;
            }
