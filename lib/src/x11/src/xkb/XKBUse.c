...
XkbNoteMapChanges(XkbMapChangesPtr old, XkbMapNotifyEvent *new, unsigned wanted)
{
    ...
    if (wanted & XkbKeyTypesMask)
    {
        if (old->changed & XkbKeyTypesMask)
        {
            ...
            if (new->first_type < first) first = new->first_type;
    ...
    if (wanted & XkbKeySymsMask)
    {
        if (old->changed & XkbKeySymsMask)
        {
            ...
            if (new->first_key_sym < first) first = new->first_key_sym;
            if (oldLast > newLast) newLast = oldLast;
    ...
    if (wanted & XkbKeyActionsMask)
    {
        if (old->changed & XkbKeyActionsMask)
        {
            ...
            if (new->first_key_act < first) first = new->first_key_act;
            if (oldLast > newLast) newLast = oldLast;
    ...
    if (wanted & XkbModifierMapMask)
    {
        if (old->changed & XkbModifierMapMask)
        {
            ...
            if (new->first_modmap_key < first) first = new->first_modmap_key;
            if (oldLast > newLast) newLast = oldLast;
    ...
    if (wanted & XkbVirtualModMapMask)
    {
        if (old->changed & XkbVirtualModMapMask)
        {
            ...
            if (new->first_vmodmap_key < first) first = new->first_vmodmap_key;
            if (oldLast > newLast) newLast = oldLast;
                       ...
                       unsigned int wanted)
{
    ...
    if ((new->request == MappingKeyboard) && (wanted & XkbKeySymsMask))
    {
        if (old->changed & XkbKeySymsMask)
        {
            ...
            if (new->first_keycode < first) first = new->first_keycode;
            if (oldLast > newLast) newLast = oldLast;
            old->first_key_sym = first;
            old->num_key_syms  = newLast - first + 1;
        }
        ...
        case XkbMapNotify:
            {
                ...
                else if (mn->nKeySyms > 0)
                {
                    ...
                    _XkbNoteCoreMapChanges(&xkbi->changes,
                                           ev,
                                           XKB_XLIB_MAP_MASK);
                    if (xkbi->changes.changed) xkbi->flags |= XkbMapPending;
                    return True;
...
#ifdef DEBUG
            fprintf(stderr,
                    "Got unknown XKEYBOARD event (%d, base=%d)\n",
                    re->type,
...
XkbIgnoreExtension(Bool ignore)
{
    ...
    {
        fprintf(stderr,
                "Explicitly %signoring XKEYBOARD\n",
                ignore ? "" : "not ");
    }
#endif
...
_XkbFreeInfo(Display *dpy)
{
    ...
    if (xkbi)
    {
        if (xkbi->desc) XkbFreeKeyboard(xkbi->desc, XkbAllComponentsMask, True);
        Xfree(xkbi);
        dpy->xkb_info = NULL;
    }
}
...
XkbUseExtension(Display *dpy, int *major_rtrn, int *minor_rtrn)
{
    ...
    if (!dpy->xkb_info)
    {
        ...
        if (debugMsg)
        {
            ...
            fprintf(
                stderr,
                "XKEYBOARD XLookupString: %sconsume shift and lock, %scontrol "
                "fallback\n",
    ...
    if ((codes = XInitExtension(dpy, XkbName)) == NULL)
    {
        ...
        dpy->flags |= XlibDisplayNoXkb;
        UnlockDisplay(dpy);
        if (debugMsg) fprintf(stderr, "XKEYBOARD extension not present\n");
        return False;
    }
    xkbi->codes = codes;
    LockDisplay(dpy);
    ...
    if (!_XReply(dpy, (xReply *)&rep, 0, xFalse) || !rep.supported)
    {
        ...
        if ((rep.serverMajor == 0) && (rep.serverMinor == 65))
        {
            ...
            if (_XReply(dpy, (xReply *)&rep, 0, xFalse) && rep.supported)
            {
                if (debugMsg) fprintf(stderr, "succeeded\n");
                fail = False;
            }
    ...
    {
        fprintf(stderr,
                "Internal Error!  XkbUseExtension succeeded with forceIgnore "
                "set\n");
    }
#endif
    UnlockDisplay(dpy);
    ...
    if (debugMsg)
        fprintf(stderr,
                "XKEYBOARD (version %d.%02d/%d.%02d) OK!\n",
                XkbMajorVersion,
                XkbMinorVersion,
                rep.serverMajor,
                rep.serverMinor);
