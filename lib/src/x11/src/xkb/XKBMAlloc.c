...
XkbAllocClientMap(XkbDescPtr xkb, unsigned which, unsigned nTotalTypes)
{
    ...
    if ((xkb == NULL) ||
        ...
        return BadValue;
    if ((which & XkbKeySymsMask) && ((!XkbIsLegalKeycode(xkb->min_key_code)) ||
                                     (!XkbIsLegalKeycode(xkb->max_key_code)) ||
                                     (xkb->max_key_code < xkb->min_key_code)))
    {
#ifdef DEBUG
        fprintf(stderr,
                "bad keycode (%d,%d) in XkbAllocClientMap\n",
                xkb->min_key_code,
                xkb->max_key_code);
#endif
        return BadValue;
    ...
    if (which & XkbKeySymsMask)
    {
        ...
        if (map->key_sym_map == NULL)
        {
            ...
            if (map->key_sym_map == NULL) return BadAlloc;
        }
    }
    if (which & XkbModifierMapMask)
    {
        ...
        if (map->modmap == NULL)
        {
            ...
            if (map->modmap == NULL) return BadAlloc;
        }
    }
    return Success;
}
...
XkbAllocServerMap(XkbDescPtr xkb, unsigned which, unsigned nNewActions)
{
    ...
    if (which & XkbExplicitComponentsMask)
    {
        ...
        if (map->explicit == NULL)
        {
            ...
            if (map->explicit == NULL) return BadAlloc;
        }
    }
    if (which & XkbKeyActionsMask)
    {
        if ((!XkbIsLegalKeycode(xkb->min_key_code)) ||
            ...
            (xkb->max_key_code < xkb->min_key_code))
            return BadMatch;
        if (nNewActions < 1) nNewActions = 1;
        if (map->acts == NULL)
        {
            map->num_acts  = 1;
            map->size_acts = 0;
        ...
        if (map->key_acts == NULL)
        {
            ...
            if (map->key_acts == NULL) return BadAlloc;
        }
    }
    if (which & XkbKeyBehaviorsMask)
    {
        ...
        if (map->behaviors == NULL)
        {
            ...
            if (map->behaviors == NULL) return BadAlloc;
        }
    }
    if (which & XkbVirtualModMapMask)
    {
        ...
        if (map->vmodmap == NULL)
        {
            ...
            if (map->vmodmap == NULL) return BadAlloc;
        }
    }
    return Success;
}
...
XkbCopyKeyType(XkbKeyTypePtr from, XkbKeyTypePtr into)
{
    ...
    *into = *from;
    ...
    if ((from->preserve) && (into->map_count > 0))
    {
        ...
        if (!into->preserve) return BadAlloc;
        memcpy(into->preserve,
               from->preserve,
    ...
    if ((from->level_names) && (into->num_levels > 0))
    {
        ...
        if (!into->level_names) return BadAlloc;
        memcpy(into->level_names,
               from->level_names,
...
XkbCopyKeyTypes(XkbKeyTypePtr from, XkbKeyTypePtr into, int num_types)
{
    ...
    if ((!from) || (!into) || (num_types < 0)) return BadMatch;
    for (i = 0; i < num_types; i++)
    {
        if ((rtrn = XkbCopyKeyType(from++, into++)) != Success) return rtrn;
    }
    return Success;
}
              ...
              int        num_lvls)
{
    ...
    if ((map) && (map->types))
    {
        for (i = 0; i < map->num_types; i++)
        {
            ...
            {
                Status status = XkbResizeKeyType(xkb,
                                                 i,
                                                 map_count,
                                                 want_preserve,
                                                 num_lvls);
    ...
    if ((!map) || (!map->types) || (map->num_types < XkbNumRequiredTypes))
    {
        ...
        if (!map) map = xkb->map;
        tmp = 0;
        if (map->num_types <= XkbKeypadIndex) tmp |= XkbKeypadMask;
        if (map->num_types <= XkbAlphabeticIndex) tmp |= XkbAlphabeticMask;
        if (map->num_types <= XkbTwoLevelIndex) tmp |= XkbTwoLevelMask;
        if (map->num_types <= XkbOneLevelIndex) tmp |= XkbOneLevelMask;
        if (XkbInitCanonicalKeyTypes(xkb, tmp, XkbNoModifier) == Success)
        {
            for (i = 0; i < map->num_types; i++)
            {
                ...
                if (map->types[i].name != name) continue;
                status = XkbResizeKeyType(xkb,
                                          i,
                                          map_count,
                                          want_preserve,
                                          num_lvls);
                 ...
                 int        new_num_lvls)
{
    ...
    switch (type_ndx)
    {
        case XkbOneLevelIndex:
            if (new_num_lvls != 1) return BadMatch;
            break;
        case XkbTwoLevelIndex:
        case XkbAlphabeticIndex:
        case XkbKeypadIndex:
            if (new_num_lvls != 2) return BadMatch;
            break;
    }
    ...
    {
        _XkbResizeArray(type->level_names,
                        type->num_levels,
                        new_num_lvls,
                        Atom);
        if (!type->level_names)
    ...
    if (new_num_lvls > type->num_levels)
    {
        ...
        for (nTotal = 1, i = xkb->min_key_code; i <= xkb->max_key_code; i++)
        {
            ...
            if (width < type->num_levels) continue;
        ...
        if (nResize > 0)
        {
            ...
            if (newSyms == NULL) return BadAlloc;
            nextMatch = 0;
            nSyms     = 1;
    ...
    if (nMatchingKeys > 0)
    {
        ...
        if (new_num_lvls > type->num_levels) firstClear = type->num_levels;
...
XkbResizeKeySyms(XkbDescPtr xkb, int key, int needed)
{
    ...
    for (i = xkb->min_key_code; i <= (int)xkb->max_key_code; i++)
    {
        ...
        if ((nKeySyms == 0) && (i != key)) continue;
        if (i == key) nKeySyms = needed;
        if (nCopy != 0)
            memcpy(&newSyms[nSyms],
                   XkbKeySymsPtr(xkb, i),
                   nCopy * sizeof(KeySym));
                      ...
                      XkbChangesPtr changes)
{
    ...
    if ((!xkb) || (minKC < XkbMinLegalKeyCode) || (maxKC > XkbMaxLegalKeyCode))
        return BadValue;
    if (minKC > maxKC) return BadMatch;
    if (minKC < xkb->min_key_code)
    {
        if (changes) changes->map.min_key_code = minKC;
    ...
    {
        if (changes) changes->map.max_key_code = maxKC;
        tmp = maxKC - xkb->max_key_code;
        if (xkb->map)
        {
            if (xkb->map->key_sym_map)
            {
                _XkbResizeArray(xkb->map->key_sym_map,
                                xkb->max_key_code + 1,
                                (maxKC + 1),
                                XkbSymMapRec);
                if (!xkb->map->key_sym_map)
            ...
            if (xkb->map->modmap)
            {
                _XkbResizeArray(xkb->map->modmap,
                                xkb->max_key_code + 1,
                                (maxKC + 1),
                                unsigned char);
                if (!xkb->map->modmap)
        ...
        if (xkb->server)
        {
            if (xkb->server->behaviors)
            {
                _XkbResizeArray(xkb->server->behaviors,
                                xkb->max_key_code + 1,
                                (maxKC + 1),
                                XkbBehavior);
                if (!xkb->server->behaviors)
            ...
            if (xkb->server->key_acts)
            {
                _XkbResizeArray(xkb->server->key_acts,
                                xkb->max_key_code + 1,
                                (maxKC + 1),
                                unsigned short);
                if (!xkb->server->key_acts)
            ...
            if (xkb->server->vmodmap)
            {
                _XkbResizeArray(xkb->server->vmodmap,
                                xkb->max_key_code + 1,
                                (maxKC + 1),
                                unsigned short);
                if (!xkb->server->vmodmap)
        ...
        if ((xkb->names) && (xkb->names->keys))
        {
            _XkbResizeArray(xkb->names->keys,
                            xkb->max_key_code + 1,
                            (maxKC + 1),
                            XkbKeyNameRec);
            if (!xkb->names->keys)
...
XkbResizeKeyActions(XkbDescPtr xkb, int key, int needed)
{
    ...
    for (i = xkb->min_key_code; i <= (int)xkb->max_key_code; i++)
    {
        ...
        if ((xkb->server->key_acts[i] == 0) && (i != key)) continue;

        nCopy = nKeyActs = XkbKeyNumActions(xkb, i);
        if (i == key)
        {
            nKeyActs = needed;
            if (needed < nCopy) nCopy = needed;
        }

        if (nCopy > 0)
            memcpy(&newActs[nActs],
                   XkbKeyActionsPtr(xkb, i),
...
XkbFreeClientMap(XkbDescPtr xkb, unsigned what, Bool freeMap)
{
    ...
    if ((xkb == NULL) || (xkb->map == NULL)) return;
    if (freeMap) what = XkbAllClientInfoMask;
    map = xkb->map;
...
XkbFreeServerMap(XkbDescPtr xkb, unsigned what, Bool freeMap)
{
    ...
    if ((xkb == NULL) || (xkb->server == NULL)) return;
    if (freeMap) what = XkbAllServerInfoMask;
    map = xkb->server;
