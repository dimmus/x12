                     ...
                     unsigned int *intervalp)
{
    ...
    if (!_XReply(dpy,
                 (xReply *)&rep,
                 (SIZEOF(xkbGetControlsReply) - SIZEOF(xReply)) >> 2,
                 xFalse))
    {
        UnlockDisplay(dpy);
        SyncHandle();
...
XkbGetControls(Display *dpy, unsigned long which, XkbDescPtr xkb)
{
    ...
    ctrls = xkb->ctrls;
    if (which & XkbControlsEnabledMask) ctrls->enabled_ctrls = rep.enabledCtrls;
    ctrls->num_groups = rep.numGroups;
    if (which & XkbGroupsWrapMask) ctrls->groups_wrap = rep.groupsWrap;
    ...
    if (which & XkbSlowKeysMask) ctrls->slow_keys_delay = rep.slowKeysDelay;
    if (which & XkbBounceKeysMask) ctrls->debounce_delay = rep.debounceDelay;
    ...
    if (which & XkbAccessXKeysMask) ctrls->ax_options = rep.axOptions;
...
XkbSetControls(Display *dpy, unsigned long which, XkbDescPtr xkb)
{
    ...
    if (which & XkbSlowKeysMask) req->slowKeysDelay = ctrls->slow_keys_delay;
    if (which & XkbBounceKeysMask) req->debounceDelay = ctrls->debounce_delay;
    if (which & XkbMouseKeysMask)
    {
        req->mkDfltBtn = ctrls->mk_dflt_btn;
    }
    if (which & XkbGroupsWrapMask) req->groupsWrap = ctrls->groups_wrap;
    if (which &
