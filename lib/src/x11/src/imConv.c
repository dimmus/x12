...
typedef int (*ucstocsConvProc)(XPointer, unsigned char *, ucs4_t, int);
...
/*ARGSUSED*/
int
_XimGetCharCode(XPointer       ucs_conv,
                KeySym         keysym,
                unsigned char *buf,
                int            nbytes)
{
    ...
    else if (cvt)
    {
        ...
        if (ucs4) count = (*cvt)((XPointer)NULL, buf, ucs4, nbytes);
    }

    if (count < 0) count = 0;
    if (count > nbytes) return nbytes;
    if (count < nbytes) buf[count] = '\0';
    return count;
}

#ifdef XKB
static int
lookup_string(XKeyEvent      *event,
              char           *buffer,
              int             nbytes,
              KeySym         *keysym,
              XComposeStatus *status)
{
    ...
    XkbSetXlibControls(event->display, XkbLC_ForceLatin1Lookup, ctrls);
    return ret;
}
#endif
                 ...
                 XComposeStatus *status)
{
    ...
    if (count > 1)
    {
        if ((unsigned)count >= sizeof(look)) return 0;
    ...
    else if ((count == 0) || (count == 1 && (symbol > 0x7f && symbol < 0xff00)))
    {
