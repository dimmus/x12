...
static int
parse_line1(char *line, char **argv, int argsize)
{
    int   argc = 0;
    char *p    = line;
...
static int
_XlcParsePath(char *path, char **argv, int argsize)
{
    char *p = path;
    int   n, i;
...
void
xlocaledir(char *buf, int buf_len)
{
    char *p   = buf;
    int   len = 0;
...
#  endif
    ...
    if (len < buf_len) strncpy(p, XLOCALEDIR, (size_t)(buf_len - len));
    buf[buf_len - 1] = '\0';
}

static void
xlocalelibdir(char *buf, int buf_len)
{
    char *p   = buf;
    int   len = 0;
...
#  endif
    ...
    if (len < buf_len) strncpy(p, XLOCALELIBDIR, (size_t)(buf_len - len));
    buf[buf_len - 1] = '\0';
}
...
static char *
resolve_name(const char *lc_name, char *file_name, MapDirection direction)
{
    FILE *fp;
...
static char *
lowercase(char *dst, const char *src)
{
    const char *s;
    char       *t;
...
/************************************************************************/
char *
_XlcFileName(XLCd lcd, const char *category)
{
    char *siname;
    ...
    if (lcd == (XLCd)NULL) return NULL;

    siname = XLC_PUBLIC(lcd, siname);

    if (category) lowercase(cat, category);
    else cat[0] = '\0';
...
int
_XlcResolveLocaleName(const char *lc_name, XLCdPublicPart *pub)
{
    ...
    for (i = 0; i < n; ++i)
    {
        ...
        if (snprintf(buf, PATH_MAX, "%s/%s", args[i], locale_alias) < PATH_MAX)
        {
            name = resolve_name(lc_name, buf, LtoR);
            if (!name)
            {
                if (!nlc_name) nlc_name = normalize_lcname(lc_name);
                if (nlc_name) name = resolve_name(nlc_name, buf, LtoR);
            }
        }
        if (name != NULL)
        {
    ...
    else dst = &pub->siname[sinamelen + 1];
...
_XlcLocaleDirName(char *dir_name, size_t dir_len, const char *lc_name)
{
    ...
    else snprintf(dir_name, dir_len, "%s/%s", target_dir, target_name);
