 ...
 */
static unsigned int
_XlcParseCT(const char **text, int *length, unsigned char *final_byte)
{
    unsigned int         ret = 0;
    unsigned char        ch;
    ...
    switch (ch = *str++)
    {
        ...
        case XctCSI:
            ...
            switch (*str++)
            {
                case XctLeftToRight:
                    if (*length < 3) return 0;
                    if (*str++ == XctDirection) ret = XctLeftToRight;
                    break;
                case XctRightToLeft:
                    if (*length < 3) return 0;
                    if (*str++ == XctDirection) ret = XctRightToLeft;
                    break;
 ...
 */
Bool
_XlcParseCharSet(XlcCharSet charset)
{
    unsigned int  type;
    ...
    switch (type)
    {
        ...
        case XctExtSeg:
            ...
            if (!(char_size >= 0 && char_size <= 4)) return False;
            break;
        case XctOtherCoding:
            char_size = 0;
            break;
 ...
 */
static CTInfo
_XlcGetCTInfo(unsigned int  type,
              unsigned char final_byte,
              const char   *text,
              int           text_len)
{
...
_XlcGetCTInfoFromCharSet(XlcCharSet charset)
{
    ...
    for (ct_info = ct_list; ct_info; ct_info = ct_info->next)
        if (ct_info->charset == charset) return ct_info;
...
XlcCharSet
_XlcAddCT(const char *name, const char *ct_sequence)
{
    ...
    if (ct_info == NULL) return charset;
    ...
    existing_info = _XlcGetCTInfo(type,
                                  ct_info->final_byte,
                                  ct_info->ext_segment,
    ...
    if (existing_info == NULL)
    {
        ...
        ct_info->next = NULL;
        if (ct_list_end) ct_list_end->next = ct_info;
        else ct_list = ct_info;
        ct_list_end = ct_info;
    }
    else
    {
...
static CheckResult
_XlcCheckCTSequence(State state, const char **ctext, int *ctext_len)
{
    XlcCharSet    charset;
    CTInfo        ct_info;
    ...
    switch (type)
    {
        ...
        case XctReturn:
            ...
            *ctext_len           = tmp_ctext_len;
            ...
            return resOK;
        case XctExtSeg:
            if (tmp_ctext_len > 2 && (tmp_ctext[0] & 0x80) &&
                (tmp_ctext[1] & 0x80))
            {
    ...
    while (ctext_len > 0 && buf_len > 0)
    {
        if (state->ext_seg_left == 0)
        {
            ...
            if (ch == XctESC)
            {
                ...
                if (ret == resOK) /* state has been modified. */
                    continue;
                if (ret == resNotInList)
                {
            ...
            if (buf_len < 6)
            {
                ...
                {
                    if (buf_len < charset->char_size) break;
                }
                else
                {
        ...
        else
        {
            ...
            if (charset)
            {
                if (charset != state->charset) break;
            }
            else
            {
       ...
       int       num_args)
{
    ...
    if (ct_info->type == XctOtherCoding)
    {
        ...
        if (ct_len < length + 3) return -1;

        memcpy(ctptr, ct_info->ct_sequence, (size_t)length);
        ctptr += length;
        ct_len -= length + 3;
    }
    else /* Test whether the charset is already active. */
             ...
             charset != state->GL_charset))
        {
            ...
            {
                if (ct_len < length + 2 + ct_info->ext_segment_len) return -1;
                ...
                if (ct_len > 0x3fff) ct_len = 0x3fff;

                memcpy(ctptr,
                       ct_info->ext_segment,
                       (size_t)ct_info->ext_segment_len);
                ctptr += ct_info->ext_segment_len;
                ct_len -= ct_info->ext_segment_len;
            }
            else
            {
                if (ct_len < length) return -1;
    ...
    if (charset->set_size)
    {
        ...
        {
            max_ch--;
            if (charset->char_size > 1 || side == XlcGR) min_ch++;
        }
    }
    ...
    if (char_size == 1)
    {
        while (csstr_len > 0 && ct_len > 0)
        {
		   ...
		   Therefore here we eliminate other control characters. */
            ...
            if (side == XlcGL) *ctptr++ = *csptr++ & 0x7f;
            else if (side == XlcGR) *ctptr++ = *csptr++ | 0x80;
            else *ctptr++ = *csptr++;
            csstr_len--;
            ct_len--;
        }
            ...
            while (csstr_len > 0 && ct_len > 0)
            {
                ...
                int           ch_size = (ch < 0xc0   ? 1
                                         ...
                                         : ch < 0xfc ? 5
                                                     : 6);
                int           i;
                if (!(csstr_len >= ch_size && ct_len >= ch_size)) break;
                for (i = ch_size; i > 0; i--)
                    *ctptr++ = *csptr++;
                csstr_len -= ch_size;
                ct_len -= ch_size;
    ...
    {
        if (side == XlcGR || side == XlcGLGR) state->GR_charset = charset;
        if (side == XlcGL || side == XlcGLGR) state->GL_charset = charset;
    }
        ...
        int       num_args)
{
    ...
    if (num_args < 1 || !((XlcCharSet)args[0] == state->GL_charset ||
...
static XlcConvMethodsRec cttocs_methods = { close_converter,
                                            cttocs,
                                            init_state };

static XlcConv
open_cttocs(XLCd        from_lcd,
...
static XlcConvMethodsRec cstoct_methods = { close_converter,
                                            cstoct,
                                            init_state };

static XlcConv
open_cstoct(XLCd        from_lcd,
            const char *from_type,
            XLCd        to_lcd,
            const char *to_type)
{
...
static XlcConvMethodsRec strtocs_methods = { close_converter,
                                             strtocs,
                                             init_state };

static XlcConv
open_strtocs(XLCd        from_lcd,
             const char *from_type,
             XLCd        to_lcd,
             const char *to_type)
{
...
static XlcConvMethodsRec cstostr_methods = { close_converter,
                                             cstostr,
                                             init_state };

static XlcConv
open_cstostr(XLCd        from_lcd,
             const char *from_type,
             XLCd        to_lcd,
             const char *to_type)
{
...
_XlcInitCTInfo(void)
{
    if (ct_list == NULL)
    {
        ...
        for (ct_data = default_ct_data; num > 0; ct_data++, num--)
        {
            ...
            else charset->source = CSsrcXLC;
        }

        /* Register CompoundText and CharSet converters.  */

        _XlcSetConverter((XLCd)NULL,
                         XlcNCompoundText,
                         (XLCd)NULL,
                         XlcNCharSet,
                         open_cttocs);
