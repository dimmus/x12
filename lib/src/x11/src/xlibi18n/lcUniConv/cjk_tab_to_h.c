 ...
 */
static void
output_title(const char *charsetname)
{
    printf("\n");
    printf("/*\n");
    printf(" * %s\n", charsetname);
...
read_table(Encoding *enc)
{
    ...
    if (c == '#')
    {
        ...
        for (;;)
        {
            ...
            if (scanf(" 0x%x", &enc->charset2uni[row][col]) != 1) exit(1);
        }
    }
    else
    {
        ...
        for (i1 = 32; i1 < 132; i1++)
            ...
            {
                i = scanf("%x", &j);
                if (i == EOF) goto read_done;
                if (i != 1) exit(1);
                if (j < 0 || j == 0xffff) j = 0xfffd;
                if (j != 0xfffd)
                {
 ...
 */
static void
find_charset2uni_pages(Encoding *enc)
{
    int row, col;
    ...
    for (row = 0; row < enc->rows; row++)
    {
        int used = 0;
        for (col = 0; col < enc->cols; col++)
            if (enc->charset2uni[row][col] != 0xfffd) used = col + 1;
 ...
 */
static void
find_charset2uni_blocks(Encoding *enc)
{
    int    n, row, lastrow;
    size_t max;
    ...
    for (row = 0; row < enc->rows; row++)
        if (enc->charsetpage[row] > 0 &&
            (row == 0 || enc->charsetpage[row - 1] == 0))
        {
            for (lastrow = row; enc->charsetpage[lastrow + 1] > 0; lastrow++)
                ;
            enc->charsetblocks[n].start = row * enc->cols;
            enc->charsetblocks[n].end =
                lastrow * enc->cols + enc->charsetpage[lastrow];
            n++;
        }
 ...
 */
static void
output_charset2uni(const char *name, Encoding *enc)
{
    ...
    for (row = 0; row < enc->rows; row++)
        if (enc->charsetpage[row] > 0)
        {
            ...
            {
                /* Start a new block. */
                for (lastrow = row; enc->charsetpage[lastrow + 1] > 0;
                     lastrow++)
                    ;
            ...
            printf("  /"
                   ...
                   "/\n ",
                   enc->row_byte(row));
            col_max = (enc->charsetpage[row + 1] > 0 ? enc->cols
                                                     : enc->charsetpage[row]);
    ...
    for (i = 0; i < enc->ncharsetblocks; i++)
    {
        ...
        if (i > 0) printf(" || ");
        if (i1_min == i1_max) printf("(c1 == 0x%02x)", i1_min);
        else printf("(c1 >= 0x%02x && c1 <= 0x%02x)", i1_min, i1_max);
    }
    printf(") {\n");
    printf("    if (n >= 2) {\n");
...
invert(Encoding *enc)
{
    ...
    for (row = 0; row < enc->rows; row++)
        for (col = 0; col < enc->cols; col++)
        {
            ...
            if (j != 0xfffd)
                enc->uni2charset[j] =
                    0x100 * enc->row_byte(row) + enc->col_byte(col);
        }
}
 ...
 */
static void
output_uni2charset_dense(const char *name, Encoding *enc)
{
    /* Like in 8bit_tab_to_h.c */
    /* bool pages[0x100]; */ /* -Wunused-but-set-variable */
    int line[0x2000];
    int tableno;
    ...
    for (j1 = 0; j1 < 0x2000; j1++)
    {
        ...
        {
            j = 8 * j1 + j2;
            if (enc->uni2charset[j] != 0) all_invalid = false;
        }
        if (all_invalid) line[j1] = -1;
        else line[j1] = 0;
    }
    tableno = 0;
    for (j1 = 0; j1 < 0x2000; j1++)
    ...
    for (t = 0; t < tableno; t++)
    {
        ...
        for (j = j1; j < j2; j++)
            if (enc->uni2charset[j] != 0) tables[t].usecount++;
    }
    {
        p = -1;
        for (t = 0; t < tableno; t++)
    ...
    printf("static int\n%s_wctomb (conv_t conv, unsigned char *r, ucs4_t wc, "
           "int n)\n",
           name);
    printf("{\n");
    printf("  if (n >= 2) {\n");
    ...
    for (j1 = 0; j1 < 0x2000;)
    {
        ...
        if (t >= 0)
        {
            ...
            else
            {
                ...
                if (tables[t].minline > 0) printf("-0x%04x", 8 * j1);
                printf("];\n");
            }
        }
        j1 = j2;
 ...
 */
static void
output_uni2charset_sparse(const char *name, Encoding *enc)
{
    bool  pages[0x100];
  ...
  printf("\n");
    ...
    for (i = 0; i < npageblocks; i++)
    {
        ...
        printf("if (wc >= 0x%04x && wc < 0x%04x)\n",
               16 * pageblocks[i].start,
               16 * pageblocks[i].end);
        printf("      summary = &%s_uni2indx_page%02x[(wc>>4)",
               name,
               pageblocks[i].start / 16);
        if (pageblocks[i].start > 0) printf("-0x%03x", pageblocks[i].start);
        printf("];\n");
    }
    printf("    if (summary) {\n");
    ...
    printf("        /* Add `summary->indx' and the number of bits set in "
           "`used'. */\n");
...
static int
row_byte_normal(int row)
{
    return 0x21 + row;
}

static int
col_byte_normal(int col)
{
    return 0x21 + col;
}

static int
byte_row_normal(int byte)
{
    return byte - 0x21;
}

static int
byte_col_normal(int byte)
{
    return byte - 0x21;
}

static void
do_normal(const char *name)
{
    Encoding enc;
...
static void
do_normal_only_charset2uni(const char *name)
{
    Encoding enc;
...
static int
row_byte_cns11643(int row)
{
    return 0x100 * (row / 94) + (row % 94) + 0x21;
}

static int
byte_row_cns11643(int byte)
{
...
static void
do_cns11643_only_uni2charset(const char *name)
{
    Encoding enc;
    int      j, x;
...
static int
row_byte_gbk1(int row)
{
    return 0x81 + row;
}

static int
col_byte_gbk1(int col)
{
    return (col >= 0x3f ? 0x41 : 0x40) + col;
}

static int
byte_row_gbk1(int byte)
{
    if (byte >= 0x81 && byte < 0xff) return byte - 0x81;
    else return -1;
}

static int
byte_col_gbk1(int byte)
{
    if (byte >= 0x40 && byte < 0x7f) return byte - 0x40;
    else if (byte >= 0x80 && byte < 0xff) return byte - 0x41;
    else return -1;
}

static void
do_gbk1(const char *name)
{
    Encoding enc;
...
static void
do_gbk1_only_charset2uni(const char *name)
{
    Encoding enc;
...
static int
row_byte_gbk2(int row)
{
    return 0x81 + row;
}

static int
col_byte_gbk2(int col)
{
    return (col >= 0x3f ? 0x41 : 0x40) + col;
}

static int
byte_row_gbk2(int byte)
{
    if (byte >= 0x81 && byte < 0xff) return byte - 0x81;
    else return -1;
}
...
static int
byte_col_gbk2(int byte)
{
    if (byte >= 0x40 && byte < 0x7f) return byte - 0x40;
    else if (byte >= 0x80 && byte < 0xa1) return byte - 0x41;
    else return -1;
}

static void
do_gbk2_only_charset2uni(const char *name)
{
    Encoding enc;
...
static void
do_gbk1_only_uni2charset(const char *name)
{
    Encoding enc;
 ...
 */
static void
read_table_ksc5601(Encoding *enc)
{
    int row, col, i, i1, i2, c, j;
    ...
    if (c == '#')
    {
        ...
        for (;;)
        {
         ...
         = KS X 1001.1992, ignore the rest. */
            if (!(i1 >= 128 + 33 && i1 < 128 + 127 && i2 >= 128 + 33 &&
                  i2 < 128 + 127))
...
static int
row_byte_big5(int row)
{
    return 0xa1 + row;
}

static int
col_byte_big5(int col)
{
    return (col >= 0x3f ? 0x62 : 0x40) + col;
}

static int
byte_row_big5(int byte)
{
    if (byte >= 0xa1 && byte < 0xff) return byte - 0xa1;
    else return -1;
}

static int
byte_col_big5(int byte)
{
    if (byte >= 0x40 && byte < 0x7f) return byte - 0x40;
    else if (byte >= 0xa1 && byte < 0xff) return byte - 0x62;
    else return -1;
}

static void
do_big5(const char *name)
{
    Encoding enc;
...
static int
row_byte_big5hkscs(int row)
{
    return 0x81 + row;
}

static int
col_byte_big5hkscs(int col)
{
    return (col >= 0x3f ? 0x62 : 0x40) + col;
}

static int
byte_row_big5hkscs(int byte)
{
    if (byte >= 0x81 && byte < 0xff) return byte - 0x81;
    else return -1;
}
...
static int
row_byte_johab_hangul(int row)
{
    return 0x84 + row;
}

static int
col_byte_johab_hangul(int col)
{
    return (col >= 0x3e ? 0x43 : 0x41) + col;
}

static int
byte_row_johab_hangul(int byte)
{
    if (byte >= 0x84 && byte < 0xd4) return byte - 0x84;
    else return -1;
}

static int
byte_col_johab_hangul(int byte)
{
    if (byte >= 0x41 && byte < 0x7f) return byte - 0x41;
    else if (byte >= 0x81 && byte < 0xff) return byte - 0x43;
    else return -1;
}

static void
do_johab_hangul(const char *name)
{
    Encoding enc;
...
static int
row_byte_sjis(int row)
{
    return (row >= 0x1f ? 0xc1 : 0x81) + row;
}

static int
col_byte_sjis(int col)
{
    return (col >= 0x3f ? 0x41 : 0x40) + col;
}

static int
byte_row_sjis(int byte)
{
    if (byte >= 0x81 && byte < 0xa0) return byte - 0x81;
    else if (byte >= 0xe0) return byte - 0xc1;
    else return -1;
}

static int
byte_col_sjis(int byte)
{
    if (byte >= 0x40 && byte < 0x7f) return byte - 0x40;
    else if (byte >= 0x80 && byte < 0xfd) return byte - 0x41;
    else return -1;
}

static void
do_sjis(const char *name)
{
    Encoding enc;
...
main(int argc, char *argv[])
{
    ...
    else if (!strcmp(name, "cns11643_inv")) do_cns11643_only_uni2charset(name);
    else if (!strcmp(name, "gbkext1")) do_gbk1_only_charset2uni(name);
    else if (!strcmp(name, "gbkext2")) do_gbk2_only_charset2uni(name);
    else if (!strcmp(name, "gbkext_inv")) do_gbk1_only_uni2charset(name);
    else if (!strcmp(name, "cp936ext")) do_gbk1(name);
    else if (!strcmp(name, "ksc5601")) do_ksc5601(name);
    else if (!strcmp(name, "big5") || !strcmp(name, "cp950ext")) do_big5(name);
    else if (!strcmp(name, "big5hkscs")) do_big5hkscs(name);
    else if (!strcmp(name, "johab_hangul")) do_johab_hangul(name);
    else if (!strcmp(name, "cp932ext")) do_sjis(name);
    else exit(1);
