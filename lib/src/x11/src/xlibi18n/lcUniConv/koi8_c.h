...
koi8_c_mbtowc(conv_t conv, ucs4_t *pwc, const unsigned char *s, int n)
{
    ...
    else *pwc = (ucs4_t)koi8_c_2uni[c - 0x80];
    return 1;
}
...
koi8_c_wctomb(conv_t conv, unsigned char *r, ucs4_t wc, int n)
{
    ...
    else if (wc >= 0x00a0 && wc < 0x00a1) c = koi8_c_page00[wc - 0x00a0];
    else if (wc >= 0x0400 && wc < 0x04ef) c = koi8_c_page04[wc - 0x0400];
    else if (wc >= 0x2216 && wc < 0x2217) c = koi8_c_page22[wc - 0x2216];
    if (c != 0)
    {
        *r = c;
        return 1;
