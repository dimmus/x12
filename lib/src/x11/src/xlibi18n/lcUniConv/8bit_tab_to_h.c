...
main(int argc, char *argv[])
{
    ...
    if (argc != 3 && argc != 4 && argc != 5) exit(1);
    charsetname   = argv[1];
...

    {
      ...
      /* Read a unicode.org style .TXT file. */
            ...
            for (;;)
            {
                ...
                ungetc(c, stdin);
                if (scanf("0x%x", &i) != 1 || !(i >= 0 && i < 0x100)) exit(1);
                do
                {
                    c = getc(stdin);
                }
                ...
                if (c == '\n' || c == '#') continue;
                if (scanf("0x%x", &charset2uni[i]) != 1) exit(1);
            }
        }
        else
        {
      /* Read a table of hexadecimal Unicode values. */
            for (i = 0; i < 0x100; i++)
            {
                if (scanf("%x", &charset2uni[i]) != 1) exit(1);
                if (charset2uni[i] < 0 || charset2uni[i] == 0xffff)
                    charset2uni[i] = 0xfffd;
            }
            if (scanf("%x", &i) != EOF) exit(1);
        }
    }
  ...
  /* Write the output file. */
    {
...

        {
            ...
            for (i1 = 0; i1 < 16; i1++)
            {
                ...
                {
                    i = 16 * i1 + i2;
                    if (charset2uni[i] != 0xfffd) all_invalid = false;
                    if (charset2uni[i] != i) all_identity = false;
                }
                if (all_invalid) line[i1] = -2;
                else if (all_identity) line[i1] = -1;
                else line[i1] = 0;
            }
            tableno = 0;
            for (i1 = 0; i1 < 16; i1++)
            ...
            some_invalid = false;
            for (i = 0; i < 0x100; i++)
                if (charset2uni[i] == 0xfffd) some_invalid = true;
            if (tableno > 0)
            {
                int t;
                for (t = 0; t < tableno; t++)
                {
                    fprintf(f,
                            "static const unsigned short %s_2uni",
                            c_charsetname);
                    if (tableno > 1) fprintf(f, "_%d", t + 1);
                    fprintf(f,
                            "[%d] = {\n",
            ...
            fprintf(f,
                    "static int\n%s_mbtowc (conv_t conv, ucs4_t *pwc, const "
            ...
            if (some_invalid)
            {
                for (i1 = 0; i1 < 16;)
                {
                    ...
                    else
                    {
                        ...
                        for (i = 16 * i1; i < 16 * i2; i++)
                            if (charset2uni[i] == 0xfffd) some_invalid = true;
                        if (some_invalid) fprintf(f, "unsigned short wc = ");
                        else fprintf(f, "*pwc = (ucs4_t) ");
                ...
                if (final_ret_reached) fprintf(f, "  return RET_ILSEQ;\n");
            }
            else
            {
                for (i1 = 0; i1 < 16;)
        ...
        pages[p] = false; */
          ...
          /* pages[j>>8] = true; */
            ...
            for (j1 = 0; j1 < 0x2000; j1++)
            {
                ...
                {
                    j = 8 * j1 + j2;
                    if (uni2charset[j] != 0) all_invalid = false;
                    if (uni2charset[j] != j) all_identity = false;
                }
                if (all_invalid) line[j1] = -2;
                else if (all_identity) line[j1] = -1;
                else line[j1] = 0;
            }
            tableno = 0;
            for (j1 = 0; j1 < 0x2000; j1++)
            ...
            for (t = 0; t < tableno; t++)
            {
                ...
                for (j = j1; j < j2; j++)
                    if (uni2charset[j] != 0) tables[t].usecount++;
            }
            ...
            }
            {
                ...
                for (t = 0; t < tableno; t++)
                    if (tables[t].usecount > 1)
                    {
                        ...
                        for (j1 = tables[t].minline; j1 <= tables[t].maxline;
                             j1++)
                        {
            ...
            for (j1 = 0; j1 < 0x2000;)
            {
                ...
                for (j2 = j1; j2 < 0x2000 && line[j2] == t; j2++)
                    ;
                if (t >= 0) j2 = tables[t].maxline + 1;
                if (!(t == -2 || (t == -1 && j1 == 0))) need_c = true;
                j1 = j2;
            }
            fix_0000 = false;
            fprintf(f,
                    "static int\n%s_wctomb (conv_t conv, unsigned char *r, "
                    "ucs4_t wc, int n)\n",
                    c_charsetname);
            fprintf(f, "{\n");
            ...
            if (need_c)
            {
                if (fix_0000) fprintf(f, "  if (c != 0 || wc == 0) {\n");
