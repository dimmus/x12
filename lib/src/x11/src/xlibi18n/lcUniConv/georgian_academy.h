...
{
    unsigned char c = *s;
    if (c >= 0x80 && c < 0xa0) *pwc = (ucs4_t)georgian_academy_2uni[c - 0x80];
    else if (c >= 0xc0 && c < 0xe7) *pwc = (ucs4_t)c + 0x1010;
    else *pwc = (ucs4_t)c;
    return 1;
}
...
georgian_academy_wctomb(conv_t conv, unsigned char *r, ucs4_t wc, int n)
{
    ...
    else if (wc >= 0x10d0 && wc < 0x10f7) c = (unsigned char)(wc - 0x1010);
