...
static XlcConv
get_converter(XLCd from_lcd, XrmQuark from_type, XLCd to_lcd, XrmQuark to_type)
{
...
Bool
_XlcSetConverter(XLCd                 from_lcd,
                 const char          *from,
                 XLCd                 to_lcd,
                 const char          *to,
                 XlcOpenConverterProc converter)
...
static int
indirect_convert(XlcConv   lc_conv,
                 XPointer *from,
                 int      *from_left,
                 XPointer *to,
                 int      *to_left,
                 ...
                 int       num_args)
{
    ...
    {
        if (from_conv->methods->reset) (*from_conv->methods->reset)(from_conv);

        if (to_conv->methods->reset) (*to_conv->methods->reset)(to_conv);
    ...
    while (*from_left > 0)
    {
        ...
        ret = (*from_conv->methods->convert)(from_conv,
                                             from,
                                             from_left,
                                             &cs,
                                             &cs_left,
        ...
        if (length > 0)
        {
            ...
            ret = (*to_conv->methods->convert)(to_conv,
                                               &cs,
                                               &cs_left,
                                               to,
                                               to_left,
...
close_indirect_converter(XlcConv lc_conv)
{
    ...
    if (conv)
    {
        if (conv->from_conv) close_converter(conv->from_conv);
        if (conv->to_conv) close_converter(conv->to_conv);
...
static XlcConvMethodsRec conv_methods = { close_indirect_converter,
                                          indirect_convert,
                                          reset_indirect_converter };

static XlcConv
open_indirect_converter(XLCd        from_lcd,
                        const char *from,
                        XLCd        to_lcd,
                        const char *to)
{
...
XlcConv
_XlcOpenConverter(XLCd from_lcd, const char *from, XLCd to_lcd, const char *to)
{
    XlcConv  conv;
    XrmQuark from_type, to_type;
...
void
_XlcCloseConverter(XlcConv conv)
{
    close_converter(conv);
}

int
_XlcConvert(XlcConv   conv,
            XPointer *from,
            int      *from_left,
            XPointer *to,
            int      *to_left,
...
{
    return (*conv->methods
                 ->convert)(conv, from, from_left, to, to_left, args, num_args);
}

void
_XlcResetConverter(XlcConv conv)
{
    if (conv->methods->reset) (*conv->methods->reset)(conv);
}
