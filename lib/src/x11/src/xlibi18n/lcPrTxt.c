...
alloc_list(Bool is_wide_char, int count, int nitems)
{
    if (is_wide_char)
    {
        ...
        if (wstr_list == NULL) return (XPointer *)NULL;
    ...
    else
    {
        ...
        if (str_list == NULL) return (XPointer *)NULL;
...
static int
_XTextPropertyToTextList(XLCd                 lcd,
                         Display             *dpy,
                         ...
                         int                 *count_ret)
{
    ...
    if (strcmp(XlcNWideChar, to_type) == 0) is_wide_char = True;
    ...
    if (text_prop->format != 8) return XConverterNotFound;

    encoding = text_prop->encoding;
    if (encoding == XA_STRING) from_type = XlcNString;
        ...
        from_type = XlcNCompoundText;
             ...
             XInternAtom(dpy, XLC_PUBLIC(lcd, encoding_name), False))
        from_type = XlcNMultiByte;
    else return XConverterNotFound;
    ...
    else
    {
        ...
        else buf_len = text_prop->nitems * XLC_PUBLIC(lcd, mb_cur_max) + 1;
    }
    buf = Xmalloc(buf_len);
    if (buf == NULL) return XNoMemory;
    to      = buf;
    to_left = buf_len;
    ...
    while (1)
    {
        if (nitems == 0 || *str_ptr == 0)
        {
            ...
            if (!do_strcpy) _XlcResetConverter(conv);
        }
        else str_ptr++;
    ...
    if (!do_strcpy) _XlcCloseConverter(conv);
    ...
    *list_ret = alloc_list(is_wide_char, *count_ret, buf_len - to_left);
    if (*list_ret) copy_list(is_wide_char, buf, *list_ret, *count_ret);
...
int
_XmbTextPropertyToTextList(XLCd                 lcd,
                           Display             *dpy,
                           const XTextProperty *text_prop,
                           char              ***list_ret,
                           int                 *count_ret)
{
    return _XTextPropertyToTextList(lcd,
                                    dpy,
                                    text_prop,
                                    XlcNMultiByte,
                                    (XPointer **)list_ret,
...
int
_XwcTextPropertyToTextList(XLCd                 lcd,
                           Display             *dpy,
                           const XTextProperty *text_prop,
                           wchar_t           ***list_ret,
                           int                 *count_ret)
...
int
_Xutf8TextPropertyToTextList(XLCd                 lcd,
                             Display             *dpy,
                             const XTextProperty *text_prop,
                             char              ***list_ret,
                             int                 *count_ret)
...
void
_XwcFreeStringList(XLCd lcd, wchar_t **list)
{
    if (list)
    {
        Xfree(*list);
