...
get_buf_size(Bool is_wide_char, XPointer list, int count)
{
    ...
    if (is_wide_char)
    {
        ...
        for (; count-- > 0; wc_list++)
        {
            if (*wc_list) length += _Xwcslen(*wc_list) + 1;
        }
        length *= 5; /* XXX */
    }
    else
...
static int
_XTextListToTextProperty(XLCd              lcd,
                         Display          *dpy,
                         ...
                         XTextProperty    *text_prop)
{
    ...
    if (strcmp(XlcNWideChar, from_type) == 0) is_wide_char = True;

    buf_len = get_buf_size(is_wide_char, list, count);
    if ((buf = Xmalloc(buf_len)) == NULL) return XNoMemory;
...
retry:
    ...
    if (is_wide_char) wc_list = (wchar_t **)list;
    else mb_list = (char **)list;
    ...
    if (nitems == 1) *value = 0;
    else memcpy(value, buf, (size_t)nitems);
    nitems--;
    Xfree(buf);
...
int
_XmbTextListToTextProperty(XLCd              lcd,
                           Display          *dpy,
                           char            **list,
                           int               count,
                           XICCEncodingStyle style,
                           XTextProperty    *text_prop)
{
    return _XTextListToTextProperty(lcd,
                                    dpy,
                                    XlcNMultiByte,
                                    (XPointer)list,
                                    count,
...
int
_XwcTextListToTextProperty(XLCd              lcd,
                           Display          *dpy,
                           wchar_t         **list,
                           int               count,
                           XICCEncodingStyle style,
...
int
_Xutf8TextListToTextProperty(XLCd              lcd,
                             Display          *dpy,
                             char            **list,
                             int               count,
                             XICCEncodingStyle style,
