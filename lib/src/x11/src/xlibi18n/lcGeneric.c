...
static XLCdPublicMethodsRec genericMethods = {
    ...
    { NULL, create, initialize, destroy, NULL }
};
...
static Bool
string_to_encoding(const char *str, char *encoding)
{
    char *next;
    long  value;
    int   base;
...
static Bool
string_to_ulong(const char *str, unsigned long *value)
{
    const char *tmp1 = str;
    int         base;
...
add_charset(CodeSet codeset, XlcCharSet charset)
{
    ...
    if ((num = codeset->num_charsets))
        new_list =
            Xreallocarray(codeset->charset_list, num + 1, sizeof(XlcCharSet));
    else new_list = Xmalloc(sizeof(XlcCharSet));
...
add_codeset(XLCdGenericPart *gen)
{
    ...
    if ((num = gen->codeset_num))
        new_list = Xreallocarray(gen->codeset_list, num + 1, sizeof(CodeSet));
    else new_list = Xmalloc(sizeof(CodeSet));

    if (new_list == NULL) goto err;
...
static Bool
add_parse_list(XLCdGenericPart *gen,
               EncodingType     type,
               const char      *encoding,
               CodeSet          codeset)
{
    ...
    if (gen->mb_parse_table == NULL)
    {
        ...
        if (gen->mb_parse_table == NULL) goto err;
    }

    if ((num = gen->mb_parse_list_num))
        new_list =
            Xreallocarray(gen->mb_parse_list, num + 2, sizeof(ParseInfo));
    else
    {
    ...
    if (gen->mb_parse_table[ch] == 0) gen->mb_parse_table[ch] = num + 1;

    new->type     = type;
    new->encoding = str;
    new->codeset  = codeset;

    if (codeset->parse_info == NULL) codeset->parse_info = new;
...
static const char *
getscope(const char *str, FontScope scp)
{
...
FontScope
_XlcParse_scopemaps(const char *str, int *size)
{
    int         num = 0, i;
...
void
_XlcDbg_printValue(const char *str, char **value, int num)
{
/*
    int i;
    for (i = 0; i < num; i++)
...
static void
dmpscope(const char *name, FontScope sc, int num)
{
/*
    int i;
...
static XlcCharSet
srch_charset_define(const char *name, int *new)
{
    XlcCharSet charset;

    *new    = 0;
    charset = _XlcGetCharSet(name);
    if (charset == NULL && (charset = _XlcCreateDefaultCharSet(name, "")))
    {
...
static SegConv
add_conversion(XLCdGenericPart *gen)
{
    SegConv new_list;
    int     num;
    ...
    {
        new_list =
            Xreallocarray(gen->segment_conv, num + 1, sizeof(SegConvRec));
    }
    else
    {
...
static ExtdSegment
create_ctextseg(char **value, int num)
{
    ExtdSegment ret;
    char       *ptr;
    char       *cset_name = NULL;
...
load_generic(XLCd lcd)
{
    ...
    /***** codeset.side *****/
        ...
        if (num > 0)
        {
            ...
            {
                if (codeset->side == XlcGR) gen->initial_state_GR = codeset;
                else gen->initial_state_GL = codeset;
            }
        }
    ...
    /***** codeset.length *****/
        ...
        if (num > 0)
        {
            ...
            if (codeset->length < 1) codeset->length = 1;
        }
    ...
    /***** codeset.ct_encoding *****/
        ...
        if (num > 0)
        {
            ...
            for (; num-- > 0; value++)
            {
                if (strlen(*value) > sizeof name) goto err;
                ...
                if (charset)
                {
                    if (add_charset(codeset, charset) == False) goto err;
                }
            }
        }

        if (codeset == NULL) break;
        codeset->cs_num = i;
        /* For VW/UDC */
        ...
        for (M = 1; M - 1 < codeset->length; M++)
        {
            ...
            if (M == 1)
            {
                ...
                codeset->byteM =
                    Xmallocarray(codeset->length, sizeof(ByteInfoListRec));
    ...
    if (gen->initial_state_GL == NULL)
    {
        ...
        for (i = 0; i < gen->codeset_num; i++)
        {
            ...
            if (codeset->side == XlcGL) gen->initial_state_GL = codeset;
        }
    }
    ...
    for (i = 0; i < gen->codeset_num; i++)
    {
        ...
        for (ii = 0; ii < codeset->num_charsets; ii++)
        {
            ...
            if (charset->string_encoding) codeset->string_encoding = True;
        }
    }
    return True;
...
initialize(XLCd lcd)
{
    ...
    {
        if ((*superclass->pub.initialize)(lcd) == False) return False;
    }
    ...
    if (load_generic(lcd) == False) return False;
...
destroy(XLCd lcd)
{
    ...
    if (superclass && superclass->pub.destroy) (*superclass->pub.destroy)(lcd);
}
