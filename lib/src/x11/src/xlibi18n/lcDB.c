    ...
    while (*p)
    {
        ...
        if (*p == SYM_COMMENT && !*quoted)
        {
                ...
                (iswhite(p[-1]) && (pos == 1 || p[-2] != SYM_BACKSLASH)))
            {
                ...
                if (len > 0 &&
                    (p[len - 1] == SYM_NEWLINE || p[len - 1] == SYM_CR))
                {
...
get_token(const char *str)
{
    switch (*str)
    {
        ...
        case SYM_BACKSLASH:
            ...
            {
                case 'x':
                    return T_NUMERIC_HEX;
                case 'd':
                    return T_NUMERIC_DEC;
                case 'o':
                    return T_NUMERIC_OCT;
            }
            return T_BACKSLASH;
        default:
            return T_DEFAULT;
...
static int
get_word(const char *str, char *word)
{
    const char *p = str;
    char       *w = word;
    Token       token;
...
static int
get_quoted_word(const char *str, char *word)
{
    const char *p = str;
    char       *w = word;
    Token       token;
...
append_value_list(void)
{
    ...
    else
    {
        ...
        value_list =
            (char **)Xreallocarray(value_list, value_num + 2, sizeof(char *));
        if (value_list == NULL)
        {
            Xfree(prev_list);
        }
    }
    if (value_list == (char **)NULL) goto err2;
...
static int
construct_name(char *name, int size)
{
    int   i;
    int   len = 0;
    char *p   = name;
...
static int
check_category_end(const char *str)
{
    const char *p;
    int         len;
...
static int
f_newline(const char *str, Token token, Database *db)
{
...
static int
f_comment(const char *str, Token token, Database *db)
{
...
static int
f_white(const char *str, Token token, Database *db)
{
    const char *p = str;
...
static int
f_semicolon(const char *str, Token token, Database *db)
{
    switch (parse_info.pre_state)
    {
        case S_NULL:
...
static int
f_left_brace(const char *str, Token token, Database *db)
{
    switch (parse_info.pre_state)
    {
        case S_NULL:
...
static int
f_right_brace(const char *str, Token token, Database *db)
{
    if (parse_info.nest_depth < 1) return 0;
...
static int
f_double_quote(const char *str, Token token, Database *db)
{
    char  word[BUFSIZE];
    char *wordp;
    int   len;

    if ((len = (int)strlen(str)) < sizeof word) wordp = word;
    else wordp = Xmalloc(len + 1);
    if (wordp == NULL) return 0;
    ...
    switch (parse_info.pre_state)
    {
        ...
        case S_VALUE:
            ...
            parse_info.bufsize =
                (int)((size_t)parse_info.bufsize + strlen(wordp));
            parse_info.pre_state = S_VALUE;
            break;
        default:
            goto err;
...
static int
f_backslash(const char *str, Token token, Database *db)
{
    return f_default(str, token, db);
}

static int
f_numeric(const char *str, Token token, Database *db)
{
    char        word[BUFSIZE];
    const char *p;
    char       *wordp;
    ...
    switch (parse_info.pre_state)
    {
        ...
        case S_VALUE:
            ...
            if ((parse_info.bufsize + token_len + (int)strlen(wordp) + 1) >=
                parse_info.bufMaxSize)
            {
                if (realloc_parse_info(
                        (int)((size_t)token_len + strlen(wordp) + 1)) == False)
                    goto err;
            }
            strncpy(&parse_info.buf[parse_info.bufsize],
                    str,
                    (size_t)token_len);
            strcpy(&parse_info.buf[parse_info.bufsize + token_len], wordp);
            parse_info.bufsize   = (int)((size_t)parse_info.bufsize +
                                       ((size_t)token_len + strlen(wordp)));
            parse_info.pre_state = S_VALUE;
            break;
        default:
            goto err;
...
static int
f_default(const char *str, Token token, Database *db)
{
    char  word[BUFSIZE], *p;
    char *wordp;
    int   len;
    ...
    {
        case S_NULL:
            if (parse_info.category != NULL) goto err;
            p = strdup(wordp);
            if (p == NULL) goto err;
...
/************************************************************************/
void
_XlcGetLocaleDataBase(XLCd        lcd,
                      const char *category,
                      const char *name,
                      char     ***value,
                      int        *count)
...
/************************************************************************/
void
_XlcDestroyLocaleDataBase(XLCd lcd)
{
    XlcDatabase     lc_db = (XlcDatabase)XLC_PUBLIC(lcd, xlocale_db);
    XlcDatabaseList p, prev;

    for (p = _db_list, prev = (XlcDatabaseList)NULL; p; prev = p, p = p->next)
    {
        if (p->lc_db == lc_db)
        {
            if ((--p->ref_count) < 1)
...
/************************************************************************/
XPointer
_XlcCreateLocaleDataBase(XLCd lcd)
{
    ...
    if (lc_db == (XlcDatabase)NULL) goto err;
