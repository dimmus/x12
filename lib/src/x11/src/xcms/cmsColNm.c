...
_XcmsColorSpaceOfString(XcmsCCC ccc, const char *color_string)
/*
 *	DESCRIPTION
 ...
 */
{
     ...
     */
    papColorSpaces =
        ((XcmsFunctionSet *)ccc->pPerScrnInfo->functionSet)->DDColorSpaces;
    if (papColorSpaces != NULL)
    {
        while (*papColorSpaces != NULL)
        {
 ...
 */
static int
_XcmsParseColorString(XcmsCCC ccc, const char *color_string, XcmsColor *pColor)
/*
 *	DESCRIPTION
 ...
 */
{
    ...
    if ((len = strlen(color_string)) >= sizeof(string_buf))
    {
        ...
        if (string_lowered == NULL) return (XcmsFailure);
    }
    else
    {
...
{
    return (
        strcmp(((const XcmsPair *)p1)->first, ((const XcmsPair *)p2)->first));
}
 ...
 */
static Status
_XcmsLookupColorName(XcmsCCC ccc, const char **name, XcmsColor *pColor)
/*
 *	DESCRIPTION
...
Retry:
    if ((len = (int)strlen(tmpName)) > 63)
    {
        ...
        if (name_lowered == NULL) return (XcmsFailure);
    }
    else
    {
 ...
 */
static int
stringSectionSize(FILE *stream, int *pNumEntries, int *pSectionSize)
/*
 *	DESCRIPTION
 ...
 */
{
    ...
    *pSectionSize = 0;
    ...
    while ((fgets(buf, XCMSDB_MAXLINELEN, stream)) != NULL)
    {
        ...
        numEntries++;
        if (numEntries >= INT_MAX) return (XcmsFailure);

        i = strlen(f1);
        if (i >= INT_MAX - sectionSize) return (XcmsFailure);
        sectionSize += i + 1;
        for (; i; i--, f1++)
        {
 ...
 */
static Status
ReadColornameDB(FILE *stream, XcmsPair *pRec, char *pString)
/*
 *	DESCRIPTION
 ...
 */
void
_XcmsCopyISOLatin1Lowered(char *dst, const char *src)
/*
 *	DESCRIPTION
 ...
 */
Status
_XcmsResolveColorString(XcmsCCC         ccc,
 ...
 *    and return.
 */
    if (_XcmsParseColorString(ccc, *color_string, pColor_exact_return) == 1)
    {
        if (result_format != XcmsUndefinedFormat &&
            pColor_exact_return->format != result_format)
        {
        /* need to be converted to the target format */
            return (XcmsConvertColors(ccc,
                                      pColor_exact_return,
                                      1,
                                      result_format,
                                      (Bool *)NULL));
 ...
 */
	 ...
	 */
	     ...
	     */
            if (ccc->whitePtAdjProc &&
                !_XcmsEqualWhitePts(ccc,
                                    pClientWhitePt,
                                    ScreenWhitePointOfCCC(ccc)))
            {
                return ((*ccc->whitePtAdjProc)(ccc,
                                               ScreenWhitePointOfCCC(ccc),
                                               pClientWhitePt,
                                               result_format,
                                               pColor_exact_return,
	 ...
	 */
	     ...
	     */
            if (ccc->whitePtAdjProc &&
                !_XcmsEqualWhitePts(ccc, &dbWhitePt, pClientWhitePt))
            {
        /*
		 ...
		 */
            ...
            else if (_XcmsEqualWhitePts(ccc, &dbWhitePt, pClientWhitePt))
            {
        /*
