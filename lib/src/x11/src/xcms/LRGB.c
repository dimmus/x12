...
static Status _XcmsGetTableType0(IntensityTbl  *pTbl,
                                 int            format,
                                 char         **pChar,
                                 unsigned long *pCount);
static Status _XcmsGetTableType1(IntensityTbl  *pTbl,
                                 int            format,
                                 char         **pChar,
                                 unsigned long *pCount);
...
XcmsColorSpace XcmsRGBiColorSpace = {
     ...
     * RGB Color Spaces
     */
XcmsColorSpace XcmsRGBColorSpace = { _XcmsRGB_prefix,  /* prefix */
                                     XcmsRGBFormat,  /* id */
...
/*
     ...
     */
static XcmsColorSpace *DDColorSpaces[] = { &XcmsRGBColorSpace,
                                           &XcmsRGBiColorSpace,
                                           NULL };
...
static const IntensityRec Default_RGB_RedTuples[] = {
...
static const IntensityRec Default_RGB_GreenTuples[] = {
    /* {unsigned short value, XcmsFloat intensity} */
    { 0x0000, 0.000000 },
    { 0x1313, 0.000000 },
    { 0x1414, 0.000832 },
...
static const IntensityRec Default_RGB_BlueTuples[] = {
    /* {unsigned short value, XcmsFloat intensity} */
    { 0x0000, 0.000000 },
    { 0x0e0e, 0.000000 },
    { 0x0f0f, 0.001341 },
 ...
 */
static Status
LINEAR_RGB_InitSCCData(Display         *dpy,
                       int              screenNumber,
                       XcmsPerScrnInfo *pPerScrnInfo)
/*
 *	DESCRIPTION
 ...
 */
{
     ...
     */
    if (!(pScreenData = pScreenDefaultData =
              (LINEAR_RGB_SCCData *)Xcalloc(1, sizeof(LINEAR_RGB_SCCData))))
    {
        return (XcmsFailure);
    }
    ...
    if (MatrixAtom == None ||
        !_XcmsGetProperty(dpy,
                          RootWindow(dpy, screenNumber),
                          MatrixAtom,
                          &format_return,
                          &nitems,
                          &nbytes_return,
	 ...
	 */
        ...
        {
            *pValue++ = (long)_XcmsGetElement(format_return, &pChar, &nitems) /
     ...
     */
    if (CorrectAtom == None || !_XcmsGetProperty(dpy,
                                                 RootWindow(dpy, screenNumber),
                                                 CorrectAtom,
                                                 &format_return,
                                                 &nitems,
	 ...
	 */
	     ...
	     */
            ...
            /* copy matrices */
            memcpy((char *)pScreenData,
                   (char *)pScreenDefaultData,
                   18 * sizeof(XcmsFloat));

            /* Create, initialize, and add map */
            if (!(pNewMap =
                      (XcmsIntensityMap *)Xcalloc(1, sizeof(XcmsIntensityMap))))
            {
                Xfree(pScreenData);
                goto Free_property_return;
            }
        ...
        if (cType == 0)
        {
            ...
            if (!(pScreenData->pRedTbl =
                      (IntensityTbl *)Xcalloc(1, sizeof(IntensityTbl))))
            {
                goto Free_property_return;
            }
            if (_XcmsGetTableType0(pScreenData->pRedTbl,
                                   format_return,
                                   &pChar,
                                   &nitems) == XcmsFailure)
            {
            ...
            else
            {
                ...
                if (_XcmsGetTableType0(pScreenData->pGreenTbl,
                                       format_return,
                                       &pChar,
                                       &nitems) == XcmsFailure)
                {
                ...
                if (_XcmsGetTableType0(pScreenData->pBlueTbl,
                                       format_return,
                                       &pChar,
                                       &nitems) == XcmsFailure)
                {
        ...
        else if (cType == 1)
        {
            ...
            if (_XcmsGetTableType1(pScreenData->pRedTbl,
                                   format_return,
                                   &pChar,
                                   &nitems) == XcmsFailure)
            {
            ...
            else
            {
                ...
                if (_XcmsGetTableType1(pScreenData->pGreenTbl,
                                       format_return,
                                       &pChar,
                                       &nitems) == XcmsFailure)
                {
                ...
                if (_XcmsGetTableType1(pScreenData->pBlueTbl,
                                       format_return,
                                       &pChar,
                                       &nitems) == XcmsFailure)
                {
        ...
        for (count = 0; count < pScreenData->pRedTbl->nEntries;
             count++, pIRec++)
        {
        ...
        if (pScreenData->pGreenTbl->pBase != pScreenData->pRedTbl->pBase)
        {
            ...
            for (count = 0; count < pScreenData->pGreenTbl->nEntries;
                 count++, pIRec++)
            {
                printf("\t0x%4x\t%f\n", pIRec->value, pIRec->intensity);
            }
        ...
        if (pScreenData->pBlueTbl->pBase != pScreenData->pRedTbl->pBase)
        {
            ...
            for (count = 0; count < pScreenData->pBlueTbl->nEntries;
                 count++, pIRec++)
            {
                printf("\t0x%4x\t%f\n", pIRec->value, pIRec->intensity);
            }
 ...
 */
static void
LINEAR_RGB_FreeSCCData(XPointer pScreenDataTemp)
/*
 *	DESCRIPTION
 *
 *	RETURNS
 ...
 */
{
    ...
    if (pScreenData && pScreenData != &Default_RGB_SCCData)
    {
        if (pScreenData->pRedTbl)
        {
            ...
            if (pScreenData->pBlueTbl)
            {
                if (pScreenData->pRedTbl->pBase != pScreenData->pBlueTbl->pBase)
                {
 ...
 */
static Status
_XcmsGetTableType0(IntensityTbl  *pTbl,
                   int            format,
                   char         **pChar,
                   unsigned long *pCount)
/*
 ...
 */
{
    ...
    nElements = pTbl->nEntries = _XcmsGetElement(format, pChar, pCount) + 1;
    if (!(pIRec = pTbl->pBase =
              (IntensityRec *)Xcalloc(nElements, sizeof(IntensityRec))))
    {
        return (XcmsFailure);
    }
    ...
    switch (format)
    {
        ...
        case 16:
            for (; nElements--; pIRec++)
            {
                ...
                pIRec->intensity =
                    _XcmsGetElement(format, pChar, pCount) / (XcmsFloat)65535.0;
            }
            break;
        case 32:
            for (; nElements--; pIRec++)
 ...
 */
static Status
_XcmsGetTableType1(IntensityTbl  *pTbl,
                   int            format,
                   char         **pChar,
                   unsigned long *pCount)
/*
 ...
 */
{
    ...
    if (!(pIRec = pTbl->pBase =
              (IntensityRec *)Xcalloc(max_index + 1, sizeof(IntensityRec))))
    {
        return (XcmsFailure);
    }
 ...
 */
static int
_XcmsValueCmp(IntensityRec *p1, IntensityRec *p2)
/*
 *	DESCRIPTION
 ...
 */
static int
_XcmsIntensityCmp(IntensityRec *p1, IntensityRec *p2)
/*
 *	DESCRIPTION
...
/* ARGSUSED */
static int
_XcmsValueInterpolation(IntensityRec *key,
                        IntensityRec *lo,
                        IntensityRec *hi,
                        IntensityRec *answer,
                        int           bitsPerRGB)
 ...
 */
static int
_XcmsIntensityInterpolation(IntensityRec *key,
                            IntensityRec *lo,
                            IntensityRec *hi,
                            IntensityRec *answer,
                            int           bitsPerRGB)
...
typedef int (*comparProcp)(char *p1, char *p2);
typedef int (*interpolProcp)(char *key,
                             char *lo,
                             char *hi,
                             char *answer,
                             int   bitsPerRGB);
 ...
 */
static int
_XcmsTableSearch(char    *key,
                 int      bitsPerRGB,
                 char    *base,
                 unsigned nel,
                 unsigned nKeyPtrSize,
                 int (*compar)(char *p1, char *p2),
                 int (*interpol)(char *key,
                                 char *lo,
                                 char *hi,
                                 char *answer,
                                 int   bitsPerRGB),
 ...
 */
{
    ...
    ((IntensityRec *)key)->value =
        ((unsigned long)(((IntensityRec *)key)->value >> (16 - bitsPerRGB)) *
         0xFFFF) /
        ((1 << bitsPerRGB) - 1);
 ...
 */
static void
_XcmsMatVec(XcmsFloat *pMat, XcmsFloat *pIn, XcmsFloat *pOut)
/*
 *      DESCRIPTION
 ...
 */
static int
XcmsLRGB_RGB_ParseString(register char *spec, XcmsColor *pColor)
/*
 *	DESCRIPTION
        ...
        for (i = 0; i < 3; i++, pShort++, spec++)
        {
            ...
            *pShort = 0;
            while (*spec != '/' && *spec != '\0')
            {
                ...
                c = *spec++;
                *pShort <<= 4;
                if (c >= '0' && c <= '9') *pShort |= c - '0';
                /* assume string in lowercase
		else if (c >= 'A' && c <= 'F')
		    *pShort |= c - ('A' - 10);
		*/
                else if (c >= 'a' && c <= 'f') *pShort |= c - ('a' - 10);
                else return (XcmsFailure);
            }
            if (n == 0) return (XcmsFailure);
            if (n < 4)
            {
                *pShort =
                    ((unsigned long)*pShort * 0xFFFF) / ((1 << n * 4) - 1);
            }
        }
    }
 ...
 */
static int
XcmsLRGB_RGBi_ParseString(register char *spec, XcmsColor *pColor)
/*
 *	DESCRIPTION
 *		This routines takes a string and attempts to convert
 ...
 */
{
    ...
    while (nColors--)
    {
        ...
        _XcmsMatVec((XcmsFloat *)pScreenData->RGBtoXYZmatrix,
                    (XcmsFloat *)&pXcmsColors_in_out->spec,
                    tmp);
 ...
 */
{
    ...
    while (nColors--)
    {
        ...
        if (!_XcmsTableSearch((char *)&keyIRec,
                              ccc->visual->bits_per_rgb,
                              (char *)pScreenData->pRedTbl->pBase,
                              (unsigned)pScreenData->pRedTbl->nEntries,
                              (unsigned)sizeof(IntensityRec),
                              (comparProcp)_XcmsIntensityCmp,
                              (interpolProcp)_XcmsIntensityInterpolation,
                              (char *)&answerIRec))
        {
            return (XcmsFailure);
        }
        ...
        memcpy((char *)&pXcmsColors_in_out->spec,
               (char *)&tmpRGB,
               sizeof(XcmsRGB));
 ...
 */
{
    ...
    while (nColors--)
    {
        ...
        if (!_XcmsTableSearch((char *)&keyIRec,
                              ...
                              (unsigned)pScreenData->pRedTbl->nEntries,
                              (unsigned)sizeof(IntensityRec),
                              (comparProcp)_XcmsValueCmp,
                              (interpolProcp)_XcmsValueInterpolation,
                              (char *)&answerIRec))
        {
            return (XcmsFailure);
        }
...
/* ARGSUSED */
int
_XcmsLRGB_InitScrnDefault(Display         *dpy,
                          int              screenNumber,
                          XcmsPerScrnInfo *pPerScrnInfo)
/*
 *	DESCRIPTION
