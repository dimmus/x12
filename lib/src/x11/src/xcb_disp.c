...
static void *
alloc_copy(const void *src, int *dstn, size_t n)
{
    void *dst;
    if (n <= 0)
    {
...
void
XSetAuthorization(char *name, int namelen, char *data, int datalen)
{
...
int
_XConnectXCB(Display *dpy, _Xconst char *display, int *screenp)
{
    char             *host;
    int               n = 0;
    xcb_connection_t *c;
    ...
    if (!xcb_parse_display(display, &host, &n, screenp)) return 0;
    ...
    else c = xcb_connect(display, NULL);
    _XUnlockMutex(_Xglobal_lock);
    ...
    if (!dpy->xcb->event_notify || !dpy->xcb->reply_notify) return 0;
...
void
_XFreeX11XCBStructure(Display *dpy)
{
    ...
    if (dpy->xcb->event_notify) xcondition_clear(dpy->xcb->event_notify);
    if (dpy->xcb->reply_notify) xcondition_clear(dpy->xcb->reply_notify);
