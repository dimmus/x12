...
#define throw_thread_fail_assert(_message, _var)                            \
    {                                                                       \
        ...
        {                                                                   \
            fprintf(stderr,                                                 \
                    "[xcb] Most likely this is a multi-threaded client "    \
...
require_socket(Display *dpy)
{
    if (dpy->bufmax == dpy->buffer)
    {
        ...
        if (!xcb_take_socket(dpy->xcb->connection,
                             return_socket,
                             dpy,
                             flags,
                             &sent))
 ...
 */
static Bool
check_internal_connections(Display *dpy)
{
    ...
    if (dpy->flags & XlibDisplayProcConni || !dpy->im_fd_info) return True;
    ...
    for (ilist = dpy->im_fd_info; ilist; ilist = ilist->next)
    {
        ...
        if (ilist->fd > highest_fd) highest_fd = ilist->fd;
    }
    assert(highest_fd >= 0);
...
append_pending_request(Display *dpy, uint64_t sequence)
{
    ...
    else dpy->xcb->pending_requests = node;
    dpy->xcb->pending_requests_tail = node;
    return node;
}

static void
dequeue_pending_request(Display *dpy, PendingRequest *req)
{
...
static int
handle_error(Display *dpy, xError *err, Bool in_XReply)
{
    _XExtension *ext;
    int          ret_code;
 ...
 */
static void
widen(uint64_t *wide, unsigned int narrow)
{
...
poll_for_event(Display *dpy, Bool queued_only)
{
    /* Make sure the Display's sequence numbers are valid */
    if (!require_socket(dpy)) return NULL;
    ...
    if (dpy->xcb->next_event)
    {
        ...
        if (!req || XLIB_SEQUENCE_COMPARE(event_sequence, <, req->sequence) ||
            (event->response_type != X_Error &&
             event_sequence == req->sequence))
        {
...
poll_for_response(Display *dpy)
{
    ...
    while (1)
    {
        ...
        {
            if (((xcb_generic_reply_t *)dpy->xcb->next_response)
                    ->response_type == X_Error)
            {
        ...
        if (error) return (xcb_generic_reply_t *)error;
    }
    return response;
}

static void
handle_response(Display *dpy, xcb_generic_reply_t *response, Bool in_XReply)
{
    ...
    switch (response->response_type)
    {
        case X_Reply:
            for (async = dpy->async_handlers; async; async = next)
            {
                ...
                if (async->handler(dpy,
                                   (xReply *)response,
                                   (char *)response,
                                   sizeof(xReply) + (response->length << 2),
                                   async->data))
                    break;
            }
            break;
		 ...
		 * be after the first 32 bytes. */
            if (response->response_type == GenericEvent &&
...
_XEventsQueued(Display *dpy, int mode)
{
    ...
    if (mode == QueuedAfterFlush) _XSend(dpy, NULL, 0);
    else if (!check_internal_connections(dpy)) return 0;
...
_XReadEvents(Display *dpy)
{
    ...
    _XSend(dpy, NULL, 0);
    if (dpy->xcb->event_owner != XlibOwnsEventQueue) return;
    if (!check_internal_connections(dpy)) return;
    ...
    while (serial == dpy->next_event_serial_num || dpy->qlen == 0)
    {
        ...
        if (response) handle_response(dpy, response, False);
    ...
    if (!dpy->xcb->event_waiter)
        while ((response = poll_for_response(dpy)))
            handle_response(dpy, response, False);
    if (xcb_connection_has_error(dpy->xcb->connection)) _XIOError(dpy);
}
 ...
 */
void
_XSend(Display *dpy, const char *data, long size)
{
    ...
    if (dpy->bufptr == dpy->buffer && !size) return;
    ...
    {
        uint64_t sequence;
        for (sequence = dpy->xcb->last_flushed + 1; sequence <= dpy_request;
             ++sequence)
...
{
    if (dpy->xcb->next_xid == inval_id) _XAllocIDs(dpy, &dpy->xcb->next_xid, 1);
}

/* _XAllocID - resource ID allocation routine. */
XID
_XAllocID(Display *dpy)
{
...
void
_XAllocIDs(Display *dpy, XID *ids, int count)
{
    int i;
#ifdef XTHREADS
...
static void
_XFreeReplyData(Display *dpy, Bool force)
{
    if (!force && dpy->xcb->reply_consumed < dpy->xcb->reply_length) return;
...
_XReply(Display *dpy, xReply *rep, int extra, Bool discard)
{
    ...
    else current = append_pending_request(dpy, dpy_request);
    ...
    while (1)
    {
        ...
        X_DPY_SET_LAST_REQUEST_READ(dpy, req->sequence);
        if (!response) dequeue_pending_request(dpy, req);
        ...
        if (error) handle_response(dpy, (xcb_generic_reply_t *)error, True);
        else if (response) handle_response(dpy, response, True);
    }
    if (!check_internal_connections(dpy)) return 0;
    ...
    if (dpy->xcb->reply_data[0] == 1)
        dpy->xcb->reply_length +=
            (((xcb_generic_reply_t *)dpy->xcb->reply_data)->length * 4);
...
void
_XEatData(Display *dpy, unsigned long n)
{
