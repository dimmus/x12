...
#undef HAVE_UNISTD_H /* defined in freetype (ftconfig.h) without ifndef */
...
void XftGlyphCore(XftDraw          *draw,
                  _Xconst XftColor *color,
                  XftFont *public,
                  int              x,
                  int              y,
...
void XftGlyphSpecCore(XftDraw          *draw,
                      _Xconst XftColor *color,
                      XftFont *public,
                      _Xconst XftGlyphSpec *glyphs,
                      int                   nglyphs);

void XftGlyphFontSpecCore(XftDraw                  *draw,
                          _Xconst XftColor         *color,
...
FcBool XftDrawRenderPrepare(XftDraw *draw);
...
/* xftfreetype.c */
FcBool _XftSetFace(XftFtFile *f,
                   FT_F26Dot6 xsize,
                   FT_F26Dot6 ysize,
                   FT_Matrix *matrix);
...
void XftMemAlloc(int kind, size_t size);

void XftMemFree(int kind, size_t size);

/* xftlist.c */
FcFontSet *XftListFontsPatternObjects(Display     *dpy,
                                      int          screen,
                                      FcPattern   *pattern,
                                      FcObjectSet *os);
...
/* xftstr.c */
int _XftMatchSymbolic(XftSymbolic *s, int n, const char *name, int def);
