                ...
                XGlyphInfo      *extents)
{
    ...
    if (nmissing) XftFontLoadGlyphs(dpy, pub, FcFalse, missing, nmissing);
    g    = glyphs;
    n    = nglyphs;
    xftg = NULL;
    while (n)
    {
        glyph = *g++;
        n--;
        if (glyph < font->num_glyphs && (xftg = font->glyphs[glyph])) break;
    }
    if (n == 0)
    {
        if (xftg) *extents = xftg->metrics;
        else memset(extents, '\0', sizeof(*extents));
    }
    else
    {
        x              = 0;
                ...
                XGlyphInfo      *extents)
{
    ...
    if (len <= NUM_LOCAL) *(glyphs = glyphs_local) = 0;
    else
    {
