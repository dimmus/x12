...
_XftDrawScreen(Display *dpy, Drawable drawable, Visual *visual)
{
                     ...
                     &depth))
    {
        for (s = 0; s < ScreenCount(dpy); s++)
        {
            if (RootWindow(dpy, s) == root) return s;
        }
    }
    /*
...
_X_EXPORT XftDraw *
XftDrawCreateAlpha(Display *dpy, Pixmap pixmap, int depth)
{
    XftDraw *draw;
...
_XftDrawFormat(XftDraw *draw)
{
    ...
    if (!info || !info->hasRender) return NULL;
    ...
    else return XRenderFindVisualFormat(draw->dpy, draw->visual);
}
...
{
    if (draw->render.pict) XRenderFreePicture(draw->dpy, draw->render.pict);
    if (draw->core.gc) XFreeGC(draw->dpy, draw->core.gc);
...
XftDrawSrcPicture(XftDraw *draw, _Xconst XftColor *color)
{
    ...
    if (!info || !info->solidFormat) return 0;
    ...
    {
        if (info->colors[i].pict && info->colors[i].screen == draw->screen &&
	 ...
	 */
        if (info->colors[i].pict) XRenderFreePicture(dpy, info->colors[i].pict);
    /*
	 * Create picture
	 */
...
{
    if (draw->visual || draw->depth != 1) return PictOpOver;
    if (color->color.alpha >= 0x8000) return PictOpOver;
...
_XftDrawRenderPrepare(XftDraw *draw)
{
    if (!draw->render.pict)
    {
        ...
        draw->render.pict =
            XRenderCreatePicture(draw->dpy, draw->drawable, format, mask, &pa);
        if (!draw->render.pict) return FcFalse;
        switch (draw->clip_type)
        {
            case XftClipTypeRegion:
                XRenderSetPictureClipRegion(draw->dpy,
                                            draw->render.pict,
                                            draw->clip.region);
                break;
            case XftClipTypeRectangles:
                XRenderSetPictureClipRectangles(draw->dpy,
                                                draw->render.pict,
...
_XftDrawCorePrepare(XftDraw *draw, _Xconst XftColor *color)
{
    if (!draw->core.gc)
    {
        ...
        if (!draw->core.gc) return FcFalse;
        switch (draw->clip_type)
        {
            case XftClipTypeRegion:
...
{
    if (!_XftDrawRenderPrepare(draw)) return 0;
              ...
              int               nglyphs)
{
    ...
    if (font->format)
    {
        ...
        if (_XftDrawRenderPrepare(draw) &&
            ...
            XftGlyphRender(draw->dpy,
                           _XftDrawOp(draw, color),
                           src,
                           pub,
                           draw->render.pict,
                           0,
               ...
               int               len)
{
    ...
    if (len <= NUM_LOCAL) glyphs = glyphs_local;
    else
    {
    ...
    if (glyphs != glyphs_local) free(glyphs);
}
    ...
    while (i < len)
    {
        ...
        if (((XftFontInt *)glyphs[i].font)->format)
        {
            ...
            if (_XftDrawRenderPrepare(draw) &&
                (src = XftDrawSrcPicture(draw, color)))
            {
                XftGlyphFontSpecRender(draw->dpy,
                                       _XftDrawOp(draw, color),
                                       src,
                                       draw->render.pict,
                                       0,
                    ...
                    int                      len)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        glyphs[i].glyph =
            XftCharIndex(draw->dpy, glyphs[i].font, chars[i].ucs4);
        glyphs[i].x = chars[i].x;
        glyphs[i].y = chars[i].y;
    }
...
_X_EXPORT Bool
XftDrawSetClip(XftDraw *draw, Region r)
{
    Region n = NULL;
     ...
     */
    if (!r && draw->clip_type == XftClipTypeNone) return True;
    ...
    if (draw->render.pict)
    {
        if (n) XRenderSetPictureClipRegion(draw->dpy, draw->render.pict, n);
        else
        {
            XRenderPictureAttributes pa;
            pa.clip_mask = None;
            XRenderChangePicture(draw->dpy, draw->render.pict, CPClipMask, &pa);
        }
    }
    if (draw->core.gc)
    {
        if (n) XSetRegion(draw->dpy, draw->core.gc, draw->clip.region);
        else XSetClipMask(draw->dpy, draw->core.gc, None);
    }
    return True;
}
                         ...
                         int                 n)
{
     ...
     */
    if (draw->clip_type == XftClipTypeRectangles && draw->clip.rect->n == n &&
        (n == 0 || (draw->clip.rect->xOrigin == xOrigin &&
                    draw->clip.rect->yOrigin == yOrigin)) &&
        !memcmp(XftClipRects(draw->clip.rect),
                rects,
                (size_t)n * sizeof(XRectangle)))
    {
        return True;
    }
...
{
    if (mode == draw->subwindow_mode) return;
    ...
    if (draw->render.pict)
    {
        ...
        XRenderChangePicture(draw->dpy,
                             draw->render.pict,
                             CPSubwindowMode,
                             &pa);
    }
    if (draw->core.gc) XSetSubwindowMode(draw->dpy, draw->core.gc, mode);
}
