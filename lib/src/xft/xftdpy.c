...
_XftCloseDisplay(Display *dpy, XExtCodes *codes _X_UNUSED)
{
     ...
     */
    if (info->defaults) FcPatternDestroy(info->defaults);
...
_XftDisplayInfoGet(Display *dpy, FcBool createIfNecessary)
{
    ...
    info->hasRender   = (XRenderQueryExtension(dpy, &event_base, &error_base) &&
                       (XRenderFindVisualFormat(
                            dpy,
                            DefaultVisual(dpy, DefaultScreen(dpy))) != NULL));
    ...
    if (info->hasRender)
    {
        ...
        if (major > 0 || (major == 0 && minor >= 10)) info->hasSolid = FcTrue;
        ...
        info->solidFormat   = XRenderFindFormat(
            ...
            (PictFormatType | PictFormatDepth | PictFormatRedMask |
             PictFormatGreenMask | PictFormatBlueMask | PictFormatAlphaMask),
            &pf,
            0);
    }
    ...
    info->max_glyph_memory =
        (unsigned long)XftDefaultGetInteger(dpy,
    ...
    info->track_mem_usage =
        XftDefaultGetBool(dpy, XFT_TRACK_MEM_USAGE, 0, FcFalse);
    if (XftDebug() & XFT_DBG_CACHE)
...
_XftDisplayManageMemory(Display *dpy)
{
    ...
    if (!info || !info->max_glyph_memory) return;
    if (XftDebug() & XFT_DBG_CACHE)
    {
    ...
    if (XftDebug() & XFT_DBG_CACHE) _XftDisplayValidateMemory(info);
}
...
XftDefaultSet(Display *dpy, FcPattern *defaults)
{
    ...
    if (!info->max_unref_fonts) info->max_unref_fonts = XFT_DPY_MAX_UNREF_FONTS;
    info->max_unref_fonts = XftDefaultGetInteger(dpy,
                                                 XFT_MAX_UNREF_FONTS,
                                                 0,
                                                 info->max_unref_fonts);
    ...
    info->track_mem_usage =
        XftDefaultGetBool(dpy, XFT_TRACK_MEM_USAGE, 0, info->track_mem_usage);
    if (XftDebug() & XFT_DBG_CACHE)
...
XftDefaultParseBool(const char *v)
{
    ...
    c0 = *v;
    if (isupper((int)c0)) c0 = (char)tolower(c0);
    if (c0 == 't' || c0 == 'y' || c0 == '1') return 1;
    if (c0 == 'f' || c0 == 'n' || c0 == '0') return 0;
    if (c0 == 'o')
    {
        c1 = v[1];
        if (isupper((int)c1)) c1 = (char)tolower(c1);
        if (c1 == 'n') return 1;
        if (c1 == 'f') return 0;
    }
    return -1;
...
_XftDefaultInitDouble(Display *dpy, FcPattern *pat, const char *option)
{
    ...
    {
        d = strtod(v, &e);
        if (e != v) return FcPatternAddDouble(pat, option, d);
    }
    return True;
}
...
_XftDefaultInitInteger(Display *dpy, FcPattern *pat, const char *option)
{
    ...
    if (v)
    {
        ...
        i = (int)strtol(v, &e, 0);
        if (e != v) return FcPatternAddInteger(pat, option, i);
    }
    return True;
}
...
_XftDefaultInit(Display *dpy)
{
    ...
    if (!_XftDefaultInitDouble(dpy, pat, FC_SCALE)) goto bail1;
    if (!_XftDefaultInitDouble(dpy, pat, FC_DPI)) goto bail1;
    if (!_XftDefaultInitBool(dpy, pat, XFT_RENDER)) goto bail1;
    if (!_XftDefaultInitInteger(dpy, pat, FC_RGBA)) goto bail1;
    if (!_XftDefaultInitInteger(dpy, pat, FC_LCD_FILTER)) goto bail1;
    if (!_XftDefaultInitBool(dpy, pat, FC_ANTIALIAS)) goto bail1;
    if (!_XftDefaultInitBool(dpy, pat, FC_EMBOLDEN)) goto bail1;
    if (!_XftDefaultInitBool(dpy, pat, FC_AUTOHINT)) goto bail1;
    if (!_XftDefaultInitInteger(dpy, pat, FC_HINT_STYLE)) goto bail1;
    if (!_XftDefaultInitBool(dpy, pat, FC_HINTING)) goto bail1;
    if (!_XftDefaultInitBool(dpy, pat, FC_MINSPACE)) goto bail1;
    if (!_XftDefaultInitInteger(dpy, pat, XFT_MAX_GLYPH_MEMORY)) goto bail1;
...
XftDefaultGetBool(Display *dpy, const char *object, int screen, Bool def)
{
    ...
    if (r != FcResultMatch || v.type != FcTypeBool) return def;
    return v.u.b;
}
...
XftDefaultGetInteger(Display *dpy, const char *object, int screen, int def)
{
    ...
    r = _XftDefaultGet(dpy, object, screen, &v);
    if (r != FcResultMatch || v.type != FcTypeInteger) return def;
    return v.u.i;
}
...
XftDefaultGetDouble(Display *dpy, const char *object, int screen, double def)
{
    ...
    r = _XftDefaultGet(dpy, object, screen, &v);
    if (r != FcResultMatch || v.type != FcTypeDouble) return def;
    return v.u.d;
}
...
XftDefaultSubstitute(Display *dpy, int screen, FcPattern *pattern)
{
    ...
    if (FcPatternGet(pattern, FC_ANTIALIAS, 0, &v) == FcResultNoMatch)
    {
        FcPatternAddBool(pattern,
                         FC_ANTIALIAS,
                         XftDefaultGetBool(dpy, FC_ANTIALIAS, screen, True));
    }
    if (FcPatternGet(pattern, FC_EMBOLDEN, 0, &v) == FcResultNoMatch)
    {
        FcPatternAddBool(pattern,
                         FC_EMBOLDEN,
                         XftDefaultGetBool(dpy, FC_EMBOLDEN, screen, False));
    }
    if (FcPatternGet(pattern, FC_HINTING, 0, &v) == FcResultNoMatch)
    {
        FcPatternAddBool(pattern,
                         FC_HINTING,
                         XftDefaultGetBool(dpy, FC_HINTING, screen, True));
    }
    if (FcPatternGet(pattern, FC_HINT_STYLE, 0, &v) == FcResultNoMatch)
    {
        FcPatternAddInteger(
            pattern,
            FC_HINT_STYLE,
            XftDefaultGetInteger(dpy, FC_HINT_STYLE, screen, FC_HINT_FULL));
    }
    if (FcPatternGet(pattern, FC_AUTOHINT, 0, &v) == FcResultNoMatch)
    {
        FcPatternAddBool(pattern,
                         FC_AUTOHINT,
                         XftDefaultGetBool(dpy, FC_AUTOHINT, screen, False));
    }
    if (FcPatternGet(pattern, FC_RGBA, 0, &v) == FcResultNoMatch)
    {
        ...
        FcPatternAddInteger(
            pattern,
            FC_RGBA,
            XftDefaultGetInteger(dpy, FC_RGBA, screen, subpixel));
    }
    if (FcPatternGet(pattern, FC_LCD_FILTER, 0, &v) == FcResultNoMatch)
    {
        FcPatternAddInteger(
            pattern,
            FC_LCD_FILTER,
            XftDefaultGetInteger(dpy, FC_LCD_FILTER, screen, FC_LCD_DEFAULT));
    }
    if (FcPatternGet(pattern, FC_MINSPACE, 0, &v) == FcResultNoMatch)
    {
        FcPatternAddBool(pattern,
                         FC_MINSPACE,
                         XftDefaultGetBool(dpy, FC_MINSPACE, screen, False));
    }
    if (FcPatternGet(pattern, FC_DPI, 0, &v) == FcResultNoMatch)
    {
        dpi = (((double)DisplayHeight(dpy, screen) * 25.4) /
               (double)DisplayHeightMM(dpy, screen));
        FcPatternAddDouble(pattern,
                           FC_DPI,
                           XftDefaultGetDouble(dpy, FC_DPI, screen, dpi));
    }
