               ...
               int              nglyphs)
{
    ...
    if (nmissing) XftFontLoadGlyphs(dpy, pub, FcTrue, missing, nmissing);

    if (!font->glyphset) goto bail1;
    ...
    if ((size_t)nglyphs > SIZE_MAX / size) goto bail1;
    ...
    for (i = 0, j = 0; i < nglyphs; i++)
    {
        ...
        if (wire >= (Glyph)font->num_glyphs || !font->glyphs[wire]) wire = 0;
        ...
        if (glyph->picture)
        {
            ...
            XRenderComposite(dpy,
                             ...
                             0,
                             0,
                             dstx - glyph->metrics.x,
                             dsty - glyph->metrics.y,
                             glyph->metrics.width,
        ...
        else
        {
            switch (width)
            {
                case 1:
                    ...
                    break;
                case 2:
                    char16[j] = (unsigned short)wire;
                    break;
                case 4:
                    char32[j] = (unsigned int)wire;
                    break;
            }
            dstx += glyph->metrics.xOff;
            dsty += glyph->metrics.yOff;
    ...
    _XftCompositeString(dpy,
                        ...
                        chars,
                        j);
    if (chars != char_local) free(chars);
bail1:
    if (glyphs_loaded) _XftFontManageMemory(dpy, pub);
}
                   ...
                   int                   nglyphs)
{
    ...
    /* Substitute default for non-existent glyphs */
        if (g >= font->num_glyphs || !font->glyphs[g]) g = 0;
        if (font->glyphs[g]) break;
    }
    if (i == nglyphs) goto bail2;
    glyph = font->glyphs[g];
        ...
        if ((glyph = font->glyphs[g]))
        {
            ...
            switch (width)
            {
                case 1:
                    ...
                    break;
                case 2:
                    char16[j] = (unsigned short)g;
                    break;
                case 4:
                    char32[j] = (unsigned int)g;
                    break;
            }
            x += glyph->metrics.xOff;
            y += glyph->metrics.yOff;
...
XftPreferFormat(Display *dpy, XRenderPictFormat *a, XRenderPictFormat *b)
{
    ...
    else if (a->depth > b->depth) prefer = a;
    else prefer = b;
    return prefer;
}
                       ...
                       int                       nglyphs)
{
    ...
    if (i == nglyphs || !firstFont) goto bail2;
               ...
               int              len)
{
    ...
    XftGlyphRender(dpy, op, src, pub, dst, srcx, srcy, x, y, glyphs, len);
    if (glyphs != glyphs_local) free(glyphs);
}
                  ...
                  int              len)
{
    ...
    for (i = 0; i < len; i++)
        glyphs[i] = XftCharIndex(
            dpy,
            pub,
            (FcChar32)((string[i * 4] << 24) | (string[i * 4 + 1] << 16) |
                       (string[i * 4 + 2] << 8) | (string[i * 4 + 3])));
    XftGlyphRender(dpy, op, src, pub, dst, srcx, srcy, x, y, glyphs, len);
    if (glyphs != glyphs_local) free(glyphs);
}
                  ...
                  int              len)
{
    ...
    for (i = 0; i < len; i++)
        glyphs[i] = XftCharIndex(
            dpy,
            pub,
            (FcChar32)((string[i * 4]) | (string[i * 4 + 1] << 8) |
                       (string[i * 4 + 2] << 16) | (string[i * 4 + 3] << 24)));
    XftGlyphRender(dpy, op, src, pub, dst, srcx, srcy, x, y, glyphs, len);
    if (glyphs != glyphs_local) free(glyphs);
}
                  ...
                  int              len)
{
    ...
    XftGlyphRender(dpy, op, src, pub, dst, srcx, srcy, x, y, glyphs, i);
    if (glyphs != glyphs_local) free(glyphs);
}
                   ...
                   int              len)
{
    ...
    XftGlyphRender(dpy, PictOpOver, src, pub, dst, srcx, srcy, x, y, glyphs, i);
    if (glyphs != glyphs_local) free(glyphs);
}
