...
static void
_XftSharpGlyphMono(XftDraw *draw, XftGlyph *glyph, int x, int y)
{
 ...
 */
static void
_XftSharpGlyphGray(XftDraw *draw, XftGlyph *glyph, int x, int y)
{
...
static void
_XftSharpGlyphRgba(XftDraw *draw, XftGlyph *glyph, int x, int y)
{
...
typedef void (*XftSharpGlyph)(XftDraw *draw, XftGlyph *glyph, int x, int y);
...
_XftSharpGlyphFind(XftDraw *draw _X_UNUSED, XftFont *public)
{
    ...
    if (!font->info.antialias) return _XftSharpGlyphMono;
                    ...
                    _Xconst XftColor *color)
{
    ...
    while (height--)
    {
        ...
        while (w--)
        {
            if (bits & bitsMask) XPutPixel(image, xspan, y, pixel);
            bitsMask = bitsMask >> 1;
            if (!bitsMask)
            {
                bits     = *src++;
...
#define cvt8888to0565(s)                                     \
    (CARD16)((((s) >> 3) & 0x001f) | (((s) >> 5) & 0x07e0) | \
             (((s) >> 8) & 0xf800))

#define cvt0565to8888(s)                              \
    (((((s) << 3) & 0xf8) | (((s) >> 2) & 0x7)) |     \
     ((((s) << 5) & 0xfc00) | (((s) >> 1) & 0x300)) | \
     ((((s) << 8) & 0xf80000) | (((s) << 3) & 0x70000)))

#define cvt8888to0555(s)                                     \
    (CARD16)((((s) >> 3) & 0x001f) | (((s) >> 6) & 0x03e0) | \
             (((s) >> 7) & 0x7c00))

#define cvt0555to8888(s)                              \
    (((((s) << 3) & 0xf8) | (((s) >> 2) & 0x7)) |     \
     ((((s) << 6) & 0xf800) | (((s) >> 0) & 0x300)) | \
     ((((s) << 9) & 0xf80000) | (((s) << 4) & 0x70000)))

#define XftIntMult(a, b, t, cast) \
    (((t) = (cast)((a) * (b) + 0x80)), ((((t) >> 8) + (t)) >> 8))
...
#define XftOverU(x, y, i, a, t)                                               \
    ((t) =                                                                    \
         (CARD16)XftIntMult(XftGet8(y, i), (a), (t), CARD16) + XftGet8(x, i), \
                        ...
                        _Xconst XftColor *color)
{
    ...
    while (height--)
    {
        ...
        while (w--)
        {
            ...
            if (m == 0xff)
            {
                if (srca == 0xff) *dst = src;
                else *dst = fbOver24(src, *dst);
            }
            else if (m)
            {
                d    = fbIn(src, m);
                    ...
                    _Xconst XftColor *color)
{
    ...
    srca = color->color.alpha >> 8;
    src  = (srca << 24 | (CARD32)((color->color.red & 0xff00) << 8) |
           (color->color.green & 0xff00) | (color->color.blue) >> 8);
    x -= xftg->metrics.x;
    y -= xftg->metrics.y;
    width  = xftg->metrics.width;
    height = xftg->metrics.height;
...
{
    if (!draw->visual) return FcFalse;
    if (draw->visual->class != TrueColor) return FcFalse;
    return FcTrue;
}
...
_XftSmoothGlyphFind(XftDraw *draw, XftFont *public)
{
    ...
    if (!font->info.antialias) return _XftSmoothGlyphMono;
    else switch (font->info.rgba)
        {
            case FC_RGBA_RGB:
            case FC_RGBA_BGR:
...
static int
XftGetImageErrorHandler(Display *dpy             _X_UNUSED,
                        XErrorEvent *error_event _X_UNUSED)
{
    return 0;
}
             ...
             int              nglyphs)
{
    ...
    while (n--)
        if (XftFontCheckGlyph(dpy, public, FcTrue, *g++, missing, &nmissing))
            glyphs_loaded = FcTrue;
    if (nmissing) XftFontLoadGlyphs(dpy, public, FcTrue, missing, nmissing);
        ...
        _XftSmoothGlyphPossible(draw))
    {
        ...
        if (!gi.width || !gi.height) goto bail1;
        ox = x - gi.x;
        oy = y - gi.y;
    /*
	 ...
	 */
        if (draw->core.use_pixmap == 0)
        {
            ...
            if (!image) draw->core.use_pixmap = XFT_ASSUME_PIXMAP;
        }
        else
        {
        ...
        if (!image && (depth = XftDrawDepth(draw)))
        {
            ...
            pix =
                XCreatePixmap(dpy, draw->drawable, gi.width, gi.height, depth);
        ...
        if (image->byte_order != XftNativeByteOrder()) XftSwapImage(image);
        while (n--)
        {
            glyph = *g++;
...
bail1:
    if (glyphs_loaded) _XftFontManageMemory(dpy, public);
}
                 ...
                 int                   nglyphs)
{
    ...
    else
    {
        ...
        for (i = 0; i < nglyphs; i++)
        {
            ...
            if (glyph >= font->num_glyphs || !(xftg = font->glyphs[glyph]))
                xftg = _XftGlyphDefault(dpy, public);
            if (xftg) (*sharp)(draw, xftg, glyphs[i].x, glyphs[i].y);
        }
    }
bail1:
    if (glyphs_loaded) _XftFontManageMemory(dpy, public);
                     ...
                     int                       nglyphs)
{
    ...
    for (i = 0; i < nglyphs; i++)
        if (((XftFontInt *)glyphs[i].font)->info.antialias) break;
