...
_XftGetFile(const FcChar8 *file, int id)
{
    ...
    if (!XftInitFtLibrary()) return NULL;
    ...
    XftMemAlloc(XFT_MEM_FILE,
                sizeof(XftFtFile) + strlen((const char *)file) + 1);
    if (XftDebug() & XFT_DBG_REF)
...
_XftLockFile(XftFtFile *f)
{
    ...
    if (!f->face)
    {
        ...
        if (FT_New_Face(_XftFTlibrary, f->file, f->id, &f->face)) --f->lock;
...
{
    if (--f->lock < 0) _XftLockError("too many file unlocks");
}
	     ...
	     */
            if (FT_Set_Char_Size(face,
                ...
                FT_Set_Char_Size(face,
                                 face->available_sizes[best].width << 6,
...
_XftReleaseFile(XftFtFile *f)
{
    ...
    if (--f->ref != 0) return;
    if (f->lock) _XftLockError("Attempt to close locked file");
    if (f->file)
    {
...
_XftIsPrime(FcChar32 i)
{
    ...
    if (i < 2) return FcFalse;
    if ((i & 1) == 0)
    {
        if (i == 2) return FcTrue;
        return FcFalse;
    }
    l = _XftSqrt(i) + 1;
    for (t = 3; t <= l; t += 2)
        if (i % t == 0) return FcFalse;
    return FcTrue;
}
...
XftFontInfoFill(Display *dpy, _Xconst FcPattern *pattern, XftFontInfo *fi)
{
    ...
    if (filename) fi->file = _XftGetFile(filename, id);
    else if (FcPatternGetFTFace(pattern, FC_FT_FACE, 0, &face) ==
                 FcResultMatch &&
             face)
        fi->file = _XftGetFaceFile(face);
    if (!fi->file) goto bail0;
    ...
    if (XftDebug() & XFT_DBG_OPEN)
        printf("XftFontInfoFill: %s: %d (%g pixels)\n",
               (filename ? filename : (const FcChar8 *)"<none>"),
               id,
               dsize);
    /*
     * Get antialias value
    ...
    mid = 1;
    while (FcPatternGetMatrix(pattern, FC_MATRIX, mid, &font_matrix) ==
           FcResultMatch)
    {
        FcMatrixInit(&fm1);
...
#  define XFT_EMBEDDED_BITMAP "embeddedbitmap"
    ...
    switch (FcPatternGetInteger(pattern, FC_CHAR_WIDTH, 0, &fi->char_width))
    {
        case FcResultNoMatch:
            ...
            break;
        case FcResultMatch:
            if (fi->char_width) fi->spacing = FC_MONO;
            break;
        default:
            goto bail1;
    }
...
{
    if (fi->file) _XftReleaseFile(fi->file);
}
...
XftFontOpenInfo(Display *dpy, FcPattern *pattern, XftFontInfo *fi)
{
    ...
    for (font = (XftFontInt *)*bucket; font;
         font = (XftFontInt *)font->hash_next)
        if (XftFontInfoEqual(&font->info, fi))
        {
            if (!font->ref++) --info->num_unref_fonts;
    ...
    if (!_XftSetFace(fi->file, fi->xsize, fi->ysize, &fi->matrix)) goto bail1;
    ...
    else charset = FcFreeTypeCharSet(face, FcConfigGetBlanks(NULL));

    antialias = fi->antialias;
    if (!(face->face_flags & FT_FACE_FLAG_SCALABLE)) antialias = FcFalse;
    ...
    alloc_size = (sizeof(XftFontInt) + (size_t)num_glyphs * sizeof(XftGlyph *) +
    ...
    if (fi->transform)
    {
        ...
        if (fi->minspace) height = ascent + descent;
        else
        {
            vector.x = 0;
    ...
    else
    {
        ...
        if (fi->minspace) height = ascent + descent;
        else height = (int)(face->size->metrics.height >> 6);
    }
    font->public.ascent  = ascent;
    font->public.descent = descent;
    font->public.height  = height;

    if (fi->char_width) font->public.max_advance_width = fi->char_width;
    else
    {
        if (fi->transform)
        {
    ...
    *bucket         = &font->public;
    ...
    font->sizeof_glyph =
        (font->track_mem_usage ? sizeof(XftGlyphUsage) : sizeof(XftGlyph));
...
XftFontOpenPattern(Display *dpy, FcPattern *pattern)
{
    ...
    if (!XftFontInfoFill(dpy, pattern, &info)) return NULL;
...
XftFontDestroy(Display *dpy, XftFont *public)
{
    ...
    if (info) info->glyph_memory -= font->glyph_memory;
    /* Clean up the info */
    XftFontInfoEmpty(dpy, &font->info);
    /* Free the glyphset */
    if (font->glyphset) XRenderFreeGlyphSet(dpy, font->glyphset);
    /* Free the glyphs */
    for (i = 0; i < font->num_glyphs; i++)
    {
        ...
        if (xftg)
        {
            if (xftg->bitmap) free(xftg->bitmap);
            free(xftg);
        }
    }
...
XftFontManageMemory(Display *dpy)
{
    ...
    while (info->num_unref_fonts > info->max_unref_fonts)
    {
        ...
        if (XftDebug() & XFT_DBG_CACHE)
            printf("freeing unreferenced font %s/%d size %dx%d\n",
                   font->info.file->file,
                   font->info.file->id,
                   (int)font->info.xsize >> 6,
                   (int)font->info.ysize >> 6);
    ...
    /* Unhook from hash list */
        for (prev = &info->fontHash[font->info.hash % XFT_NUM_FONT_HASH]; *prev;
...
{
    if (_XftFTlibrary) return FcTrue;
    if (FT_Init_FreeType(&_XftFTlibrary)) return FcFalse;
    return FcTrue;
}
