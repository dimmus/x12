...
_XftValidateGlyphUsage(XftFontInt *font)
{
    if (font->newest != FT_UINT_MAX)
    {
        ...
        for (forward = 1, next = x1st->newer;
             xuse != NULL && next != font->newest;
             next = xuse->newer)
        {
            ...
            if (++forward > font->total_inuse)
            {
                printf("Xft: too many in-use glyphs (%d vs %d)\n",
                       forward,
                       font->total_inuse);
                if (forward > font->total_inuse + 10) break;
            }
             ...
             next = xuse->older)
        {
            ...
            if (++reverse > font->total_inuse)
            {
                printf("Xft: too many in-use glyphs (%d vs %d)\n",
                       reverse,
                       font->total_inuse);
                if (reverse > font->total_inuse + 10) break;
            }
            xuse = (XftGlyphUsage *)font->glyphs[next];
        }
        if (reverse < font->total_inuse)
                             ...
                             m3x3           m)
{
    ...
    /* fall-through */
        ...
        case FT_PIXEL_MODE_LCD:
            if (mode != FT_RENDER_MODE_LCD) return -1;
    ...
    /* horz pixel triplets are packed into 32-bit ARGB values */
        ...
        case FT_PIXEL_MODE_LCD_V:
            if (mode != FT_RENDER_MODE_LCD_V) return -1;
 ...
 */
static void
_scaled_fill_xrender_bitmap(FT_Bitmap *target, FT_Bitmap *source, m3x3 m)
{
    ...
    if (src_pitch < 0) src_buf -= ((unsigned)src_pitch * (source->rows - 1));
    ...
    for (y = height; y > 0; y--, dst_line += pitch)
    {
        ...
        /* compute target pixel location in source space */
            ...
            {
                if (vector.x < 0 || vector.x >= source->width) continue;
                if (vector.y < 0 || vector.y >= source->rows) continue;
            }
                ...
                case FT_PIXEL_MODE_BGRA: /* scale by averaging all relevant source pixels, keep BGRA format */
                    {
                        ...
                        for (sample_y = -sampling_height;
                             sample_y < sampling_height + 1;
                             ++sample_y)
                        {
                            ...
                            for (sample_x = -sampling_width;
                                 sample_x < sampling_width + 1;
                                 ++sample_x)
                            {
                        ...
                        for (i = 0; i < 4; ++i)
                            dst_line[4 * x + i] =
                                (unsigned char)(bgra[i] / sample_count);
                        break;
                    }
            }
        }
                     ...
                     int            bgr)
{
...

    {
        ...
        if (src_pitch < 0) srcLine -= ((unsigned)src_pitch * (ftbit->rows - 1));
                ...
                if (subpixel) /* convert mono to ARGB32 values */
                {
                         ...
                         h--, srcLine += src_pitch, dstLine += pitch)
                    {
                        ...
                        for (x = 0; x < width; x++)
                        {
                         ...
                         FT_RENDER_MODE_NORMAL) /* convert mono to 8-bit gray */
                {
                    for (h = height; h > 0;
                         h--, srcLine += src_pitch, dstLine += pitch)
                    {
                  ...
                  int              nglyph)
{
    ...
    if (font->info.color) mode = FT_RENDER_MODE_NORMAL;
...
#define ROUND(x) (((x) + 32) & -64)
            ...
            for (xc = 0; xc <= 1; xc++)
            {
                for (yc = 0; yc <= 1; yc++)
                {
                    vector.x = glyphslot->metrics.horiBearingX +
                               xc * glyphslot->metrics.width;
                    vector.y = glyphslot->metrics.horiBearingY -
                               yc * glyphslot->metrics.height;
	 ...
	 */
        ...
        if (XftDebug() & XFT_DBG_GLYPH)
        {
            ...
            if (XftDebug() & XFT_DBG_GLYPHV)
            {
                ...
                line = ftbit->buffer;
                if (ftbit->pitch < 0) line -= ftbit->pitch * (height - 1);
        ...
        m3x3_uniform(m);
        size = _compute_xrender_bitmap_size(&local,
                                            glyphslot,
                                            mode,
                                            glyph_transform ? &font->info.matrix
                                                            : NULL,
                                            m);
        if (size < 0) continue;
	 ...
	 */
        if (!need_bitmaps &&
            ((unsigned long)size > (info->max_glyph_memory / 100)))
            continue;
	 ...
	 */
        if (size > bufSize)
        {
            if (bufBitmap != bufLocal) free(bufBitmap);
            bufBitmap = (unsigned char *)malloc((size_t)size);
            if (!bufBitmap) continue;
            bufSize = size;
        }
	 ...
	 */
        ...
        if (font->format)
        {
            ...
            {
                Pixmap pixmap = XCreatePixmap(dpy,
                                              DefaultRootWindow(dpy),
                                              local.width,
                                              local.rows,
                                              32);
                GC     gc     = XCreateGC(dpy, pixmap, 0, NULL);
                XImage image  = { (int)local.width,
                                  (int)local.rows,
                                  0,
                                  ZPixmap,
                                  (char *)bufBitmap,
                                  ...
                                  32,
                                  32,
                                  (int)(local.width * 4 - (unsigned)local.pitch),
                                  32,
                                  0,
                                  0,
                                  0,
                ...
                XPutImage(dpy,
                          ...
                          local.rows);
                xftg->picture =
                    XRenderCreatePicture(dpy, pixmap, font->format, 0, NULL);
		 ...
		 */
                if (font->info.matrix.xy || font->info.matrix.yx)
                    xftg->glyph_memory +=
                        font->max_glyph_memory - (unsigned long)size;
                else xftg->glyph_memory += (size_t)size * 255;
            }
            else
                XRenderAddGlyphs(dpy,
                                 font->glyphset,
                                 &glyph,
                                 &xftg->metrics,
                                 1,
        ...
        else
        {
            if (size)
            {
                ...
                if (xftg->bitmap) memcpy(xftg->bitmap, bufBitmap, (size_t)size);
            }
            else xftg->bitmap = NULL;
        }
        ...
        if (font->track_mem_usage)
        {
            ...
            else
            {
                ...
                if (XftDebug() & XFT_DBG_USAGE)
                    printf("alloc %p -> %d: %p USE %d.%d, %p NEW %d.%d %p OLD "
                           "%d.%d\n",
                           (void *)font,
                           glyphindex,
                           (void *)xuse,
            ...
            font->total_inuse++;
            if (XftDebug() & XFT_DBG_USAGE) _XftValidateGlyphUsage(font);
        }
    }
    if (bufBitmap != bufLocal) free(bufBitmap);
                    ...
                    int              nglyph)
{
    ...
    while (nglyph--)
    {
        ...
        if (xftg->glyph_memory)
        {
            ...
            if (font->format)
            {
                if (xftg->picture) XRenderFreePicture(dpy, xftg->picture);
            ...
            else if (xftg->bitmap) free(xftg->bitmap);
            font->glyph_memory -= xftg->glyph_memory;
            if (info) info->glyph_memory -= xftg->glyph_memory;
        }
        ...
        if (font->track_mem_usage)
        {
            ...
            if (xuse->older != FT_UINT_MAX)
            {
                ...
                {
                    if (font->newest == xuse->older) font->newest = FT_UINT_MAX;
                    else font->newest = xuse->older;
                }
            }
                  ...
                  int     *nmissing)
{
    ...
    if (glyph >= font->num_glyphs) return FcFalse;
     ...
     */
    if (glyph != 0 && font->track_mem_usage && font->total_inuse > 10 &&
        font->newest != FT_UINT_MAX && font->newest != glyph)
    {
        XftGlyphUsage *xuse = (XftGlyphUsage *)xftg;
...
_X_EXPORT FcBool
XftCharExists(Display *dpy _X_UNUSED, XftFont *pub, FcChar32 ucs4)
{
    if (pub->charset) return FcCharSetHasChar(pub->charset, ucs4);
    return FcFalse;
}
...
_X_EXPORT FT_UInt
XftCharIndex(Display *dpy, XftFont *pub, FcChar32 ucs4)
{
    XftFontInt *font = (XftFontInt *)pub;
    FcChar32    ent, offset;
    FT_Face     face;

    if (!font->hash_value) return 0;
    ...
    while (font->hash_table[ent].ucs4 != ucs4)
    {
        ...
        {
            if (!XftCharExists(dpy, pub, ucs4)) return 0;
