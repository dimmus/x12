...
static XEXT_GENERATE_FIND_DISPLAY(find_display,
                                  xevi_info,
                                  xevi_extension_name,
                                  &xevi_extension_hooks,
                                  0,
                                  NULL) Bool XeviQueryExtension(Display *dpy)
{
...
notInList(VisualID32 *visual, int sz_visual, VisualID newVisualid)
{
    while (sz_visual-- > 0)
    {
        if (*visual == newVisualid) return False;
        visual++;
    }
    return True;
}
...
Status
XeviGetVisualInfo(register Display    *dpy,
                  VisualID            *visual,
                  int                  n_visual,
                  ...
                  int                 *n_info_return)
{
    ...
    *evi_return    = NULL;
    ...
    if (!*evi_return || !temp_xInfo || !temp_conflict)
    {
        ...
        if (temp_xInfo) Xfree(temp_xInfo);
        if (temp_conflict) Xfree(temp_conflict);
        return BadAlloc;
    }
