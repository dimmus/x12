                                 ...
                                 _Xconst XRenderPictureAttributes *attributes)
{
    ...
    if (valuemask & CPRepeat) *value++ = (unsigned long)attributes->repeat;
    if (valuemask & CPAlphaMap) *value++ = attributes->alpha_map;
    ...
    if (valuemask & CPClipMask) *value++ = attributes->clip_mask;
    ...
    if (valuemask & CPPolyEdge) *value++ = (unsigned long)attributes->poly_edge;
    if (valuemask & CPPolyMode) *value++ = (unsigned long)attributes->poly_mode;
    if (valuemask & CPDither) *value++ = attributes->dither;
                     ...
                     _Xconst XRenderPictureAttributes *attributes)
{
    ...
    _XRenderProcessPictureAttributes(dpy, req, valuemask, attributes);
    UnlockDisplay(dpy);
    SyncHandle();
}
                                ...
                                int                 n)
{
    ...
    RenderSimpleCheckExtension(dpy, info);
    LockDisplay(dpy);
    _XRenderSetPictureClipRectangles(dpy,
                                     info,
                                     picture,
                                     xOrigin,
                                     yOrigin,
...
void
XRenderSetPictureClipRegion(Display *dpy, Picture picture, Region r)
{
    XRenderExtDisplayInfo *info = XRenderFindDisplay(dpy);
    XRectangle            *xr;
    unsigned long          total;
...
void
XRenderSetPictureTransform(Display *dpy, Picture picture, XTransform *transform)
{
    XRenderExtDisplayInfo         *info = XRenderFindDisplay(dpy);
...
void
XRenderFreePicture(Display *dpy, Picture picture)
{
    XRenderExtDisplayInfo *info = XRenderFindDisplay(dpy);
...
Picture
XRenderCreateSolidFill(Display *dpy, const XRenderColor *color)
{
    XRenderExtDisplayInfo     *info = XRenderFindDisplay(dpy);
    Picture                    pid;
...
Picture
XRenderCreateLinearGradient(Display               *dpy,
...
Picture
XRenderCreateConicalGradient(Display                *dpy,
