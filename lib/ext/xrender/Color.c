...
XRenderParseColor(Display *dpy, char *spec, XRenderColor *def)
{
	 ...
	 */
		...
		*/
            ...
            if (n < 4)
            {
                *pShort = (unsigned short)(((unsigned long)*pShort * 0xFFFF) /
                                           (unsigned long)((1 << n * 4) - 1));
            }
        }
        def->red   = elements[0];
    ...
    else
    {
        ...
        if (!XParseColor(dpy, colormap, spec, &coreColor)) return 0;
    ...
    def->green =
        (unsigned short)((unsigned)(def->green * def->alpha) / 0xffffU);
