                 ...
                 int                 nbyte_images)
{
    ...
    if (nbyte_images & 3) nbyte_images += 4 - (nbyte_images & 3);
    RenderSimpleCheckExtension(dpy, info);
    LockDisplay(dpy);
...
#define MAX_8 252
    ...
    req->length =
        (CARD16)(req->length +
                 ((len + 3) >> 2));  /* convert to number of 32-bit words */
...
#define MAX_8 252
	 ...
	 */
        ...
        while (nchars)
        {
            ...
            BufAlloc(xGlyphElt *, elt, SIZEOF(xGlyphElt)) elt->len =
                (CARD8)this_chars;
            elt->deltax = (INT16)xDst;
            elt->deltay = (INT16)yDst;
            xDst        = 0;
...
#define MAX_16 254
	 ...
	 */
        elen =
            SIZEOF(xGlyphElt) * ((nchars + MAX_16 - 1) / MAX_16) + nchars * 2;
        len += (elen + 3) >> 2;
    }
...
#define MAX_32 254
	 ...
	 */
        ...
        nchars = elts[i].nchars;
        elen =
            SIZEOF(xGlyphElt) * ((nchars + MAX_32 - 1) / MAX_32) + nchars * 4;
        len += (elen + 3) >> 2;
    }
