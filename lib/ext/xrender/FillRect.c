...
#define size \
    (SIZEOF(xRenderFillRectanglesReq) + FRCTSPERBATCH * SIZEOF(xRectangle))
                     ...
                     unsigned int          height)
{
    ...
    if (req->reqType == info->codes->major_opcode &&
        req->renderReqType == X_RenderFillRectangles && req->op == op &&
        req->dst == dst && req->color.red == color->red &&
        req->color.green == color->green && req->color.blue == color->blue &&
