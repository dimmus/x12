...
static XRRScreenConfiguration *
_XRRGetScreenInfo(Display *dpy, XExtDisplayInfo *info, Window window);
 ...
 */
static XRRScreenConfiguration *
_XRRValidateCache(Display *dpy, XExtDisplayInfo *info, int screen)
{
    if ((screen >= 0) && (screen < ScreenCount(dpy)) && XextHasExtension(info))
    {
        ...
        if (configs[screen] == NULL)
            configs[screen] =
                _XRRGetScreenInfo(dpy, info, RootWindow(dpy, screen));
...
Rotation
XRRRotations(Display *dpy, int screen, Rotation *current_rotation)
{
...
/* given a screen, return the information from the (possibly) cached data */
XRRScreenSize *
XRRSizes(Display *dpy, int screen, int *nsizes)
{
    XRRScreenConfiguration *config;
    XExtDisplayInfo        *info = XRRFindDisplay(dpy);
...
short *
XRRRates(Display *dpy, int screen, int sizeID, int *nrates)
{
    XRRScreenConfiguration *config;
    XExtDisplayInfo        *info = XRRFindDisplay(dpy);
...
/* given a screen, return the information from the (possibly) cached data */
Time
XRRTimes(Display *dpy, int screen, Time *config_timestamp)
{
    XRRScreenConfiguration *config;
    XExtDisplayInfo        *info = XRRFindDisplay(dpy);
...
_XRRGetScreenInfo(Display *dpy, XExtDisplayInfo *info, Window window)
{
    ...
    {
        if (getting_version) DeqAsyncHandler(dpy, &async);
        SyncHandle();
        return NULL;
    }
    if (getting_version)
    {
        ...
        xrri->has_rates =
            _XRRHasRates(xrri->minor_version, xrri->major_version);
    }
    ...
    if (rep.length < INT_MAX >> 2)
    {
        ...
        nbytesRead =
            (long)(rep.nSizes * SIZEOF(xScreenSizes) +
	 ...
	 */
        ...
        rbytes =
            sizeof(XRRScreenConfiguration) +
            (rep.nSizes * sizeof(XRRScreenSize) + rep.nrateEnts * sizeof(int));
    ...
    scp->screen = ScreenOfDisplay(dpy, XRRRootToScreen(dpy, rep.root));
...
XRRGetScreenInfo(Display *dpy, Window window)
{
    XRRScreenConfiguration *config;
    XExtDisplayInfo        *info = XRRFindDisplay(dpy);
    XRRFindDisplay(dpy);
...
Status
XRRSetScreenConfigAndRate(Display                *dpy,
                          XRRScreenConfiguration *config,
                          Drawable                draw,
                          int                     size_index,
                          Rotation                rotation,
...
Status
XRRSetScreenConfig(Display                *dpy,
                   XRRScreenConfiguration *config,
                   Drawable                draw,
                   int                     size_index,
                   Rotation                rotation,
                   Time                    timestamp)
{
    return XRRSetScreenConfigAndRate(dpy,
                                     config,
                                     draw,
                                     size_index,
                                     rotation,
