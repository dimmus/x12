...
doGetScreenResources(Display *dpy, Window window, int poll)
{
    ...
    req->randrReqType =
        poll ? X_RRGetScreenResources : X_RRGetScreenResourcesCurrent;
    req->window = window;
    ...
    if (rep.length < INT_MAX >> 2)
    {
        ...
        nbytesRead = (long)(rep.nCrtcs * 4 + rep.nOutputs * 4 +
	 ...
	 */
        ...
        rbytes = (sizeof(XRRScreenResources) + rep.nCrtcs * sizeof(RRCrtc) +
                  rep.nOutputs * sizeof(RROutput) +
                  rep.nModes * sizeof(XRRModeInfo) + rep.nbytesNames +
                  rep.nModes);    /* '\0' terminate names */
...
Status
XRRGetScreenSizeRange(Display *dpy,
                      Window   window,
                      int     *minWidth,
                      int     *minHeight,
                      int     *maxWidth,
