...
XETC *
XECreateTC(Display *dpy, CARD32 valuemask, XETCValues *value)
{
    ...
    if (!XETrapQueryExtension(dpy,
                              &(tc->eventBase),
                              &(tc->errorBase),
                              &(tc->extOpcode)))
    {
        String       params     = XTrapExtName;
        unsigned int num_params = 1L;
        XtWarningMsg("CantLoadExt",
                     "XECreateTC",
                     "XTrapToolkitError",
                     "Can't load %s extension",
                     &params,
                     &num_params);
    ...
    {   /* We can't seem to communicate with the extension! */
        String       params     = XTrapExtName;
        unsigned int num_params = 1L;
        XtWarningMsg("CantComm",
                     "XECreateTC",
                     "XTrapToolkitError",
                     "Can't communicate with extension %s",
                     &params,
                     &num_params);
        (void)XtFree(
            (XtPointer)tc->xbuff);/* de-allocate memory just alloc'd */
        (void)XtFree((XtPointer)tc);           /* free the allocated TC */
        last_tc->next = NULL; /* Clear now nonexistant forward pointer */
        return (NULL);
    }
...
static int
CheckChangeBits(XETrapFlags *dest, XETrapFlags *src, INT32 bit)
{
    int chg_flag = False;
 ...
 */
int
XEChangeTC(XETC *tc, CARD32 mask, XETCValues *values)
{
    int                  status = True;
    ...
    { /* Statistics need changing */
        if (CheckChangeBits(&(tval->v.flags),
                            &(values->v.flags),
                            XETrapStatistics))
        {
...
XEFreeTC(XETC *tc)
{
    ...
    if (tc)
    {
        while (list->next != NULL)
        {
            ...
            else list = list->next; /* Update the list pointer */
        }
        if (tc->values.req_cb)
        {
 ...
 */
int
XETrapSetMaxPacket(XETC *tc, Bool set_flag, CARD16 size)
{
    XETCValues tcv;
    int        status = True;
...
int
XETrapSetCommandKey(XETC *tc, Bool set_flag, KeySym cmd_key, Bool mod_flag)
{
    XETCValues tcv;
    int        status = True;
    KeyCode    cmd_keycode;
...
int
XETrapSetTimestamps(XETC *tc, Bool set_flag, Bool delta_flag)
{
    XETCValues tcv;
    int        status = True;
...
int
XETrapSetWinXY(XETC *tc, Bool set_flag)
{
    XETCValues tcv;
    int        status = True;
...
int
XETrapSetCursor(XETC *tc, Bool set_flag)
{
    XETCValues tcv;
    int        status = True;
...
int
XETrapSetXInput(XETC *tc, Bool set_flag)
{
    XETCValues tcv;
    int        status = True;
...
int
XETrapSetColorReplies(XETC *tc, Bool set_flag)
{
    XETCValues tcv;
    int        status = True;
...
int
XETrapSetGrabServer(XETC *tc, Bool set_flag)
{
    XETCValues tcv;
    int        status = True;
...
int
XETrapSetStatistics(XETC *tc, Bool set_flag)
{
    XETCValues tcv;
    int        status = True;
...
int
XETrapSetRequests(XETC *tc, Bool set_flag, ReqFlags requests)
{
    XETCValues tcv;
    int        status = True;
    int        i;
...
int
XETrapSetEvents(XETC *tc, Bool set_flag, EventFlags events)
{
    XETCValues tcv;
    int        status = True;
    int        i;
    ...
    else
    {
        switch (type)
        {
            case KeyPress:
                ...
                if ((*next_key == XEKeyIsOther) && *gate_closed == True)
                {
                    break;
                }
                *gate_closed = True;
