...
#  define GetOldReq(name, req, old_length)                                  \
      WORD64ALIGN                                                           \
      if ((dpy->bufptr + SIZEOF(x##name##Req)) > dpy->bufmax) _XFlush(dpy); \
      ...
      {                                                         \
          ...
          if (!--ctl$gw_soft_ast_lock_depth)                    \
              ...
              {                                                 \
                  ...
                  if (ctl$gb_reenable_asts != 0) sys$setast(1); \
              }                                                 \
      }
 ...
 */
int
XEGetVersionRequest(XETC *tc, XETrapGetVersRep *ret)
{
    int                status     = True;
    Display           *dpy        = tc->dpy;
    ...
    int                numlongs =
        (SIZEOF(xXTrapGetVersReply) - SIZEOF(xReply) + SIZEOF(CARD32) - 1) /
        SIZEOF(CARD32);
    LockDisplay(dpy);
...
int
XEGetAvailableRequest(XETC *tc, XETrapGetAvailRep *ret)
{
    int                 status     = True;
    Display            *dpy        = tc->dpy;
    CARD32              X_XTrapGet = tc->extOpcode;
    ...
    int                 numlongs =
        (SIZEOF(xXTrapGetAvailReply) - SIZEOF(xReply) + SIZEOF(CARD32) - 1) /
        SIZEOF(CARD32);
    LockDisplay(dpy);
    GetReq(XTrapGet, reqptr);
...
{   /* protocol changed between V3.1 and V3.2! */
    ...
    memcpy((char *)reqptr->config_flags_valid,
           (char *)tc->values.v.flags.valid,
           4);
    memcpy((char *)reqptr->config_flags_data,
           (char *)tc->values.v.flags.data,
           4);
    memcpy((char *)reqptr->config_flags_req,
           (char *)tc->values.v.flags.req,
           XETrapMaxRequest);
    memcpy((char *)reqptr->config_flags_event,
           (char *)tc->values.v.flags.event,
           XETrapMaxEvent);
...
int
XEFlushConfig(XETC *tc)
{
    return ((tc->dirty) ? XEConfigRequest(tc) : True);
}

int
XEResetRequest(XETC *tc)
{
    int        status  = True;
    Display   *dpy     = tc->dpy;
...
int
XEGetLastInpTimeRequest(XETC *tc, XETrapGetLastInpTimeRep *ret)
{   /* this was broken in V3.1! */
    ...
    int                 numlongs =
        (SIZEOF(xXTrapGetLITimReply) - SIZEOF(xReply) + SIZEOF(CARD32) - 1) /
        SIZEOF(CARD32);
    LockDisplay(dpy);
    GetReq(XTrap, reqptr);
...
#ifndef _XINPUT
int
XESimulateXEventRequest(XETC  *tc,
                        CARD8  type,
                        CARD8  detail,
                        CARD16 x,
                        CARD16 y,
...
#endif
int
XEGetCurrentRequest(XETC *tc, XETrapGetCurRep *ret)
{
    int               status  = True;
    Display          *dpy     = tc->dpy;
    CARD32            X_XTrap = tc->extOpcode;
    ...
    int               numlongs =
        (SIZEOF(xXTrapGetCurReply) - SIZEOF(xReply) + SIZEOF(CARD32) - 1) /
        SIZEOF(CARD32);
    status = XEFlushConfig(tc); /* Flushout any pending configuration first */
    if (status == True)
    {
        ...
        memcpy((char *)ret->config.flags.req,
               rep.data_config_flags_req,
               XETrapMaxRequest);
        memcpy((char *)ret->config.flags.event,
               rep.data_config_flags_event,
               XETrapMaxEvent);
...
int
XEGetStatisticsRequest(XETC *tc, XETrapGetStatsRep *ret)
{
    int                 status  = True;
    Display            *dpy     = tc->dpy;
    CARD32              X_XTrap = tc->extOpcode;
...
#  ifndef VECTORED_EVENTS
            int numlongs =
                (1060 - sizeof(xReply) + sizeof(long) - 1) / sizeof(long);
#  else
            int numlongs =
                (1544 - sizeof(xReply) + sizeof(long) - 1) / sizeof(long);
#  endif
            status = _XReply(dpy, (xReply *)&rep, numlongs, xTrue);
            if (status == True)
