...
Boolean (*XETrapGetEventHandler(XETC *tc, CARD32 id))(XETrapDataEvent *event,
                                                      XETC            *tc)
{
    return ((id < XETrapNumberEvents) ? tc->eventFunc[id] : NULL);
}

Boolean (*XETrapSetEventHandler(
    XETC  *tc,
    CARD32 id,
    Boolean (*pfunc)(XETrapDataEvent *event, XETC *tc)))(XETrapDataEvent *event,
                                                         XETC            *tc)
{
...
Boolean
XETrapDispatchEvent(XEvent *pevent, XETC *tc)
{
    ...
    if (firstEvent != 0 && id >= firstEvent && id <= lastEvent)
    {
        ...
        {
            status =
                (*tc->eventFunc[id - firstEvent])((XETrapDataEvent *)pevent,
                                                  tc);
        }
    }
    else
...
XtInputMask
XETrapAppPending(XtAppContext app)
{
...
void
XETrapAppMainLoop(XtAppContext app, XETC *tc)
{
    XEvent      event;
    XtInputMask imask;
...
int
XETrapAppWhileLoop(XtAppContext app, XETC *tc, Bool *done)
{
    XEvent      event;
    XtInputMask imask;
    int         status = True;
...
int
XETrapWaitForSomething(XtAppContext app)
{
#ifndef vms
...
#  define IS_AFTER(t1, t2)        \
    ...
    {   /* check timeout queue */
         ...
         * milliseconds 
         */
        status = lib$sub_times(&(te_ptr->te_timer_value.low),
                               &cur_time,
                               &result_time);
        /*
        ...
        else if ((status & 1) == 1)
        {
            ...
            return (XMultiplexInput(app->count,
                                    &(app->list[0L]),
                                    app->Input_EF_Mask,
                                    quotient,
                                    0L,
