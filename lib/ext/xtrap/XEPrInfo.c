...
void
XEPrintRelease(FILE *ofp, XETrapGetAvailRep *pavail)
{
    fprintf(ofp,
            "\tRelease:   %d.%d-%d\n",
            XETrapGetAvailRelease(pavail),
            XETrapGetAvailVersion(pavail),
            XETrapGetAvailRevision(pavail));
}

void
XEPrintTkRelease(FILE *ofp, XETC *tc)
{
    fprintf(ofp,
            "\tRelease:   %d.%d-%d\n",
            XEGetRelease(tc),
...
void
XEPrintPlatform(FILE *ofp, XETrapGetAvailRep *pavail)
{
    fprintf(ofp,
            "\tPlatform:  %s (0x%02x)\n",
...
void
XEPrintAvailFlags(FILE *ofp, XETrapGetAvailRep *pavail)
{
    CARD8 f[4L];
...
void
XEPrintAvailPktSz(FILE *ofp, XETrapGetAvailRep *pavail)
{
    fprintf(ofp, "\tMax Packet Size: %d\n", XETrapGetAvailMaxPktSize(pavail));
}

void
XEPrintStateFlags(FILE *ofp, XETrapGetCurRep *pcur)
{
    CARD8 f[2];
...
void
XEPrintMajOpcode(FILE *ofp, XETrapGetAvailRep *pavail)
{
    fprintf(ofp, "\tMajor Opcode:  %d\n", (int)XETrapGetAvailOpCode(pavail));
}

void
XEPrintCurXY(FILE *ofp, XETrapGetAvailRep *pavail)
{
    fprintf(ofp,
            "\tCurrent (x,y):  (%d,%d)\n",
...
void
XEPrintTkFlags(FILE *ofp, XETC *tc)
{
    CARD8 f[2];
...
void
XEPrintLastTime(FILE *ofp, XETC *tc)
{
    fprintf(ofp, "\tLast Relative Time:  %d\n", (int)XETrapGetTCTime(tc));
}

void
XEPrintCfgFlags(FILE *ofp, XETrapGetCurRep *pcur)
{
    CARD8 f[4L];
...
void
XEPrintRequests(FILE *ofp, XETrapGetCurRep *pcur)
{
    long i;
...
void
XEPrintEvents(FILE *ofp, XETrapGetCurRep *pcur)
{
    int i;
...
void
XEPrintCurPktSz(FILE *ofp, XETrapGetCurRep *pcur)
{
    fprintf(ofp, "\tMax Packet Size: %d\n", XETrapGetCurMaxPktSize(pcur));
}

void
XEPrintCmdKey(FILE *ofp, XETrapGetCurRep *pcur)
{
    fprintf(ofp, "\tcmd_key: 0x%02x\n", XETrapGetCurCmdKey(pcur));
}

void
XEPrintEvtStats(FILE *ofp, XETrapGetStatsRep *pstats, XETC *tc)
{
    int i;
    fprintf(ofp, "\tX Events:\n");
...
void
XEPrintReqStats(FILE *ofp, XETrapGetStatsRep *pstats, XETC *tc)
{
    int i;
    fprintf(ofp, "\tX Requests:\n");
    for (i = 0L; i < 256L; i++)
...
void
XEPrintAvail(FILE *ofp, XETrapGetAvailRep *pavail)
{
...
void
XEPrintTkState(FILE *ofp, XETC *tc)
{
...
void
XEPrintCurrent(FILE *ofp, XETrapGetCurRep *pcur)
{
...
void
XEPrintStatistics(FILE *ofp, XETrapGetStatsRep *pstats, XETC *tc)
{
