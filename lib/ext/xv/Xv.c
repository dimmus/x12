...
static char *
xv_error_string(Display *dpy, int code, XExtCodes *codes, char *buf, int n);
...
static XEXT_GENERATE_CLOSE_DISPLAY(xv_close_display, xv_info)
    ...
    static XEXT_GENERATE_FIND_DISPLAY(xv_find_display,
                                      xv_info,
                                      xv_extension_name,
                                      &xv_extension_hooks,
                                      XvNumEvents,
        ...
        static XEXT_GENERATE_ERROR_STRING(xv_error_string,
                                          xv_extension_name,
                                          XvNumErrors,
                                          xv_error_list)

            int XvQueryExtension(Display      *dpy,
...
XvGrabPort(Display *dpy, XvPortID port, Time time)
{
    ...
    if (_XReply(dpy, (xReply *)&rep, 0, xTrue) == 0) rep.result = GrabSuccess;
    ...
    *num = 0;
    ...
    if (rep.num_attributes)
    {
            ...
            (rep.text_size < (INT_MAX / 2) - 1))
        {
            ...
            size =
                (rep.num_attributes * sizeof(XvAttribute)) + rep.text_size + 1;
            ret = Xmalloc(size);
        }
