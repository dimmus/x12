...
Bool
XF86DGAQueryExtension(Display *dpy, int *event_basep, int *error_basep)
{
    return XDGAQueryExtension(dpy, event_basep, error_basep);
}

Bool
XF86DGAQueryVersion(Display *dpy, int *majorVersion, int *minorVersion)
{
    return XDGAQueryVersion(dpy, majorVersion, minorVersion);
}

Bool
XF86DGAGetVideoLL(Display      *dpy,
                  int           screen,
                  unsigned int *offset,
                  int          *width,
                  int          *bank_size,
...
Bool
XF86DGAGetViewPortSize(Display *dpy, int screen, int *width, int *height)
{
    XExtDisplayInfo             *info = xdga_find_display(dpy);
...
Bool
XF86DGASetViewPort(Display *dpy, int screen, int x, int y)
{
    XExtDisplayInfo        *info = xdga_find_display(dpy);
...
Bool
XF86DGAGetVidPage(Display *dpy, int screen, int *vpage)
{
    XExtDisplayInfo        *info = xdga_find_display(dpy);
...
Bool
XF86DGASetVidPage(Display *dpy, int screen, int vpage)
{
    XExtDisplayInfo       *info = xdga_find_display(dpy);
...
Bool
XF86DGAInstallColormap(Display *dpy, int screen, Colormap cmap)
{
    XExtDisplayInfo            *info = xdga_find_display(dpy);
...
Bool
XF86DGAViewPortChanged(Display *dpy, int screen, int n)
{
    XExtDisplayInfo             *info = xdga_find_display(dpy);
...
AddMap(void)
{
    ...
    if (!mapList[numMaps]) return NULL;
...
AddScr(void)
{
    ...
    if (!scrList[numScrs]) return NULL;
...
MapPhysAddress(unsigned long address, unsigned long size)
{
    ...
    if (pagesize == -1) pagesize = getpagesize();
#endif
#ifdef PAGE_SIZE
...
#  define MAP_FILE 0
    ...
    if (mapFd < 0)
    {
        if ((mapFd = open(DEV_MEM, O_RDWR)) < 0) return NULL;
    }
    vaddr = (void *)mmap(NULL,
                         size + delta,
                         PROT_READ | PROT_WRITE,
                         MAP_FILE | MAP_SHARED,
                         mapFd,
                         (off_t)offset);
    if (vaddr == (void *)-1) return NULL;

    if (!vaddr)
    {
        if (!(mp = AddMap())) return NULL;
        mp->physaddr = address;
        mp->size     = size;
        mp->delta    = delta;
        mp->vaddr    = vaddr;
...
Bool
XF86DGADirectVideo(Display *dis, int screen, int enable)
{
    ScrPtr sp;
    MapPtr mp = NULL;

    if ((sp = FindScr(dis, screen))) mp = sp->map;
                ...
                int     *ram)
{
    ...
    *addr = MapPhysAddress(offset, *bank);
    ...
    if ((mp = FindMap(offset, *bank))) sp->map = mp;
