...
XEXT_GENERATE_FIND_DISPLAY(xdga_find_display,
                           xdga_info,
                           "XFree86-DGA",
...
Bool
XDGAQueryExtension(Display *dpy, int *event_basep, int *error_basep)
{
    XExtDisplayInfo *info = xdga_find_display(dpy);
...
Bool
XDGAQueryVersion(Display *dpy, int *majorVersion, int *minorVersion)
{
    XExtDisplayInfo       *info = xdga_find_display(dpy);
    ...
    *minorVersion = rep.minorVersion;
    ...
    if (*majorVersion >= 2)
    {
        ...
        for (i = 0, j = info->codes->first_event; i < XF86DGANumberEvents;
             i++, j++)
        {
...
Bool
XDGASetClientVersion(Display *dpy)
{
    XExtDisplayInfo          *info = xdga_find_display(dpy);
...
Bool
XDGAOpenFramebuffer(Display *dpy, int screen)
{
    XExtDisplayInfo          *info = xdga_find_display(dpy);
...
XDGAMode *
XDGAQueryModes(Display *dpy, int screen, int *num)
{
...
XDGADevice *
XDGASetMode(Display *dpy, int screen, int mode)
{
    XExtDisplayInfo  *dinfo = xdga_find_display(dpy);
    ...
    if (_XReply(dpy, (xReply *)&rep, 0, xFalse))
    {
        if (rep.length)
        {
            ...
            if (dev)
            {
                ...
                if (dev->data) dev->data += rep.offset;
            }
...
void
XDGASetViewport(Display *dpy, int screen, int x, int y, int flags)
{
    XExtDisplayInfo     *info = xdga_find_display(dpy);
...
void
XDGAInstallColormap(Display *dpy, int screen, Colormap cmap)
{
    XExtDisplayInfo         *info = xdga_find_display(dpy);
...
void
XDGASelectInput(Display *dpy, int screen, long mask)
{
    XExtDisplayInfo     *info = xdga_find_display(dpy);
...
void
XDGACopyTransparentArea(Display      *dpy,
                        int           screen,
                        int           srcx,
                        int           srcy,
                        unsigned int  width,
...
int
XDGAGetViewportStatus(Display *dpy, int screen)
{
    XExtDisplayInfo            *info = xdga_find_display(dpy);
    ...
    req->screen     = screen;
    if (!_XReply(dpy, (xReply *)&rep, 0, xFalse)) status = rep.status;
    UnlockDisplay(dpy);
    SyncHandle();
    return status;
}
...
void
XDGASync(Display *dpy, int screen)
{
    XExtDisplayInfo *info = xdga_find_display(dpy);
    xXDGASyncReply   rep;
    xXDGASyncReq    *req;
...
void
XDGAChangePixmapMode(Display *dpy, int screen, int *x, int *y, int mode)
{
    XExtDisplayInfo           *info = xdga_find_display(dpy);
...
Colormap
XDGACreateColormap(Display *dpy, int screen, XDGADevice *dev, int alloc)
{
    XExtDisplayInfo        *info = xdga_find_display(dpy);
...
static Bool DGAMapPhysical(int,
                           const char *,
                           unsigned char *,
                           CARD32,
                           CARD32,
...
)
{
    ...
    while (pMap != NULL)
    {
        if (pMap->screen == screen) return True;
        pMap = pMap->next;
    }
...
XDGAUnmapFramebuffer(int screen)
{
    ...
    if (!pPrev) _Maps = pMap->next;
    else pPrev->next = pMap->next;

    Xfree(pMap);
}

static Bool
DGAMapPhysical(int            screen,
...
#  define MAP_FILE 0
#endif
    if (!name) name = DEV_MEM;
    if ((pMap->fd = open(name, O_RDWR)) < 0) return False;
    pMap->virtual = mmap(NULL,
                         size,
                         PROT_READ | PROT_WRITE,
                         MAP_FILE | MAP_SHARED,
                         pMap->fd,
                         (off_t)(uintptr_t)base);
    if (pMap->virtual == (void *)-1) return False;
