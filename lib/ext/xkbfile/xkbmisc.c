...
_XkbKSCheckCase(KeySym ks)
{
    ...
    switch (set)
    {
        case 0:                    /* latin 1 */
            ...
            break;
        case 1:                    /* latin 2 */
            if (((ks >= XK_Aogonek) && (ks <= XK_Zabovedot) &&
                 (ks != XK_breve)) ||
        ...
        case 2:                    /* latin 3 */
            ...
            break;
        case 3:                    /* latin 4 */
            if (((ks >= XK_Rcedilla) && (ks <= XK_Tslash)) || (ks == XK_ENG) ||
        ...
        case 18:                   /* latin 8 */
            if ((ks == XK_Babovedot) ||
                ...
                (ks == XK_Mabovedot) || (ks == XK_Pabovedot) ||
                (ks == XK_Sabovedot) || (ks == XK_Wdiaeresis) ||
                ((ks >= XK_Wcircumflex) && (ks <= XK_Ycircumflex)))
            {
                rtrn |= _XkbKSUpper;
            }
            if ((ks == XK_babovedot) || (ks == XK_dabovedot) ||
                (ks == XK_fabovedot) || (ks == XK_mabovedot) ||
                ((ks >= XK_wgrave) && (ks <= XK_wacute)) || (ks == XK_ygrave) ||
                       ...
                       int       *lvl_rtrn)
{
    ...
    if ((!xkb) || (!XkbKeycodeInRange(xkb, key)) || (!grp_inout)) return False;
    ...
    if (mods_inout != NULL)
    {
        ...
        {        /* find the shift level */
            ...
            for (i = 0, entry = type->map; i < type->map_count; i++, entry++)
            {
                ...
                {
                    if (lvl_rtrn != NULL) *lvl_rtrn = entry->level;
                    if (type->preserve) preserve = type->preserve[i].mask;
                    break;
                }
            }
        }
                          ...
                          unsigned             need)
{
    ...
    want |= (complete | need);
    if (want & XkmSymbolsMask) want |= XkmKeyNamesMask | XkmTypesMask;
    ...
    if (xkb != NULL)
    {
        ...
        finfo.defined = 0;
        finfo.xkb     = xkb;
        if (!XkbDetermineFileType(&finfo, XkbXKBFile, NULL)) return False;
    }
    else old_names = NULL;
    ...
    if (xkb != NULL)
    {
        ...
        {
            if (!xkb->indicators) wantConfig &= ~XkmIndicatorsMask;
        }
    ...
    if ((xkb != NULL) && (old_names != NULL))
    {
        ...
        {
            if (old_names->symbols == None) return False;
        ...
        {
            if (old_names->geometry == None) return False;
...
XkbConvertXkbComponents(Bool toXkm, unsigned orig)
{
    ...
    else
    {
        ...
        if (orig & XkmKeyNamesMask) rtrn |= XkbNamesMask | XkbIndicatorMapMask;
...
XkbDetermineFileType(XkbFileInfoPtr finfo, int format, int *opts_missing)
{
    ...
    if ((!finfo) || (!finfo->xkb)) return False;
    if (opts_missing) *opts_missing = 0;
    xkb     = finfo->xkb;
    present = 0;
    ...
    if ((present & (~XkmSemanticsLegal)) == 0)
    {
        ...
        {
            if (opts_missing) *opts_missing = XkmSemanticsOptional & (~present);
    ...
    else if ((present & (~XkmLayoutLegal)) == 0)
    {
        ...
        {
            if (opts_missing) *opts_missing = XkmLayoutOptional & (~present);
    ...
    else if ((present & (~XkmKeymapLegal)) == 0)
    {
        ...
        {
            if (opts_missing) *opts_missing = XkmKeymapOptional & (~present);
...
static unsigned char componentSpecLegal[] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0xa7, 0xff, 0x83, 0xfe, 0xff, 0xff,
    0x87, 0xfe, 0xff, 0xff, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0xff, 0xff, 0x7f, 0xff, 0xff, 0xff, 0x7f, 0xff
};
...
XkbNameMatchesPattern(char *name, char *ptrn)
{
    while (ptrn[0] != '\0')
    {
        ...
        if (ptrn[0] == '?')
        {
            if (UNMATCHABLE(name[0])) return False;
        }
        else if (ptrn[0] == '*')
        {
        ...
        else if (ptrn[0] != name[0]) return False;
        name++;
        ptrn++;
    }
