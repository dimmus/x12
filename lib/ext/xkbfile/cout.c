                  ...
                  XkbKeyTypePtr type)
{
    ...
    fprintf(file,
            "static XkbModsRec preserve_%s[%d]= {\n",
            prefix,
            type->map_count);
...
WriteTypeInitFunc(FILE *file, Display *dpy, XkbDescPtr xkb)
{
    ...
    for (i = 0, type = xkb->map->types; i < xkb->map->num_types; i++, type++)
    {
        ...
        if (type->name != None)
            fprintf(file,
                    "    dflt_types[%d].name= GET_ATOM(dpy,\"%s\");\n",
                    i,
...
WriteCHdrKeyTypes(FILE *file, Display *dpy, XkbDescPtr xkb)
{
    ...
    for (i = 0, type = map->types; i < map->num_types; i++, type++)
    {
        ...
        if (type->map_count > 0)
        {
            ...
            entry = type->map;
            fprintf(file,
                    "static XkbKTMapEntryRec map_%s[%d]= {\n",
                    prefix,
                    type->map_count);
            ...
            if (type->preserve) WriteTypePreserve(file, dpy, prefix, xkb, type);
        }
        if (type->level_names != NULL)
        {
            fprintf(file,
                    "static Atom lnames_%s[%d];\n",
                    prefix,
                    type->num_levels);
        }
        fprintf(file, "\n");
    ...
    for (i = 0, type = map->types; i < (unsigned)map->num_types; i++, type++)
    {
        ...
        if (type->map_count > 0) fprintf(file, "	map_%s,", prefix);
        else fprintf(file, "	NULL,");
        if (type->preserve) fprintf(file, "	preserve_%s,\n", prefix);
        else fprintf(file, "	NULL,\n");
        if (type->level_names != NULL)
...
WriteCHdrClientMap(FILE *file, Display *dpy, XkbDescPtr xkb)
{
    ...
    if (!WriteCHdrKeyTypes(file, dpy, xkb)) return False;
    if (!WriteCHdrSymbols(file, xkb)) return False;
...
WriteCHdrIndicators(FILE *file, Display *dpy, XkbDescPtr xkb)
{
    ...
    if (xkb->indicators == NULL) return True;
         ...
         i++, imap++)
    {
        ...
        if (xkb->names && (xkb->names->indicators[i] != None)) nNames++;
    }
    fprintf(file, "\n    }\n};\n");
    if (nNames > 0)
    {
        ...
        for (i = 0; i < XkbNumIndicators; i++)
        {
            ...
            if (xkb->names->indicators[i] == None) continue;
                     ...
                     int            section_num)
{
    ...
    if (section == NULL)
    {
        ...
        {
            fprintf(file,
                    "static XkbDoodadRec g_doodads[%d];\n",
                    geom->num_doodads);
        }
        fprintf(file, "static void\n");
    ...
    else
    {
        ...
        {
            fprintf(file,
                    "static XkbDoodadRec doodads_s%02d[%d];\n",
                    section_num,
                    section->num_doodads);
        }
        fprintf(file, "static void\n");
    ...
    for (d = 0; d < nd; d++, doodad++)
    {
        ...
        switch (doodad->any.type)
        {
            ...
            case XkbSolidDoodad:
                fprintf(file,
                        "    doodads[%d].shape.color_ndx= %d;\n",
                        d,
                        doodad->shape.color_ndx);
                fprintf(file,
                        "    doodads[%d].shape.shape_ndx= %d;\n",
                        d,
                        doodad->shape.shape_ndx);
                break;
            case XkbTextDoodad:
                fprintf(file,
                        "    doodads[%d].text.width= %d;\n",
                        d,
                        doodad->text.width);
                fprintf(file,
                        "    doodads[%d].text.height= %d;\n",
                        d,
                        doodad->text.height);
                fprintf(file,
                        "    doodads[%d].text.color_ndx= %d;\n",
                        d,
                        doodad->text.color_ndx);
                fprintf(file,
                        "    doodads[%d].text.text= \"%s\";\n",
                        d,
                        XkbStringText(doodad->text.text, XkbCFile));
                fprintf(file,
                        "    doodads[%d].text.font= \"%s\";\n",
                        d,
                        XkbStringText(doodad->text.font, XkbCFile));
                break;
            case XkbIndicatorDoodad:
                fprintf(file,
                        "    doodads[%d].indicator.shape_ndx= %d;\n",
                        d,
                        doodad->indicator.shape_ndx);
                fprintf(file,
                        "    doodads[%d].indicator.on_color_ndx= %d;\n",
                        d,
                        doodad->indicator.on_color_ndx);
                fprintf(file,
                        "    doodads[%d].indicator.off_color_ndx= %d;\n",
                        d,
                        doodad->indicator.off_color_ndx);
                break;
            case XkbLogoDoodad:
                fprintf(file,
                        "    doodads[%d].logo.color_ndx= %d;\n",
                        d,
                        doodad->logo.color_ndx);
                fprintf(file,
                        "    doodads[%d].logo.shape_ndx= %d;\n",
                        d,
                        doodad->logo.shape_ndx);
                fprintf(file,
                        "    doodads[%d].logo.logo_name= \"%s\";\n",
                        d,
                      ...
                      int           section_num)
{
    ...
    if (section->num_overlays < 1) return True;
    for (o = 0, ol = section->overlays; o < section->num_overlays; o++, ol++)
    {
        for (r = 0, row = ol->rows; r < ol->num_rows; r++, row++)
        {
            fprintf(file,
                    "static XkbOverlayKeyRec olkeys_s%02dr%02d[%d]= {\n",
                    section_num,
                    r,
                    row->num_keys);
        ...
        fprintf(file,
                "static XkbOverlayRowRec olrows_s%02d[%d]= {\n",
                section_num,
                section->num_rows);
        for (r = 0, row = ol->rows; r < ol->num_rows; r++, row++)
        {
            fprintf(file,
                    "%s	{ %4d, %4d, %4d, olkeys_s%02dr%02d }",
                    (r == 0 ? "" : ",\n"),
                    row->row_under,
                    row->num_keys,
                    row->num_keys,
    ...
    fprintf(file,
            "static XkbOverlayRec overlays_s%02d[%d]= {\n",
            section_num,
                  ...
                  int           section_num)
{
    ...
    {
        fprintf(file,
                "static XkbKeyRec keys_s%02dr%02d[]= {\n",
                section_num,
                r);
        for (k = 0, key = row->keys; k < row->num_keys; k++, key++)
        {
    ...
    {
        fprintf(file,
                "%s	{ %4d, %4d, %2d, %2d, %1d, keys_s%02dr%02d, ",
                (r == 0 ? "" : ",\n"),
                row->top,
                row->left,
                row->num_keys,
...
WriteCHdrGeomSections(FILE *file, XkbDescPtr xkb, XkbGeometryPtr geom)
{
         ...
         s++, section++)
    {
        ...
        {
            fprintf(file,
                    "    _InitS%02dDoodads(dpy,geom,&sections[%d]);\n",
                    s,
                    s);
        }
        if (section->num_overlays > 0)
        {
            fprintf(file,
                    "    _InitS%02dOverlays(dpy,geom,&sections[%d]);\n",
                    s,
                    s);
        }
    }
...
XkbWriteCFile(FILE *out, char *name, XkbFileInfo *result)
{
    ...
    Bool (*func)(FILE * /* file */, XkbFileInfo * /* result */
    );
