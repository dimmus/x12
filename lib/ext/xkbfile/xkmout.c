...
xkmPutCountedBytes(FILE *file, char *ptr, unsigned count)
{
    ...
    if (count == 0) return xkmPutCARD32(file, (unsigned long)0);
...
{
    if (str == NULL) return xkmPutCARD32(file, (unsigned long)0);
...
SizeXKMKeyTypes(XkbFileInfo *result, xkmSectionInfo *toc, int *offset_inout)
{
    ...
    for (i = 0, type = xkb->map->types; i < xkb->map->num_types; i++, type++)
    {
        ...
        if (type->preserve) size += SIZEOF(xkmModsDesc) * type->map_count;
        if (type->level_names)
        {
            Atom *names;
...
WriteXKMKeyTypes(FILE *file, XkbFileInfo *result)
{
    ...
    for (i = 0; i < xkb->map->num_types; i++, type++)
    {
        ...
        if (type->level_names != NULL) wire.nLevelNames = type->num_levels;
            ...
            if (xkb->server->explicit[i] & XkbExplicitKeyTypesMask)
            {
                ...
                for (g = XkbKeyNumGroups(xkb, i) - 1; g >= 0; g--)
                {
                    if (xkb->server->explicit[i] & (1 << g))
                    {
                        ...
                        if (name != NULL) size += xkmSizeCountedString(name);
                    }
                }
            }
            if (XkbKeyHasActions(xkb, i)) size += nSyms * SIZEOF(xkmActionDesc);
...
WriteXKMSymbols(FILE *file, XkbFileInfo *result, XkmInfo *info)
{
    ...
    {
        if (xkb->names->groups[i] != None) tmp |= (1 << i);
    }
    size += xkmPutCARD8(file, xkb->min_key_code);
    size += xkmPutCARD8(file, xkb->max_key_code);
            ...
            if (xkb->server->explicit[i] & XkbExplicitKeyTypesMask)
            {
                ...
                for (g = 0; g < XkbKeyNumGroups(xkb, i); g++)
                {
                    if (xkb->server->explicit[i] & (1 << g))
                    {
                        ...
                        if (typeName[g] != NULL) wireMap.flags |= (1 << g);
                    }
                }
            }
            if (XkbKeyHasActions(xkb, i)) wireMap.flags |= XkmKeyHasActions;
            if (xkb->server->behaviors[i].type != XkbKB_Default)
...
static unsigned
SizeXKMIndicators(XkbFileInfo    *result,
                  XkmInfo        *info,
                  xkmSectionInfo *toc,
                  int            *offset_inout)
{
...
WriteXKMGeomDoodad(FILE *file, XkbFileInfo *result, XkbDoodadPtr doodad)
{
    ...
    switch (doodad->any.type)
    {
        ...
        case XkbLogoDoodad:
            ...
            break;
        default:
            _XkbLibError(_XkbErrIllegalDoodad,
                         "WriteXKMGeomDoodad",
                         doodad->any.type);
            return 0;
    }
                  ...
                  xkmSectionInfo *toc_rtrn)
{
    ...
    if (SizeXKMKeycodes(result, &toc_rtrn[num_toc], &total_size)) num_toc++;
               ...
               xkmSectionInfo *toc_rtrn)
{
    ...
    if (SizeXKMVirtualMods(result, info, &toc_rtrn[num_toc], &total_size))
        num_toc++;
    if (SizeXKMKeyTypes(result, &toc_rtrn[num_toc], &total_size)) num_toc++;
    return num_toc;
}
                ...
                xkmSectionInfo *toc_rtrn)
{
    ...
    if (SizeXKMIndicators(result, info, &toc_rtrn[num_toc], &total_size))
        num_toc++;
    if (SizeXKMGeometry(result, &toc_rtrn[num_toc], &total_size)) num_toc++;
    return num_toc;
}
    ...
    for (i = 0; i < num_toc; i++)
    {
        ...
        switch (toc[i].type)
        {
            ...
            case XkmVirtualModsIndex:
                ...
                break;
            default:
                _XkbLibError(_XkbErrIllegalTOCType,
                             "WriteXKMFile",
                             toc[i].type);
                return False;
        }
