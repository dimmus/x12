...
{
    if (line->line != line->buf) _XkbFree(line->line);
    line->line_num = 1;
    line->num_line = 0;
    line->sz_line  = DFLT_LINE_SIZE;
    line->line     = line->buf;
...
#define ADD_CHAR(l, c)                                                      \
    ((l)->num_line < (l)->sz_line ? (int)((l)->line[(l)->num_line++] = (c)) \
    ...
    while ((!endOfFile) && (line->num_line == 0))
    {
        ...
        while (((ch = getc(file)) != '\n') && (ch != EOF))
        {
            ...
            if (isspace(ch))
            {
                ...
                if (ch == EOF) break;
                if ((ch != '\n') && (line->num_line > 0)) spacePending = True;
                ungetc(ch, file);
            }
            else
            {
...
/*	else line->num_line++;*/
    ...
    funlockfile(file);
    if ((line->num_line == 0) && (endOfFile)) return False;
    ADD_CHAR(line, '\0');
    return True;
}
...
static const char *cname[MAX_WORDS] = { "model",  "layout",   "variant",
                                        "option", "keycodes", "symbols",
    ...
    while ((tok = _XStrtok(str, " ", strtok_buf)) != NULL)
    {
        ...
        for (i = 0; i < MAX_WORDS; i++)
        {
            ...
            if (strncmp(cname[i], tok, len) == 0)
            {
                ...
                present |= (1 << i);
                if (i == LAYOUT) l_ndx_present |= 1 << ndx;
                if (i == VARIANT) v_ndx_present |= 1 << ndx;
...
MatchOneOf(char *wanted, char *vals_defined)
{
    ...
    for (str = vals_defined, next = NULL; str != NULL; str = next)
    {
        ...
        if ((len == want_len) && (strncmp(wanted, str, len) == 0)) return True;
    }
    return False;
}
          ...
          XkbRF_GroupPtr group)
{
    ...
    if (tmp.name[OPTION]) rule->flags |= XkbRF_Option;
    else if (append) rule->flags |= XkbRF_Append;
    else rule->flags |= XkbRF_Normal;
...
MakeMultiDefs(XkbRF_MultiDefsPtr mdefs, XkbRF_VarDefsPtr defs)
{
    ...
    if (defs->layout)
    {
        ...
        else
        {
            ...
            if (mdefs->layout[1] == NULL) return False;
    ...
    if (defs->variant)
    {
        ...
        else
        {
            ...
            if (mdefs->variant[1] == NULL) return False;
...
Apply(char *src, char **dst)
{
    if (src)
    {
        ...
        {
            if (*dst == NULL) *dst = _XkbDupString(src);
        }
    }
}
...
{
    rule->flags &=
        ~XkbRF_PendingMatch; /* clear the flag because it's applied */
...
CheckGroup(XkbRF_RulesPtr rules, char *group_name, char *name)
{
    ...
    if (i == rules->num_groups) return False;
                     ...
                     XkbRF_RulesPtr       rules)
{
    ...
    if (rule->model != NULL)
    {
        ...
        else
        {
            if (rule->model[0] == '$')
            {
                if (!CheckGroup(rules, rule->model, mdefs->model)) return 0;
            }
            else
            {
                if (strcmp(rule->model, mdefs->model) != 0) return 0;
            }
        }
    }
    if (rule->option != NULL)
...
XkbRF_ApplyPartialMatches(XkbRF_RulesPtr rules, XkbComponentNamesPtr names)
{
    ...
    {
        if ((rule->flags & XkbRF_PendingMatch) == 0) continue;
        XkbRF_ApplyRule(rule, names);
    }
}
                      ...
                      int                  flags)
{
    ...
    {
        if ((rule->flags & flags) != flags) continue;
                    ...
                    XkbComponentNamesPtr names)
{
    ...
    if (names->types) names->types = XkbRF_SubstituteVars(names->types, &mdefs);
    if (names->compat)
...
XkbRF_AddRule(XkbRF_RulesPtr rules)
{
    ...
    else if (rules->num_rules >= rules->sz_rules)
    {
        rules->sz_rules *= 2;
        rules->rules =
            _XkbTypedReallocF(rules->rules, rules->sz_rules, XkbRF_RuleRec);
    }
    if (!rules->rules)
    {
...
XkbRF_AddGroup(XkbRF_RulesPtr rules)
{
    ...
    else if (rules->num_groups >= rules->sz_groups)
    {
        rules->sz_groups *= 2;
        rules->groups =
            _XkbTypedReallocF(rules->groups, rules->sz_groups, XkbRF_GroupRec);
    }
    if (!rules->groups)
    {
...
XkbRF_LoadRulesByName(char *base, char *locale, XkbRF_RulesPtr rules)
{
    ...
    if (locale)
    {
        if (strlen(base) + strlen(locale) + 2 > PATH_MAX) return False;
        snprintf(buf, sizeof(buf), "%s-%s", base, locale);
    }
    else
    {
        if (strlen(base) + 1 > PATH_MAX) return False;
        strcpy(buf, base);
    }
...
XkbRF_AddVarToDescribe(XkbRF_RulesPtr rules, char *name)
{
    ...
    else if (rules->num_extra >= rules->sz_extra)
    {
        ...
        rules->extra = _XkbTypedReallocF(rules->extra,
                                         rules->sz_extra,
...
XkbRF_LoadDescriptions(FILE *file, XkbRF_RulesPtr rules)
{
    ...
    for (; GetInputLine(file, &line, False); line.num_line = 0)
    {
        if (line.line[0] == '!')
        {
            ...
            if (strcmp(tok, "model") == 0) headingtype = HEAD_MODEL;
            ...
            else
            {
                ...
                if (extra_ndx < 0)
                {
                    ...
                    if (var) extra_ndx = var - rules->extra;
...
XkbRF_LoadDescriptionsByName(char *base, char *locale, XkbRF_RulesPtr rules)
{
    ...
    if (locale)
    {
        if (strlen(base) + strlen(locale) + 6 > PATH_MAX) return False;
        snprintf(buf, sizeof(buf), "%s-%s.lst", base, locale);
    }
    else
    {
        if (strlen(base) + 5 > PATH_MAX) return False;
...
XkbRF_Load(char *base, char *locale, Bool wantDesc, Bool wantRules)
{
    ...
    if ((!base) || ((!wantDesc) && (!wantRules))) return NULL;
    if ((rules = _XkbTypedCalloc(1, XkbRF_RulesRec)) == NULL) return NULL;
...
XkbRF_Create(int szRules, int szExtra)
{
    ...
    if (szExtra > 0)
    {
        ...
        if (!rules->extra)
        {
            if (rules->rules) _XkbFree(rules->rules);
            _XkbFree(rules);
            return NULL;
        }
    }
...
XkbRF_ClearVarDescriptions(XkbRF_DescribeVarsPtr var)
{
    ...
    {
        if (var->desc[i].name) _XkbFree(var->desc[i].name);
        if (var->desc[i].desc) _XkbFree(var->desc[i].desc);
        var->desc[i].name = var->desc[i].desc = NULL;
    }
    if (var->desc) _XkbFree(var->desc);
    var->desc = NULL;
    return;
}
...
XkbRF_GetNamesProp(Display *dpy, char **rf_rtrn, XkbRF_VarDefsPtr vd_rtrn)
{
    ...
    rtrn = XGetWindowProperty(dpy,
                              ...
                              &bytes_after,
                              &data);
    if (rtrn != Success) return False;
    if (rf_rtrn) *rf_rtrn = NULL;
    ...
    if (out && (*out) && rf_rtrn) *rf_rtrn = _XkbDupString(out);
    out += strlen(out) + 1;

    if (out < end)
    {
        if (*out) vd_rtrn->model = _XkbDupString(out);
        out += strlen(out) + 1;
    }

    if (out < end)
    {
        if (*out) vd_rtrn->layout = _XkbDupString(out);
        out += strlen(out) + 1;
    }

    if (out < end)
    {
        if (*out) vd_rtrn->variant = _XkbDupString(out);
        out += strlen(out) + 1;
    }

    if (out < end)
    {
        if (*out) vd_rtrn->options = _XkbDupString(out);
        out += strlen(out) + 1;
    }
    /*
...
XkbRF_FreeVarDefs(XkbRF_VarDefsPtr var_defs, Bool freeVarDefs)
{
    ...
    if (freeVarDefs) _XkbFree(var_defs);
}
