...
#define XkmInsureTypedSize(p, o, n, t) \
    ((p) = ((t *)XkmInsureSize((char *)(p), (o), (n), sizeof(t))))
...
XkmGetCARD16(FILE *file, int *pNRead)
{
    ...
    if ((fread(&val, 2, 1, file) == 1) && (pNRead)) (*pNRead) += 2;
    return val;
}
...
XkmGetCARD32(FILE *file, int *pNRead)
{
    ...
    if ((fread(&val, 4, 1, file) == 1) && (pNRead)) (*pNRead) += 4;
    return val;
}
...
XkmGetCountedString(FILE *file, char *str, int max_len)
{
    ...
    if (count > 0) nRead += XkmSkipPadding(file, count);
    return nRead;
}
...
ReadXkmVirtualMods(FILE *file, XkbFileInfo *result, XkbChangesPtr changes)
{
    ...
    for (i = tmp = 0, bit = 1; i < XkbNumVirtualMods; i++, bit <<= 1)
    {
        if (bound & bit)
        {
            ...
            if (changes) changes->map.vmods |= bit;
            tmp++;
        }
    }
    if ((i = XkbPaddedSize(tmp) - tmp) > 0) nRead += XkmSkipPadding(file, i);
    ...
    for (i = 0, bit = 1; i < XkbNumVirtualMods; i++, bit <<= 1)
    {
        ...
        if (named & bit)
        {
            if (nRead += XkmGetCountedString(file, name, 100))
            {
                ...
                if (changes) changes->names.changed_vmods |= bit;
            }
        }
    }
    return nRead;
...
ReadXkmKeycodes(FILE *file, XkbFileInfo *result, XkbChangesPtr changes)
{
    ...
    {
        if (minKC < xkb->min_key_code) xkb->min_key_code = minKC;
...
#define WANTED (XkbKeycodesNameMask | XkbKeyNamesMask | XkbKeyAliasesMask)
    ...
    if (nAl > 0)
    {
        ...
        if (changes) changes->names.changed |= XkbKeyAliasesMask;
    }
    if (changes) changes->names.changed |= XkbKeyNamesMask;
    return nRead;
}
...
ReadXkmKeyTypes(FILE *file, XkbFileInfo *result, XkbChangesPtr changes)
{
    ...
    for (i = 0; i < num_types; i++, type++)
    {
        ...
        if (wire.preserve)
        {
            ...
            XkmInsureTypedSize(type->preserve,
                               type->map_count,
                               &tmp,
                               XkbModsRec);
        ...
        if (wire.nLevelNames > 0)
        {
            ...
            XkmInsureTypedSize(type->level_names,
                               type->num_levels,
                               &width,
                               Atom);
            if (type->level_names != NULL)
            {
                for (n = 0; n < wire.nLevelNames; n++)
                {
                    if ((tmp = XkmGetCountedString(file, buf, 100)) < 1)
                        return -1;
                    nRead += tmp;
                    if (strlen(buf) == 0) type->level_names[n] = None;
...
ReadXkmCompatMap(FILE *file, XkbFileInfo *result, XkbChangesPtr changes)
{
    ...
    nRead += XkmSkipPadding(file, 1);
    if (XkbAllocCompatMap(xkb, XkbAllCompatMask, num_si) != Success) return -1;
    compat         = xkb->compat;
    ...
    if (groups)
    {
        ...
        if (changes) changes->compat.changed_groups |= groups;
    }
    return nRead;
}
...
ReadXkmIndicators(FILE *file, XkbFileInfo *result, XkbChangesPtr changes)
{
    ...
    while (nLEDs-- > 0)
    {
        ...
        nRead += tmp;
        if (buf[0] != '\0') name = XkbInternAtom(xkb->dpy, buf, False);
        else name = None;
...
ReadXkmSymbols(FILE *file, XkbFileInfo *result)
{
    ...
    xkb = result->xkb;
    if ((tmp = XkmGetCountedString(file, buf, 100)) < 1) return -1;
    nRead += tmp;
    minKC         = XkmGetCARD8(file, &nRead);
    maxKC         = XkmGetCARD8(file, &nRead);
    ...
    if ((xkb->map == NULL) || (xkb->server == NULL)) return -1;
    if (xkb->min_key_code < 8) xkb->min_key_code = minKC;
    if (xkb->max_key_code < 8) xkb->max_key_code = maxKC;
    if ((minKC >= 8) && (minKC < xkb->min_key_code)) xkb->min_key_code = minKC;
    ...
    if (totalVModMaps > 0)
    {
        ...
        for (i = 0; i < totalVModMaps; i++)
        {
            ...
            if (tmp > 0) xkb->server->vmodmap[v.key] = v.vmods;
        }
    }
    return nRead;
}
    ...
    switch (doodadWire.any.type)
    {
        ...
        case XkbIndicatorDoodad:
            ...
            doodad->indicator.off_color_ndx =
                doodadWire.indicator.off_color_ndx;
            break;
        case XkbLogoDoodad:
                   ...
                   XkbSectionPtr  section)
{
    ...
    ol = XkbAddGeomOverlay(section,
                           XkbInternAtom(dpy, buf, False),
                           olWire.num_rows);
    if (!ol) return nRead;
...
#ifdef DEBUG
        fprintf(stderr,
                "Warning! Too many TOC entries; last %d ignored\n",
                size_toc - max_toc);
#endif
        size_toc = max_toc;
    }
...
XkmFindTOCEntry(xkmFileInfo *finfo, xkmSectionInfo *toc, unsigned type)
{
    ...
    {
        if (toc[i].type == type) return &toc[i];
    }
    return NULL;
}
                   ...
                   unsigned       *loaded_rtrn)
{
    ...
    switch (tmpTOC.type)
    {
        ...
        case XkmTypesIndex:
            ...
            if ((loaded_rtrn) && (nRead >= 0)) *loaded_rtrn |= XkmTypesMask;
            break;
        case XkmCompatMapIndex:
            nRead += ReadXkmCompatMap(file, result, NULL);
            if ((loaded_rtrn) && (nRead >= 0)) *loaded_rtrn |= XkmCompatMapMask;
            break;
        case XkmKeyNamesIndex:
            nRead += ReadXkmKeycodes(file, result, NULL);
            if ((loaded_rtrn) && (nRead >= 0)) *loaded_rtrn |= XkmKeyNamesMask;
            break;
        case XkmSymbolsIndex:
            nRead += ReadXkmSymbols(file, result);
            if ((loaded_rtrn) && (nRead >= 0)) *loaded_rtrn |= XkmSymbolsMask;
            break;
        ...
        case XkmGeometryIndex:
            ...
            if ((loaded_rtrn) && (nRead >= 0)) *loaded_rtrn |= XkmGeometryMask;
            break;
        default:
...
XkmReadFileSectionName(FILE *file, xkmSectionInfo *toc)
{
    ...
    switch (toc->type)
    {
        ...
        default:
            _XkbLibError(_XkbErrBadImplementation,
                         XkbConfigText(toc->type, XkbMessage),
                         0);
            break;
    }
    return NULL;
...
XkmReadFile(FILE *file, unsigned need, unsigned want, XkbFileInfo *result)
{
    ...
    if (!XkmReadTOC(file, &fileInfo, MAX_TOC, toc)) return which;
    ...
    if (result->xkb == NULL) result->xkb = XkbAllocKeyboard();
