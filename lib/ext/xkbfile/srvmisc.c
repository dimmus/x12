...
XkbWriteToServer(XkbFileInfo *result)
{
    ...
    xkb = result->xkb;
    dpy = xkb->dpy;
    if (!XkbSetMap(dpy, XkbAllMapComponentsMask, xkb)) return False;
    if (!XkbSetIndicatorMap(dpy, ~0, xkb)) return False;
    if (!XkbSetCompatMap(dpy, XkbAllCompatMask, xkb, True)) return False;
                  ...
                  XkbFileInfo *result)
{
    ...
    if ((result == NULL) || (dpy == NULL)) return which;
...
XkbChangeKbdDisplay(Display *newDpy, XkbFileInfo *result)
{
    ...
    if ((result->xkb == NULL) || (result->xkb->dpy == newDpy)) return Success;
    xkb    = result->xkb;
    oldDpy = xkb->dpy;
    if (xkb->names)
    {
        ...
        names->phys_symbols =
            XkbChangeAtomDisplay(oldDpy, newDpy, names->phys_symbols);
                 ...
                 s++, section++)
            {
                ...
                if (section->doodads)
                {
                    ...
                    for (d = 0, doodad = section->doodads;
                         d < section->num_doodads;
                         d++, doodad++)
                    {
                        doodad->any.name =
                            XkbChangeAtomDisplay(oldDpy,
                                                 newDpy,
                                                 doodad->any.name);
                    }
                }
