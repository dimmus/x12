...
tbGetBuffer(unsigned size)
{
    ...
    if ((BUFFER_SIZE - tbNext) <= size) tbNext = 0;
...
tbGetBufferString(const char *str)
{
    ...
    if (rtrn != NULL) memcpy(rtrn, str, size);
...
XkbVModIndexText(Display *dpy, XkbDescPtr xkb, unsigned ndx, unsigned format)
{
    ...
    if (xkb && xkb->names) vmodNames = xkb->names->vmods;
    else vmodNames = NULL;

    tmp = NULL;
    if (ndx >= XkbNumVirtualMods) tmp = strdup("illegal");
                ...
                unsigned   format)
{
    ...
    if ((modMask == 0) && (mask == 0))
    {
        ...
        if (format == XkbCFile) snprintf(rtrn, rtrnsize, "0");
        else snprintf(rtrn, rtrnsize, "none");
        return rtrn;
    }
    if (modMask != 0) mm = XkbModMaskText(modMask, format);
    else mm = NULL;
    ...
    if (str) len += strlen(str) + (mm == NULL ? 0 : 1);
    if (len >= BUFFER_SIZE) len = BUFFER_SIZE - 1;
...
static const char *modNames[XkbNumModifiers] = { "Shift", "Lock", "Control",
                                                 "Mod1",  "Mod2", "Mod3",
                                                 "Mod4",  "Mod5" };
...
XkbModIndexText(unsigned ndx, unsigned format)
{
    ...
    {
        if (ndx < XkbNumModifiers) strcpy(buf, modNames[ndx]);
        else if (ndx == XkbNoModifier) strcpy(buf, "none");
...
XkbKeysymText(KeySym sym, unsigned format)
{
    ...
    if (sym == NoSymbol) strcpy(rtrn = buf, "NoSymbol");
...
static char *siMatchText[5] = { "NoneOf",
                                "AnyOfOrNone",
                                "AnyOf",
                                "AllOf",
                                "Exactly" };
...
static const char *imWhichNames[] = { "base",
                                      "latched",
                                      "locked",
                                      "effective",
                                      "compat" };
...
static const char *nknNames[] = { "keycodes", "geometry", "deviceID" };
...
XkbNKNDetailMaskText(unsigned detail, unsigned format)
{
    ...
    else
    {
        prefix = "XkbNKN_";
        plen   = 7;
        if (format == XkbCFile) suffix = "Mask";
        else suffix = "";
        slen = strlen(suffix);
    }
...
static const char *ctrlNames[] = {
    ...
    "mouseKeys",       "mouseKeysAccel", "accessXKeys", "accessXTimeout",
...
XkbStringText(char *str, unsigned format)
{
    ...
    else if (format == XkbXKMFile) return str;
...
XkbGeomFPText(int val, unsigned format)
{
    ...
    else
    {
        ...
        if (frac != 0)
        {
            ...
            else snprintf(buf, bufsize, "%d.%d", whole, frac);
        }
        else snprintf(buf, bufsize, "%d", whole);
    }
    return buf;
}
...
XkbDoodadTypeText(unsigned type, unsigned format)
{
    ...
    if (format == XkbCFile)
    {
        ...
        else snprintf(buf, bufsize, "UnknownDoodad%d", type);
    }
    else
    {
        ...
        else snprintf(buf, bufsize, "unknown%d", type);
    }
    return buf;
}

static char *actionTypeNames[XkbSA_NumActions] = {
    "NoAction",      "SetMods",      "LatchMods",   "LockMods",
    "SetGroup",      "LatchGroup",   "LockGroup",   "MovePtr",
    "PtrBtn",        "LockPtrBtn",   "SetPtrDflt",  "ISOLock",
    "Terminate",     "SwitchScreen", "SetControls", "LockControls",
    "ActionMessage", "RedirectKey",  "DeviceBtn",   "LockDeviceBtn"
};
                  ...
                  int       *sz)
{
    ...
    if (act->flags & XkbSA_UseModMapMods) TryCopyStr(buf, "modMapMods", sz);
    ...
    if (act->flags & XkbSA_ClearLocks) TryCopyStr(buf, ",clearLocks", sz);
    if (act->flags & XkbSA_LatchToLock) TryCopyStr(buf, ",latchToLock", sz);
    return True;
}

/*ARGSUSED*/
static Bool
CopyGroupActionArgs(Display   *dpy,
                    XkbDescPtr xkb,
                    XkbAction *action,
                    char      *buf,
                    int       *sz)
{
    ...
    if (act->type == XkbSA_LockGroup) return True;
    if (act->flags & XkbSA_ClearLocks) TryCopyStr(buf, ",clearLocks", sz);
    if (act->flags & XkbSA_LatchToLock) TryCopyStr(buf, ",latchToLock", sz);
    return True;
}
                ...
                int       *sz)
{
    ...
    TryCopyStr(buf, tbuf, sz);
    if (act->flags & XkbSA_NoAcceleration) TryCopyStr(buf, ",!accel", sz);
    return True;
}

/*ARGSUSED*/
static Bool
CopyPtrBtnArgs(Display   *dpy,
               XkbDescPtr xkb,
               XkbAction *action,
               char      *buf,
               int       *sz)
...
/*ARGSUSED*/
static Bool
CopySetPtrDfltArgs(Display   *dpy,
                   XkbDescPtr xkb,
                   XkbAction *action,
                   char      *buf,
                   int       *sz)
    ...
    else
    {
        ...
        else if (act->real_mods || tmp)
        {
            if (act->real_mods)
            {
                ...
                if (tmp) TryCopyStr(buf, "+", sz);
            }
            if (tmp)
                TryCopyStr(buf,
                           XkbVModMaskText(dpy, xkb, 0, tmp, XkbXKBFile),
                           sz);
        }
        else TryCopyStr(buf, "none", sz);
    }
...
/*ARGSUSED*/
static Bool
CopySwitchScreenArgs(Display   *dpy,
                     XkbDescPtr xkb,
                     XkbAction *action,
                     char      *buf,
                     int       *sz)
{
    ...
    TryCopyStr(buf, tbuf, sz);
    if (act->flags & XkbSA_SwitchApplication) TryCopyStr(buf, ",!same", sz);
...
/*ARGSUSED*/
static Bool
CopySetLockControlsArgs(Display   *dpy,
                        XkbDescPtr xkb,
                        XkbAction *action,
                        char      *buf,
                        int       *sz)
{
    ...
    if (tmp == 0) TryCopyStr(buf, "none", sz);
...
/*ARGSUSED*/
static Bool
CopyActionMessageArgs(Display   *dpy,
                      XkbDescPtr xkb,
                      XkbAction *action,
                      char      *buf,
                      int       *sz)
{
    ...
    if ((act->flags & all) == 0) TryCopyStr(buf, "none", sz);
    else if ((act->flags & all) == all) TryCopyStr(buf, "all", sz);
    else if (act->flags & XkbSA_MessageOnPress) TryCopyStr(buf, "KeyPress", sz);
...
static Bool
CopyRedirectKeyArgs(Display   *dpy,
                    XkbDescPtr xkb,
                    XkbAction *action,
                    char      *buf,
                    int       *sz)
{
    ...
    TryCopyStr(buf, tbuf, sz);
    if ((act->mods_mask == 0) && (vmods_mask == 0)) return True;
    ...
    else
    {
        ...
        {
            tmp = XkbVModMaskText(dpy,
                                  xkb,
                                  act->mods_mask & act->mods,
                                  vmods_mask & vmods,
                                  XkbXKBFile);
            TryCopyStr(buf, ",mods= ", sz);
            TryCopyStr(buf, tmp, sz);
...
XkbActionText(Display *dpy, XkbDescPtr xkb, XkbAction *action, unsigned format)
{
    ...
    if (format == XkbCFile)
    {
        snprintf(buf,
                 sizeof(buf),
                 "{ %20s, { 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, "
                 "0x%02x } }",
...
XkbBehaviorText(XkbDescPtr xkb, XkbBehavior *behavior, unsigned format)
{
    ...
    else
    {
        ...
        else if (type == XkbKB_RadioGroup)
        {
            ...
            if (permanent) snprintf(tmp, tmpsize, "permanentRadioGroup= %d", g);
            else snprintf(tmp, tmpsize, "radioGroup= %d", g);
        }
