...
#ifdef HAVE_CONFIG_H
#  include <config.h>
#elif defined(HAVE_DIX_CONFIG_H)
...
ScanIdent(FILE *file, int ch, XkbCFScanResultPtr val_rtrn)
{
    ...
    {
        if (i < XKBCF_MAX_STR_LEN) str[i++] = ch;
    }
    if ((ch != EOF) && (ch != ' ') && (ch != '\t')) ungetc(ch, file);
    str[i] = '\0';
    return XkbCF_Ident;
}
...
ScanString(FILE *file, int quote, XkbCFScanResultPtr val_rtrn)
{
    ...
    while (((ch = getc(file)) != EOF) && (ch != '\n') && (ch != quote))
    {
        ...
        if (nInBuf < XKBCF_MAX_STR_LEN - 1) _XkbCF_rtrn[nInBuf++] = ch;
    }
    if (ch == quote)
    {
...
XkbCFScan(FILE *file, XkbCFScanResultPtr val_rtrn, XkbConfigRtrnPtr rtrn)
{
    ...
    while ((ch == '\t') || (ch == ' '));
    if (isalpha(ch)) return ScanIdent(file, ch, val_rtrn);
    else if (isdigit(ch)) return ScanInteger(file, ch, val_rtrn);
    else if (ch == '"') return ScanString(file, ch, val_rtrn);
    else if (ch == '\n')
    {
        rtrn->line++;
        return XkbCF_EOL;
    }
    else if (ch == ';') return XkbCF_Semi;
    else if (ch == '=') return XkbCF_Equals;
    else if (ch == '+')
    {
        ch = getc(file);
        if (ch == '=') return XkbCF_PlusEquals;
        if ((ch != EOF) && (ch != ' ') && (ch != '\t')) ungetc(ch, file);
        return XkbCF_Plus;
    }
    else if (ch == '-')
    {
        ch = getc(file);
        if (ch == '=') return XkbCF_MinusEquals;
        if ((ch != EOF) && (ch != ' ') && (ch != '\t')) ungetc(ch, file);
        return XkbCF_Minus;
    }
    else if (ch == EOF) return XkbCF_EOF;
                  ...
                  XkbConfigUnboundModPtr last)
{
    ...
    if (_XkbStrCaseCmp(name, "shift") == 0) last->mods |= ShiftMask;
    else if (_XkbStrCaseCmp(name, "lock") == 0) last->mods |= LockMask;
    else if ((_XkbStrCaseCmp(name, "control") == 0) ||
             (_XkbStrCaseCmp(name, "ctrl") == 0))
        last->mods |= ControlMask;
    else if (_XkbStrCaseCmp(name, "mod1") == 0) last->mods |= Mod1Mask;
    else if (_XkbStrCaseCmp(name, "mod2") == 0) last->mods |= Mod2Mask;
    else if (_XkbStrCaseCmp(name, "mod3") == 0) last->mods |= Mod3Mask;
    else if (_XkbStrCaseCmp(name, "mod4") == 0) last->mods |= Mod4Mask;
    else if (_XkbStrCaseCmp(name, "mod5") == 0) last->mods |= Mod5Mask;
    else
    {
        if (last->name != NULL)
        {
...
XkbCFBindMods(XkbConfigRtrnPtr rtrn, XkbDescPtr xkb)
{
    ...
    if (rtrn->num_unbound_mods < 1) return 0;
    if ((xkb == NULL) || (xkb->names == NULL)) return -1;
...
XkbCFApplyMods(XkbConfigRtrnPtr rtrn, int what, XkbConfigModInfoPtr info)
{
    ...
    if (rtrn->num_unbound_mods < 1) return True;

    for (n = 0, mod = rtrn->unbound_mods; n < rtrn->num_unbound_mods;
         n++, mod++)
    {
        if (mod->what != what) continue;
        ...
        {
            if (mod->merge == XkbCF_MergeSet) info->replace = True;
            info->mods |= mod->mods;
              ...
              XkbConfigRtrnPtr   rtrn)
{
    ...
    switch (field->field_id)
    {
        ...
        case _XkbCF_Keymap:
            str = &rtrn->keymap;
            goto process_str;
        case _XkbCF_Keycodes:
            str = &rtrn->keycodes;
            goto process_str;
        case _XkbCF_Geometry:
            str = &rtrn->geometry;
            goto process_str;
        case _XkbCF_PhysSymbols:
            str = &rtrn->phys_symbols;
            goto process_str;
        case _XkbCF_Symbols:
            str = &rtrn->symbols;
            goto process_str;
...
process_str:
        ...
        case _XkbCF_InternalMods:
            ...
            last = NULL;
            while ((tok != XkbCF_EOL) && (tok != XkbCF_Semi) &&
                   (tok != XkbCF_EOF))
            {
                ...
                if (merge == XkbCF_MergeSet) merge = XkbCF_MergeAdd;
                tok = XkbCFScan(file, &val, rtrn);
                if ((tok != XkbCF_EOL) && (tok != XkbCF_EOF) &&
                    (tok != XkbCF_Semi))
                {
                    if (tok != XkbCF_Plus)
                    {
        ...
        case _XkbCF_InitialCtrls:
            ...
            ctrls = NULL;
            if (tok == XkbCF_PlusEquals) ctrls = &rtrn->initial_ctrls;
        ...
        case _XkbCF_AccessXTimeoutCtrlsOff:
            ...
            else if ((tok == XkbCF_PlusEquals) || (tok == XkbCF_Equals))
            {
                if (field->field_id == _XkbCF_AccessXTimeoutCtrlsOff)
                {
                    ...
                    if (tok == XkbCF_Equals) rtrn->replace_axt_ctrls_off = True;
                }
                else
                {
                    ctrls = &rtrn->axt_ctrls_on;
                    opts  = &rtrn->axt_opts_on;
                    if (tok == XkbCF_Equals) rtrn->replace_axt_ctrls_on = True;
                }
                *ctrls = 0;
            }
            else
        ...
        case _XkbCF_InitialFeedback:
            ...
            opts = NULL;
            if (tok == XkbCF_PlusEquals) opts = &rtrn->initial_opts;
            else if (tok == XkbCF_MinusEquals) opts = &rtrn->initial_opts_clear;
            else if (tok == XkbCF_Equals)
            {
        ...
        case _XkbCF_AccessXTimeoutOptsOn:
            opts = NULL;
            if (tok == XkbCF_MinusEquals) opts = &rtrn->axt_opts_ignore;
            else if ((tok == XkbCF_PlusEquals) || (tok == XkbCF_Equals))
            {
                if (field->field_id == _XkbCF_AccessXTimeoutOptsOff)
                {
                    opts = &rtrn->axt_opts_off;
                    if (tok == XkbCF_Equals) rtrn->replace_axt_opts_off = True;
                }
                else
                {
                    opts = &rtrn->axt_opts_on;
                    if (tok == XkbCF_Equals) rtrn->replace_axt_opts_on = True;
                }
                *opts = 0;
            }
            else
...
pival:
        ...
        case _XkbCF_GroupsWrap:
            ...
            if (tok == XkbCF_Ident)
            {
                ...
                {
                    rtrn->groups_wrap =
                        XkbSetGroupInfo(0, XkbClampIntoRange, 0);
                }
                else
                {
            ...
            {
                rtrn->groups_wrap =
                    XkbSetGroupInfo(0, XkbRedirectIntoRange, val.ival - 1);
            }
            else
            {
                rtrn->error = XkbCF_ExpectedOORGroupBehavior;
...
DefaultApplyControls(XkbConfigRtrnPtr rtrn, XkbDescPtr xkb)
{
    ...
    if (XkbAllocControls(xkb, XkbAllControlsMask) != Success) return False;
    ctrls = xkb->ctrls;
    if (rtrn->replace_initial_ctrls) ctrls->enabled_ctrls = rtrn->initial_ctrls;
    ...
    if (rtrn->repeat_delay > 0) ctrls->repeat_delay = rtrn->repeat_delay;
    ...
    if (rtrn->debounce_delay > 0) ctrls->debounce_delay = rtrn->debounce_delay;
    if (rtrn->mk_delay > 0) ctrls->mk_delay = rtrn->mk_delay;
    if (rtrn->mk_interval > 0) ctrls->mk_interval = rtrn->mk_interval;
    if (rtrn->mk_time_to_max > 0) ctrls->mk_time_to_max = rtrn->mk_time_to_max;
    if (rtrn->mk_max_speed > 0) ctrls->mk_max_speed = rtrn->mk_max_speed;
    if (rtrn->mk_curve > 0) ctrls->mk_curve = rtrn->mk_curve;
...
#define _XkbCFNumDfltFields \
    (sizeof(_XkbCFDfltFields) / sizeof(XkbConfigFieldRec))
                     ...
                     XkbDescPtr         xkb)
{
    ...
    if ((fields == NULL) || (rtrn == NULL) || (xkb == NULL)) return False;
...
XkbCFAddPrivate(XkbConfigRtrnPtr rtrn, XkbConfigFieldsPtr fields, XPointer ptr)
{
    ...
    if ((rtrn == NULL) || (fields == NULL)) return NULL;
...
XkbCFFreeRtrn(XkbConfigRtrnPtr rtrn, XkbConfigFieldsPtr fields, XkbDescPtr xkb)
{
    ...
    if ((fields == NULL) || (rtrn == NULL)) return;
    while (fields != NULL)
    {
        if (fields->finish != NULL)
           ...
           XkbConfigRtrnPtr   rtrn)
{
    ...
    if ((file == NULL) || (fields == NULL) || (rtrn == NULL)) return False;
        ...
        if (tok == XkbCF_Ident)
        {
                 ...
                 tmp = tmp->next)
            {
                ...
                for (i = 0, f = tmp->fields; (i < tmp->num_fields) && (!done);
                     i++, f++)
                {
                    if (_XkbStrCaseCmp(val.str, f->field) != 0) continue;
                    if ((*tmp->parser)(file, tmp, f, xkb, rtrn)) done = True;
                    else goto BAILOUT;
                }
            }
        }
...
BAILOUT:
    for (tmp = fields; tmp != NULL; tmp = tmp->next)
    {
        if (tmp->finish) (*tmp->finish)(tmp, xkb, rtrn, XkbCF_CleanUp);
    }
    return False;
}
