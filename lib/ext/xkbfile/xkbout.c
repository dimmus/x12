...
WriteXKBVModDecl(FILE *file, Display *dpy, XkbDescPtr xkb, int showValue)
{
    ...
    if (xkb->names != NULL) vmodNames = xkb->names->vmods;
    else vmodNames = NULL;
    ...
    for (i = nMods = 0; i < XkbNumVirtualMods; i++)
    {
        if ((vmodNames != NULL) && (vmodNames[i] != None))
        {
            ...
            if ((showValue != VMOD_HIDE_VALUE) && (xkb->server) &&
                (xkb->server->vmods[i] != XkbNoModifierMask))
            {
                    ...
                    void            *priv)
{
    ...
    if (xkb->indicators != NULL)
    {
        for (i = 0; i < XkbNumIndicators; i++)
        {
            ...
            if (xkb->indicators->phys_indicators & (1 << i)) type = "    ";
            else type = "    virtual ";
                    ...
                    void            *priv)
{
    ...
    for (i = 0; i < xkb->map->num_types; i++, type++)
    {
        ...
        for (n = 0; n < type->map_count; n++, entry++)
        {
            ...
            str = XkbVModMaskText(dpy,
                                  xkb,
                                  entry->mods.real_mods,
                                  entry->mods.vmods,
                                  XkbXKBFile);
            fprintf(file, "        map[%s]= Level%d;\n", str, entry->level + 1);
            if ((type->preserve) &&
                ((type->preserve[n].real_mods) || (type->preserve[n].vmods)))
            {
                     ...
                     void              *priv)
{
    ...
    if (addOn) (*addOn)(file, result, False, True, XkmIndicatorsIndex, priv);
    fprintf(file, "    };\n");
    return True;
}
                     ...
                     void            *priv)
{
    ...
    for (i = 0; i < XkbNumKbdGroups; i++)
    {
        ...
        if ((gc->real_mods == 0) && (gc->vmods == 0)) continue;
        fprintf(file,
                "    group %d = %s;\n",
                i + 1,
                XkbVModMaskText(xkb->dpy,
                   ...
                   void            *priv)
{
    ...
    for (i = xkb->min_key_code; i <= xkb->max_key_code; i++)
    {
        ...
        if ((int)XkbKeyNumSyms(xkb, i) < 1) continue;
        if (XkbFindKeycodeByName(xkb, xkb->names->keys[i].name, True) != i)
            continue;
        simple = True;
        fprintf(file,
        ...
        if (srv->explicit)
        {
                ...
                (showImplicit))
            {
                ...
                {
                    if (XkbKeyKeyTypeIndex(xkb, i, g) != typeNdx) multi = True;
                }
                if (multi)
                {
                ...
                else
                {
                    fprintf(
                        ...
                        "\n%s      type= \"%s\",",
                        comment,
                        XkbAtomText(dpy, map->types[typeNdx].name, XkbXKBFile));
                }
                simple = False;
            }
                ...
                int           indent)
{
    ...
    if (first != indent) iStr = XkbIndentText(indent);
    ...
    if (shape)
    {
        if (outline == shape->approx) fprintf(file, "approx= ");
        else if (outline == shape->primary) fprintf(file, "primary= ");
    }
    fprintf(file, "{");
                ...
                XkbOverlayPtr  ol)
{
    ...
    for (nOut = r = 0, row = ol->rows; r < ol->num_rows; r++, row++)
    {
        for (k = 0, key = row->keys; k < row->num_keys; k++, key++)
        {
            ...
            if (nOut == 0) fprintf(file, "%s    %6s=%6s", i_str, under, over);
            else if ((nOut % 4) == 0)
...
static Bool
WriteXKBSection(FILE *file, Display *dpy, XkbSectionPtr s, XkbGeometryPtr geom)
{
    register int i;
    XkbRowPtr    row;
    int          dfltKeyColor = 0;
    ...
    for (i = 0, row = s->rows; row && i < s->num_rows; i++, row++)
    {
        ...
        if (row->vertical) fprintf(file, "            vertical;\n");
        if (row->num_keys > 0)
        {
            register int       k;
            ...
            for (k = 0, key = row->keys; k < row->num_keys; k++, key++)
            {
                ...
                if (key->color_ndx != dfltKeyColor) forceNL = 1;
                if (k == 0)
                {
                    fprintf(file, "                ");
                    nThisLine = 0;
                    ...
                    void            *priv)
{
    ...
    dpy  = xkb->dpy;
    geom = xkb->geom;
    if (geom->name == None) fprintf(file, "xkb_geometry {\n\n");
    else
        fprintf(file,
                "xkb_geometry \"%s\" {\n\n",
    ...
    switch (result->type)
    {
        ...
        case XkmIndicatorsIndex:
            _XkbLibError(_XkbErrBadImplementation,
                         XkbConfigText(result->type, XkbMessage),
                         0);
            return False;
    }
    if (out == NULL)
