...
int     *minorVersion;
{
    ...
    *minorVersion = rep.minorVersion;
    UnlockDisplay(dpy);
    SyncHandle();
    if (*majorVersion > 0 || *minorVersion > 5) XF86MiscSetClientVersion(dpy);
...
Bool
                       XF86MiscGetMouseSettings(dpy, mouseinfo)
Display               *dpy;
...
Bool
                       XF86MiscSetMouseSettings(dpy, mouseinfo)
Display               *dpy;
XF86MiscMouseSettings *mouseinfo;
{
    XExtDisplayInfo              *info = find_display(dpy);
...
Bool
                     XF86MiscSetKbdSettings(dpy, kbdinfo)
Display             *dpy;
XF86MiscKbdSettings *kbdinfo;
{
    XExtDisplayInfo            *info = find_display(dpy);
...
int
         XF86MiscSetGrabKeysState(dpy, enable)
Display *dpy;
Bool     enable;
{
    XExtDisplayInfo               *info = find_display(dpy);
...
Bool
                   XF86MiscGetFilePaths(dpy, filpaths)
Display           *dpy;
XF86MiscFilePaths *filpaths;
{
    ...
    if (rep.configlen)
    {
        ...
        {
            _XEatData(dpy,
                      ((rep.configlen + 3) & ~3) + ((rep.modulelen + 3) & ~3) +
                          ((rep.loglen + 3) & ~3));
            return False;
        }
    }
    ...
    if (rep.modulelen)
    {
        if (!(filpaths->modulepath = Xcalloc(rep.modulelen + 1, 1)))
        {
            _XEatData(dpy,
                      ((rep.configlen + 3) & ~3) + ((rep.modulelen + 3) & ~3) +
                          ((rep.loglen + 3) & ~3));
            if (filpaths->configfile) Xfree(filpaths->configfile);
            return False;
        }
    }
        ...
        if (!(filpaths->logfile = Xcalloc(rep.loglen + 1, 1)))
        {
            _XEatData(dpy,
                      ((rep.configlen + 3) & ~3) + ((rep.modulelen + 3) & ~3) +
                          ((rep.loglen + 3) & ~3));
            if (filpaths->configfile) Xfree(filpaths->configfile);
            if (filpaths->modulepath) Xfree(filpaths->modulepath);
            return False;
        }
    }

    if (rep.configlen) _XReadPad(dpy, filpaths->configfile, rep.configlen);
    else filpaths->configfile = "";

    if (rep.modulelen) _XReadPad(dpy, filpaths->modulepath, rep.modulelen);
    else filpaths->modulepath = "";

    if (rep.loglen) _XReadPad(dpy, filpaths->logfile, rep.loglen);
    else filpaths->logfile = "";
...
Status
            XF86MiscPassMessage(dpy, screen, msgtype, msgval, retmsg)
Display    *dpy;
int         screen;
const char *msgtype;
const char *msgval;
