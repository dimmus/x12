...
XkbUI_SimpleInit(Display *dpy, Window win, int width, int height)
{
    ...
    if ((!dpy) || (win == None) || (width < 1) || (height < 1)) return NULL;
           ...
           XkbUI_ViewOptsPtr opts)
{
    ...
    if ((!dpy) || (!xkb) || (!xkb->geom) || (win == None) || (width < 1) ||
        (height < 1))
        return NULL;
    ...
    {
        if (opts->present & XkbUI_BackgroundMask) view->opts.bg = opts->bg;
        if (opts->present & XkbUI_ForegroundMask) view->opts.fg = opts->fg;
        ...
        if (opts->present & XkbUI_ColormapMask) view->opts.cmap = opts->cmap;
    }
    ...
    view->gc =
        XCreateGC(view->dpy, view->win, GCForeground | GCBackground, &xgcv);
...
{
    if ((!view) || (!opts)) return BadValue;
    if (opts->present == 0) return Success;
    if (opts->present & XkbUI_BackgroundMask) view->opts.bg = opts->bg;
    if (opts->present & XkbUI_ForegroundMask) view->opts.fg = opts->fg;
    if (opts->present & XkbUI_LabelModeMask)
        view->opts.label_mode = opts->label_mode;
...
{
    if ((!view) || (!opts_rtrn)) return BadValue;
    *opts_rtrn = view->opts;
    return Success;
}
...
{
    if ((!view) || (!view->xkb) || (!view->xkb->geom)) return BadValue;
           ...
           Bool          key)
{
    ...
    for (maxPts = 4, o = 0, ol = shape->outlines; o < shape->num_outlines;
         o++, ol++)
    {
        if ((shape->num_outlines > 1) && (ol == shape->approx)) continue;
        if (ol->num_points > maxPts) maxPts = ol->num_points;
    }
    ...
    for (o = 0, ol = shape->outlines; o < shape->num_outlines; o++, ol++)
    {
        ...
        if (ol->num_points == 1)
        {
            uipts[0].x = xoff;
            uipts[0].y = yoff;
            uipts[1].x = xoff + gpts[0].x;
            uipts[1].y = yoff;
            uipts[2].x = xoff + gpts[0].x;
            uipts[2].y = yoff + gpts[0].y;
            uipts[3].x = xoff;
            uipts[3].y = yoff + gpts[0].y;
            p          = 4;
        }
        else if (ol->num_points == 2)
        {
            uipts[0].x = xoff + gpts[0].x;
            uipts[0].y = yoff + gpts[0].y;
            uipts[1].x = xoff + gpts[1].x;
            uipts[1].y = yoff + gpts[0].y;
            uipts[2].x = xoff + gpts[1].x;
            uipts[2].y = yoff + gpts[1].y;
            uipts[3].x = xoff + gpts[0].x;
            uipts[3].y = yoff + gpts[1].y;
            p          = 4;
        }
        else
        {
        ...
        if (rangle != 0.0) _RotatePoints(rangle, rotx, roty, p, uipts);
        if (key)
        {
            if (o == 0)
            {
                XSetForeground(view->dpy,
                               view->gc,
          ...
          Bool          key)
{
    ...
    uipts[3].x = x1;
    uipts[3].y = y2;
    if (rangle != 0.0) _RotatePoints(rangle, 0, 0, 4, uipts);
    if (key)
    {
            ...
            XkbDoodadPtr  doodad)
{
    ...
    switch (doodad->any.type)
    {
        ...
        case XkbIndicatorDoodad:
            shape =
                XkbIndicatorDoodadShape(view->xkb->geom, &doodad->indicator);
            solid = True;
            break;
        case XkbLogoDoodad:
    ...
    if (shape) _DrawShape(view, rangle, x, y, x, y, shape, solid);
    return;
}

static void
_DrawRow(XkbUI_ViewPtr view, double rangle, int xoff, int yoff, XkbRowPtr row)
{
    register int k, x, y;
    XkbKeyPtr    key;
...
XkbUI_SetKeyAppearance(XkbUI_ViewPtr view, KeyCode kc, unsigned int flags)
{
    ...
    if ((!view) || (!view->xkb)) return False;
    xkb = view->xkb;
    if ((kc < xkb->min_key_code) || (kc > xkb->max_key_code)) return False;
    old             = view->state[kc];
    view->state[kc] = (flags & (~XkbUI_Obscured));
    if (old & XkbUI_Obscured) view->state[kc] |= XkbUI_Obscured;
    else if (old != view->state[kc]) _RedrawKey(view, kc);
    return True;
}
                             ...
                             unsigned int  flags)
{
    ...
    if ((!view) || (!view->xkb) || (!name)) return False;
