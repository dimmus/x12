    ...
    for (screens = xcb_render_query_pict_formats_screens_iterator(formats);
        ...
        for (depths = xcb_render_pictscreen_depths_iterator(screens.data);
            ...
            for (visuals = xcb_render_pictdepth_visuals_iterator(depths.data);
                ...
                if (visuals.data->visual == visual) return visuals.data;
    return 0;
}
    ...
    int                                          count)
{
         ...
         xcb_render_pictforminfo_next(&i))
    {
        if (mask & XCB_PICT_FORMAT_ID)
            if (ptemplate->id != i.data->id) continue;
        if (mask & XCB_PICT_FORMAT_TYPE)
            if (ptemplate->type != i.data->type) continue;
        if (mask & XCB_PICT_FORMAT_DEPTH)
            if (ptemplate->depth != i.data->depth) continue;
        if (mask & XCB_PICT_FORMAT_RED)
            if (ptemplate->direct.red_shift != i.data->direct.red_shift)
                continue;
        if (mask & XCB_PICT_FORMAT_RED_MASK)
            if (ptemplate->direct.red_mask != i.data->direct.red_mask) continue;
        ...
        if (mask & XCB_PICT_FORMAT_COLORMAP)
            if (ptemplate->colormap != i.data->colormap) continue;
        if (count-- == 0) return i.data;
    }
    return 0;
}
    ...
    xcb_pict_standard_t                          format)
{
        ...
        {
         {
                ...
                XCB_PICT_FORMAT_RED_MASK | XCB_PICT_FORMAT_GREEN |
                XCB_PICT_FORMAT_GREEN_MASK | XCB_PICT_FORMAT_BLUE |
                XCB_PICT_FORMAT_BLUE_MASK | XCB_PICT_FORMAT_ALPHA_MASK,
         },
        ...
        {
         {
                ...
                XCB_PICT_FORMAT_RED_MASK | XCB_PICT_FORMAT_GREEN_MASK |
                XCB_PICT_FORMAT_BLUE_MASK | XCB_PICT_FORMAT_ALPHA |
                XCB_PICT_FORMAT_ALPHA_MASK, },
    ...
    if (format < 0 ||
        format >= sizeof(standardFormats) / sizeof(*standardFormats))
        return 0;
