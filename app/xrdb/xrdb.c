...
{
    if (n > 0 && (SIZE_MAX / n) < s) return NULL;
...
FindFirst(const char *string, char dest, int *lines)
{
    ...
    for (;;)
    {
        ...
        else if (*string == dest) return string;
        if (*string == '\n' && lines) (*lines)++;
        string++;
    }
}
...
GetEntries(Entries *entries, Buffer *buff, int bequiet)
{
         ...
         str = line + 1, lineno += lines_skipped)
    {
        line = FindFirst(str, '\n', &lines_skipped);
        lineno++;
        if (!line) line = buff->buff + buff->used;
        if (*str == '!') continue;
        if (*str == '\n') continue;
        if (!bequiet && *str == '#')
        {
...
GetEntriesString(Entries *entries, char *str)
{
    if (str && *str)
    {
        Buffer buff = { .buff = str, .used = strlen(str) };
...
ReadFile(Buffer *b, FILE *input)
{
    ...
    while (!feof(input) && (bytes = fread(buf, 1, BUFSIZ, input)) > 0)
    {
        ...
        if (show_cpp) fwrite(buf, 1, bytes, stdout);
    }
    AppendToBuffer(b, "", 1);
}
...
AddDef(String *buff, const char *title, const char *value)
{
    ...
    if (buff->used)
    {
        if (oper == OPSYMBOLS) addstring(buff, "\n-D");
        else addstring(buff, " -D");
    }
    else addstring(buff, "-D");
    addtokstring(buff, title);
    if (value && (value[0] != '\0'))
...
{
#ifdef PATHETICCPP
    if (need_real_defines) AddDef(buff, title, value);
    else
#endif
        if (value && (value[0] != '\0'))
    {
...
static const char *ClassNames[] = { "StaticGray",  "GrayScale", "StaticColor",
...
{
    Entry   entry  = { .usable = False, .tag = NULL, .value = NULL };
    Entries phoney = { .used = 0, .room = 1, .entry = &entry };

    GetEntries(&phoney, b, 1);
    if (phoney.used < 1) return NULL;
    for (size_t i = 0; i < db->used; i++)
    {
        Entry *e = &db->entry[i];

        if (!e->usable) continue;
        if (strcmp(e->tag, entry.tag)) continue;
        e->usable = False;
        if (strcmp(e->value, entry.value)) return e;
        return NULL;
    }
    return NULL;
}
...
EditFile(Entries *new, FILE *in, FILE *out)
{
    ...
    while (in)
    {
        ...
        while (1)
        {
            ...
            buff[0] = '\0';
            if (!fgets(buff, BUFSIZ, in)) goto cleanup;
        ...
        if ((e = FindEntry(new, &b)))
            fprintf(out, "%s:\t%s\n", e->tag, e->value);
        else fwrite(b.buff, 1, b.used, out);
    }
cleanup:
    for (size_t i = 0; i < new->used; i++)
    {
        Entry *e = &new->entry[i];
        if (e->usable) fprintf(out, "%s:\t%s\n", e->tag, e->value);
    }
}
...
addstring(String *arg, const char *s)
{
    ...
    {
        if (arg->val) arg->val = realloc(arg->val, arg->room + CHUNK_SIZE);
        else arg->val = malloc(arg->room + CHUNK_SIZE);
        if (arg->val == NULL) fatal("%s: Not enough memory\n", ProgramName);
        arg->room += CHUNK_SIZE;
    }
    if (arg->used) strcat(arg->val, s);
    else strcpy(arg->val, s);
    arg->used += strlen(s);
}
...
addtokstring(String *arg, const char *s)
{
    ...
    for (c = copy; *s && c < &copy[sizeof(copy) - 1]; s++)
    {
        if (!isalpha(*s) && !isdigit(*s) && *s != '_') *c++ = '_';
        else *c++ = *s;
    }
    *c = 0;
    addstring(arg, copy);
...
main(int argc, char *argv[])
{
    ...
    /* TODO: needs to be replaced with XrmParseCommand */
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg[0] == '-')
        {
            ...
            {
                oper = OPGET;
                if (++i >= argc) Syntax("-get requires an argument");
...
#  endif
    ...
    InitBuffer(&buffer);
    if (whichResources == RGLOBAL) Process(DefaultScreen(dpy), False, True);
    else if (whichResources == RSCREEN) Process(DefaultScreen(dpy), True, True);
    else if (whichResources == RSCREENS ||
             (oper != OPLOAD && oper != OPMERGE && oper != OPOVERRIDE))
    {
        if (whichResources == RALL && oper != OPSYMBOLS)
        {
            if (need_newline) printf("! screen-independent resources\n");
        ...
        for (int i = 0; i < ScreenCount(dpy); i++)
        {
            if (need_newline)
            {
                if (oper == OPSYMBOLS) printf("# screen %d symbols\n", i);
                else
                {
            ...
            if (need_newline)
            {
                if (oper != OPSYMBOLS) printf("#endif\n");
    ...
    if (fp) unlink(filename);
    if (retainProp) XSetCloseDownMode(dpy, RetainPermanent);
    XCloseDisplay(dpy);
    exit(0);
}
...
StoreProperty(Display *display, Window root, Atom res_prop)
{
    ...
    if (mode != PropModeReplace) XUngrabServer(display);
}
...
Process(int scrno, Bool doScreen, Bool execute)
{
    ...
    else if (oper == OPGET)
    {
        if (xdefs && resource_name != NULL)
        {
            ...
            Bool        found = XrmGetResource(xrdb,
                                        resource_name,
                                        resource_name,
                                        &type,
                                        &value);
            if (found == True && value.addr != NULL) printf("%s\n", value.addr);
            XrmDestroyDatabase(xrdb);
        }
    }
    else if (oper == OPREMOVE)
    {
        if (xdefs) XDeleteProperty(dpy, root, res_prop);
    }
    else if (oper == OPEDIT)
    {
    ...
    else
    {
        ...
        if (cpp_program)
        {
            ...
            if (cp && ((cp[3] == '\0') || cp[3] == ' ')) cpp_addflags = "-P";
        }
#ifdef PATHETICCPP
        if (need_real_defines)
        {
...
#  endif
        ...
        if (need_real_defines)
        {
            unlink(tmpname2);
#  ifdef WIN32
            if (tmpname3[strlen(tmpname3) - 1] != 'X') unlink(tmpname3);
#  endif
        }
#endif
        ...
        if (execute)
        {
            ...
            else XDeleteProperty(dpy, root, res_prop);
        }
    }
    if (execute) FreeEntries(&newDB);
    if (doScreen) XFree(xdefs);
}
...
ReProcess(int scrno, Bool doScreen)
{
    ...
    {
        if (buffer.used > 1 || !doScreen) StoreProperty(dpy, root, res_prop);
        else XDeleteProperty(dpy, root, res_prop);
    }
    FreeEntries(&newDB);
}
