...
#define DESCRIPTOR(name, string)                     \
    struct dsc$descriptor_s name = { strlen(string), \
                                     DSC$K_DTYPE_T,  \
                                     DSC$K_CLASS_S,  \
                                     string }

int
trnlnm(char *in, int id, char *out)
{
    ...
    struct itemlist
    {
        ...
        char *buffer_addr;
        int  *return_length;
    } itmlst[] = { 4,           LNM$_INDEX, &foo,    0, 255,
                   LNM$_STRING, out,        &outlen, 4, LNM$_MAX_INDEX,
                   &num,        &len,       0,       0 };
...
spawn(void)
{
   ...
   */
    ...
    status =
        ptd$set_event_notification(tt_chan, &send_xon, 0, 0, PTD$C_SEND_XON);
    if (!(status & SS$_NORMAL)) CloseDown(status);

    status =
        ptd$set_event_notification(tt_chan, &send_xoff, 0, 0, PTD$C_SEND_XOFF);
    if (!(status & SS$_NORMAL)) CloseDown(status);

    status =
        ptd$set_event_notification(tt_chan, &send_bell, 0, 0, PTD$C_SEND_BELL);
    if (!(status & SS$_NORMAL)) CloseDown(status);

    status = ptd$set_event_notification(tt_chan,
                                        &char_change,
                                        0,
                                        0,
                                        PTD$C_CHAR_CHANGED);
...
static void
tt_echo_ast(TT_BUF_STRUCT *buff_addr)
{
    int status;
...
int
tt_write(const char *tt_write_buf, size_t size)
{
    int            status;
...
static void
tt_read_ast(TT_BUF_STRUCT *buff_addr)
{
    int status;
    ...
    else CloseDown(buff_addr->status);
...
static void
freeBuff(TT_BUF_STRUCT *buff_addr)
{
    int ast_stat;
    int status;
