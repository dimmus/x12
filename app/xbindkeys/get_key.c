...
#define max(a, b) ((a) > (b) ? (a) : (b))
    ...
    {
        hintp->x =
            DisplayWidth(dpy, DefaultScreen(dpy)) + hintp->x - hintp->width;
    }
...
get_key_binding(Display *dpy, char **argv, int argc)
{
    ...
    set_sizehints(dpy,
                  &hints,
                  OUTER_WINDOW_MIN_WIDTH,
                  OUTER_WINDOW_MIN_HEIGHT,
        ...
        switch (event.type)
        {
            case KeyRelease:
                    ...
                    event.xkey.keycode > max_keycode)
                {
                    fprintf(stderr,
                            "Note:\n"
                            "  The keycode %d cannot be used, as it's not "
                            "between the\n"
            ...
            case ButtonRelease:
                ...
                if (have_to_get_binding == 1) done = 1;
                break;
