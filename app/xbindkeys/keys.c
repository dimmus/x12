...
modifier_to_string(unsigned int modifier, char *str)
{
    ...
    {
        if (str[0]) strncat(str, "+", STR_KEY_LEN);
            ...
            if (d != NULL)
            {
                ...
                printf("    %s%s%s%s\n",
                       ...
                       (XKeysymToString(*XGetKeyboardMapping(
                            ...
                            key->key.code,
                            1,
                            &keysyms_per_keycode_return)) != NULL)
                           ? XKeysymToString(*XGetKeyboardMapping(
                                 d,
                                 key->key.code,
                                 1,
           ...
           char        *command)
{
    ...
    if (command != NULL)
    {
        ...
        if (key->command != NULL) strncpy(key->command, command, size);
    }
    else
    {
        key->command = NULL;
...
free_key(Keys_t *key)
{
    ...
    if (key->command != NULL) free(key->command);
}

int
remove_key(KeyType_t    type,
           EventType_t  event_type,
           KeySym       keysym,
           KeyCode      keycode,
           unsigned int button,
           unsigned int modifier)
{
    ...
    if (found_index != -1)
    {
        if (verbose) printf("Removing key index %d\n", found_index);
...
run_command(char *command)
{
    ...
    if (verbose) printf("Start program with fork+exec call\n");
  ...
  //  execlp ("sh", "sh", "-c", key->command, NULL);
    ...
    if (pid > 0) wait(NULL);
#else
    if (verbose) printf("Start program with system call\n");
