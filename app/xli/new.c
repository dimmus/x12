...
unsigned long
colorsToDepth(long unsigned int ncolors)
{
    unsigned long a;
...
void
resizeRGBMapData(RGBMap *rgb, unsigned int size)
{
    CURRFUNC("resizeRGBMapData");
    rgb->red =
        (Intensity *)lrealloc((byte *)rgb->red, sizeof(Intensity) * size);
    rgb->green =
        (Intensity *)lrealloc((byte *)rgb->green, sizeof(Intensity) * size);
    rgb->blue =
        (Intensity *)lrealloc((byte *)rgb->blue, sizeof(Intensity) * size);
    rgb->size = size;
}
...
static Image *
newImage(unsigned width, unsigned height)
{
    Image *image;
...
Image *
newBitImage(unsigned width, unsigned height)
{
    Image       *image;
    ...
    *(image->rgb.red + 1) = *(image->rgb.green + 1) = *(image->rgb.blue + 1) =
        0;
    image->rgb.used = 2;
    image->depth    = 1;
    linelen         = ((width + 7) / 8);
...
freeImageData(Image *image)
{
    ...
    if (!TRUEP(image)) freeRGBMapData(&(image->rgb));
    lfree(image->data);
}
...
lmalloc(unsigned int size)
{
    ...
    {
        size = 1;
        if (globals._Xdebug) fprintf(stderr, "lmalloc given zero size!\n");
    }
...
lcalloc(unsigned int size)
{
    ...
    {
        size = 1;
        if (globals._Xdebug) fprintf(stderr, "lcalloc given zero size!\n");
    }
...
byte *
lrealloc(byte *old, unsigned int size)
{
    byte *area;
    ...
    {
        size = 1;
        if (globals._Xdebug) fprintf(stderr, "lrealloc given zero size!\n");
    }
