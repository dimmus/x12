...
void
compress_cmap(Image *image, unsigned int verbose)
{
    ...
    if (!RGBP(image) || image->rgb.compressed) return;
    ...
    used  = (unsigned char *)lcalloc(sizeof(unsigned char) *
                                    depthToColors(image->depth));
    ...
    for (x = 0; x < image->rgb.used; x++)
    {
        ...
        g = green[x];
        b = blue[x];
        if (fast[r >> 11][g >> 11][b >> 11])
        { /* if matches fast check */
            /* then do a linear search */
            for (y = x + 1; y < image->rgb.used; y++)
            {
                if (r == red[y] && g == green[y] && b == blue[y]) break;
            }
            if (y < image->rgb.used)
            { /* found match */
                map[x] = y;
    ...
    if (badcount)
        if (verbose) printf("%d out-of-range pixels, ", badcount);
        else
            fprintf(stderr,
    ...
    if (verbose)
    {
        if (!unusedcount && !dupcount) printf("no improvment\n");
        else
        {
            if (dupcount)
