...
static boolean PCX_LoadImage(ZFILE *zf,
                             int    in_bpr,
                             int    img_bpr,
                             Image *image,
                             int    rows);  /* Routine to load a PCX file */
...
int
pcxIdent(char *fullname, char *name)
{
    ZFILE        *zf;
    int           ret;
    int           xmin;
...
Image *
pcxLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ZFILE          *zf;
    ...
    {
        fprintf(stderr,
                "pcxLoad: %s - Unable to handle Color PCX image\n",
                name);
        zclose(zf);
        return ((Image *)NULL);
    }
...
static boolean
PCX_LoadImage(ZFILE *zf, int in_bpr, int img_bpr, Image *image, int rows)
{
	...
	*/
    ...
    while ((b = zgetc(zf)) != EOF)
    {
        ...
        for (i = 0; i < cnt; i++)
        {
            ...
            if (bytes_this_row < img_bpr) *ptr++ = (unsigned char)(255 - b);
