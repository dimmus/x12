...
#define GAMMA_NOT_EQUAL(g1, g2) \
    ((g1) > ((g2) + 0.00001) || (g1) < ((g2) - 0.00001))
...
char imageInWindow(DisplayInfo  *dinfo,
                   Image        *image,
...
int  doGeneralOption(OptionId      opid,
                     char        **argv,
                     ImageOptions *persist_ops,
                     ImageOptions *image_ops);
int  doLocalOption(OptionId      opid,
                   char        **argv,
                   boolean       setpersist,
                   ImageOptions *persist_ops,
                   ImageOptions *image_ops);
void read_trail_opt(ImageOptions *image_ops,
                    ZFILE        *file,
                    Image        *image,
                    boolean       verbose);
...
/* send.c */
void        sendXImage(XImageInfo  *xii,
                       int          src_x,
                       int          src_y,
                       int          dst_x,
                       int          dst_y,
...
int     zeof(ZFILE *zf);
void    zunread(ZFILE *zf, const byte *buf, int len);
