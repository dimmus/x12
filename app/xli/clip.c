     ...
     ImageOptions *imgopp)
{
   ...
   */
    ...
    {
        if (clipx > simage->width) dclipw = 0;
        else dclipw = simage->width - clipx;
        border_shows = TRUE;
    }
    if (dclipx + dclipw > clipw)
    {
        if (dclipx > clipw) dclipw = 0;
        else dclipw = clipw - dclipx;
    }

    if (clipy + dcliph > simage->height)
    {
        if (clipy > simage->height) dcliph = 0;
        else dcliph = simage->height - clipy;
        border_shows = TRUE;
    }
    if (dclipy + dcliph > cliph)
    {
        if (dclipy > cliph) dcliph = 0;
        else dcliph = cliph - dclipy;
    }

    if (globals.verbose)
    {
        if (border_shows) printf("(Adding border)");
        printf("...");
        fflush(stdout);
    }
   ...
   */
    ...
    /* Gamma correct the background colour to suit image */
        red   = (int)(0.5 +
                    65535 * pow((double)red / 65535.0, 1.0 / simage->gamma));
        green = (int)(0.5 + 65535 * pow((double)green / 65535.0,
                                        1.0 / simage->gamma));
        blue  = (int)(0.5 +
                     65535 * pow((double)blue / 65535.0, 1.0 / simage->gamma));
        ...
        /* Failed to find exact color */
                    ...
                    { /* can't fit another */
                        compress_cmap(simage,
                                      FALSE);  /* try compressing image */
                        border_pv = simage->rgb.used;
                    }
                    if (border_pv < simage->rgb.size)
                    { /* can fit another */
                    ...
                    { /* else need more color space */
                        Image *tmp;
                        if (simage->depth >= 16)
                        { /* don't create irgb with depth > 16 */
                            tmp       = expandtotrue(simage);
                            border_pv = RGB_TO_TRUE(red, green, blue);
                        }
                        else
                        { /* add more depth (copes with bitmap) */
     ...
     */
            ...
            if (border_shows) fill(dimage, 0, 0, clipw, cliph, border_pv);
        ...
        case ITRUE:
            ...
            else dimage = newTrueImage(clipw, cliph);
            if (border_shows) fill(dimage, 0, 0, clipw, cliph, border_pv);
