...
static void
freeCtable(char **ctable, int ncolors)
        /* color table */

{
    ...
    for (i = 0; i < ncolors; i++)
        if (ctable[i] != NULL) lfree(ctable[i]);
    lfree((byte *)ctable);
}
...
xpmoption(char *s)
{
    ...
    {
        if (!strcmp(s, xpmkeys[i])) return i;
    }
    printf("Argument to -xpm must be a xpm color context m, g4, g or c. "
           "(ignored)\n");
    return XPMKEY_NONE;
}
...

{
    ...
    *format = *w = *h = *ncolors = 0;
    for (; maxlines-- > 0;)
    {
        ...
        {
            if (*imagetitle) lfree(*imagetitle);
            return (FALSE);
        }
        if (!strncmp(buf, "#define", 7))
        {
            ...
            if (!(p = rindex(what, '_'))) p = what;
            else p++;
            if (!strcmp(p, "format")) *format = value;
            else if (!strcmp(p, "width")) *w = value;
            else if (!strcmp(p, "height")) *h = value;
            else if (!strcmp(p, "ncolors")) *ncolors = value;

            /* this one is ugly
			 */

            else if (!strcmp(p, "pixel"))
            {  /* this isn't pretty but it works */
                ...
                *(--p) = '\0';
                if (!(p = rindex(what, '_'))) p = what;
                if (strcmp(++p, "chars")) continue;
                *cpp = value;
            }
        }
        else if ((sscanf(buf, "static char * %s", what) == 1) &&
                 (p = rindex(what, '_')) &&
                 (!strcmp(p + 1, "colors[]") || !strcmp(p + 1, "mono[]")))
        {
            zunread(zf,
                    buf,
                    strlen(buf)); /* stuff it back so we can read it again */
            break;
        }
        ...
        else if ((sscanf(buf, "/* %s */", what) == 1) && !strcmp(what, "XPM"))
        {
            *format = XPM_FORMAT3;
            break;
        }
    ...
    if (*format == XPM_FORMAT2C || *format == XPM_FORMAT3)
    {
        ...
        for (b = 0; ((c = zgetc(zf)) != EOF) && (c != '"') && b < (BUFSIZ - 1);
             b++)
        {
            if (c == '\\') c = zgetc(zf);
            if (c == EOF)
    ...
    { /* get the name in the image if there is */
        *p          = '\0'; /* one */
        *imagetitle = dupString(what);
    }
...
Image *
xpixmapLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ZFILE       *zf;
    ...
    if (imagetitle != NULL) image->title = imagetitle;
    else image->title = dupString(name);
    ...
    for (donecmap = 0; !donecmap;)
    { /* until we have read a colormap */
        if (format == XPM_FORMAT1)
        {
            ...
            { /* keep reading lines */
                ...
                {
                    fprintf(stderr,
                            "xpixmapLoad: %s - unable to find a colormap\n",
                            name);
                     ...
                     !strcmp(p + 1, "pixels[]")))
                {
                    if (!strcmp(p + 1, "pixels[]"))
                    {
                        ...
                        {
                            fprintf(stderr,
                                    "xpixmapLoad: %s - colormap is missing\n",
                                    name);
                            freeCtable(ctable, ncolors);
                            freeImage(image);
                            zclose(zf);
        ...
        for (a = 0; a < ncolors; a++)
        {
            ...
            *(ctable + a) = (char *)lmalloc(sizeof(char *) * ncolors);
            ...
            {
                fprintf(stderr,
                        "xpixmapLoad: %s - file ended in the colormap\n",
                        name);
                freeCtable(ctable, ncolors);
                freeImage(image);
                zclose(zf);
            ...
            if (format != XPM_FORMAT1)
            {
                ...
                { /* failed to find that color key type */
                    for (b = XPMKEY_C; b >= XPMKEY_M && p == NULL; b--)
                    { /* try all the rest */
                        for (p = buf; p != NULL;)
                        {
                            if ((p = nword(p, what)) != NULL &&
                                !strcmp(what, xpmkeys[b]))
                            {
                                if (nword(p, what))
                                {
                                    ...
                                    fprintf(
                                        stderr,
                                        "xpixmapLoad: %s - couldn't find color "
                                        "key '%s', switching to '%s'\n",
                                        name,
                                        xpmkeys[colkey],
                                        xpmkeys[b]);
                                    colkey = b;
                ...
                {
                    fprintf(stderr,
                            "xpixmapLoad: %s - file is corrupted\n",
                            name);
                    freeCtable(ctable, ncolors);
                    freeImage(image);
                    zclose(zf);
            ...
            {
                fprintf(stderr,
                        "xpixmapLoad: %s - Bad color name '%s'\n",
                        name,
                        p);
    ...
    for (y = 0; y < h; y++)
    {
        ...
        for (x = 0; x < w; x++)
        {
            ...
            for (b = 0, val = 0; b < cpp; b++)
            {
                ...
                {
                    fprintf(stderr,
                            "xpixmapLoad: %s - Short read of X Pixmap\n",
                            name);
                    zclose(zf);
                    lfree((byte *)clookup);
                    return (image);
            ...
            if (val < cmin || val >= cmax)
            {
                fprintf(
                    stderr,
                    "xpixmapLoad: %s - Pixel data doesn't match color data\n",
                    name);
                a = 0;
            }
            a = clookup[val - cmin];
...
int
xpixmapIdent(char *fullname, char *name)
{
    ZFILE       *zf;
    char        *imagetitle;
    unsigned int w, h; /* image dimensions */
    ...
    if (imagetitle == NULL) imagetitle = dupString(name);

    printf("%s is a %dx%d X Pixmap image (Version %d) with %d colors titled "
           "'%s'\n",
           name,
           w,
           h,
