...
/* load a named image */
Image *
loadImage(ImageOptions *image_ops, boolean verbose)
{
    ...
    {
        if (errno == ENOENT) printf("%s: image not found\n", image_ops->name);
        else if (errno == EISDIR) printf("%s: directory\n", image_ops->name);
        else perror(fullname);
        return (NULL);
    }
    ...
    else
    {
        ...
        {
            image = ImageTypes[a].loader(fullname, image_ops, verbose);
            if (image)
            {
                zreset(NULL);
                return (image);
