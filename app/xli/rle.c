...
static int   background[3] = { 0, 0, 0 };   /* our background color */

int
rleIdent(char *fullname, char *name)
{
    ZFILE *rlefile;
    int    x_len, y_len;
    int    rv;
        ...
        /* now figure out the picture type */
            ...
            switch (sv_globals.sv_ncolors)
            {
                ...
                /* black and white with a map */
                        ...
                        default:
                            printf(
                                " 8 bit RLE image with an illegal color map\n");
                            return 0;
                    }
                    break;
                case 3:
                    switch (sv_globals.sv_ncmap)
                    {
                        ...
                        case 3:
                            ...
                            break;
                        default:
                            printf(" 24 bit color RLE image with an illegal "
                                   "color map\n");
                            return 0;
                    }
                    break;
                default:
                    printf(" RLE image with an illegal number (%d)of color "
                           "planes\n",
...
Image *
rleLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ...
    /* Check comments in file for gamma specification */
    {
        ...
        if ((v = rle_getcom("image_gamma", &sv_globals)) != NULL)
        {
            ...
            if (img_gam == UNSET_GAMMA) img_gam = 1.0;
            else img_gam = 1.0 / img_gam; /* convert to display gamma */
        }
        else if ((v = rle_getcom("display_gamma", &sv_globals)) != NULL)
        {
            img_gam = atof(v);
            /* Protect */
            if (UNSET_GAMMA == img_gam) img_gam = 1.0;
        }
    }
    ...
    switch (sv_globals.sv_ncolors)
    {
        case 0:
            fprintf(stderr,
                    "rleLoad: %s - no color channels to display\n",
                    name);
            zclose(rlefile);
            rle_free_setup(&sv_globals);
            return (NULL);
        case 1:
            switch (sv_globals.sv_ncmap)
            {
                ...
                case 3:
                    ptype =
                        SC_M; /* single channel encoded color with decoding map */
                    break;
                default:
                    fprintf(stderr,
                            "rleLoad: %s - Illegal number of maps for one "
                            "color channel\n",
                            name);
                    zclose(rlefile);
                    rle_free_setup(&sv_globals);
        ...
        case 3:
            switch (sv_globals.sv_ncmap)
            {
                ...
                default:
                    fprintf(stderr,
                            "rleLoad: %s - Illegal number of maps for color "
                            "picture\n",
                            name);
                    zclose(rlefile);
                    rle_free_setup(&sv_globals);
        ...
        default:
            fprintf(stderr,
                    "rleLoad: %s - too many of color channels (%d)\n",
                    name,
                    sv_globals.sv_ncolors);
            zclose(rlefile);
            rle_free_setup(&sv_globals);
    ...
    if (verbose)
    {
        ...
        if (img_gam != UNSET_GAMMA) printf(", with gamma of %4.2f\n", img_gam);
        else printf(", with unknown gamma\n");
    }
    znocache(rlefile);
    ...
    if (ptype == C_NM || ptype == C_M)
    { /* 24 bit color type result */
        depth        = 3;
    ...
    switch (ptype)
    {
        ...
        case BW_NM:
            for (j = y_len; j > 0; j--, bufp -= (x_len * depth))
            {
                ...
                if (rle_getrow(&sv_globals, bufpp) < 0) break;
            }
            break;
        case BW_M:
            for (j = y_len; j > 0; j--, bufp -= (x_len * depth))
            {
                ...
                register int            i;
                if (rle_getrow(&sv_globals, scan) < 0) break;
                for (i = x_len, r = &scan[0][0], dp = bufp; i > 0;
                     i--, r++, dp++)
                    *dp = fmaps[0][*r];
            }
            break;
