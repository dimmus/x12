...
static int
isXWD(char *name, ZFILE *zf, XWDHeader *header, int verbose)
{
    GenericXWDHeader gh;
    int              a;
    int              type = NOT_XWD;

    if (zread(zf, (byte *)&gh, sizeofGenericXWDHeader) !=
        sizeofGenericXWDHeader)
        return (0);
    ...
    {
        if (memToValLSB(gh.file_version, 4) != XWD_VERSION) return (0);
        else type = XWD_LSB;
    }
    else type = XWD_MSB;
    ...
    if (header->header_size < sizeofGenericXWDHeader) return (0);
    ...
    if (verbose)
    {
        ...
        if (type == XWD_LSB) printf(" LSB header, ");
        else printf(" MSB header, ");
        if (header->pixmap_format != ZPixmap || header->pixmap_depth == 1)
            if (header->bitmap_bit_order == LSBFirst)
                printf(" LSB bit, %d units,", header->bitmap_unit);
            else printf(" MSB bit, %d units,", header->bitmap_unit);
        if (header->byte_order == LSBFirst) printf(" LSB byte order");
...
int
xwdIdent(char *fullname, char *name)
{
    ZFILE    *zf;
    XWDHeader header;
    int       ret;
...
static Image *
loadXYBitmap(char *name, ZFILE *zf, XWDHeader header, int type)
{
    Image *image;
    ...
    if (header.bitmap_unit > 7) /* supposed to be 8, 16, or 32 but appears */
        unit =
            header.bitmap_unit / 8; /* to often be the byte count.  this will */
    else /* accept either. */ unit = header.bitmap_unit;
    xoffset = (header.xoffset / (unit * 8)) * unit;
    if (header.bytes_per_line) dlinelen = header.bytes_per_line;
    else dlinelen = unit * header.pixmap_width;
    ...
    for (y = 0; y < header.pixmap_height; y++)
    {
        ...
        if (header.bitmap_bit_order == LSBFirst) flipBits(line, dlinelen);
...
static Image *
loadXYPixmap(char *name, ZFILE *zf, XWDHeader header, int type)
{
    Image        *image;
    int           plane;
    int           dlinelen; /* length of scan line in data file */
    ...
    image    = newRGBImage(header.pixmap_width,
                        header.pixmap_height,
                        header.pixmap_depth);
...
static Image *
loadZPixmap(char *name, ZFILE *zf, XWDHeader header, int type)
{
    Image        *image;
    int           dlinelen; /* length of scan line in data file */
    ...
    else depth = header.pixmap_depth;

    pixmask = ((unsigned long)0xffffffff) >> (32 - header.pixmap_depth);
    pixlen  = image->pixlen;
    if (header.bytes_per_line) dlinelen = header.bytes_per_line;
    else dlinelen = depth * header.pixmap_width;
    ilinelen = image->width * image->pixlen;
    ...
    for (y = 0; y < header.pixmap_height; y++)
    {
        ...
        switch (depth)
        {
            ...
            case 8:
                for (x = 0; x < header.pixmap_width; x++)
                {
                    pixel = ((unsigned long)*(dptr++)) &
                            pixmask; /* loader isn't needed */
            ...
            default:
                fprintf(stderr,
                        "xwdLoad: %s - ZPixmaps of depth %d are not supported "
                        "(sorry).\n",
                        name,
                        header.pixmap_depth);
                lfree(line);
...
Image *
xwdLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ZFILE    *zf;
    ...
    if (header.pixmap_depth == 1) header.pixmap_format = XYBitmap;
    ...
    { /* color format */
        ...
        else /*  ZPixmap */ image = loadZPixmap(name, zf, header, type);
        if (image == NULL)
        {
            zclose(zf);
            return NULL;
        ...
        if (type == XWD_MSB)
            for (a = 0, cmapptr = (byte *)cmap; a < header.ncolors;
                 a++, cmapptr += sizeofXWDColor)
            {
