...
#if (defined(SYSV) || defined(VMS)) && !defined(SVR4) && !defined(__hpux) && \
    !defined(_CRAY)
...
static char *
find_text(int file, char *key, char **junk, int *jlen, int show, int delete);
...
main(int argc, char **argv)
{
    ...
    if (argv[a][0] == '-')
    {
        if (argv[a][1] == 'c')
        {
            if (argv[a][2] != '\000') text = &argv[a][2];
            else if (argc >= 3) text = argv[++a];
            else usage(pname);
            func = CHANGE;
        }
        else if (argv[a][1] == 'd')
        {
            if (argv[a][2] != '\000') usage(pname);
            func = DELETE;
        }
        else if (argv[a][1] == 'x')
        {
    ...
    for (; a < argc; a++) /* Do all the files */
    {
        ...
        if (func == XLI)
        {
            if (nameopt)
            {
                ...
                if (file != -1)
                {
                    if ((text = find_text(file, key, &junk, &jlen, 1, 0)) !=
                        NULL)
                        printf(" %s -name %s", text, fname);
                    else printf(" -name %s", fname);
                    close(file);
                    continue;
                }
                printf(" -name %s", fname);
        ...
        else if (func == SHOW)
        {
            ...
            if (file != -1)
            {
                ...
                if (close(file) != 0)
                {
                    fprintf(stderr, "%s: Error closing %s, ", pname, fname);
                    perror("");
                    exit(-1);
                }
                continue;
        ...
        else if (func == DELETE)
        {
            ...
            {
                fprintf(stderr,
                        "%s: Unable to open file '%s' for deleting options, ",
                        pname,
                        fname);
                perror("");
                exit(-1);
            ...
            if ((text = find_text(file, key, &junk, &jlen, 0, 1)) != NULL)
            {
                ...
                {
                    fprintf(stderr, "%s: Error truncating %s, ", pname, fname);
                    perror("");
                    exit(-1);
                }
            }
        ...
        else if (func == CHANGE)
        {
            ...
            {
                fprintf(stderr,
                        "%s: Unable to open file '%s' for changing options, ",
                        pname,
                        fname);
                perror("");
                exit(-1);
            ...
            if (write(file, key, klen) != klen || write(file, tt + 1, 5) != 5 ||
                write(file, text, tlen) != tlen || write(file, tt, 5) != 5 ||
                write(file, key, klen) != klen ||
                write(file, junk, jlen) != jlen)
            {
                fprintf(stderr, "%s: Error writing to file %s, ", pname, fname);
                perror("");
                exit(-1);
            }
            fsize = lseek(file, 0L, 1);
...
find_text(int file, char *key, char **junk, int *jlen, int show, int delete)
{
    ...
    if (lseek(file, -sk, 2) == -1L) goto seekerr;
    ...
    for (pp = &buf1[sk - 1];; pp--)
    {
        ...
        if (strncmp(key, pp - klen + 1, klen) == 0) break;
    }
    ...
    *jlen = count + (buf1 - pp) - 1;
    ...
    foff += count;
    if (buf[0] != '"' || !isdigit(buf[1]) || !isdigit(buf[2]) ||
    ...
    sk = tlen + klen + 10;
    if ((foff = lseek(file, -(sk < foff ? sk : foff), 1)) == -1L) goto seekerr;
...
fail:
    *jlen = 0;
    if (lseek(file, 0L, 2) != -1L) return NULL;
seekerr:
    *jlen = 0;
    if (show) return NULL;
    fprintf(stderr, "%s: Error seeking on file %s, ", pname, fname);
    perror("");
    exit(-1);
}
