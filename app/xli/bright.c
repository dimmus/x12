...
void
brighten(Image *image, unsigned int percent, unsigned int verbose)
{
    int          a;
    unsigned int newrgb;
    float        fperc;
    ...
    if (BITMAPP(image)) return;

    if (GAMMA_NOT_EQUAL(image->gamma, 1.0)) gammacorrect(image, 1.0, verbose);
...
void
gammacorrect(Image *image, float target_gam, unsigned int verbose)
{
    int          a;
    int          gammamap[256];
    unsigned int size;
    ...
    if (BITMAPP(image))
    { /* bitmap gamma looks ok on any screen */
    ...
    if (verbose)
    {
        printf("  Adjusting image gamma from %4.2f to %4.2f for image "
               "processing...",
               image->gamma,
               target_gam);
        fflush(stdout);
    ...
    switch (image->type)
    {
        case IRGB:
            for (a = 0; a < image->rgb.used; a++)
            {
                *(image->rgb.red + a) = gammamap[(*(image->rgb.red + a)) >> 8]
                                        << 8;
                *(image->rgb.green + a) =
                    gammamap[(*(image->rgb.green + a)) >> 8] << 8;
                *(image->rgb.blue + a) = gammamap[(*(image->rgb.blue + a)) >> 8]
                                         << 8;
            }
            break;
 ...
 */
void
defaultgamma(Image *image)
{
    if (BITMAPP(image))
    {  /* We can't change the gamma of a bitmap */
    ...
    if (RGBP(image))
    {  /* Assume 8 bit mapped images are gamma corrected */
    ...
    if (TRUEP(image))
    {  /* Assume a 24 bit image is linear */
        image->gamma = 1.0;
        if (globals.verbose)
        {
...
static void
setupNormalizationArray(unsigned int min,
                        unsigned int max,
                        byte        *array,
...
Image *
normalize(Image *image, unsigned int verbose)
{
    unsigned int a, x, y;
    ...
    switch (image->type)
    {
        ...
        case ITRUE:
            ...
            for (y = 0; y < image->height; y++)
                for (x = 0; x < image->width; x++)
                {
                    if (*srcptr < min) min = *srcptr;
                    if (*srcptr > max) max = *srcptr;
                    srcptr++;
                    if (*srcptr < min) min = *srcptr;
                    if (*srcptr > max) max = *srcptr;
                    srcptr++;
