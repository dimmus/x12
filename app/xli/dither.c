...
Image *
dither(Image *cimage, unsigned int verbose)
{
   ...
   */
    if (RGBP(cimage) && (cimage->depth <= 16))
    {
        ...
        for (i = 0; i < cimage->rgb.used; i++)
            grey[i] = ((int)colorIntensity(cimage->rgb.red[i],
                                           cimage->rgb.green[i],
                                           cimage->rgb.blue[i]));
    }
    else
   ...
   */
    ...
    /* dither the current line */
        if (i & 0x01) RightToLeft(curr, next, cimage->width);
        else LeftToRight(curr, next, cimage->width);
