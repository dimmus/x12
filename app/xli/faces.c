...
static int
nextInt(ZFILE *zf, unsigned int len)
{
    int c;
    int value = 0;
    int count;
        ...
        unsigned int *dp)
{
    ...
    {
        if (!strcmp(buf, "\n")) break;
        if (!strncmp(buf, "FirstName:", 10)) strcpy(fname, buf + 11);
        else if (!strncmp(buf, "LastName:", 9)) strcpy(lname, buf + 10);
...
Image *
facesLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ZFILE       *zf;
   ...
   */
    ...
    for (x = 0; x < image->rgb.size; x++)
        *(image->rgb.red + x)      = *(image->rgb.green + x) =
            *(image->rgb.blue + x) = (65536 / image->rgb.size) * x;
   ...
   */
    ...
    for (y = 0; y < h; y++)
    {
        ...
        for (x = 0; x < w; x++)
        {
            ...
            {
                fprintf(stderr,
                        "facesLoad: %s - Bad image data (returning partial "
                        "image)\n",
                        fullname);
                zclose(zf);
                return (image);
...
int
facesIdent(char *fullname, char *name)
{
    ZFILE       *zf;
    char         fname[BUFSIZ];
    char         lname[BUFSIZ];
