...
static byte root_weave_bits[] = { 0xe0, 0xb0, 0xd0, 0x70 };
...
main(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        {
            if (opid == NAME && argv[++i] == NULL) continue;

            if (nimages + 1 >= maximages)
            {
                maximages *= 2;
                images =
                    (ImageOptions *)lrealloc((byte *)images,
        ...
        {
            i +=
                doGeneralOption(opid, &argv[i], &persist_ops, &images[nimages]);
        }
        else if (isLocalOption(opid))
        {
            i += doLocalOption(opid,
                               &argv[i],
                               TRUE,
                               &persist_ops,
                               &images[nimages]);
    ...
    {
        printf("-fit and -visual options are mutually exclusive (ignoring "
               "-visual)\n");
        globals.visual_class = -1;
    }
    if (!nimages && !globals.set_default) exit(0);

    if (globals.identify)
    { /* identify the named image(s) */
        for (i = 0; i < nimages; i++)
    ...
    for (i = 0; i < nimages; i++)
    {
        ...
        {
            io->xzoom = io->yzoom =
                io->iscale < 0 ? 100 << -io->iscale : 100 >> io->iscale;
        }

        if (!(inew = loadImage(io, globals.verbose))) continue;

        first = (first < 0);

        if (inew->flags & FLAG_ISCALE) io->xzoom = io->yzoom = 0;

        if (io->gamma != UNSET_GAMMA) inew->gamma = io->gamma;
        else if (UNSET_GAMMA == inew->gamma) defaultgamma(inew);

        /* Process any other options that need doing here */
        if (io->border)
        {
            xliParseXColor(&globals.dinfo, io->border, &io->bordercol);
            xliGammaCorrectXColor(&io->bordercol, DEFAULT_DISPLAY_GAMMA);
        }
            ...
            !io->xzoom && !io->yzoom && !io->center)
        {
            ...
            io->xzoom = io->yzoom =
                (((wr < hr) ^ (globals.onroot && globals.fillscreen)) ? wr
                                                                      : hr) *
                    100 +
                0.5;
        }
        ...
        if (idisp)
        {
            if (io->center)
            {
                io->atx = (idisp->width - inew->width) / 2;
                io->aty = (idisp->height - inew->height) / 2;
            }
            if (!idisp->title)
            {
		 ...
		 */
        if (first && globals.onroot &&
            (winwidth || winheight || io->center || io->ats ||
             globals.fullscreen || globals.fillscreen))
        {
            int atx = io->atx, aty = io->aty;

            if (!winwidth) winwidth = globals.dinfo.width;
            if (!winheight) winheight = globals.dinfo.height;
        ...
        if (globals.onroot || ((i + 1 < nimages) && (images[i + 1].merge)))
            continue;

        switchval = imageInWindow(&globals.dinfo, idisp, io, argc, argv);
        ...
        /* next image */
            ...
            case 'n':
                if (i >= (nimages - 1) && globals.go_to != NULL)
                {
                    ...
                    for (j = 0; j < nimages; j++)
                    {
                        if (!strcmp(images[j].name, globals.go_to))
                        {
                            i = j - 1;
                            break;
                        }
                    ...
                    if (j >= nimages)
                        fprintf(stderr,
                                "Target for -goto %s was not found\n",
                                globals.go_to);
                }
                break;
        ...
        /* previous image */
            ...
            case '9':
                ...
                if (globals.verbose) printf("Image Gamma now %f\n", io->gamma);
                i--;
                break;
