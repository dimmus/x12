...
{
    fprintf(stderr,
            "Memory has been exhausted; Last function was %s, operation cannot "
            "continue (sorry).\n",
            globals.lastfunc);
...
information about the error is available, sorry.\n");
    ...
    if (globals._Xdebug) /* dump core if -debug is on */
        abort();
    printf("If you run xli again with the -dumpcore flag, then it will core "
           "dump\n");
...
tail(char *path)
{
    ...
    t = path;
    for (s = 0; *(path + s) != '\0'; s++)
        if (*(path + s) == '/') t = path + s + 1;
    return (t);
}

Image *
processImage(DisplayInfo *dinfo, Image *iimage, ImageOptions *options)
{
        ...
        (options->clipw != 0) || (options->cliph != 0))
    {
        tmpimage = clip(image,
                        ...
                        options);
        if (tmpimage != image && iimage != image) freeImage(image);
        image = tmpimage;
    }
    if (options->rotate)
    {
    ...
    if (options->xzoom || options->yzoom)
    {
        ...
        tmpimage =
            zoom(image, options->xzoom, options->yzoom, globals.verbose, TRUE);
        if (tmpimage != image && iimage != image) freeImage(image);
        image = tmpimage;
    }
    ...
    if (options->colors &&
        (TRUEP(image) || (RGBP(image) && (options->colors < image->rgb.used))))
    {
        tmpimage = reduce(image,
                          options->colors,
                          options->colordither,
                          UNSET_GAMMA,
                          globals.verbose);
    ...
    if (options->dither && (image->depth > 1))
    {
        ...
        if (options->dither == 1) tmpimage = dither(image, globals.verbose);
        else tmpimage = halftone(image, globals.verbose);
        if (tmpimage != image && iimage != image) freeImage(image);
        image = tmpimage;
...
#  else
    ...
    while (n != 0)
    {
        ...
        if (b != 0)
        {
            ...
            int           i;
            f = c & (((unsigned long)-1) >>
                     ((sizeof(unsigned long) - sizeof(char)) * 8));
...
initBEHexTable(void)
{
    ...
    if (BEHexTableInitialized) return;
...
initLEHexTable(void)
{
    ...
    if (LEHexTableInitialized) return;
...
int
hstoi(unsigned char *s, int n)
{
    int i, c;
    int value = 0;

    if (!LEHexTableInitialized) initLEHexTable();
    for (i = 0; i < n; i++)
    {
        c = s[i];
        c = LEHexTable[c & 0xff];
