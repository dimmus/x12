...
get_err_string(int errno)
{
    ...
    {
        if (fbm_err_strings[i].err_no == errno) return fbm_err_strings[i].name;
    }
    return "";
}
...
fbmin_open_image(ZFILE *s)
{
  ...
  /* Now extract relevant features of FBM file header */
    ...
    if (fbmin_image_test() != FBMIN_SUCCESS) return FBMIN_ERR_BAD_SD;
...
static int
fbmin_close_file(void)
{
  /* make sure there's a file open */
    if (!file_open) return FBMIN_ERR_NFO;
...
fbmin_image_test(void)
{
    ...
    {
        fprintf(stderr,
                "Invalid number of planes (%d) on input %s\n",
                fbmin_img_depth,
                "(must be 1 or 3)");
        return FBMIN_ERR_BAD_SD;
    }
    ...
    {
        fprintf(stderr,
                "Invalid number of bits (%d) on input %s\n",
                fbmin_img_bits,
                "(must be [1..8])");
        return FBMIN_ERR_BAD_SD;
    }
    ...
    {
        fprintf(stderr,
                "Invalid number of physbits (%d) on input %s\n",
                fbmin_img_physbits,
                "(must be 1 or 8)");
        return FBMIN_ERR_BAD_SD;
    }
    ...
    {
        fprintf(stderr,
                "Invalid plane length (%d) < (%d) * (%d) on input\n",
                fbmin_img_plnlen,
                fbmin_img_height,
                fbmin_img_rowlen);
        return FBMIN_ERR_BAD_SD;
...
tellAboutImage(char *name)
{
    printf("%s is a %dx%d FBM image, depth %d with %d colors\n",
           ...
           fbmin_img_width,
           fbmin_img_height,
           fbmin_img_bits * fbmin_img_depth,
           fbmin_img_clrlen / 3);
}

Image *
fbmLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ZFILE         *zf;
    ...
    if (fbmin_img_depth == 1)
        if (fbmin_img_bits == 1)
            ...
            image =
                newRGBImage(fbmin_img_width, fbmin_img_height, fbmin_img_bits);
    else /* must be 3 */
   ...
   */
    if (fbmin_img_depth == 1 && fbmin_img_clrlen > 0)
    {
        ...
        {
            fprintf(stderr,
                    "fbmLoad: %s - can't read colormap (%d bytes)\n",
                    name,
                    fbmin_img_clrlen);
            fbmin_close_file();
            lfree(cm);
    ...
    if (fbmin_img_depth == 1)
    {
        if (fbmin_img_bits == 1)
        {
            ...
            for (j = 0; j < fbmin_img_height; j++, pixptr += ((width + 7) / 8))
            {
                ...
                {
                    fprintf(stderr,
                            "fbmLoad: %s - Short read within image data\n",
                            name);
                    if (cm != NULL) lfree(cm);
                    lfree(rb);
                    fbmin_close_file();
                ...
                {
                    fprintf(stderr,
                            "fbmLoad: %s - Short read within row padding\n",
                            name);
                    if (cm != NULL) lfree(cm);
                    lfree(rb);
                    fbmin_close_file();
    ...
    {    /* depth == 3 case */
        int   xx;
        byte *rb;
        rb = (byte *)lmalloc(width > rowpad
                                 ? (width > plnpad ? width : plnpad)
        ...
        for (k = 0; k < fbmin_img_depth; k++)
        {
            ...
            {
                fprintf(stderr,
                        "fbmLoad: %s - Short read within plane padding\n",
                        name);
                if (cm != NULL) lfree(cm);
                lfree(rb);
                fbmin_close_file();
...
int
fbmIdent(char *fullname, char *name)
{
    ZFILE       *zf;
    unsigned int ret;
    int          retv;
