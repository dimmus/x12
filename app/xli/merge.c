               ...
               unsigned int cliph)
{
    ...
    for (y = 0; y < cliph; y++)
    {
        ...
        {
            if (flip)
                if (*srcpixel & srcmask) *dstpixel &= ~dstmask;
                else *dstpixel |= dstmask;
            else if (*srcpixel & srcmask) *dstpixel |= dstmask;
            else *dstpixel &= ~dstmask;
            dstmask >>= 1;
            srcmask >>= 1;
            if (dstmask == 0)
    ...
    if (globals.verbose) printf("done\n");
    return (dst);
}
          ...
          unsigned int cliph)
{
  ...
  /* Move towards our output gamma to minimize gamma changes */
    ...
    {
        if (FABS(src->gamma - globals.display_gamma) >
            FABS(dst->gamma - globals.display_gamma))
            gammacorrect(src, dst->gamma, globals.verbose);
        else gammacorrect(dst, src->gamma, globals.verbose);
    }

    switch (src->type)
    {
        case IBITMAP:
            fg         = RGB_TO_TRUE(src->rgb.red[1],
                             src->rgb.green[1],
                             src->rgb.blue[1]);
            bg         = RGB_TO_TRUE(src->rgb.red[0],
                             src->rgb.green[0],
                             src->rgb.blue[0]);
            ...
            for (y = 0; y < cliph; y++)
            {
                ...
                if (dst->pixlen == 3) /* the ususal case */
                    ...
                    for (x = 0; x < clipw; x++)
                    {
                        valToMem((*srcpixel & srcmask ? fg : bg),
                                 dstpixel,
                                 dst->pixlen);
                        dstpixel += dst->pixlen;
                        srcmask >>= 1;
...
Image *
merge(Image *idst, Image *isrc, int atx, int aty, ImageOptions *imgopp)
{
    int    clipw, cliph;
   ...
   */
    ...
    clipw = src->width;
    cliph = src->height;
    if ((atx + clipw < 0) || (aty + cliph < 0) || (atx >= (int)dst->width) ||
        (aty >= (int)dst->height)) /* not on dst, ignore */
        return dst;

    if (atx + clipw > dst->width) clipw = dst->width - atx;
    if (aty + cliph > dst->height) cliph = dst->height - aty;
   ...
   */
    ...
    if (atx < 0 || aty < 0)
    {
        ...
        if (src != tmp &&
            src != isrc) /* free imtermediate, but preserve input */
            freeImage(src);
        src = tmp;
    }
    ...
    else
    {
        ...
        {  /* convert to true */
            ...
            if (dst != tmp && dst != idst) freeImage(dst);
            dst = tmp;
        }
        outimage = anyToTrue(src,
                             dst,
                             (unsigned int)atx,
                             (unsigned int)aty,
                             clipw,
  ...
  /* do the right thing */
    if (dst != outimage && dst != idst) freeImage(dst);
    if (src != outimage && src != isrc) freeImage(src);
