...
static boolean
pcd_read(pcdHeader *hp, long unsigned int size)
{
    /* Read in the header block */
    while (size > PCDBLOCK)
    {
        if (zread(hp->zf, hp->buf, PCDBLOCK) != PCDBLOCK) return TRUE;

        size -= PCDBLOCK;
        hp->off += hp->nob;
        hp->nob = PCDBLOCK;
    }
    if (zread(hp->zf, hp->buf, size) != size) return TRUE;
    hp->off += hp->nob;
    hp->nob = size;
...
/* return TRUE on error */
static boolean
pcd_read_to_buf(pcdHeader *hp, byte *buf, long unsigned int size)
{
    if (zread(hp->zf, buf, size) != size) return TRUE;
...
/* bit fetch macros) */
static boolean
pcd_skip(pcdHeader *hp, long unsigned int toff)
{
    if (toff < pcd_offset(hp))
    {
        fprintf(stderr, "pcdLoad: internal error - tried to seek backwards\n");
        return TRUE;
    }

    if (toff > pcd_offset(hp))
        if (pcd_read(hp, toff - pcd_offset(hp))) return TRUE;
...
static boolean
read_pcdHeader(pcdHeader *hp, char *name)
{
    int i, w, h, sz;

    CURRFUNC("read_pcdHeader");
    if (zread(hp->zf, hp->buf, 32) != 32) return FALSE;

    /* See if it is consistent with a pcd files */
    for (i = 0; i < 32; i++)
        if (hp->buf[i] != 0xff) return FALSE;
    ...
    /* Read in the header block */
    if (pcd_read(hp, PCDBLOCK)) return FALSE;

    /* Image format ident string (?) */
    bcopy(&hp->buf[0x0800 & PCDBMASK], hp->magic, 7);
    hp->magic[7] = '\000';
    if (strcmp(hp->magic, "PCD_IPI") != 0) return FALSE; /* ? */
    ...
    if (hp->rotate == PCD_NO_ROTATE) w = BASEW * 4, h = BASEH * 4;
    else w = BASEH * 4, h = BASEW * 4;
    for (sz = SZ_16B;; sz--, w /= 2, h /= 2)
    {
        if (sz == SZ_B16 || (w <= hp->width && h <= hp->height &&
        ...
        (hp->yzoom != 0 && hp->yzoom != 100))
    {
        ...
        if (hp->xzoom != 0 && hp->xzoom != 100)
            tw = (int)((float)hp->width * (float)hp->xzoom / 100.0 + 0.5);
        else tw = hp->width;

        if (hp->yzoom != 0 && hp->yzoom != 100)
            th = (int)((float)hp->height * (float)hp->yzoom / 100.0 + 0.5);
        else th = hp->height;
        ...
        for (sz = SZ_B16;; sz++, w *= 2, h *= 2)
        {
            if (sz == SZ_16B || (w >= tw && h >= th))
            {
                ...
                hp->width  = w;
                hp->height = h;
                hp->xzoom  = (unsigned int)((float)tw / (float)w * 100.0 + 0.5);
                hp->yzoom  = (unsigned int)((float)th / (float)h * 100.0 + 0.5);
                break;
            }
        }
    }
...
int
pcdIdent(char *fullname, char *name)
{
    pcdHeader hdr;
...
Image *
pcdLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    pcdHeader     hdr;
    Image        *image;
    unsigned long toff;
    ...
    if (read_baseband(&hdr, toff, sf)) goto data_error;
    ...
    if (sf >= 2)
    {
        if (pcd_skip(
                &hdr,
                384 * PCDBLOCK)) /* skip to start of greater than base stuff */
            goto data_short;
        sf /= 2;
...
int
read_baseband(pcdHeader *hp, int toff, int sf)
{
    int   h, w, hw;
    byte *Lp, *C1p, *C2p;
    w  = hp->width / sf; /* width */
    ...
    if (pcd_skip(hp, toff)) goto data_short;
    ...
    {
        /* First row of luma */
        if (pcd_read_to_buf(hp, Lp, w)) goto data_short;
        Lp += w;
        ...
        /* C1 + C2 */
        if (pcd_read_to_buf(hp, C1p, hw)) goto data_short;
        C1p += hw;
        if (pcd_read_to_buf(hp, C2p, hw)) goto data_short;
        C2p += hw;
    }
    return FALSE;
 ...
 */
void
upsample(pcdHeader *hp, byte *data, int sf)
/* start point */
...
byte roundtab[768] = {
    ...
    224, 225, 226, 227, 228, 229, 230, 231, 232, 233, 234, 235, 236, 237, 237,
    238, 239, 240, 240, 241, 241, 242, 243, 243, 244, 244, 244, 245, 245, 246,
    246, 246, 247, 247, 247, 248, 248, 248, 248, 249, 249, 249, 249, 249, 250,
    250, 250, 250, 250, 251, 251, 251, 251, 251, 251, 251, 251, 252, 252, 252,
    252, 252, 252, 252, 252, 252, 252, 253, 253, 253, 253, 253, 253, 253, 253,
    253, 253, 253, 253, 253, 253, 253, 253, 254, 254, 254, 254, 254, 254, 254,
...
void
pcd_ycc_to_rgb(pcdHeader *hp, Image *image)
{
    byte          *Lp, *C1p, *C2p;
    unsigned char *odat;
    int            x, y, w, h;
    ...
    if (pcdlimit == NULL) pcd_ycc_rgb_init();
    ...
    switch (hp->rotate)
    {
        ...
        case PCD_ACLOCK_ROTATE:
            ...
            for (y = 0; y < h; y += RBLOCK)
            {
                int soff, ww, wh;
                wh = h - y;
                if (wh > RBLOCK) wh = RBLOCK;
                for (x = 0; x < w; x += RBLOCK)
                {
                    ww = w - x;
                    if (ww > RBLOCK) ww = RBLOCK;
                    soff = h - 1 - y + (x * h);
                    rot_block(Lp + soff,
                              C1p + soff,
                              C2p + soff,
...
/* Virtical output increment */
{
    ...
    for (y = 0; y < h; y++)
    {
        for (x = 0; x < w; x++)
        {
            ...
            *(op + 1) =
                pcdround[(l + pcdC1_g_tab[c1] + pcdC2_g_tab[c2]) >> SCALEBITS];
...
#define get_byte(dest, hp)                                 \
    {                                                      \
        if ((hp)->bytes_left <= 0)                         \
            if (pcd_read((hp), PCDBLOCK)) goto data_short; \
        --(hp)->bytes_left;                                \
        dest = *((hp)->bp++);                              \
    }
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (length > 12)
        {
            if (!lht)
            {
                ...
                hufftab =
                    (huff *)lrealloc((byte *)hufftab, sizeof(huff) * (1 << 16));
                for (sz = sizeof(huff) * (1 << 12);
                     sz < (sizeof(huff) * (1 << 16));
                     sz *= 2)
...
data_error:
    if (hufftab != NULL) lfree((byte *)hufftab);
    return NULL;
}
...
#define load_byte(hp)                                       \
    {                                                       \
        ...
        --(hp)->bytes_left;                                 \
        (hp)->bits_left += 8;                               \
        (hp)->bits |= ((unsigned int)(*((hp)->bp++)))       \
                      << (BITS_PER_WORD - (hp)->bits_left); \
    }

/* Get (and consume) 1 bit */
#define get1bit(dest, hp)                           \
    {                                               \
        if ((hp)->bits_left < 1) load_byte(hp);     \
...
static int
gethuffdata(pcdHeader *hp, int p, int tb, int db, int sf)
...
/* scale factor from final size */
{
    ...
    for (i = 0; i < p; i++)
        if (huffs[i] != NULL) lfree((byte *)huffs[i]);
