...
static int
getNextEventWithTimeout(Display *disp, XEvent *event)
{
#if (defined(SYSV) || defined(SVR4)) && !defined(__hpux) && !defined(_CRAY)
    struct pollfd pfd[1];
#else
    ...
    while ((AlarmWentOff == 0))
    {
        ...
        nfound = select(ConnectionNumber(disp) + 1,
                        &rmask,
                        (fd_set *)0,
                        (fd_set *)0,
                        /*(struct timeval *) */ 0);
                        ...
                        /* supplied and returned */
{
    ...
    if (winheight > height) pixy = (winheight - height) / 2;
    else
    {
...
static void
blitImage(XImageInfo  *ximageinfo,
          unsigned int width,
          unsigned int height,
          int          x,
          int          y,
...
void
cleanUpWindow(DisplayInfo *dinfo)
{
    Display *disp = dinfo->disp;
    if (ImageWindow) XDestroyWindow(disp, ImageWindow);
    ImageWindow = 0;
    if (ViewportWin) XDestroyWindow(disp, ViewportWin);
    ViewportWin = 0;
}
...
static void
setViewportColormap(Display *disp, int scrn, Visual *visual)
{
    XSetWindowAttributes swa;
    Atom                 cmap_atom = None;
...
static Visual *
bestVisualOfClassAndDepth(Display *disp,
                          int      scrn,
                          int class,
                          unsigned int depth)
{
           ...
           unsigned int *rdepth)
{
    ...
    switch (image->type)
    {
		 ...
		 */
            ...
            {
                visual =
                    bestVisualOfClassAndDepth(disp, scrn, TrueColor, depth);
                if (!visual)
                    visual = bestVisualOfClassAndDepth(disp,
                                                       scrn,
                                                       DirectColor,
                                                       depth);
            }
            ...
            {
                visual =
                    bestVisualOfClassAndDepth(disp, scrn, DirectColor, depth);
                if (!visual)
                    visual =
                        bestVisualOfClassAndDepth(disp, scrn, TrueColor, depth);
            }
            ...
            if (!visual)
                visual =
                    bestVisualOfClassAndDepth(disp, scrn, StaticColor, depth);
            if (!visual)
                visual =
                    bestVisualOfClassAndDepth(disp, scrn, GrayScale, depth);
            if (!visual)
                visual =
                    bestVisualOfClassAndDepth(disp, scrn, StaticGray, depth);
            break;
...
static void
bestVisualOfClass(Display      *disp,
                  int           scrn,
                  Image        *image,
                  int           visual_class,
                  Visual      **rvisual,
...
char
imageInWindow(DisplayInfo  *dinfo,
              Image        *image,
              ...
              char        **argv)
{
    ...
    else
    {
        ...
        if (globals.visual_class == -1)
        {
            ...
            if (globals.verbose && (visual != DefaultVisual(disp, scrn)))
        ...
        else
        {
            ...
            bestVisualOfClass(disp,
                              scrn,
                              image,
                              globals.visual_class,
                              &visual,
                              &depth);
            if (!visual)
            {
                bestVisual(disp, scrn, image, &visual, &depth);
                fprintf(stderr,
                        "Server does not support %s visual, using %s\n",
    ...
    if (ViewportWin)
    {
        ...
        else if (!options->gray && !BITMAPP(image)) private_cmap = 1;
    }
    if (!(xii = imageToXImage(disp,
                              scrn,
                              visual,
                              depth,
                              image,
    ...
    if (!ViewportWin)
    {
        ViewportWin    = XCreateWindow(disp,
        ...
        if ((proto_atom != None) && (delete_atom != None))
        {
            XChangeProperty(disp,
                            ...
                            32,
                            PropModePrepend,
                            (unsigned char *)&delete_atom,
                            1);
        }
        if (globals.focus)
        {
            XSetTransientForHint(disp, ViewportWin, atoi(getenv("WINDOWID")));
        }
        paint = 0;
    }
    else
    ...
    ImageWindow          = XCreateWindow(disp,
                                ViewportWin,
                                winx,
                                winy,
                                image->width,
    ...
    if ((DoesBackingStore(ScreenOfDisplay(disp, scrn)) == NotUseful &&
        ...
        globals.use_pixmap)
    {
        if (((pixmap = ximageToPixmap(disp, ImageWindow, xii)) == None) &&
            globals.verbose)
    ...
    if (lastx || globals.fullscreen || globals.fillscreen) sh.flags |= USSize;
    else sh.flags |= PSize;
    if (globals.fullscreen || globals.fillscreen)
    {
        sh.x = sh.y = 0;
    ...
    for (;;)
    {
        ...
        switch (event.any.type)
        {
            ...
            case KeyPress:
                {
                    ...
                    if (XLookupString(&event.key, buf, 128, &ks, &status) != 1)
                        break;
                    ret = buf[0];
                    if (isupper(ret)) ret = tolower(ret);
                    switch (ret)
                    {
                        case ' ':
                        case 'l':
                        ...
                        case '>':
                        case 'x':
                            if ('x' == ret && !globals.delete) break;

                            if (delay > 0) alarm(0);
                            cursor          = swa_view.cursor;
                            swa_view.cursor = XCreateFontCursor(disp, XC_watch);
                            XChangeWindowAttributes(disp,
                                                    ViewportWin,
                                                    CWCursor,
                                                    &swa_view);
                            XFreeCursor(disp, cursor);
            ...
            case MotionNotify:
                ...
                /*XSync(disp, FALSE); */
                while (XCheckTypedEvent(disp, MotionNotify, (XEvent *)&event) ==
                       TRUE)
                {
                    mousex = event.button.x;
                    mousey = event.button.y;
            ...
            case EnterNotify:
                if (globals.install) XInstallColormap(disp, xii->cmap);
                break;

            case LeaveNotify:
                if (globals.install) XUninstallColormap(disp, xii->cmap);
                break;
