...
static void
describe_png(char *name, png_struct *png, png_info *info)
{
    int bd, ct;
    ...
    printf("%s is a%s %ux%u %d bit deep %s PNG image",
           ...
           : (ct & PNG_COLOR_MASK_COLOR) ? "RGB"
                                         : "grayscale");
    if (ct & PNG_COLOR_MASK_ALPHA) printf(" with an alpha channel");
    if (png_get_valid(png, info, PNG_INFO_tRNS)) printf(" with transparency");
    if (png_get_valid(png, info, PNG_INFO_bKGD))
    {
        ...
        {
            printf("#%0*x%0*x%0*x", d, bg->red, d, bg->green, d, bg->blue);
        }
    }
...
static void
xli_png_read_data(png_struct *png, png_byte *data, png_size_t length)
{
    ZFILE *zfp;
...
static void
xli_png_error(png_struct *png_ptr, png_const_charp error_msg)
{
...
Image *
pngLoad(char *fullname, ImageOptions *opt, boolean verbose)
{
    ZFILE      *zfp;
    png_struct *png;
    png_info   *info;
    ...
    /* open the file */
    if (!check_png(fullname)) return (Image *)0;
    ...
    if (verbose) describe_png(fullname, png, info);
    ...
    if ((color_type & PNG_COLOR_MASK_ALPHA) ||
        (png_get_valid(png, info, PNG_INFO_tRNS) &&
         (opt->bg || png_get_valid(png, info, PNG_INFO_bKGD))))
    {
        png_color_16 bg;
        int          expand = 0;
        ...
        {
            XColor xc;
            int shift = ((color_type & PNG_COLOR_MASK_ALPHA) && 16 == bit_depth)
                            ? 0
                            : 8;
            ...
            bg.gray =
                (int)(xc.red * .299 + xc.green * .587 + xc.blue * .114 + .5) >>
                shift;
        }
    ...
    if (bit_depth < 8 && (bit_depth > 1 || PNG_COLOR_TYPE_GRAY != color_type))
    {
        if (PNG_COLOR_TYPE_GRAY == color_type) orig_depth = bit_depth;
        png_set_packing(png);
    }
    /* tell libpng to strip 16 bit depth files down to 8 bits */
    if (bit_depth > 8) png_set_strip_16(png);

    if (png_get_interlace_type(png, info)) png_set_interlace_handling(png);
    ...
    {
        assert(image->width * image->pixlen == png_get_rowbytes(png, info));
    }
...

    {
        ...
        for (i = 0; i < num_text; ++i)
        {
            ...
            if (!strcmp(text[i].key, TITLE_KEYWORD)) title = text[i].text;
        }
...
pngIdent(char *fullname, char *name)
{
    ...
    if (!check_png(fullname)) return 0;
