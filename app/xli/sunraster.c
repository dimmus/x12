...
static void
babble(char *name, struct rheader *header)
{
    printf("%s is a", name);
...
int
sunRasterIdent(char *fullname, char *name)
{
    ZFILE         *zf;
...
static boolean
sunread(ZFILE *zf, byte *buf, unsigned int len, unsigned int enc, char *name)
...

{
        ...
        while (len--)
            ...
            else
            {
                if (zread(zf, &repchar, 1) != 1)
                {
                    fprintf(
                        stderr,
                        "sunRasterLoad: %s - Bad read on encoded image data\n",
                        name);
                    return FALSE;
                }
                if (repchar == RESC)
                {
                    ...
                    {
                        fprintf(stderr,
                                "sunRasterLoad: %s - Bad read on encoded image "
                                "data\n",
                                name);
                        return FALSE;
                    }
                    if (remaining == 0) *(buf++) = RESC;
                    else
                    {
                        if (zread(zf, &repchar, 1) != 1)
                        {
                            fprintf(stderr,
                                    "sunRasterLoad: %s - Bad read on encoded "
                                    "image data\n",
                                    name);
                            return FALSE;
                        }
   ...
   */
    ...
    else
    {
        ...
        {
            fprintf(stderr,
                    "sunRasterLoad: %s - Bad read on standard image data\n",
                    name);
            return FALSE;
        }
    }
...
Image *
sunRasterLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ZFILE         *zf;
   ...
   */
    ...
    switch (depth)
    {
        ...
        default:
            fprintf(
                stderr,
                "sunRasterLoad: %s - Bad depth %d (only 1, 8, 24 are valid)\n",
                name,
                depth);
            zclose(zf);
            return NULL;
   ...
   */
    ...
    if (mapsize)
    {
        ...
        {
            fprintf(stderr,
                    "sunRasterLoad: %s - Warning, true color image colormap "
                    "being ignored\n",
                    name);
        }
        else
        { /* can handle 8 bit indexed rgb */
            if (mapsize > image->rgb.size)
            {
                fprintf(stderr,
                        "sunRasterLoad: %s - Warning, colormap is too big for "
                        "depth\n",
                        name);
   ...
   */
    ...
    else
    {
        ...
        else linelen = image->width * image->pixlen;
