...
static void
ComputeRGBMap(Box *boxes, int colors, short unsigned int *rgbmap, int ditherf);
...
static void    CopyToNewImage(Image          *inimage,
                              Image          *outimage,
...
Image *
reduce(Image *image, unsigned colors, int ditherf, float gamma, int verbose)
{
    unsigned short *rgbmap;
    ...
    Histogram = (unsigned long *)lcalloc(ColormaxI * ColormaxI * ColormaxI *
                                         sizeof(long));
    Boxes     = (Box *)lmalloc(colors * sizeof(Box));
    rgbmap    = (unsigned short *)lmalloc(ColormaxI * ColormaxI * ColormaxI *
    ...
    switch (image->type)
    {
        case IRGB:
            if (verbose)
            {
                printf("  Reducing RGB image color usage to %d colors...",
                       colors);
                fflush(stdout);
            }
        ...
        case ITRUE:
            if (verbose)
            {
                printf("  Converting true color image to RGB image with %d "
                       "colors...",
                       colors);
                fflush(stdout);
            }
        ...
        default:
            {
                lfree((unsigned char *)Histogram);
                lfree((unsigned char *)Boxes);
                lfree((unsigned char *)rgbmap);
                return (
                    image); /* not something we can reduce, thank you anyway */
            }
    }
 ...
 */
static void
QuantHistogram(Image *image, Box *box)
{
    register byte          *pixel;
...
CutBoxes(Box *boxes, int colors)
{
    ...
    boxes[0].high[REDI] = boxes[0].high[GREENI] = boxes[0].high[BLUEI] =
        ColormaxI;
    ...
    for (curbox = 1; curbox < colors;)
    {
        ...
        if (n == curbox) break; /* all are un-cutable */
        if (CutBox(&boxes[n], &boxes[curbox])) curbox++; /* cut successfully */
    }
...
BoxStats(Box *box)
{
    ...
    for (color = 0; color < 3; color++)
    {
        ...
        box->weightedvar +=
            var - box->mean[color] * box->mean[color] * (float)box->weight;
    }
...
CutBox(Box *box, Box *newbox)
{
    ...
    for (i = 0; i < 3; i++)
    {
        if (FindCutpoint(box, i, &newboxes[i][0], &newboxes[i][1]))
            totalvar[i] =
                newboxes[i][0].weightedvar + newboxes[i][1].weightedvar;
        else totalvar[i] = HUGE; /* Not cutable on that axis */
    }
	 ...
	 */
    if (totalvar[REDI] < HUGE && totalvar[REDI] <= totalvar[GREENI] &&
    ...
    else if (totalvar[GREENI] < HUGE && totalvar[GREENI] <= totalvar[REDI] &&
...
FindCutpoint(Box *box, int color, Box *newbox1, Box *newbox2)
{
    ...
    for (i = minindex; i <= maxindex; i++)
    {
        ...
        u += (float)(i * box->freq[color][i]) / (float)box->weight;
...
static void
ComputeRGBMap(Box *boxes, int colors, unsigned short *rgbmap, int ditherf)
{
    register int i;
    ...
    else
    {
		 ...
		 */
        bfill((char *)rgbmap,
              ColormaxI * ColormaxI * ColormaxI * sizeof(*rgbmap),
              0xff);
 ...
 */
static void
SetRGBmap(int boxnum, Box *box, unsigned short *rgbmap)
{
    register int r, g, b;
...
static void
find_nnearest(int rval, int gval, int bval, NN *nnp, Image *outimage)
{
    long       bdist, mdist;
    ...
    rval =
        ((rval & ~NNmaxc) << 1) + NNmaxc; /* closest color to center of cell */
...
#define QERR(r1,                                              \
             ...
             r2,                                              \
             g2,                                              \
             b2) /* sum of squares distance (in 2 * scale) */ \
    (SQUARE(r1 - (r2 >> 7)) + SQUARE(g1 - (g2 >> 7)) + SQUARE(b1 - (b2 >> 7)))
...
found_small:
    ...
    {
        if (*ds <= mdist) goto found_neighbor;
        ds++;
        if (*ds <= mdist) goto found_neighbor;
        ds++;
        if (*ds <= mdist) goto found_neighbor;
...
find_nearest(int rval, int gval, int bval, NN *nna, Image *outimage)
{
    long           bdist, ndist;
    int            i, bindex = 0;
    int            lastc;
    ...
    nnindex = ((((rval >> NNcBits) << NNBits) | (gval >> NNcBits)) << NNBits) |
              (bval >> NNcBits);
    ...
    lastc = nnp->length - 1;
    red = &nnp->red[lastc], green = &nnp->green[lastc],
    blue = &nnp->blue[lastc];

    rval = (rval & ~Colormaxc) +
           (ColormaxcI >> 1); /* closest color to center of 15 bit color */
               ...
               int             verbose)
{
    ...
    if (!ditherf)
    {
        ...
        {
            case IRGB:
                if (outimage->pixlen == 1 &&
                    inimage->pixlen == 1) /* most common */
            ...
            case ITRUE:
                if (inimage->pixlen == 3 && outimage->pixlen == 1)
                { /* common */
                    for (y = inimage->height; y > 0; y--)
                    {
                        register unsigned long temp;
    ...
    { /* else use dithering */
        ...
        if (verbose)
        {
            printf("\n  (Using color dithering to reduce the impact of "
                   "restricted colors)...");
        }
        ...
        if (UNSET_GAMMA == gamma || (gamma / inimage->gamma) == 1.0)
        ...
        else /* Create map that accounts for quantizing effect of output */
            ...
            for (x = 0; x < outimage->rgb.size; x++)
            {
                ...
                val =
                    (int)(0.5 + 255 * pow(val / 255.0, inimage->gamma / gamma));
                val =
                    (int)(0.5 + 255 * pow(val / 255.0, gamma / inimage->gamma));
                ored[x] = val;
        ...
        switch (inimage->type)
        {
            case IRGB:
                {
                    Intensity *red   = inimage->rgb.red,
                              *green = inimage->rgb.green,
                              *blue  = inimage->rgb.blue;
                    if (outimage->pixlen == 1 && inimage->pixlen == 1)
                    { /* most common */
                        for (x = inimage->width, ip = epixel; x > 0;
                             x--, pixel++)
                        {
                            *ip++ = red[*pixel] >> 8;
                            ;
                        ...
                        for (y = inimage->height; y > 0; y--)
                        {
                                 ...
                                 x--)
                            {
                                ...
                                register int           rval, gval, bval, color;

                                rgbindex = ((rval = sat[nextr]) & 0xf8)
                                           << (Bits + Bits - cBits);
                                rgbindex |= ((gval = sat[nextg]) & 0xf8)
                                            << (Bits - cBits);
                                rgbindex |= (bval = sat[nextb]) >> cBits;
                                *dpixel = color = rgbmap[rgbindex];
                                if (color ==
                                    0xffff) /* Create nearest color mapping on demand */
                                    *dpixel = color = rgbmap[rgbindex] =
                                        find_nearest(rval,
                                                     gval,
                                                     bval,
                                ...
                                rval -= ored[color];
                                nextr = *ip++ +
                                        err7[rval]; /* this line, next pixel */
                                ip[-1] = err1[rval]; /* next line, next pixel */
                                ip[-4] +=
                                    (red[*pixel] >> 8) +
                                    err5[rval]; /* next line, this pixel */
                                ip[-7] +=
                                    err3[rval]; /* next line, last pixel */
                                gval -= ogreen[color];
                                nextg = *ip++ +
                                        err7[gval]; /* this line, next pixel */
                                ip[-1] = err1[gval]; /* next line, next pixel */
                                ip[-4] +=
                                    (green[*pixel] >> 8) +
                                    err5[gval]; /* next line, this pixel */
                                ip[-7] +=
                                    err3[gval]; /* next line, last pixel */
                                bval -= oblue[color];
                                nextb = *ip++ +
                                        err7[bval]; /* this line, next pixel */
                                ip[-1] = err1[bval]; /* next line, next pixel */
                                ip[-4] +=
                                    (blue[*pixel++] >> 8) +
                                    err5[bval]; /* next line, this pixel */
                                ip[-7] +=
                                    err3[bval]; /* next line, last pixel */
                            }
                        }
                    }
                    else
            ...
            case ITRUE:
                if (inimage->pixlen == 3 && outimage->pixlen == 1)
                { /* common */
                    for (x = inimage->width * 3, ip = epixel; x > 0;
                         x--, ip++, pixel++)
                    {
                        *ip = *pixel;
                    }
                    for (y = inimage->height; y > 0; y--)
                    {
                             ...
                             x--)
                        {
                            ...
                            *dpixel = color = rgbmap[rgbindex];
                            ...
                            rval -= ored[color];
                            nextr =
                                *ip++ + err7[rval]; /* this line, next pixel */
                            ip[-1] = err1[rval]; /* next line, next pixel */
                            ip[-4] += *pixel++ +
                                      err5[rval]; /* next line, this pixel */
                            ...
                            gval -= ogreen[color];
                            nextg =
                                *ip++ + err7[gval]; /* this line, next pixel */
                            ip[-1] = err1[gval]; /* next line, next pixel */
                            ip[-4] += *pixel++ +
                                      err5[gval]; /* next line, this pixel */
                            ...
                            bval -= oblue[color];
                            nextb =
                                *ip++ + err7[bval]; /* this line, next pixel */
                            ip[-1] = err1[bval]; /* next line, next pixel */
                            ip[-4] += *pixel++ +
                                      err5[bval]; /* next line, this pixel */
        ...
        lfree((unsigned char *)nna);
    }
}

/* expand an image into a true color image */
Image *
expandtotrue(Image *image)
{
    Pixel        fg, bg;
    Image       *new_image;
    int          x, y;
    ...
    {
        case IBITMAP:
            fg      = RGB_TO_TRUE(image->rgb.red[1],
                             image->rgb.green[1],
                             image->rgb.blue[1]);
            bg      = RGB_TO_TRUE(image->rgb.red[0],
                             image->rgb.green[0],
                             image->rgb.blue[0]);
            line    = image->data;
...
Image *
expandbittoirgb(Image *image, int depth)
{
    Image       *new_image;
    int          x, y;
    byte        *spixel, *dpixel, *line;
...
Image *
expandirgbdepth(Image *image, int depth)
{
    Image *new_image;
    int    i, x, y;
    byte  *spixel, *dpixel;
    ...
    CURRFUNC("expandirgb");
    if (TRUEP(image)) return (image);
    if (BITMAPP(image)) return expandbittoirgb(image, depth);

    if (((depth + 7) / 8) == image->pixlen)
