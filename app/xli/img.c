...
static void IMG_WriteByte(unsigned char c,
                          register int  cols,
                          unsigned int  bpl);   /* Write output byte */
...
int
imgIdent(char *fullname, char *name)
{
    ZFILE     *zf;    /* Input file */
...
Image *
imgLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ...
    if ((i = IMG_ReadHeader(zf, &header)) != 0)
    {
        ...
        switch (i)
        {
            ...
            case 3:
                fprintf(stderr,
                        "imgLoad: %s - Color images are not supported (yet)\n",
                        name);
                break;
            case 4:
            ...
            default:
                fprintf(stderr,
                        "imgLoad: %s - Unsupported image pattern length\n",
                        name);
                break;
        }
        return ((Image *)NULL);
    ...
    while (RowCount < header.lines)
    {
        ...
        {   /* monochrome mode bits */
            creps = RUN_LENGTH(i);
            ichr  = (RUN_COLOR(i) == 1) ? BYTE_BLACK : BYTE_WHITE;
            for (i = 0; i < creps; i++)
...
static int
IMG_ReadHeader(ZFILE *zf, IMG_Header *h)
{
    ...
    if (tlen != (DEF_HLEN * 2)) return (1);   /* not enough data */
    ...
    if ((h->vers != DEF_VERSION) || (h->hlen < DEF_HLEN) || (h->colors < 0) ||
        (h->colors > 256) || (h->pixw < 1) || (h->pixh < 1) || (h->llen < 1) ||
        (h->llen > (MAX_SCANLINE * 8)) || (h->lines < 1) || (h->lines > 8192) ||
    ...
    if (Debug)
    {
        ...
        fprintf(stderr,
                "\t Pixel Size:  %d x %d (microns=1000th mm)\n",
                h->pixw,
                h->pixh);
        fprintf(stderr,
    ...
    if (h->colors != 1) return (3);
    if (h->llen > (MAX_SCANLINE * 8)) return (4);
    if (h->patlen > MAX_LITERAL) return (5);
...
static void
IMG_WriteByte(unsigned char c, register int cols, unsigned int bpl)
{
    register int            i;
