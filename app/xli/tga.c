...
read_tgaHeader(ZFILE *zf, tgaHeader *hp, char *name)
{
    ...
    if (zread(zf, buf, TGA_HEADER_LEN) != TGA_HEADER_LEN) return FALSE;
    ...
    if (hp->CoMapType != 0 && hp->CoMapType != 1) return FALSE;

    if (hp->ImgType != 1 && hp->ImgType != 2 && hp->ImgType != 3 &&
        hp->ImgType != 9 && hp->ImgType != 10 && hp->ImgType != 11)
        return FALSE;

    if (hp->CoSize != 0 && hp->CoSize != 15 && hp->CoSize != 16 &&
        hp->CoSize != 24 && hp->CoSize != 32)
        return FALSE;

    if (hp->PixelSize != 8 && hp->PixelSize != 16 && hp->PixelSize != 24 &&
        hp->PixelSize != 32)
        return FALSE;

    if (hp->IntrLve != 0 && hp->IntrLve != 1 && hp->IntrLve != 2) return FALSE;
    ...
    if (hp->CoMapType != 0 && hp->PixelSize > 16) return FALSE;

    /* other numbers mustn't be silly */
    if ((hp->Index + hp->Length) > 65535 || (hp->X_org + hp->Width) > 65535 ||
        (hp->Y_org + hp->Height) > 65535)
        return FALSE;

    /* setup run-length encoding flag */
    if (hp->ImgType == TGA_RLEMap || hp->ImgType == TGA_RLERGB ||
...
int
tgaIdent(char *fullname, char *name)
{
    ZFILE    *zf;
    tgaHeader hdr;
...
Image *
tgaLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ZFILE       *zf;
    tgaHeader    hdr;
    Image       *image;
    ...
    if (hdr.IDLength)
    {
        ...
        {
            fprintf(stderr,
                    "tgaLoad: %s - Short read within Identifier String\n",
                    hdr.name);
            zclose(zf);
            return NULL;
        }
    ...
    if (hdr.CoMapType != 0) /* must be 8 or 16 bit mapped type */
    {
        ...
        {
            image->rgb.red[i] = image->rgb.green[i] = image->rgb.blue[i] = 0;
        }
        switch (hdr.CoSize)
        {
            case 8:  /* Gray scale color map */
                for (; i < (hdr.Index + hdr.Length); i++)
                {
                    ...
                    {
                        fprintf(stderr,
                                "tgaLoad: %s - Short read within Colormap\n",
                                hdr.name);
                        freeImage(image);
                        zclose(zf);
                        return NULL;
    ...
    else if (hdr.PixelSize == 8)  /* Have to handle gray as pseudocolor */
    {
        ...
        for (i = 0; i < 256; i++)
        {
            image->rgb.red[i] = image->rgb.green[i] = image->rgb.blue[i] = i
                                                                           << 8;
        }
        image->rgb.used = 256;
    }
    ...
    if (hdr.IntrLve == TGA_IL_Four) hretrace = 4 * span - hdr.Width;
    else if (hdr.IntrLve == TGA_IL_Two) hretrace = 2 * span - hdr.Width;
    else hretrace = span - hdr.Width;
    ...
    for (y = hdr.Height; y > 0; y--, data += hretrace)
    {
        ...
        for (x = hdr.Width; x > 0; x--)
        {
            ...
            { /* Deal with run length encoded. */
                if (zread(zf, buf, 1) != 1) goto data_short;
                if (buf[0] & 0x80)
                { /* repeat count */
            ...
            {  /* read a pixel and decode into RGB */
                switch (hdr.PixelSize)
                {
                    ...
                    case 15:
                        if (zread(zf, buf, 2) != 2) goto data_short;
                        buf[2] = buf[0] & 0x1F;  /* B */
                        buf[3] = ((buf[1] & 0x03) << 3) +
                                 ((unsigned)(buf[0] & 0xE0) >> 5); /* G */
                        buf[4] = (unsigned)(buf[1] & 0x7C) >> 2; /* R */
                        break;

                    case 24: /* 8 bits of B G R */
                        if (zread(zf, &buf[2], 3) != 3) goto data_short;
                        break;

                    case 32: /* 8 bits of B G R + alpha */
                        if (zread(zf, &buf[2], 4) != 4) goto data_short;
                        break;
                }
                rd_count--;
            }
