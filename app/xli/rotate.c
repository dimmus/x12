 ...
 */
Image *
rotate(Image *iimage, int rotate, int verbose)
{
    ...
    for (rot = 0; rot < rotate; rot += 90)
    {
        switch (simage->type)
        {
            ...
            case IRGB:
                dimage =
                    newRGBImage(simage->height, simage->width, simage->depth);
                for (x = 0; x < simage->rgb.used; x++)
                {
                    *(dimage->rgb.red + x)   = *(simage->rgb.red + x);
                    *(dimage->rgb.green + x) = *(simage->rgb.green + x);
        ...
        if (simage != iimage) freeImage(simage);
        simage = dimage;
    }
