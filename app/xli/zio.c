...
int
zread(ZFILE *zf, byte *buf, int len)
{
    int           lentoread = len;
    ...
    while (len > 0 && !zf->eof)
    {
        ...
        {
            if (zf->nocache)
            { /* If the cache is turned off, read directly */
                if (len > 1)
                { /* more than a byte being read */
                    len -= uuread(zf, buf, len);
                    if (len > 0) zf->eof = TRUE;
                    continue;
                }
                ...
                if (zf->endptr == zf->bufptr) zf->eof = TRUE;
                continue;
            }
            /* else fall through to deal with first cached buffered read */
        }
        else
        { /* we must move on to the next cache buffer */
            if (zf->bufptr == NULL)
            { /* If this is a first re-read of the cache */
        ...
        else /* link buffer into chain */ dataptr->next = zf->dataptr;
        dataptr    = zf->dataptr;
        zf->bufptr = dataptr->buf;
...
static void
_zaptocache(ZFILE *zf, char *cbuf, int blen)
{
    struct cache *newdat, *dat;
    int           len;
    while (blen > 0)
    { /* until we've stashed all of input */
        len = blen;
        ...
        {
            for (dat = zf->data; dat->next != NULL; dat = dat->next)
                ; /* go to end of chain */
            dat->next = newdat;
            dat->eof  = FALSE;
        }
...
void
zreset(char *filename)
{
    int    a;
    ZFILE *zf;

    if (TRUE == ZForceCache) return;
    ...
    if (!filename)
    {
        for (a = 0; a < MAX_ZFILES; a++)
            if (ZFileTable[a].filename) zreset(ZFileTable[a].filename);
        return;
    }
    for (zf = ZFileTable; zf < (ZFileTable + MAX_ZFILES); zf++)
        if (zf->filename && !strcmp(filename, zf->filename)) break;
...
_zreset(ZFILE *zf)
{
    ...
    if (zf->dataptr != zf->data)
        fprintf(stderr,
                "zreset: warning: ZFILE for %s was not closed properly\n",
                zf->filename);
    while (zf->data)
    {
        old      = zf->data;
    ...
    zf->dataptr  = NULL;
    zf->nocache  = FALSE;
    if (zf->auxb != NULL && zf->auxb != zf->buf) lfree((byte *)zf->auxb);
    zf->auxb   = NULL;
    zf->bufptr = NULL;
    zf->endptr = NULL;
    zf->eof    = FALSE;
...
_zclear(ZFILE *zf)
{
    ...
    if (zf->auxb != NULL && zf->auxb != zf->buf) lfree(zf->auxb);
    ...
    for (zf = ZFileTable; zf < (ZFileTable + MAX_ZFILES); zf++)
        if (zf->filename && !strcmp(name, zf->filename))
        {
            ...
            if (zf->nocache)
            {
                ...
                {
                    fprintf(stderr,
                            "zopen: caching was disabled by previous caller; "
                            "can't reopen stdin\n");
                    return (NULL);
                }
                fprintf(stderr,
                        "zopen: warning: caching was disabled by previous "
                        "caller\n");
...
#  else /* #else its a unix compressed file, so use uncompress */
    else if ((strlen(name) > (unsigned)2) &&
...
#  endif
    ...
    while (uutry-- > 0)
    {
        if (fgets(uuibuf, UULEN, zf->stream) != NULL)
        {
            ...
            if (blen < 0)
            { /* doesn't look like reasonable junk */
                _zaptocache(zf, uuibuf, -blen);
                break;
            }
            _zaptocache(zf, uuibuf, blen); /* keep zfile data cached */
            if (!strncmp(uuibuf, "begin ", 6) && isdigit(uuibuf[6]) &&
                isdigit(uuibuf[7]) && isdigit(uuibuf[8]))
            {
                /* Found a begin line */
...
_zgetc(ZFILE *zf)
{
    ...
    if (zf->bufptr < zf->endptr) return *zf->bufptr++;
    if (zread(zf, &c, 1) > 0) return (c);
    else return (EOF);
}

char *
zgets(char *buf, unsigned int size, ZFILE *zf)
{
 ...
 */
void
zunread(ZFILE *zf, const byte *buf, int len)
{
    byte *new;
...
znocache(ZFILE *zf)
{
    if (FALSE == ZForceCache) zf->nocache = TRUE;
}

void
zforcecache(boolean v)
{
    ZForceCache = v;
}
...
zrewind(ZFILE *zf)
{
    if (zf->nocache)
    {
        ...
        {
            fprintf(stderr,
                    "zrewind: caching was disabled by previous caller; can't "
                    "rewind\n");
            return (FALSE);
        }
        fprintf(stderr,
                "zrewind: warning: caching was disabled by previous caller\n");
        return !_zreopen(zf);
    }
    zf->dataptr = zf->data;
    if (zf->auxb != NULL && zf->auxb != zf->buf) lfree(zf->auxb);
...
int
uuread(ZFILE *zf, byte *buf, int len)
{
    int cl, ld = 0;
    if (!zf->uudecode)
    {
        if (zf->type == ZSTANDARD) return fread(buf, 1, len, zf->stream);
        ...
        for (;;)
        {
            ...
            if (ld == len) break;
            buf += cl;
            if (feof(zf->stream)) break;
        }
        return ld;
    }
    for (;;)
    {
        if (zf->uunext >= zf->uuend)
        { /* if buffer empty */
            zf->uunext = zf->uubuf;
...
#define SHORTALOWANCE \
    6 /* alowance for truncation of ' 's at the end of lines */

int
uugetline(ZFILE *zf, byte *obuf)
{
    ...
    { /* until we return something */
        ...
        if (*ibuf == 'M' && (len + SHORTALOWANCE) >= ulen &&
            uuCheck(ibuf + 1, ulen <= len ? ulen : len))
        {
            if (len < ulen)
            { /* we have a slightly short line */
...
int
uuExplodeLine(byte *icp, byte *ocp)
{
    unsigned int x, y, z;
    int          i, len;
    i = len = DEC(*icp);
