...
/* ARGSUSED */
int
mcidasIdent(char *fullname, char *name)
{
    ZFILE          *zf;
    struct area_dir dir;
    int             r;
  ...
  /* a simple sanity check */
    if (dir.idate > 99999 || dir.itime > 999999) r = 0;
...
Image *
mcidasLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ZFILE            *zf;
   ...
   */
    ...
    image        = newRGBImage(dir.esiz, dir.lsiz, 8 * dir.bands);
   ...
   */
    ...
    for (y = 0; y < 255; y++)
    {
        *(image->rgb.red + y)      = *(image->rgb.green + y) =
    ...
    {
        fprintf(stderr,
                "mcidasLoad: %s - Short read within image data\n",
                name);
        zclose(zf);
        return (image);
    }
