...
void
xliDefaultDispinfo(DisplayInfo *dinfo)
{
    dinfo->width    = 0;
    dinfo->height   = 0;
    dinfo->disp     = NULL;
...
boolean
xliOpenDisplay(DisplayInfo *dinfo, char *name)
{
    Display *disp;
    int      scrn;
    if (!(disp = XOpenDisplay(globals.dname))) return FALSE; /* failed */
...
char *
xliDisplayName(char *name)
{
    return XDisplayName(name);
}

void
xliCloseDisplay(DisplayInfo *dinfo)
{
...
int
errorHandler(Display *disp, XErrorEvent *error)
{
    char errortext[BUFSIZ];

    XGetErrorText(disp, error->error_code, errortext, BUFSIZ);
    fprintf(stderr,
            "xli: X Error: %s on 0x%lx\n",
            errortext,
            error->resourceid);
    if (globals._Xdebug) abort();
    else return (0);
}

/* Return the default visual class */
int
xliDefaultVisual(void)
{
...
void
tellAboutDisplay(DisplayInfo *dinfo)
{
    Screen *screen;
    int     a, b;
    if (dinfo->disp)
    {
        ...
        printf("Server: %s Version %d\n",
               ServerVendor(dinfo->disp),
               VendorRelease(dinfo->disp));
        ...
        for (a = 0; a < screen->ndepths; a++)
        {
            ...
            for (b = 0; b < screen->depths[a].nvisuals; b++)
                printf(" %s",
                       nameOfVisualClass(screen->depths[a].visuals[b].class));
            printf("\n");
        }
    }
    else
...
int
xliParseXColor(DisplayInfo *dinfo, char *spec, XColor *xcolor)
{
    int stat;

    if (dinfo->disp)
    {
        stat = XParseColor(globals.dinfo.disp,
                           globals.dinfo.colormap,
                           spec,
                           xcolor);
    }
    else
...
static int
saParseXColor(DisplayInfo *dinfo, char *spec, XColor *xcolor)
{
    int                  i, n, r, g, b;
    ...
    if (n > 0)
    {
        for (i = 0; i < n; i++)
        {
            if (isupper(spec[i])) spec[i] = tolower(spec[i]);
        }
        if (spec[0] == '#')
        {
            switch (n - 1)
...
void
xliGammaCorrectXColor(XColor *xlicolor, double gamma)
{
    xlicolor->red =
        (int)(0.5 + 65535.0 * pow((double)xlicolor->red / 65535.0, gamma));
    xlicolor->green =
        (int)(0.5 + 65535.0 * pow((double)xlicolor->green / 65535.0, gamma));
    xlicolor->blue =
        (int)(0.5 + 65535.0 * pow((double)xlicolor->blue / 65535.0, gamma));
}
