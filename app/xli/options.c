...
OptionId
optionNumber(char *arg)
{
    int a, b;

    if ((*arg) != '-') return (OPT_NOTOPT);
...
help(char *option)
{
    ...
    if (option)
    {
        if (!helpOnOption(option)) listOptions();
        printf("\
...
help facility.\n");
    for (;;)
    {
        printf("help> ");
        buf[BUFSIZ - 1] = '\0';
        if (fgets(buf, BUFSIZ - 1, stdin) == NULL) break; /* EOF */
        ...
        if (!strcmp(buf, "?"))
            ;
        else if (!strcmp(buf, "quit") || !strcmp(buf, ".")) exit(0);
        else if (!strcmp(buf, "`?'")) literalMindedUser("the question mark");
        ...
        else if (helpOnOption(buf)) continue;
        listOptions();
        printf("\
                ...
                ImageOptions *image_ops)
{
    ...
    switch (opid)
    {
        ...
        case DELAY:
            if (!argv[++a]) break;
            persist_ops->delay = image_ops->delay = atoi(argv[a]);
        ...
        case DISPLAY:
            if (argv[++a]) globals.dname = argv[a];
            break;

        case DISPLAYGAMMA:
            if (argv[++a]) globals.display_gamma = atof(argv[a]);
            break;
        ...
        case GEOMETRY:
            if (argv[++a]) globals.user_geometry = argv[a];
            break;

        case GOTO:
            if (argv[++a]) globals.go_to = argv[a];
            break;
        ...
        case VISUAL:
            if (argv[++a]) globals.visual_class = visualClassFromName(argv[a]);
            break;
              ...
              ImageOptions *image_ops)
{
    ...
    switch (opid)
    {
        case AT:
            if (!argv[++a]) break;
            if (sscanf(argv[a], "%d,%d", &image_ops->atx, &image_ops->aty) != 2)
            {
                printf("Bad argument to -at\n");
                usage(globals.argv0);
            /* NOTREACHED */
            }
            else image_ops->ats = TRUE;
            break;

        case BACKGROUND:
            if (argv[++a]) image_ops->bg = argv[a];
            break;

        case BORDER:
            if (argv[++a]) image_ops->border = argv[a];
            if (setpersist) persist_ops->border = image_ops->border;
            break;
        ...
        case BRIGHT:
            if (argv[++a])
            {
                ...
                if (setpersist) persist_ops->bright = image_ops->bright;
            }
            break;
        ...
        case GAMMA:
            if (argv[++a])
            {
                ...
                if (setpersist) persist_ops->gamma = image_ops->gamma;
            }
            break;

        case GRAY:
            image_ops->gray = 1;
            if (setpersist) persist_ops->gray = 1;
            break;
        ...
        case COLORDITHER:
            ...
            if (setpersist) persist_ops->colordither = 1;
            break;
        ...
        case COLORS:
            ...
            if (setpersist) persist_ops->colors = image_ops->colors;
            break;

        case DITHER:
            image_ops->dither = 1;
            if (setpersist) persist_ops->dither = 1;
            break;

        case EXPAND:
            image_ops->expand = 1;
            if (setpersist) persist_ops->expand = 1;
            break;

        case FOREGROUND:
            if (argv[++a]) image_ops->fg = argv[a];
            break;

        case HALFTONE:
            image_ops->dither = 2;
            if (setpersist) persist_ops->dither = 2;
            break;
        ...
        case ISCALE:
            if (argv[++a])
            {
                ...
                if (setpersist)
                {
                    persist_ops->iscale_auto = image_ops->iscale_auto;
                }
            }
            break;
        ...
        case NORMALIZE:
            ...
            if (setpersist) persist_ops->normalize = image_ops->normalize;
            break;
        ...
        case ROTATE:
            ...
            {
                printf(
                    "Argument to -rotate must be a multiple of 90 (ignored)\n");
                image_ops->rotate = 0;
            }
            else
        ...
        case SMOOTH:
            ...
            if (setpersist) persist_ops->smooth = image_ops->smooth;
            break;

        case TITLE:
            if (argv[++a]) image_ops->title = argv[a];
            break;
        ...
        case XZOOM:
            if (argv[++a])
            {
                ...
                if (setpersist) persist_ops->xzoom = image_ops->xzoom;
            }
            break;
        ...
        case YZOOM:
            if (argv[++a])
            {
                ...
                if (setpersist) persist_ops->yzoom = image_ops->yzoom;
            }
            break;
...
void
read_trail_opt(ImageOptions *image_ops,
               ZFILE        *file,
               Image        *image,
               boolean       verbose)
{
    CURRFUNC("read_trail_opt");
    if (!image_ops->done_to)
    { /* then try and read trailing options */
        ...
        for (pp = buf;; pp++)
        {
            for (; pp < &buf[nib] && *pp != sc; pp++)
                ;
            if (pp > &buf[nib - klen]) goto fail;
            if (strncmp(key, pp, klen) == 0) break;
        }
        ...
        if (!isdigit(buf[klen]) || !isdigit(buf[klen + 1]) ||
            !isdigit(buf[klen + 2]) || !isdigit(buf[klen + 3]) ||
            buf[klen + 4] != '"')
            goto fail;
        buf[klen + 4] = '\000';
        tlen          = atoi(&buf[klen]);
        ...
        if (strncmp(key, &buf[tlen + klen + 10], klen) != 0) goto fail;
        ti = klen + 5 + tlen;
        if (buf[ti] != '"' || !isdigit(buf[ti + 1]) || !isdigit(buf[ti + 2]) ||
            !isdigit(buf[ti + 3]) || !isdigit(buf[ti + 4]))
            goto fail;
        buf[ti + 5] = '\000';
        if (tlen != atoi(&buf[ti + 1])) goto fail;
...
notfail:;
    ...
    if (image_ops->to_argv && verbose)
    { /* remaind user that there are trailing options */
        int a;
        {
 ...
 */
static int
parse_args(char *as, int sl, char **argv)
{
    ...
    for (no = 0, sp = as; sp < as + sl; sp++)
    {
        switch (state)
        {
            case WSPACE:
                ...
                state = ARG;
                no++;
                if (argv != NULL) *argv++ = sp; /* point to start of string */
                break;
            case ARG:
                if (!isspace(*sp)) break;
                state = WSPACE;
...
nameOfVisualClass(int class)
{
    ...
    for (a = 0; VisualClassName[a].name; a++)
        if (VisualClassName[a].class == class) return (VisualClassName[a].name);
