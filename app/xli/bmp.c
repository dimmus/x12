...
#define GULONG4(bp)                                          \
    ((unsigned long)(bp)[0] + 256 * (unsigned long)(bp)[1] + \
...
static boolean
read_bmpHeader(ZFILE *zf, bmpHeader *hp, char *name)
{
    unsigned char buf[WIN_INFOHEADER_LEN]; /* largest we'll need */

    if (zread(zf, buf, BMP_FILEHEADER_LEN) != BMP_FILEHEADER_LEN) return FALSE;

    if (buf[0] != 'B' || buf[1] != 'M') return FALSE; /* bad magic number */
    ...
    if (zread(zf, buf, 4) != 4) return FALSE;

    hp->biSize = GULONG4(&buf[0]);
    if (hp->biSize == WIN_INFOHEADER_LEN) hp->class = C_WIN;
    else if (hp->biSize == OS2_INFOHEADER_LEN) hp->class = C_OS2;
    else return FALSE;
    ...
    if (hp->biBitCount != 1 && hp->biBitCount != 4 && hp->biBitCount != 8 &&
        hp->biBitCount != 24)
    {
        fprintf(stderr, "bmpLoad: %s - Illegal image BitCount\n", hp->name);
        return FALSE;
    }
    if ((hp->biCompression != BI_RGB && hp->biCompression != BI_RLE8 &&
    ...
    if (hp->biBitCount < 24)
    {
        if (hp->biClrUsed == 0 || hp->biClrUsed > (1 << hp->biBitCount))
...
int
bmpIdent(char *fullname, char *name)
{
    ZFILE    *zf;
    bmpHeader hdr;
...
Image *
bmpLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ZFILE    *zf;
    bmpHeader hdr;
    Image    *image;
    ...
    if (hdr.biBitCount < 24)
    {
        ...
        else image = newRGBImage(hdr.biWidth, hdr.biHeight, hdr.biBitCount);
        image->title = dupString(hdr.name);

        if (hdr.class == C_WIN) n++;
        for (i = 0; i < hdr.biClrUsed; i++)
        {
            if (zread(zf, buf, n) != n)
            {
                fprintf(stderr,
                        "bmpLoad: %s - Short read within Colormap\n",
                        hdr.name);
                freeImage(image);
                zclose(zf);
                return NULL;
    ...
    {  /* else must be a true color image */
    ...
    if (hdr.class == C_WIN)
        skip = hdr.bfOffBits -
               (BMP_FILEHEADER_LEN + WIN_INFOHEADER_LEN + 4 * hdr.biClrUsed);
    else
        skip = hdr.bfOffBits -
               (BMP_FILEHEADER_LEN + OS2_INFOHEADER_LEN + 3 * hdr.biClrUsed);

    while (skip > 0)
    {
        if (zgetc(zf) == EOF) goto data_short;
        skip--;
    }
    ...
    else if (hdr.biBitCount == 4)
    {
        ...
        {
            int d, e;
            bzero((char *)image->data, image->width * image->height);
            for (x = y = 0;;)
            {
                int i, f;
                if ((d = zgetc(zf)) == EOF) goto data_short;
                if (d != 0)
                { /* run of pixels */
                    x += d;
                    if (x > image->width || y > image->height)
                    {
                        ...
                        /* ignore this run */
                        x -= d;
                        if ((e = zgetc(zf)) == EOF) goto data_short;
                        continue;
                    }
                    if ((e = zgetc(zf)) == EOF) goto data_short;
                    f = e & 0xf;
                ...
                if (d == 2)
                { /* delta */
                    if ((d = zgetc(zf)) == EOF || (e = zgetc(zf)) == EOF)
                        goto data_short;
                    x += d;
                    data += d;
                    y += e;
    ...
    else if (hdr.biBitCount == 8)
    {
        ...
        if (hdr.biCompression == BI_RLE8)
        {
            ...
            for (x = y = 0;;)
            {
                ...
                if (zread(zf, data, d) != d) goto data_short;
                data += d;
