...
#define PUSH_PIXEL(p)                                    \
    {                                                    \
        if (pstk_idx == PSTK_SIZE) return GIFIN_ERR_PSO; \
        else pstk[pstk_idx++] = (p);                     \
    }
...
#define GIFIN_PUSH_STRING(code)                                         \
    {                                                                   \
        ...
        ps_code = code;                                                 \
                                                                        \
        while (((unsigned int)ps_code) < STAB_SIZE &&                   \
               prefix[ps_code] != NULL_CODE)                            \
        {                                                               \
            PUSH_PIXEL(extnsn[ps_code]);                                \
            ps_code = prefix[ps_code];                                  \
        }                                                               \
                                                                        \
        if (((unsigned int)ps_code) >= STAB_SIZE) return GIFIN_ERR_TAO; \
        PUSH_PIXEL(extnsn[ps_code]);                                    \
        ps_rslt = extnsn[ps_code];                                      \
        if (((unsigned int)ps_rslt) >= STAB_SIZE) return GIFIN_ERR_TAO; \
    }
...
get_err_string(int errno)
{
    ...
    {
        if (gif_err_strings[i].err_no == errno) return gif_err_strings[i].name;
    }
    return "";
}
...
static int
gifin_load_cmap(BYTE (*cmap)[256], int ncolors)
{
    int i;

    for (i = 0; i < ncolors; i++)
    {
        if (zread(ins, buf, 3) != 3) return GIFIN_ERR_EOF;
...
gifin_open_file(ZFILE *s)
{
    ...
    /* check GIF signature */
    if (zread(ins, buf, GIF_SIG_LEN) != GIF_SIG_LEN) return GIFIN_ERR_EOF;

    buf[GIF_SIG_LEN] = '\0';
    if (strcmp((char *)buf, GIF_SIG) == 0) gifin_version = GIF87a;
    else if (strcmp((char *)buf, GIF_SIG_89) == 0) gifin_version = GIF89a;
    else return GIFIN_ERR_BAD_SIG;

    /* read screen descriptor */
    if (zread(ins, buf, GIF_SD_SIZE) != GIF_SD_SIZE) return GIFIN_ERR_EOF;
    ...
    {
        if (buf[4] & 0x08 || buf[6] != 0) return GIFIN_ERR_BAD_SD;
    }
    else
    {
        gifin_g_cmap_sorted = ((buf[4] & 0x08) != 0);
        if (buf[6] != 0) gifin_aspect = ((double)buf[6] + 15.0) / 64.0;
    }
    ...
    if (gifin_g_cmap_flag)
    {
        ...
        if ((errno = gifin_load_cmap(gifin_g_cmap, gifin_g_ncolors)) !=
            GIFIN_SUCCESS)
            return errno;
    }
    else
...
static int
gifin_read_data_block(void)
{
    /* read the data block header */
    buf_cnt = zgetc(ins);

    /* read the data block body */
    if (zread(ins, buf, buf_cnt) != buf_cnt) return GIFIN_ERR_EOF;
...
gifin_skip_extension(void)
{
    ...
    do
    {
        if ((errno = gifin_read_data_block()) != GIFIN_SUCCESS) return errno;
    }
    while (buf_cnt > 0);
...
gifin_open_image(void)
{
    ...
    do
    {
        ...
        {
            if ((errno = gifin_skip_extension()) != GIFIN_SUCCESS) return errno;
        }
    }
    ...
    if (zread(ins, buf, GIF_ID_SIZE) != GIF_ID_SIZE) return GIFIN_ERR_EOF;
    ...
    if (gifin_l_cmap_flag)
    {
        ...
        if ((errno = gifin_load_cmap(gifin_l_cmap, gifin_l_ncolors)) !=
            GIFIN_SUCCESS)
            return errno;
    }
    else
...
gifin_add_string(int p, int e)
{
    ...
    table_size += 1;
    if (table_size > STAB_SIZE) return GIFIN_ERR_TAO;
    return GIFIN_SUCCESS;
}
...
{
    printf("%s is a %dx%d %s%s image with %d colors\n",
           name,
           gifin_img_width,
           gifin_img_height,
...
gifLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ...
    if (verbose) tellAboutImage(name);
    znocache(zf);
    image = newRGBImage(
        gifin_img_width,
        gifin_img_height,
        ...
        for (pass = 0; pass < 4; pass++)
        {
            ...
            while (y < gifin_img_height)
            {
                ...
                for (x = 0; x < gifin_img_width; x++)
                {
                    ...
                    {
                        fprintf(stderr,
                                "gifLoad: %s - Short read within image data, "
                                "'%s'\n",
                                name,
   ...
   */
    ...
    zclose(zf);
    if (gifin_aspect != 1.0)
    { /* correct for GIF89a aspect ratio */
        Image *timage;
        printf("  Correcting for GIF89a pixel aspect ratio of %2.4f...",
               gifin_aspect);
        if (gifin_aspect < 1.0) /* tall pixels - image will look too short */
            timage =
                zoom(image, 0, (int)(100.0 / gifin_aspect + 0.5), FALSE, FALSE);
        else /* wide pixels - image will look too thin */
            timage =
                zoom(image, (int)(100.0 * gifin_aspect + 0.5), 0, FALSE, FALSE);
        if (timage != image) freeImage(image);
        image = timage;
        printf("done\n");
    }
    return (image);
