...
static void
xli_jpg_error_exit(j_common_ptr cinfo)
{
...
static void
xli_jpg_init_src(j_decompress_ptr cinfo)
{}

static boolean
xli_jpg_fill_buf(j_decompress_ptr cinfo)
{
...
static void
xli_jpg_skip(j_decompress_ptr cinfo, long int n)
{
    xli_jpg_src *src = (xli_jpg_src *)cinfo->src;
...
static void
xli_jpg_term(j_decompress_ptr cinfo)
{
    ...
    {
        zunread(src->zfp,
                (byte *)src->pub.next_input_byte,
                src->pub.bytes_in_buffer);
    }
}

static void
xli_jpg_zio_src(j_decompress_ptr cinfo, ZFILE *zfp)
{
    xli_jpg_src *src;
...
static void
describe_jpeg(j_decompress_ptr cinfo, char *filename)
{
    printf("%s is a %dx%d JPEG image, color space ",
           filename,
           cinfo->image_width,
           cinfo->image_height);
...
static unsigned int
xli_jpg_getc(j_decompress_ptr cinfo)
{
    struct jpeg_source_mgr *datasrc = cinfo->src;

    if (datasrc->bytes_in_buffer == 0) (*datasrc->fill_input_buffer)(cinfo);
...
static boolean
xli_jpg_com(j_decompress_ptr cinfo)
{
    long         length;
    unsigned int ch;
...
Image *
jpegLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ZFILE                        *zfp;
    ...
    if (verbose) jpeg_set_marker_processor(&cinfo, JPEG_COM, xli_jpg_com);

    jpeg_read_header(&cinfo, TRUE);
    if (verbose) describe_jpeg(&cinfo, fullname);
    ...
    {
        image_ops->iscale = 0;
        while (image_ops->iscale < 3 &&
               (cinfo.image_width >> image_ops->iscale >
                    globals.dinfo.width * .9 ||
                cinfo.image_height >> image_ops->iscale >
                    globals.dinfo.height * .9))
            image_ops->iscale += 1;
        cinfo.scale_denom = 1 << image_ops->iscale;
        if (verbose) printf("auto-scaling to 1/%d\n", cinfo.scale_denom);
    }
    znocache(zfp);
    ...
    if (cinfo.scale_denom > 1) image->flags |= FLAG_ISCALE;
