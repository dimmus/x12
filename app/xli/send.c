...
Pixmap
ximageToPixmap(Display *disp, Window parent, XImageInfo *xii)
{
    int (*old_handler)();
    Pixmap pixmap;
    ...
    XSync(disp, False);
    pixmap = XCreatePixmap(disp,
                           parent,
                           xii->ximage->width,
                           xii->ximage->height,
                           xii->depth);
    (void)XSetErrorHandler(old_handler);
    if (GotError) return (None);
    xii->drawable = pixmap;
...
static int
shmErrorTrap(Display *d, XErrorEvent *ev)
{
    GotError = 1;
    if (ev->error_code != BadAccess) pixmapErrorTrap(d, ev);

    return 0;
}

static void
createImage(XImageInfo *xii,
            Image      *image,
            Visual     *visual,
            int         depth,
            int         format)
{
    ...
    {
        xii->ximage    = XShmCreateImage(xii->disp,
                                      visual,
                                      depth,
                                      format,
                                      NULL,
        ...
        if (xii->shm.shmid >= 0)
        {
            ...
            xii->ximage->data = xii->shm.shmaddr = shmat(xii->shm.shmid, 0, 0);
    ...
    xii->ximage       = XCreateImage(xii->disp,
                               visual,
                               depth,
                               format,
                               0,
                               ...
                               8,
                               0);
    xii->ximage->data = lmalloc(image->height * xii->ximage->bytes_per_line);
}

XImageInfo *
imageToXImage(Display      *disp,
              int           scrn,
              Visual       *visual,
              unsigned int  ddepth,
              Image        *image,
      ...
      /* goody goody */
                ...
                default:
                    if (visual->map_entries > 2)
                    {
                        ...
                        if (ncols >= 256)
                            ncols -=
                                DEFAULT_COLORS; /* allow for some default colors */
                        dimage = reduce(image,
                                        ncols,
                                        options->colordither,
                                        display_gamma,
...
retry_direct: /* tag we hit if a DirectColor allocation fails on
       ...
       */
                ...
                {
                    if (pixval & visual->red_mask) redcolors <<= 1;
                    if (pixval & visual->green_mask) greencolors <<= 1;
                    if (pixval & visual->blue_mask) bluecolors <<= 1;
                }
       ...
       */
                ...
                {
                    fprintf(stderr,
                            "Warning: inconsistency in color information (this "
                            "may be ugly)\n");
                }
                ...
                {
                    if (redbottom < 256) redtop = redbottom + redstep;
                    if (greenbottom < 256) greentop = greenbottom + greenstep;
                    if (bluebottom < 256) bluetop = bluebottom + bluestep;
	   ...
	   */
                        ...
                        fprintf(stderr,
                                "imageToXImage: XAllocColor failed on a "
	 ...
	 */
                    ...
                    while ((greenbottom < 256) && (greenbottom < greentop))
                        greenvalue[greenbottom++] =
                            xcolor.pixel & visual->green_mask;
                    while ((bluebottom < 256) && (bluebottom < bluetop))
                        bluevalue[bluebottom++] =
                            xcolor.pixel & visual->blue_mask;
                }
            }
            break;
...
retry: /* this tag is used when retrying because we couldn't get a fit */
            xii->index = (Pixel *)lmalloc(sizeof(Pixel) *
                                          (image->rgb.used + NOFLASH_COLORS));
     ...
     */
            ...
            if (private_cmap)
            { /* user asked us to use a private cmap */
                newmap = 1;
                fit    = 0;
            }
       ...
       */
                ...
                for (a = 0; a < image->rgb.used; a++)
                {
                    ...
                    if (!XAllocColor(disp, xii->cmap, &xcolor))
                    {
                        if ((visual->class == StaticColor) ||
                            (visual->class == StaticGray))
                        {
                            printf("imageToXImage: XAllocColor failed on a "
                                   "static visual\n");
                            xii->no = a;
         ...
         */
                    ...
                    for (a = 0; a < image->rgb.used;
                         a++) /* count entries we got */
                        if (!XAllocColorCells(disp,
                                              xii->cmap,
                                              False,
                                              NULL,
                                              0,
                    ...
                    if (a <= 2)
                    {
                        ...
                        if (verbose)
                        {
                            printf("  Cannot fit into default colormap, "
                                   "dithering...");
                            fflush(stdout);
                        }
                    ...
                    if (a < image->rgb.used)
                    { /* if image has too many colors, reduce it */
                        Image *dimage;
                        dimage = reduce(image,
                                        a,
                                        options->colordither,
                    ...
                    XGrabServer(
                        disp); /* stop someone else snarfing the colors */
                    XFreeColors(disp, xii->cmap, xii->index, a, 0);
                    for (a = 0; a < image->rgb.used; a++)
                    {
                        if (dogamma)
	   ...
	   */
                        ...
                        {
                            printf(
                                "XAllocColor failed while fitting colormap!\n");
                            XUngrabServer(disp);
                            xii->no = a;
                            freeXImage(image, xii);
                            return (NULL);
       ...
       * the colors writable.
       */
                    if (verbose) printf("  Using private colormap\n");
	 ...
	 */
                    ...
                    {
                        fprintf(stderr,
                                "imageToXImage: Color allocation failed!\n");
                        lfree((byte *)xii->index);
                        xii->index = NULL;
                        freeXImage(image, xii);
                        return (NULL);
   ...
   */
        ...
        case IBITMAP:
            {
                ...
                dst      = (unsigned char *)xii->ximage->data;
                src      = image->data;
     ...
     */
            ...
            dbits =
                bitsPerPixelAtDepth(disp, scrn, ddepth); /* bits per pixel */
            xii->depth = ddepth;
                ...
                case TrueColor:
                    {
                        ...
                        src  = image->data;
                        data = (unsigned char *)xii->ximage->data;
                        for (y = image->height; y--;)
                        {
                            for (p = pixels, x = image->width; x--;
                                 src += image->pixlen, ++p)
                            ...
                            if (dogamma)
                            {
                                ...
                                {
                                    *p =
                                        RGB_TO_TRUE(GAMMA8(TRUE_RED(*p)) << 8,
                                                    GAMMA8(TRUE_GREEN(*p)) << 8,
                                                    GAMMA8(TRUE_BLUE(*p)) << 8);
                                }
                            }
                            for (p = pixels, x = image->width; x--; ++p)
                            {
                                *p = redvalue[TRUE_RED(*p)] |
                                     greenvalue[TRUE_GREEN(*p)] |
       ...
       */
                    ...
                    else
                    {
                        ...
                        for (y = image->height; y--;)
                        {
                            ...
                            {
                                for (x = image->width; x--;
                                     src += image->pixlen, dst += dpixlen)
                                {
                                    valToMem(
                                        xii->index[c + memToVal(src,
                                                                image->pixlen)],
                                        dst,
                                        dpixlen);
                                }
    ...
    if (verbose && dogamma)
        printf("  Have adjusted image from %4.2f to display gamma of %4.2f\n",
               image->gamma,
               display_gamma);
    if (image != orig_image) freeImage(image);
    return (xii);
}
           ...
           unsigned    h)
{
    ...
    if (!xii->gc)
    {
        ...
        if (xii->ximage->depth == 1)
        {
            ...
            xii->gc        = XCreateGC(xii->disp,
                                xii->drawable,
                                GCFunction | GCForeground | GCBackground,
                                &gcv);
        }
        else xii->gc = XCreateGC(xii->disp, xii->drawable, GCFunction, &gcv);
    }

    if (src_x < 0 || src_y < 0 || src_x + w > xii->ximage->width ||
        src_y + h > xii->ximage->height)
        return;

    if (xii->shm.shmid >= 0)
    {
        XShmPutImage(xii->disp,
                     xii->drawable,
                     xii->gc,
                     xii->ximage,
                     src_x,
