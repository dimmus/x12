...
doSmooth(Image *isrc)
{
   ...
   */
    ...
    if (src != tmp && src != isrc) freeImage(src);
    src = tmp;
...
Image *
smooth(Image *isrc, int iterations, int verbose)
{
    int    a;
    Image *src = isrc, *tmp;

    if (GAMMA_NOT_EQUAL(src->gamma, 1.0)) gammacorrect(src, 1.0, verbose);
