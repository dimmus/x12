...
readPathsAndExts(char *name)
{
    ...
    while (fscanf(f, fmt, tokenbuf) > 0)
    {
        ...
        {
            secnum = PATHSECTION;
            if (sscanf(tokenbuf + strlen(PathToken), "%s", buf) != 1) continue;
        }
        else if (!strncmp(tokenbuf, ExtToken, strlen(ExtToken)))
        {
            secnum = EXTSECTION;
            if (sscanf(tokenbuf + strlen(ExtToken), "%s", buf) != 1) continue;
        }
        else
        {
            strcpy(buf, tokenbuf);
        ...
        switch (secnum)
        {
            ...
            case PATHSECTION:
                if (NumPaths < BUFSIZ - 1) Paths[NumPaths++] = expandPath(buf);
                else
                {
            ...
            case EXTSECTION:
                if (NumExts < BUFSIZ - 1) Exts[NumExts++] = dupString(buf);
                else
                {
...
findImage(char *name, char *fullname)
{
    ...
    if (!stat(fullname, &sbuf)) return (fileIsOk(fullname, &sbuf));
#ifndef NO_COMPRESS
    strcat(fullname, ".Z");
    if (!stat(fullname, &sbuf)) return (fileIsOk(fullname, &sbuf));
#endif
