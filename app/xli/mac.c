...
macin_open_image(ZFILE *s)
{
  ...
  /* read in the mac file header */
    ...
    if (mhdr[0] != MAC_MAGIC) return MACIN_ERR_BAD_SD;
  ...
  /* Get image name  (if available) */
    ...
    else    /* else read rest of header */
        if (zread(ins, (byte *)hp, MAC_HDR_LEN - ADD_HDR_LEN) !=
            MAC_HDR_LEN - ADD_HDR_LEN)
            return MACIN_ERR_EOF;
...
static int
macin_close_file(void)
{
  /* make sure there's a file open */
    if (!file_open) return MACIN_ERR_NFO;
...
int
macIdent(char *fullname, char *name)
{
    ZFILE       *zf;
    unsigned int ret;
