...
static boolean
isXbitmap(ZFILE *zf, unsigned int *w, unsigned int *h, int *v10p, char **title)
{
    char  line[MAX_SIZE];
  ...
  /* get width/height values */
     ...
     */
        ...
        {
            if (!(type = rindex(name_and_type, '_'))) type = name_and_type;
            else type++;

            if (!strcmp("width", type)) *w = (unsigned int)value;
            if (!strcmp("height", type)) *h = (unsigned int)value;
        }
...
Image *
xbitmapLoad(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ZFILE       *zf;
    ...
    if (v10p)
    {
        ...
        for (y = 0; y < h; y++)
        {
            for (x = 0; x < dlinelen; x++)
            {
                ...
                {
                    fprintf(stderr,
                            "xbitmapLoad: %s - short read on X bitmap file\n",
                            name);
                    zclose(zf);
                    return (image);
                }
                *(dataptr++) = value >> 8;
                if (++x < linelen) *(dataptr++) = value & 0xff;
            }
        }
    }
    else
...
int
xbitmapIdent(char *fullname, char *name)
{
    ZFILE       *zf;
    char        *title;
    unsigned int w, h;
  ...
  /* See if it's an x bitmap */
    ...
    if (title == NULL) title = dupString(name);
