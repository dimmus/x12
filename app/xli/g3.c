...
static tableentry *
g3_hashfind(tableentry **hash, int length, int code, int a, int b)
{
    unsigned int pos;
    tableentry  *te;
...
static int
g3_getfaxrow(ZFILE *fd, byte *bitrow)
{
    int         col;
    ...
    while (!g3_eof)
    {
        ...
        do
        {
            ...
            if (g3_rawzeros >= 11)
            {
                ...
                if (nextbit == 1)
                {
                    if (col == 0) /* 6 consecutive EOLs mean end of document */
                        g3_eof = (++g3_eols >= 5);
                    else g3_eols = 0;
        ...
        switch (te->tabid)
        {
            ...
            case TBTABLE:
                count += te->count;
                if (col + count > MAXCOLS) count = MAXCOLS - col;
                if (count > 0)
                {
                    if (color)
                    {
                    ...
                    else g3_bitson(bitrow, col, count);
                }
                curcode = 0;
                curlen  = 0;
                color   = !color;
...
g3_skiptoeol(ZFILE *fd)
{
    ...
    if (!maxbits) g3_error = "G3: unable to skip to eol";
    return (0);
}

static int
g3_rawgetbit(ZFILE *fd)
{
    int        b;
    static int shdata;
...
Image *
g3Load(char *fullname, ImageOptions *image_ops, boolean verbose)
{
    ZFILE *fd;
...
boolean
g3Ident(char *fullname, char *name)
{
    ZFILE *fd;
    int    i, retv;
    ...
    maxlinelen = BITS_TO_BYTES(MAXCOLS);
    if ((retv = g3_ident(fd))) printf("%s is a G3 FAX image.\n", name);
    zclose(fd);
    return retv;
}
