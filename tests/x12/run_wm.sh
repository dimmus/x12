...
"$XWM" -v -f "$ROOT/app/xwm/system.xwmrc" >"$LOG.xwm" 2>&1 &
XWM_PID=$!
sleep 0.5
...
# Startup markers must be present for a healthy WM claim.
if ! grep -q 'WM selection OK' "$LOG.xwm"; then
  echo "xwm: missing startup marker 'WM selection OK'; log:" >&2
  cat "$LOG.xwm" >&2 || true
  exit 1
fi
...
if ! grep -q 'manage client=' "$LOG.xwm"; then
  echo "xwm: no manage log line after wm_manage; log:" >&2
  cat "$LOG.xwm" >&2 || true
  exit 1
fi
