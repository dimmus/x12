...
BuiltinDirsDup(const BuiltinDirPtr a_dirs, int a_dirs_len)
{
    BuiltinDirPtr dirs = NULL;
    int           i    = 0;

    if (!a_dirs) return NULL;
 ...
 */
static int
BuiltinDirRestore(BuiltinDirPtr a_cur, const BuiltinDirPtr a_saved)
{
    if (!a_cur) return 1;
    if (!a_saved) return 0;

    if (a_saved->font_name)
        memmove(a_cur->font_name,
                a_saved->font_name,
                strlen(a_saved->font_name));
    return 0;
}
                   ...
                   int                 a_tab_len)
{
    ...
    if (!a_cur_tab) return 1;
    if (!a_saved_tab) return 0;

    for (i = 0; i < a_tab_len; i++)
    {
        if (BuiltinDirRestore(&a_cur_tab[i], &a_saved_tab[i])) return 1;
    }
    return 0;
}

static BuiltinAliasPtr
BuiltinAliasesDup(const BuiltinAliasPtr a_aliases, int a_aliases_len)
{
    BuiltinAliasPtr aliases = NULL;
    int             i       = 0;

    if (!a_aliases) return NULL;
 ...
 */
static int
BuiltinAliasRestore(BuiltinAliasPtr a_cur, const BuiltinAliasPtr a_save)
{
    if (!a_cur) return 1;
    if (!a_save) return 0;
    if (a_save->alias_name)
        memmove(a_cur->alias_name,
                a_save->alias_name,
                strlen(a_save->alias_name));
    if (a_save->font_name)
                      ...
                      int                   a_tab_len)
{
    ...
    for (i = 0; i < a_tab_len; i++)
    {
        if (BuiltinAliasRestore(&a_cur_tab[i], &a_saved_tab[i])) return 1;
    }
    return 0;
}
...
BuiltinReadDirectory(const char *directory, FontDirectoryPtr *pdir)
{
    ...
    {
        saved_builtin_dir =
            BuiltinDirsDup((const BuiltinDirPtr)builtin_dir, builtin_dir_count);
    }
    ...
    {
        saved_builtin_alias =
            BuiltinAliasesDup((const BuiltinAliasPtr)builtin_alias,
                              builtin_alias_count);
    }
