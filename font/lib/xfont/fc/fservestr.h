...
#define fsUnpack_XFontInfoHeader(packet, structure)                          \
    ...
    (structure)->drawDirection =                                             \
        ((packet)->font_header_draw_direction == LeftToRightDrawDirection)   \
            ...
            : RightToLeft;                                                   \
    (structure)->inkInside =                                                 \
        ((packet)->font_header_flags & FontInfoInkInside) != 0;              \
                                                                             \
    ...
    fsUnpack_XCharInfo((packet)->font_header_min_bounds,                     \
                       &(structure)->minbounds);                             \
    fsUnpack_XCharInfo((packet)->font_header_min_bounds,                     \
                       &(structure)->ink_minbounds);                         \
    fsUnpack_XCharInfo((packet)->font_header_max_bounds,                     \
                       &(structure)->maxbounds);                             \
    fsUnpack_XCharInfo((packet)->font_header_max_bounds,                     \
                       &(structure)->ink_maxbounds)

extern void    _fs_init_fontinfo(FSFpePtr conn, FontInfoPtr pfi);
extern int     _fs_convert_props(fsPropInfo   *pi,
                                 fsPropOffset *po,
                                 pointer       pd,
                                 FontInfoPtr   pfi);
extern int     _fs_convert_lfwi_reply(FSFpePtr                   conn,
                                      FontInfoPtr                pfi,
                                      fsListFontsWithXInfoReply *fsrep,
                                      fsPropInfo                *pi,
                                      fsPropOffset              *po,
                                      pointer                    pd);
extern int     fs_build_range(FontPtr        pfont,
                              Bool           range_flag,
                              unsigned int   count,
                              int            item_size,
                              unsigned char *data,
