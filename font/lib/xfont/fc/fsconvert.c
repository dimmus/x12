...
_fs_init_fontinfo(FSFpePtr conn, FontInfoPtr pfi)
{
    if (conn->fsMajorVersion == 1)
    {
        ...
        pfi->lastRow  = n;
        pfi->defaultCh =
            ((pfi->defaultCh >> 8) & 0xff) + ((pfi->defaultCh & 0xff) << 8);
    }
...
int
_fs_convert_props(fsPropInfo *pi, fsPropOffset *po, pointer pd, FontInfoPtr pfi)
{
    FontPropPtr  dprop;
    int          i, nprops;
    char        *is_str;
...
#define BAD_RESOURCE 0xe0000000
    ...
    if (nprops < 0 || nprops > SIZE_MAX / (sizeof(FontPropRec) + sizeof(char)))
        return -1;
    ...
    for (i = 0; i < nprops; i++, dprop++, is_str++)
    {
        ...
        if ((local_off.name.position >= pi->data_len) ||
            (local_off.name.length > (pi->data_len - local_off.name.position)))
            goto bail;
        dprop->name =
            MakeAtom(&pdc[local_off.name.position], local_off.name.length, 1);
...
int
_fs_convert_lfwi_reply(FSFpePtr                   conn,
                       FontInfoPtr                pfi,
                       ...
                       pointer                    pd)
{
    ...
    if (_fs_convert_props(pi, po, pd, pfi) == -1) return AllocError;
               ...
               fsRange      **ranges)
{
    ...
    if (!fsd->glyphs_to_get) return AccessDone;
    ...
    if (fsfont->pDefault && ENCODING_UNDEFINED(fsfont->pDefault))
    {
        ...
        if ((err = add_range(&range, nranges, ranges, FALSE)) != Successful)
            return err;
    ...
    else
    {
        ...
        while (count--)
        {
            ...
            {
                if (row1 < firstrow || row1 > lastrow || col1 < firstcol ||
                    col1 > lastcol)
                    continue;
            }
...
expand_glyph_range:;
    ...
    return access_done ? AccessDone : Successful;
}
...
void
_fs_clean_aborted_loadglyphs(FontPtr  pfont,
    ...
    if (fsfont->encoding)
    {
        ...
        for (i = 0; i < num_expected_ranges; i++)
        {
            ...
            for (row = expected_ranges[i].min_char_high;
                 ...
                 row++)
            {
                register CharInfoPtr encoding =
                    fsfont->encoding +
                    ((row - pfont->info.firstRow) *
                         (pfont->info.lastCol - pfont->info.firstCol + 1) +
                     expected_ranges[i].min_char_low - pfont->info.firstCol);
...
#define CHECK_ENCODING(cnum)                                              \
    (pci = encoding + (cnum),                                             \
     fsd->glyphs_to_get ? (pci->bits == &_fs_glyph_undefined ||           \
                                   pci->bits == &_fs_glyph_requested      \
    ...
    switch (charEncoding)
    {
        case Linear8Bit:
        case TwoD8Bit:
            if (pFont->info.firstRow > 0) break;
            if (pFont->info.allExist && pDefault)
            {
                while (err == Successful && count--)
                {
                    c = (*chars++) - firstCol;
                    if (c < numCols) *glyphs++ = CHECK_ENCODING(c);
                    else *glyphs++ = pDefault;
                }
            }
            else
            {
                ...
                {
                    c = (*chars++) - firstCol;
                    if (c < numCols && CHECK_ENCODING(c)->bits) *glyphs++ = pci;
                    else if (pDefault) *glyphs++ = pDefault;
                }
            }
            break;
        case Linear16Bit:
                ...
                xCharInfo    **glyphs) /* RETURN */
{
    ...
    if (pDefault) pDefault = encoding + (pDefault - fsdata->encoding);
    firstCol   = pFont->info.firstCol;
    numCols    = pFont->info.lastCol - firstCol + 1;
    glyphsBase = glyphs;
    ...
    switch (charEncoding)
    {
        ...
        case TwoD8Bit:
            ...
            if (pFont->info.allExist && pDefault)
            {
                ...
                {
                    c = (*chars++) - firstCol;
                    if (c < numCols) *glyphs++ = (xCharInfo *)&encoding[c];
                    else *glyphs++ = (xCharInfo *)pDefault;
                }
            }
            else
            {
                while (count--)
                {
                    c = (*chars++) - firstCol;
                    if (c < numCols) *glyphs++ = (xCharInfo *)(encoding + c);
                    else if (pDefault) *glyphs++ = (xCharInfo *)pDefault;
                }
            }
            break;
        case Linear16Bit:
...
_fs_unload_font(FontPtr pfont)
{
    ...
    if (find_old_font(fsd->fontid)) DeleteFontClientID(fsd->fontid);
               ...
               fsBitmapFormatMask fmask)
{
    ...
    CheckFSFormat(format,
                  BitmapFormatMaskBit | BitmapFormatMaskByte |
