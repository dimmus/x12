...
static int _fs_resize(FSBufPtr buf, long size);

static void _fs_downsize(FSBufPtr buf, long size);
...
_fs_poll_connect(XtransConnInfo trans_conn, int timeout)
{
    ...
    if (ret == 0) return FSIO_BLOCK;
    if (ret < 0) return FSIO_ERROR;
    return FSIO_READY;
}
...
_fs_connect(char *servername, int *err)
{
    ...
    if (i < 0)
    {
        if (i == TRANS_IN_PROGRESS) ret = FSIO_BLOCK;
        else ret = FSIO_ERROR;
    }
    else ret = FSIO_READY;
...
_fs_fill(FSFpePtr conn)
{
    ...
    if (_fs_flush(conn) < 0) return FSIO_ERROR;
    /*
...
_fs_start_read(FSFpePtr conn, long size, char **buf)
{
    ...
    if (fs_inqueued(conn) < size)
    {
        ...
        ret = _fs_fill(conn);
        if (ret == FSIO_ERROR) return ret;
        if (ret == FSIO_BLOCK || fs_inqueued(conn) < size) return FSIO_BLOCK;
    }
    if (buf) *buf = conn->inBuf.buf + conn->inBuf.remove;
    return FSIO_READY;
}
...
_fs_flush(FSFpePtr conn)
{
    ...
    if (conn->fs_fd < 0) return FSIO_ERROR;
...
_fs_resize(FSBufPtr buf, long size)
{
    ...
    {
        new_size =
            ((buf->remove + size + FS_BUF_INC) / FS_BUF_INC) * FS_BUF_INC;
        new = realloc(buf->buf, new_size);
        if (!new) return FSIO_ERROR;
        buf->buf  = new;
        buf->size = new_size;
    }
    return FSIO_READY;
...
_fs_io_init(FSFpePtr conn)
{
    ...
    if (!conn->outBuf.buf) return FALSE;
...
{
    if (conn->outBuf.buf) free(conn->outBuf.buf);
    if (conn->inBuf.buf) free(conn->inBuf.buf);
}
...
_fs_do_write(FSFpePtr conn, const char *data, long len, long size)
{
    ...
    if (conn->fs_fd == -1) return FSIO_ERROR;
...
_fs_wait_for_readable(FSFpePtr conn, int ms)
{
    ...
    for (;;)
    {
        ...
        if (result < 0)
        {
            if (ECHECK(EINTR) || ECHECK(EAGAIN)) continue;
            else return FSIO_ERROR;
        }
        if (result == 0) return FSIO_BLOCK;
        if (FD_ISSET(conn->fs_fd, &r_mask)) return FSIO_READY;
        return FSIO_ERROR;
    }
}
