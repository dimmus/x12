...
static const char *pnpSerial[] = { "BaudRate",    "1200", "DataBits", "7",
    ...
    if ((len = pnpgets(pInfo, buf, &prePNP)) > 0)
    {
        if (!prePNP)
        {
                ...
                (t = pnpproto(&pnpid)) != NULL)
            {
                xf86MsgVerb(X_INFO,
                            2,
                            "%s: PnP-detected protocol ID: %d\n",
                            pInfo->name,
                            t->val);
                return (t->val);
            }
        }
        else return prepnpparse(pInfo, buf);
    }
    return PROT_UNKNOWN;
}
...
MouseGetPnpProtocol(InputInfoPtr pInfo)
{
    ...
    {
        if (val == MouseGetSerialPnpProtocol(pInfo)) return val;
    }
...
#ifdef EXTMOUSEDEBUG
    if (mPriv->disablePnPauto) xf86ErrorF("Mouse: Enabling PnP\n");
#endif
    mPriv->disablePnPauto = FALSE;

    if (mPriv->soft) return getPs2ProtocolPnP(pInfo);
    else return probePs2ProtocolPnP(pInfo);
}
...
pnpgets(InputInfoPtr pInfo, char *buf, Bool *prePNP)
{
    ...
    if ((i = xf86GetSerialModemState(pInfo->fd)) == -1) return 0;
    i |= XF86_M_DTR; /* DTR = 1 */
    i &= ~XF86_M_RTS; /* RTS = 0 */
    if (xf86SetSerialModemState(pInfo->fd, i) == -1) goto disconnect_idle;
    usleep(200000);
    ...
    /* try to read something */
    if (xf86WaitForInput(pInfo->fd, 200000) <= 0) goto connect_idle;
#endif
    ...
    *prePNP = FALSE;
    ...
    while (xf86ReadSerial(pInfo->fd, &c, 1) == 1)
    {
        ...
        if (xf86WaitForInput(pInfo->fd, 200000) <= 0) break;
    }
    if (i <= 0)
    {
    ...
    if (buf[i - 1] != c) goto connect_idle;
    return i;
...
pnpparse(InputInfoPtr pInfo, pnpid_t *id, char *buf, int len)
{
    ...
    if ((id->nserial > 0) || (id->nclass > 0) || (id->ncompat > 0) ||
        (id->ndescription > 0))
    {
...
{
    if (buf[0] == 'M' && buf[1] == '3') return PROT_MS;
    return PROT_UNKNOWN;
}
...
pnpproto(pnpid_t *id)
{
     ...
     * ID separated by ','.
     */
    if (id->ncompat <= 0) return NULL;
    for (i = 0; i < id->ncompat; ++i)
    {
        for (j = i; id->compat[i] != ','; ++i)
            if (i >= id->ncompat) break;
        if (i > j)
        {
...
gettoken(symtab_t *tab, char *s, int len)
{
    ...
    {
        if (strncmp(tab[i].name, s, len) == 0) break;
    }
    return &tab[i];
}
...
{
    if (xf86WaitForInput(pInfo->fd, 200000) <= 0) return FALSE;
...
ps2GetDeviceID(InputInfoPtr pInfo)
{
    ...
    usleep(30000);
    xf86FlushInput(pInfo->fd);
    if (!ps2SendPacket(pInfo, packet, sizeof(packet))) return -1;
    while (1)
    {
        if (!readMouse(pInfo, &u)) return -1;
        if (u != 0xFA) break;
    }
#ifdef DEBUG
...
#endif
    if (!ps2SendPacket(pInfo, packet, sizeof(packet))) return FALSE;
...
probePs2ProtocolPnP(InputInfoPtr pInfo)
{
    ...
    { /* Reset PS2 device */
        ...
        if (ret != PROT_UNKNOWN) ps2EnableDataReporting(pInfo);
    }
    return ret;
}
...
getPs2ProtocolPnP(InputInfoPtr pInfo)
{
    ...
    while (--count)
        if (ps2DisableDataReporting(pInfo)) break;
