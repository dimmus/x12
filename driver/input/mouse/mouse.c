...
#else
#  define BLOCK_HANDLER_ARGS \
      pointer data, struct timeval **waitTime, pointer LastSelectMask
...
static void ps2BlockHandler(pointer          data,
...
#undef MOUSE
_X_EXPORT InputDriverRec MOUSE = { .driverVersion   = 1,
                                   .driverName      = "mouse",
                                   .Identify        = NULL,
 ...
 */
static const char *msDefaults[]    = { "BaudRate",    "1200", "DataBits", "7",
                                       "StopBits",    "1",    "Parity",   "None",
                                       "FlowControl", "None", "VTime",    "0",
                                       "VMin",        "1",    NULL };
/* MouseSystems */
static const char *mlDefaults[]    = { "BaudRate",    "1200", "DataBits", "8",
                                       "StopBits",    "2",    "Parity",   "None",
                                       "FlowControl", "None", "VTime",    "0",
                                       "VMin",        "1",    NULL };
/* MMSeries */
static const char *mmDefaults[]    = { "BaudRate",    "1200", "DataBits", "8",
                                       "StopBits",    "1",    "Parity",   "Odd",
                                       "FlowControl", "None", "VTime",    "0",
                                       "VMin",        "1",    NULL };
/* Hitachi Tablet */
static const char *mmhitDefaults[] = {
    "BaudRate",    "1200", "DataBits", "8", "StopBits", "1", "Parity", "None",
    "FlowControl", "None", "VTime",    "0", "VMin",     "1", NULL
};
/* AceCad Tablet */
...
MouseCommonOptions(InputInfoPtr pInfo)
{
    ...
    pMse->emulate3Timeout =
        xf86SetIntOption(pInfo->options, "Emulate3Timeout", 50);
    if (pMse->emulate3Buttons || pMse->emulate3ButtonsSoft)
    {
        MessageType from = X_CONFIG;
        if (pMse->emulate3ButtonsSoft) from = X_DEFAULT;
        xf86Msg(from,
                "%s: Emulate3Buttons, Emulate3Timeout: %d\n",
                pInfo->name,
                pMse->emulate3Timeout);
    }
    ...
    if (pMse->pDragLock) free(pMse->pDragLock);
    pMse->pDragLock = NULL;
    ...
    if (s)
    {
        ...
        while ((s1 != NULL) && (lock = strtol(s1, &s1, 10)) != 0)
        {
            ...
            {
                xf86Msg(X_WARNING,
                        "DragLock: Invalid button number = %d\n",
                        lock);
                break;
            };
        ...
        for (i = 0; i < NIB_COUNT; i++)
        {
            ...
            for (j = 0; j < NIB_SIZE; j++)
            {
                ...
                if (ff >= 0)
                {
                     ...
                     */
                    if (otherbits)
                        pLock->nib_table[i][j] = pLock->nib_table[i][fM] |
    ...
    if (s)
    {
        ...
        else if (sscanf(s, "%d %d %d %d", &b1, &b2, &b3, &b4) >= 2 && b1 > 0 &&
                 b1 <= MSE_MAXBUTTONS && b2 > 0 && b2 <= MSE_MAXBUTTONS)
        {
            pMse->negativeZ = 1 << (b1 - 1);
            pMse->positiveZ = 1 << (b2 - 1);
            if (b3 > 0 && b3 <= MSE_MAXBUTTONS && b4 > 0 &&
                b4 <= MSE_MAXBUTTONS)
            {
        ...
        {
            xf86Msg(X_WARNING,
                    "%s: Invalid ZAxisMapping value: \"%s\"\n",
                    pInfo->name,
                    s);
        }
        free(s);
    ...
    if (xf86SetBoolOption(pInfo->options, "EmulateWheel", FALSE))
    {
        ...
        {
            xf86Msg(X_WARNING,
                    "%s: Invalid EmulateWheelButton value: %d\n",
                    pInfo->name,
                    wheelButton);
            wheelButton = 4;
        }
        ...
        pMse->wheelInertia =
            xf86SetIntOption(pInfo->options, "EmulateWheelInertia", 10);
        if (pMse->wheelInertia <= 0)
        {
            xf86Msg(X_WARNING,
                    "%s: Invalid EmulateWheelInertia value: %d\n",
                    pInfo->name,
                    pMse->wheelInertia);
            pMse->wheelInertia = 10;
        }
        pMse->wheelButtonTimeout =
            xf86SetIntOption(pInfo->options, "EmulateWheelTimeout", 200);
        if (pMse->wheelButtonTimeout <= 0)
        {
            xf86Msg(X_WARNING,
                    "%s: Invalid EmulateWheelTimeout value: %d\n",
                    pInfo->name,
                    pMse->wheelButtonTimeout);
        ...
        if (s)
        {
            ...
            {
                xf86Msg(X_WARNING,
                        "%s: Invalid XAxisMapping value: \"%s\"\n",
                        pInfo->name,
                        s);
            }
            if (msg)
        ...
        if (s)
        {
            ...
            {
                xf86Msg(X_WARNING,
                        "%s: Invalid YAxisMapping value: \"%s\"\n",
                        pInfo->name,
                        s);
            }
            if (msg)
        ...
        if (!yFromConfig)
        {
            ...
            xf86Msg(X_DEFAULT,
                    "%s: YAxisMapping: buttons %d and %d\n",
                    pInfo->name,
                    pMse->negativeY,
                    pMse->positiveY);
        }
        xf86Msg(X_CONFIG,
                "%s: EmulateWheel, EmulateWheelButton: %d, "
    ...
    for (i = pMse->buttons - 1; i >= 0; i--)
    {
        ...
        if (f > pMse->buttons) pMse->buttons = f;
    }
    if (origButtons != pMse->buttons) buttons_from = X_CONFIG;
    ...
    if (s)
    {
        ...
        if ((sscanf(s, "%d %d", &b1, &b2) == 2) && (b1 > 0) &&
            (b1 <= MSE_MAXBUTTONS) && (b2 > 0) && (b2 <= MSE_MAXBUTTONS))
        {
            if (asprintf(&msg, "buttons %d and %d", b1, b2) == -1) msg = NULL;
        ...
        {
            xf86Msg(X_WARNING,
                    "%s: Invalid DoubleClickButtons value: \"%s\"\n",
                    pInfo->name,
                    s);
        }
        if (msg)
...
MouseHWOptions(InputInfoPtr pInfo)
{
    ...
    if (mPriv == NULL) return;

    if ((mPriv->soft = xf86SetBoolOption(pInfo->options, "AutoSoft", FALSE)))
    {
...
ProtocolIDToName(MouseProtocolID id)
{
    ...
    switch (id)
    {
        ...
        default:
            for (i = 0; mouseProtocols[i].name; i++)
                if (id == mouseProtocols[i].id) return mouseProtocols[i].name;
            return "Invalid";
    }
}
...
ProtocolIDToClass(MouseProtocolID id)
{
    ...
    switch (id)
    {
        ...
        default:
            for (i = 0; mouseProtocols[i].name; i++)
                if (id == mouseProtocols[i].id) return mouseProtocols[i].class;
            return MSE_NONE;
    }
}
...
InitProtocols(void)
{
    ...
    if (osInfo) return TRUE;

    osInfo = OSMouseInit(0);
    if (!osInfo) return FALSE;
    if (!osInfo->SupportedInterfaces) return FALSE;
    ...
    for (i = 0; mouseProtocols[i].name; i++)
        if (mouseProtocols[i].id == PROT_PS2) mouseProtocols[i].id = PROT_BM;
#endif
...
MouseFindDevice(InputInfoPtr pInfo, const char *protocol)
{
    ...
    if (!osInfo->FindDevice) return NULL;

    xf86Msg(X_WARNING,
            "%s: No Device specified, looking for one...\n",
            pInfo->name);
    device = osInfo->FindDevice(pInfo, protocol, 0);
    if (!device)
        xf86Msg(X_ERROR, "%s: Cannot find which device to use.\n", pInfo->name);
    else xf86Msg(X_PROBED, "%s: Device: \"%s\"\n", pInfo->name, device);

    return device;
}

static const char *
MousePickProtocol(InputInfoPtr     pInfo,
                  const char      *device,
                  const char      *protocol,
                  MouseProtocolID *protocolID_out)
{
    ...
    switch (protocolID)
    {
        case PROT_WSMOUSE:
        case PROT_VUID:
            if (osInfo->PreInit) osInfo->PreInit(pInfo, protocol, 0);
            break;
        case PROT_UNKNOWN:
            ...
            {
                if (!device) MouseFindDevice(pInfo, protocol);
                if (osInfo->PreInit)
                {
...
MousePreInit(InputDriverPtr drv, InputInfoPtr pInfo, int flags)
{
    ...
    if (!InitProtocols()) return BadAlloc;
    ...
    if (protocolID == PROT_UNKNOWN) goto out;
    ...
    if (pInfo->fd == -1)
    {
        ...
        else
        {
            ...
            if (pMse->mousePriv) free(pMse->mousePriv);
            free(pMse);
            pInfo->private = NULL;
            rc             = BadValue;
            goto out;
...
static void
MouseInitButtonLabels(Atom *btn_labels)
{
    int  i;
    Atom unknown_btn;
...
static int
MouseSetProperty(DeviceIntPtr       device,
                 Atom               atom,
                 ...
                 BOOL               checkonly)
{
    ...
    if (atom == prop_mbemu)
    {
        ...
        if (!checkonly) Emulate3ButtonsSetEnabled(pInfo, *((BOOL *)val->data));
    }
    else if (atom == prop_mbtimeout)
    {
        ...
        if (!checkonly) pMse->emulate3Timeout = *((CARD32 *)val->data);
    }
...
MouseInitProperties(DeviceIntPtr device)
{
     ...
     */
    prop_mbemu =
        MakeAtom(MOUSE_PROP_MIDBUTTON, strlen(MOUSE_PROP_MIDBUTTON), TRUE);
    rc = XIChangeDeviceProperty(device,
                                prop_mbemu,
                                XA_INTEGER,
                                8,
                                PropModeReplace,
    ...
    prop_mbtimeout = MakeAtom(MOUSE_PROP_MIDBUTTON_TIMEOUT,
                              strlen(MOUSE_PROP_MIDBUTTON_TIMEOUT),
                              TRUE);
    rc             = XIChangeDeviceProperty(device,
                                prop_mbtimeout,
                                XA_INTEGER,
...
MouseReadInput(InputInfoPtr pInfo)
{
    ...
    while ((c = XisbRead(pMse->buffer)) >= 0)
    {
        ...
        if (pMse->collectData && pMse->autoProbe)
            if (pMse->collectData(pMse, u)) continue;
        ...
        if (pBufP >= pMse->protoPara[4])
        {
                ...
                (u & pMse->protoPara[5]) == pMse->protoPara[6])
            {
                 ...
                 */
                    ...
                    default:
                        buttons |=
                            ((int)(u & 0x20) >> 4) | (pMse->lastButtons & 0x05);
                        break;
                }
                goto post_event;
            }
        ...
        if (!pMse->dataGood(pMse)) continue;
...
REDO_INTERPRET:
        ...
        switch (pMse->protocolID)
        {
            ...
            case PROT_MMHIT: /* MM_HitTablet */
                buttons = pBuf[0] & 0x07;
                if (buttons != 0) buttons = 1 << (buttons - 1);
            ...
            case PROT_MMPS2: /* MouseMan+ PS/2 */
                buttons = (pBuf[0] & 0x04) >> 1 | /* Middle */
                          ...
                          (pBuf[0] & 0x01) << 2; /* Left */
                dx = (pBuf[0] & 0x10) ? pBuf[1] - 256 : pBuf[1];
                if (((pBuf[0] & 0x48) == 0x48) && (abs(dx) > 191) &&
...
MouseProc(DeviceIntPtr device, int what)
{
    ...
    defined(WSMOUSEIO_SETVERSION)
                ...
                else pMse->buffer = XisbNew(pInfo->fd, 64);
                if (!pMse->buffer)
                {
                    xf86CloseSerial(pInfo->fd);
                    pInfo->fd = -1;
                    ...
                    else
                    {
                        ...
                        if (mPriv != NULL)
                        {
                            if (pMse->protocolID != PROT_AUTO)
                            {
                                ...
                                else mPriv->autoState = AUTOPROBE_H_GOOD;
                            }
                            else
                            {
                                if (mPriv->soft)
                                    mPriv->autoState = AUTOPROBE_NOPROTO;
                                else mPriv->autoState = AUTOPROBE_H_NOPROTO;
                            }
                        }
...
static char hitachMap[16] = { 0, 2, 1, 3, 8,  10, 9,  11,
                              4, 6, 5, 7, 12, 14, 13, 15 };
...
buttonTimer(InputInfoPtr pInfo)
{
    ...
    {
        LogMessageVerbSigSafe(X_WARNING,
                              -1,
...
Emulate3ButtonsSetEnabled(InputInfoPtr pInfo, Bool enable)
{
    ...
    if (pMse->emulate3Buttons == enable) return;
    ...
    {
        if (pMse->emulate3Pending) buttonTimer(pInfo);
...
Emulate3ButtonsSoft(InputInfoPtr pInfo)
{
    ...
    if (!pMse->emulate3ButtonsSoft) return TRUE;
...
static void
MouseBlockHandler(BLOCK_HANDLER_ARGS)
{
...
static void
MouseWakeupHandler(WAKEUP_HANDLER_ARGS)
{
    InputInfoPtr pInfo = (InputInfoPtr)data;
    MouseDevPtr  pMse  = (MouseDevPtr)pInfo->private;
    int          ms;
...
MouseDoPostEvent(InputInfoPtr pInfo, int buttons, int dx, int dy)
{
    ...
    if (pMse->doubleClickSourceButtonMask)
    {
        ...
        else pMse->doubleClickOldSourceState = 0;
    ...
    if (pMse->emulateWheel)
    {
        ...
        if (pMse->wheelButton == 0) wheelButtonMask = 0;
        else wheelButtonMask = 1 << (pMse->wheelButton - 1);
        ...
        if (change & wheelButtonMask)
        {
            if (buttons & wheelButtonMask)
            {
                ...
                pMse->wheelButtonExpires =
                    GetTimeInMillis() + pMse->wheelButtonTimeout;
        ...
        else ms = pMse->wheelButtonExpires - GetTimeInMillis();
    ...
    if (dx || dy) xf86PostMotionEvent(pInfo->dev, 0, 0, 2, dx, dy);
    ...
    if (change)
    {
        ...
        if (pMse->pDragLock)
        {
            ...
            if (pLock->masterTS) pLock->simulatedDown |= (realbuttons & change);

            /* if any button pressed, no longer in master drag lock state */
            if (realbuttons & change) pLock->masterTS = 0;
...
static void
MousePostEvent(InputInfoPtr pInfo,
               int          truebuttons,
               int          dx,
               int          dy,
               int          dz,
               int          dw)
{
    ...
    if (pMse->protocolID == PROT_MMHIT) b = reverseBits(hitachMap, truebuttons);
    else b = reverseBits(reverseMap, truebuttons);

    /* Remap mouse buttons */
    b &= (1 << MSE_MAXBUTTONS) - 1;
    for (i = 0; b; i++)
    {
        if (b & 1) buttons |= pMse->buttonMap[i];
        b >>= 1;
    }
    ...
    do
    {
        ...
        dx = dy = 0;
        if (zbutton || wbutton) MouseDoPostEvent(pInfo, buttons, 0, 0);
        if (--zbuttoncount <= 0) zbutton = 0;
        if (--wbuttoncount <= 0) wbutton = 0;
    }
...
SetupMouse(InputInfoPtr pInfo)
{
    ...
    if (automatic)
    {
        if (name)
        {
            ...
            for (size_t i = 0; i < sizeof(pMse->protoPara); i++)
                if (protoPara[i] != -1) pMse->protoPara[i] = protoPara[i];
    ...
    if (pMse->oldProtocolID != pMse->protocolID)
    {
        ...
        {
            pointer tmp =
                xf86OptionListCreate(mouseProtocols[pMse->protocolID].defaults,
                                     -1,
                                     0);
    ...
    if (!automatic) xf86SetSerial(pInfo->fd, pInfo->options);

    if (!initMouseHW(pInfo)) return FALSE;
...
initMouseHW(InputInfoPtr pInfo)
{
    ...
    switch (pMse->protocolID)
    {
        ...
        case PROT_THINKING: /* ThinkingMouse */
            ...
            {
                xf86WriteSerial(pInfo->fd, s, 1);
                if ((xf86WaitForInput(pInfo->fd, 1000000) <= 0)) break;
        ...
        case PROT_MMPS2: /* MouseMan+, FirstMouse+ */
            {
                static unsigned char seq[] = { 230, 232, 0,   232, 3,   232,
                                               2,   232, 1,   230, 232, 3,
                                               232, 1,   232, 2,   232, 3 };
                param                      = seq;
                paramlen                   = sizeof(seq);
        ...
        case PROT_THINKPS2: /* ThinkingMouse */
            {
                static unsigned char seq[] = { 243, 10, 232, 0,  243, 20,
                                               243, 60, 243, 40, 243, 20,
                                               243, 20, 243, 60, 243, 40,
                                               243, 20, 243, 20 };
                param                      = seq;
                paramlen                   = sizeof(seq);
            }
...
REDO:
            ...
            else
            {
                ...
                {
                    if (pMse->sampleRate >= 200) c2[1] = 200;
                    else if (pMse->sampleRate >= 100) c2[1] = 100;
                    else if (pMse->sampleRate >= 80) c2[1] = 80;
                    else if (pMse->sampleRate >= 60) c2[1] = 60;
                    else if (pMse->sampleRate >= 40) c2[1] = 40;
                    else c2[1] = 20;
                }
                else
                {
                ...
                {
                    if (pMse->resolution >= 200) c2[1] = 3;
                    else if (pMse->resolution >= 100) c2[1] = 2;
                    else if (pMse->resolution >= 50) c2[1] = 1;
                    else c2[1] = 0;
                }
                else
                {
                ...
                {
                    xf86Msg(X_INFO,
                            "%s: ps2EnableDataReporting: failed\n",
                            pInfo->name);
                    xf86FlushInput(pInfo->fd);
                    if (!count--) return TRUE;
                    goto REDO;
                ...
                {
                    xf86Msg(X_INFO,
                            "%s: ps2EnableDataReporting: succeeded\n",
                            pInfo->name);
                }
            }
            /*
...
#  endif
    ...
    if (mousepriv->current == 0 && (mousepriv->lastEvent - prevEvent) < 4000)
        return FALSE;

    if (mousepriv->current > 0 && (mousepriv->lastEvent - prevEvent) >= 1000)
    {
    ...
    if (mousepriv->inReset) mousepriv->inReset = FALSE;

#  ifdef EXTMOUSEDEBUG
    LogMessageVerbSigSafe(X_INFO,
                          -1,
                          "Mouse Current: %i 0x%x\n",
                          mousepriv->current,
                          val);
#  endif
    ...
    /* PS/2 */
    {
        ...
        if (seq[mousepriv->current] == val) expectReset = TRUE;
...
#  endif
    ...
    if (!expectReset) mousepriv->current = 0;
    else mousepriv->current++;
    return ret;
}

static void
ps2BlockHandler(pointer data, struct timeval **waitTime, pointer LastSelectMask)
{
    InputInfoPtr pInfo     = (InputInfoPtr)data;
    MouseDevPtr  pMse      = (MouseDevPtr)pInfo->private;
    mousePrivPtr mousepriv = (mousePrivPtr)pMse->mousePriv;
    ...
    if (mousepriv->inReset)
    {
        ...
        RemoveBlockAndWakeupHandlers(ps2BlockHandler,
                                     ps2WakeupHandler,
                                     (pointer)pInfo);
}
...
static MouseProtocolID hardProtocolList[] = {
    PROT_MSC,    PROT_MM,       PROT_LOGI,     PROT_LOGIMAN, PROT_MMHIT,
    PROT_GLIDE,  PROT_IMSERIAL, PROT_THINKING, PROT_ACECAD,  PROT_THINKPS2,
    PROT_MMPS2,  PROT_GLIDEPS2, PROT_NETSCPS2, PROT_EXPPS2,  PROT_IMPS2,
    PROT_GENPS2, PROT_NETPS2,   PROT_MS,       PROT_UNKNOWN
};

static MouseProtocolID softProtocolList[] = {
    PROT_MSC,   PROT_MM,       PROT_LOGI,     PROT_LOGIMAN, PROT_MMHIT,
    PROT_GLIDE, PROT_IMSERIAL, PROT_THINKING, PROT_ACECAD,  PROT_THINKPS2,
    PROT_MMPS2, PROT_GLIDEPS2, PROT_NETSCPS2, PROT_IMPS2,   PROT_GENPS2,
    PROT_MS,    PROT_UNKNOWN
};
...
autoOSProtocol(InputInfoPtr pInfo, int *protoPara)
{
    ...
    if (!name && osInfo->GuessProtocol)
    {
        ...
        if (name) protocolID = ProtocolNameToID(name);
    }
...
createProtoList(MouseDevPtr pMse, MouseProtocolID *protoList)
{
         ...
         i++)
    {
        ...
        while (1)
        {
            ...
            if (j > para[4] && ++header_count > para[4]) break;
            /* we have not found anything that looks like a header */
            if (!next_header_candidate) break;
...
createSerialDefaultsList(void)
{
    ...
    {
        if (!mouseProtocols[j].defaults) continue;
        for (k = 0; k < i; k++)
            if (mouseProtocols[j].defaults == serialDefaultsList[k]) continue;
        i++;
        serialDefaultsList =
            (void **)XNFrealloc(serialDefaultsList, sizeof(void *) * (i + 1));
...
validCount(mousePrivPtr mPriv, Bool inSync, Bool lostSync)
{
    ...
    mPriv->badCount +=
        lostSync ? BAD_INC_CERTAINTY_WHEN_SYNC_LOST : BAD_INC_CERTAINTY;
    ...
    while (1)
    {
        switch (mPriv->autoState)
        {
            ...
            case AUTOPROBE_H_SETPROTO:
                ...
                if ((pMse->protocolID =
                         hardProtocolList[mPriv->protocolID++]) == PROT_UNKNOWN)
                {
                    mPriv->protocolID = 0;
                    break;
                }
            ...
            case AUTOPROBE_H_VALIDATE1:
                ...
                switch (validCount(mPriv, inSync, lostSync))
                {
                    ...
                    case STATE_VALID:
                        xf86Msg(X_INFO,
                                "Mouse autoprobe: selecting %s protocol\n",
                                ProtocolIDToName(pMse->protocolID));
            ...
            case AUTOPROBE_COLLECT:
                ...
                if (mPriv->count <= NUM_MSE_AUTOPROBE_BYTES) return;
            ...
            case AUTOPROBE_AUTODETECT:
                ...
                if (pMse->protocolID != PROT_AUTO)
                    mPriv->autoState = AUTOPROBE_GOOD;
                else mPriv->autoState = AUTOPROBE_NOPROTO;
                break;
            ...
            case AUTOPROBE_SWITCHSERIAL:
                {
                    ...
                    if (!serialDefaultsList) createSerialDefaultsList();
                    ...
                    {
                        pointer tmp =
                            xf86OptionListCreate(serialDefaults, -1, 0);
...
autoGood(MouseDevPtr pMse)
{
    ...
    switch (mPriv->autoState)
    {
        ...
        case AUTOPROBE_VALIDATE2:
        case AUTOPROBE_H_VALIDATE2:
            if (mPriv->goodCount < PROBE_UNCERTAINTY / 2) return TRUE;
            /* FALLTHROUGH */
        default:
            return FALSE;
    }
...
SetMouseProto(MouseDevPtr pMse, MouseProtocolID protocolID)
{
    ...
    if ((pMse->protocolID >= 0) && (pMse->protocolID < PROT_NUMPROTOS))
        memcpy(pMse->protoPara,
               proto[pMse->protocolID],
               sizeof(pMse->protoPara));

    if (pMse->emulate3ButtonsSoft) pMse->emulate3Buttons = TRUE;
}
...
static pointer
xf86MousePlug(pointer module, pointer options, int *errmaj, int *errmin)
{
...
_X_EXPORT XF86ModuleData mouseModuleData = { &xf86MouseVersionRec,
                                             xf86MousePlug,
                                             xf86MouseUnplug };
