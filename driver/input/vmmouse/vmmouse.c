...
#define VMMOUSE_DRIVER_VERSION                                     \
    (PACKAGE_VERSION_MAJOR * 65536 + PACKAGE_VERSION_MINOR * 256 + \
     PACKAGE_VERSION_PATCHLEVEL)
#define VMMOUSE_DRIVER_VERSION_STRING                     \
    VMW_STRING(PACKAGE_VERSION_MAJOR)                     \
    "." VMW_STRING(PACKAGE_VERSION_MINOR) "." VMW_STRING( \
        PACKAGE_VERSION_PATCHLEVEL)
...
static Bool         VMMouseConvertProc(InputInfoPtr pInfo,
                                       int          first,
                                       int          num,
                                       int          v0,
                                       int          v1,
...
static void VMMousePostEvent(InputInfoPtr pInfo,
                             int          buttons,
                             int          dx,
                             int          dy,
                             int          dz,
...
#if GET_ABI_MAJOR(ABI_XINPUT_VERSION) < 7
static const char *reqSymbols[] = { "InitPointerDeviceStruct",
                                    "LoaderSymbol",
                                    "LoadSubModule",
...
static char reverseMap[32] = { 0,  4,  2,  6,  1,  5,  3,  7,  8,  12, 10,
                               14, 9,  13, 11, 15, 16, 20, 18, 22, 17, 21,
                               19, 23, 24, 28, 26, 30, 25, 29, 27, 31 };
...
#if GET_ABI_MAJOR(ABI_XINPUT_VERSION) < 12
static int VMMouseNewPreInit(InputDriverPtr drv, InputInfoPtr pInfo, int flags);
...
#  endif
    ...
    */
    if (!xorgHWAccess)
    {
        if (xf86EnableIO()) xorgHWAccess = TRUE;
        else return NULL;
    }
...
VMMouseDoPostEvent(InputInfoPtr pInfo, int buttons, int dx, int dy)
{
    ...
    if (mouseMoved)
    {
#if defined CALL_CONVERSION_PROC && GET_ABI_MAJOR(ABI_XINPUT_VERSION) < 12
        /*
...
static void
VMMousePostEvent(InputInfoPtr pInfo,
                 int          buttons,
                 int          dx,
                 int          dy,
                 int          dz,
                 int          dw)
{
    ...
    switch (pMse->negativeZ)
    {
        ...
        case MSE_MAPTOY:
            if (dz != 0)
            {
                if (mPriv->isCurrRelative) dy = dz;
                else dy += dz;
                dz = 0;
            }
            break;
        default: /* buttons */
            buttons &= ~(pMse->negativeZ | pMse->positiveZ | pMse->negativeW |
                         pMse->positiveW);
            if (dw < 0 || dz < -1)
            {
    ...
    {
        buttons &= ~zbutton;
        if (mPriv->isCurrRelative) VMMouseDoPostEvent(pInfo, buttons, 0, 0);
        else VMMouseDoPostEvent(pInfo, buttons, dx, dy);
    }
}
    ...
    */
    ...
    if (s)
    {
                 ...
                 b1 <= MSE_MAXBUTTONS && b2 > 0 && b2 <= MSE_MAXBUTTONS)
        {
            ...
            if (msg) sprintf(msg, "buttons %d and %d", b1, b2);
...
#  endif
		...
		*/
                        ...
                        {
                            xf86Msg(X_ERROR,
                                    "VMWARE(0): vmmouse enable failed\n");
        ...
        GET_ABI_MINOR(ABI_XINPUT_VERSION) >=  \
    1901
        case DEVICE_ABORT:
            if (pInfo->fd != -1)
            {
                VMMousePrivPtr mPriv = (VMMousePrivPtr)pMse->mousePriv;
                if (mPriv->vmmouseAvailable) VMMouseClient_Disable();
                break;
            }
#endif
    }
    ...
    while ((numPackets = VMMouseClient_GetInput(&vmmouseInput)))
    {
        ...
        if (numPackets == VMMOUSE_ERROR)
        {
            ...
            LogMessageVerbSigSafe(
                X_INFO,
                -1,
                "VMWARE(0): re-requesting absolute mode after reset\n");
            break;
        }
...
#if GET_ABI_MAJOR(ABI_XINPUT_VERSION) < 12
static Bool
VMMouseConvertProc(InputInfoPtr pInfo,
                   int          first,
                   int          num,
                   int          v0,
                   int          v1,
...
static pointer
VMMousePlug(pointer module, pointer options, int *errmaj, int *errmin)
{
...
static XF86ModuleVersionInfo VMMouseVersionRec = {
    ...
    VMMOUSE_PACKAGE_VERSION_MAJOR,
    VMMOUSE_PACKAGE_VERSION_MINOR,
    VMMOUSE_PACKAGE_VERSION_PATCHLEVEL,
    ABI_CLASS_XINPUT,
    ABI_XINPUT_VERSION,
    MOD_CLASS_XINPUT,
 ...
 */
_X_EXPORT XF86ModuleData vmmouseModuleData = { &VMMouseVersionRec,
                                               VMMousePlug,
                                               VMMouseUnplug };
#endif /* XFree86LOADER */
