...
keyboard_activity(Display *display)
{
    ...
    if (ignore_modifier_combos)
    {
        for (i = 0; i < KEYMAP_SIZE; i++)
        {
            ...
            {
                if (old_key_state[i] & ~keyboard_mask[i]) ret = ActivityNone;
                else ret = ActivityReset;
                break;
            }
        }
    }
...
setup_keyboard_mask(Display *display, int ignore_modifier_keys)
{
    ...
    if (ignore_modifier_keys)
    {
        ...
        for (i = 0; i < 8 * modifiers->max_keypermod; i++)
        {
            ...
            if (kc != 0) clear_bit(keyboard_mask, kc);
        }
...
is_modifier_pressed(const struct xrecord_callback_results *cbres)
{
    ...
    for (i = 0; i < MAX_MODIFIERS; ++i)
        if (cbres->pressed_modifiers[i]) return 1;
...
record_main_loop(Display *display, double idle_time)
{
    ...
    XRecordEnableContextAsync(dpy_data,
                              context,
                              xrecord_callback,
                              (XPointer)&cbres);
    ...
    while (1)
    {
        ...
        if (FD_ISSET(fd, &read_fds))
        {
            ...
            while (XEventsQueued(dpy_data, QueuedAlready) > 0)
            {
                ...
                fprintf(stderr,
                        "bad event received, major opcode %d\n",
                        event.type);
            }
        ...
        if (ret == 0 && pad_disabled)
        { /* timeout => enable event */
            toggle_touchpad(True);
        }
    } /* end while(1) */
...
dp_get_device(Display *dpy)
{
    ...
    while (ndevices--)
    {
        if (info[ndevices].type == touchpad_type)
        {
            ...
            while (nprops--)
            {
                if (properties[nprops] == touchpad_off_prop) break;
            }
            if (nprops < 0)
            {
                fprintf(stderr,
...
main(int argc, char *argv[])
{
    ...
    if (!(dev = dp_get_device(display))) exit(2);
    ...
    if (use_xrecord)
    {
        if (check_xrecord(display)) record_main_loop(display, idle_time);
        else
        {
            fprintf(stderr,
                    "Use of XRecord requested, but failed to "
                    " initialize.\n");
            exit(4);
        }
    }
