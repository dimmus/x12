...
static Atom
InitTypedAtom(DeviceIntPtr dev,
              const char  *name,
              Atom         type,
              int          format,
              int          nvalues,
...
InitSoftButtonProperty(InputInfoPtr pInfo)
{
    ...
    if (!para->has_secondary_buttons) return;
    ...
    if (values[0] || values[1] || values[2] || values[4] || values[5] ||
        values[6] || values[7])
...
InitDeviceProperties(InputInfoPtr pInfo)
{
    ...
    prop_middle_timeout     = InitAtom(pInfo->dev,
                                   SYNAPTICS_PROP_MIDDLE_TIMEOUT,
                                   32,
                                   1,
                                   &para->emulate_mid_button_time);
    prop_twofinger_pressure = InitAtom(pInfo->dev,
                                       SYNAPTICS_PROP_TWOFINGER_PRESSURE,
                                       32,
                                       1,
                                       &para->emulate_twofinger_z);
    prop_twofinger_width    = InitAtom(pInfo->dev,
    ...
    if (priv->has_scrollbuttons)
    {
        ...
        prop_buttonscroll_repeat =
            InitAtom(pInfo->dev,
                     ...
                     2,
                     values);
        prop_buttonscroll_time = InitAtom(pInfo->dev,
    ...
    prop_lockdrags      = InitAtom(pInfo->dev,
                              SYNAPTICS_PROP_LOCKED_DRAGS,
                              8,
                              1,
                              &para->locked_drags);
    prop_lockdrags_time = InitAtom(pInfo->dev,
    ...
    prop_circscroll = InitAtom(pInfo->dev,
    ...
    prop_circscroll_dist = InitFloatAtom(pInfo->dev,
    ...
    prop_circpad = InitAtom(pInfo->dev,
                            SYNAPTICS_PROP_CIRCULAR_PAD,
                            8,
                            1,
                            &para->circular_pad);
    prop_palm    = InitAtom(pInfo->dev,
    ...
    prop_pressuremotion = InitTypedAtom(pInfo->dev,
    ...
    prop_pressuremotion_factor =
        InitFloatAtom(pInfo->dev,
                      SYNAPTICS_PROP_PRESSURE_MOTION_FACTOR,
                      2,
                      fvalues);

    prop_grab = InitAtom(pInfo->dev,
                         SYNAPTICS_PROP_GRAB,
                         8,
                         1,
    ...
    if (para->clickpad) InitSoftButtonProperty(pInfo);

    values[0] = para->hyst_x;
    values[1] = para->hyst_y;
    prop_noise_cancellation =
        InitAtom(pInfo->dev, SYNAPTICS_PROP_NOISE_CANCELLATION, 32, 2, values);
            ...
            BOOL               checkonly)
{
    ...
    if (property == prop_edges)
    {
        ...
        if (edges[0] > edges[1] || edges[2] > edges[3]) return BadValue;
    ...
    else if (property == prop_finger)
    {
        ...
        if (finger[0] > finger[1]) return BadValue;
    ...
    else if (property == prop_scrolldist)
    {
        ...
        if (dist[0] == 0 || dist[1] == 0) return BadValue;
    ...
    else if (property == prop_buttonscroll)
    {
        ...
        if (!priv->has_scrollbuttons) return BadMatch;
    ...
    else if (property == prop_pressuremotion)
    {
        ...
        if (press[0] > press[1]) return BadValue;
    ...
    else if (property == prop_softbutton_areas)
    {
        ...
        areas = (int *)prop->data;
        if (!SynapticsIsSoftButtonAreasValid(areas)) return BadValue;

        memcpy(para->softbutton_areas[BOTTOM_RIGHT_BUTTON_AREA],
               areas,
               4 * sizeof(int));
        memcpy(para->softbutton_areas[BOTTOM_MIDDLE_BUTTON_AREA],
               areas + 4,
               4 * sizeof(int));
    }
    else if (property == prop_secondary_softbutton_areas)
    {
        ...
        memcpy(para->softbutton_areas[TOP_RIGHT_BUTTON_AREA],
               areas,
               4 * sizeof(int));
        memcpy(para->softbutton_areas[TOP_MIDDLE_BUTTON_AREA],
               areas + 4,
               4 * sizeof(int));
    }
    else if (property == prop_noise_cancellation)
    {
        ...
        if (hyst[0] < 0 || hyst[1] < 0) return BadValue;
        para->hyst_x = hyst[0];
        para->hyst_y = hyst[1];
    }
