...
ps2_special_cmd(int fd, byte cmd)
{
    ...
    if (!ps2_putbyte(fd, PS2_CMD_SET_SCALING_1_1)) return FALSE;
...
ps2_synaptics_model_id(int fd, struct PS2SynapticsHwInfo *synhw)
{
    ...
    synhw->model_id = 0;
    if (ps2_send_cmd(fd, SYN_QUE_MODEL) && ps2_getbyte(fd, &mi[0]) &&
...
ps2_synaptics_capability(int fd, struct PS2SynapticsHwInfo *synhw)
{
    ...
    synhw->ext_cap      = 0;
    if (ps2_send_cmd(fd, SYN_QUE_CAPABILITIES) && ps2_getbyte(fd, &cap[0]) &&
        ...
        if (SYN_CAP_VALID(synhw))
        {
            if (SYN_EXT_CAP_REQUESTS(synhw))
            {
                if (ps2_send_cmd(fd, SYN_QUE_EXT_CAPAB) &&
                    ps2_getbyte(fd, &cap[0]) && ps2_getbyte(fd, &cap[1]) &&
...
ps2_synaptics_identify(int fd, struct PS2SynapticsHwInfo *synhw)
{
    ...
    synhw->identity = 0;
    if (ps2_send_cmd(fd, SYN_QUE_IDENTIFY) && ps2_getbyte(fd, &id[0]) &&
...
static Bool
ps2_query_is_synaptics(InputInfoPtr               pInfo,
                       int                        fd,
                       struct PS2SynapticsHwInfo *synhw)
{
    for (int i = 0; i < 3; i++)
    {
        if (ps2_synaptics_disable_device(fd)) break;
    }
...
PS2QueryHardware(InputInfoPtr pInfo)
{
    ...
    if (!ps2_query_is_synaptics(pInfo, pInfo->fd, synhw)) return FALSE;
    ...
    if (!ps2_synaptics_identify(pInfo->fd, synhw)) return FALSE;

    if (!ps2_synaptics_model_id(pInfo->fd, synhw)) return FALSE;

    if (!ps2_synaptics_capability(pInfo->fd, synhw)) return FALSE;

    mode = SYN_BIT_ABSOLUTE_MODE | SYN_BIT_HIGH_RATE;
    if (SYN_ID_MAJOR(synhw) >= 4) mode |= SYN_BIT_DISABLE_GESTURE;
    if (SYN_CAP_EXTENDED(synhw)) mode |= SYN_BIT_W_MODE;
    if (!ps2_synaptics_set_mode(pInfo->fd, mode)) return FALSE;
...
static Bool
ps2_synaptics_get_packet(InputInfoPtr                        pInfo,
                         struct PS2SynapticsHwInfo          *synhw,
                         ...
                         struct CommData                    *comm)
{
    ...
    while ((c = XisbRead(comm->buffer)) >= 0)
    {
        ...
        if ((c == 0x00) && (comm->lastByte == 0xAA))
        {
            ...
            else PS2DBG("faked reset received\n");
        }
        comm->lastByte = u;
        ...
        if (comm->protoBufTail >= 6)
        {
            if (!ps2_packet_ok(synhw, comm))
            {
                ...
                {
                    comm->outOfSync = 0;
                    PS2DBG("Synaptics synchronization lost too long -> reset "
                           "touchpad.\n");
                    ...
                    struct SynapticsHwState            *hwRet)
{
    ...
    if (!ps2_synaptics_get_packet(pInfo, synhw, proto_ops, comm)) return FALSE;
    ...
    { /* newer protos... */
        ...
        if (SYN_CAP_EXTENDED(synhw))
        {
            ...
            if (SYN_CAP_FOUR_BUTTON(synhw))
            {
                ...
                if (hw->left) hw->up = !hw->up;
                hw->down = ((buf[3] & 0x02)) ? 1 : 0;
                if (hw->right) hw->down = !hw->down;
            }
            if (SYN_CAP_MULTI_BUTTON_NO(synhw))
            {
