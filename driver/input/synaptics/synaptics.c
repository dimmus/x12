...
static int  HandleState(InputInfoPtr,
...
static XF86ModuleVersionInfo VersionRec = {
    ...
    SYNAPTICS_VERSION_MAJOR,
    SYNAPTICS_VERSION_MINOR,
    SYNAPTICS_VERSION_PATCHLEVEL,
    ABI_CLASS_XINPUT,
    ABI_XINPUT_VERSION,
    MOD_CLASS_XINPUT,
    { 0, 0, 0, 0 }
...
_X_EXPORT XF86ModuleData synapticsModuleData = { &VersionRec,
                                                 &SetupProc,
                                                 NULL };
...
SetDeviceAndProtocol(InputInfoPtr pInfo)
{
    ...
    {
        if ((!device || !proto) && protocols[i].proto_ops->AutoDevProbe &&
            protocols[i].proto_ops->AutoDevProbe(pInfo, device))
            break;
        else if (proto && !strcmp(proto, protocols[i].name)) break;
    }
    free(proto);
    free(device);
...
static void
calculate_tap_hysteresis(SynapticsPrivate *priv,
                         int               range,
                         int              *fingerLow,
                         int              *fingerHigh)
{
 ...
 */
static int
set_percent_option(pointer     options,
                   const char *optname,
                   const int   range,
                   const int   offset,
                   const int   default_value)
{
    ...
    else result = xf86SetIntOption(options, optname, default_value);
...
SynapticsIsSoftButtonAreasValid(int *values)
{
    ...
    if ((((values[RBL] != 0) && (values[RBR] != 0)) &&
         (values[RBL] > values[RBR])) ||
        (((values[RBT] != 0) && (values[RBB] != 0)) &&
         (values[RBT] > values[RBB])))
        return FALSE;

    /* Check middle button area */
    if ((((values[MBL] != 0) && (values[MBR] != 0)) &&
         (values[MBL] > values[MBR])) ||
        (((values[MBT] != 0) && (values[MBB] != 0)) &&
         (values[MBT] > values[MBB])))
        return FALSE;
    ...
    if (!right_disabled && ((values[RBL] && values[RBL] == values[RBR]) ||
                            (values[RBT] && values[RBT] == values[RBB])))
        return FALSE;

    if (!middle_disabled && ((values[MBL] && values[MBL] == values[MBR]) ||
                            ...
                            int          offset)
{
    ...
    for (i = 0; i < 8 && *next_num != '\0'; i++)
    {
        ...
        if (value > INT_MAX || value < -INT_MAX) goto fail;
    ...
    for (i = 0; in_percent && i < 8; i++)
    {
        ...
        if ((in_percent & (1 << i)) == 0 || values[i] == 0) continue;
    ...
    if (!SynapticsIsSoftButtonAreasValid(values)) goto fail;
...
set_default_parameters(InputInfoPtr pInfo)
{
    ...
    pars->clickpad =
        xf86SetBoolOption(opts, "ClickPad", pars->clickpad); /* Probed */
    if (pars->clickpad)
        pars->has_secondary_buttons =
            xf86SetBoolOption(opts,
    ...
    pars->grab_event_device =
        xf86SetBoolOption(opts, "GrabEventDevice", grab_event_device);
                             ...
                             double            acc)
{
    ...
    if (accelfct > para->max_speed * acc) accelfct = para->max_speed * acc;
    else if (accelfct < para->min_speed) accelfct = para->min_speed;
    ...
    if (priv->moving_state == MS_TOUCHPAD_RELATIVE)
    {
        ...
        {
            accelfct *= minFctr + (priv->hwState->z - minZ) *
                                      (maxFctr - minFctr) / (maxZ - minZ);
        }
    }
...
SetupProc_fail:
    ...
    if (priv->comm.buffer) XisbFree(priv->comm.buffer);
...
SynapticsUnInit(InputDriverPtr drv, InputInfoPtr pInfo, int flags)
{
    ...
    if (priv && priv->timer) free(priv->timer);
    if (priv && priv->proto_data) free(priv->proto_data);
    if (priv && priv->scroll_events_mask)
        valuator_mask_free(&priv->scroll_events_mask);
    if (priv && priv->open_slots) free(priv->open_slots);
    free(pInfo->private);
    pInfo->private = NULL;
...
DeviceOn(DeviceIntPtr dev)
{
    ...
    priv->comm.buffer = XisbNew(pInfo->fd, INPUT_BUFFER_SIZE);
    if (!priv->comm.buffer) goto error;

    xf86FlushInput(pInfo->fd);

    /* reinit the pad */
    if (!QueryHardware(pInfo)) goto error;
...
DeviceInit(DeviceIntPtr dev)
{
    ...
    if (NULL != (pVel = GetDevicePredictableAccelData(dev)))
    {
        ...
        xf86IDrvMsg(pInfo,
                    X_CONFIG,
                    "(accel) MinSpeed is now constant deceleration "
                    "%.1f\n",
                    tmpf);
    ...
    if (!priv->hwState) goto fail;

    priv->local_hw_state = SynapticsHwStateAlloc(priv);
    if (!priv->local_hw_state) goto fail;
 ...
 */
static void
relative_coords(SynapticsPrivate *priv,
                int               x,
                int               y,
                double           *relX,
                double           *relY)
...
circular_edge_detection(SynapticsPrivate *priv, int x, int y)
{
    ...
    if (relR > 1)
    {
        ...
        if (relX > M_SQRT1_2) edge |= RIGHT_EDGE;
        else if (relX < -M_SQRT1_2) edge |= LEFT_EDGE;

        if (relY < -M_SQRT1_2) edge |= TOP_EDGE;
        else if (relY > M_SQRT1_2) edge |= BOTTOM_EDGE;
    }
...
edge_detection(SynapticsPrivate *priv, int x, int y)
{
    ...
    if (priv->synpara.circular_pad) return circular_edge_detection(priv, x, y);

    if (x > priv->synpara.right_edge) edge |= RIGHT_EDGE;
    else if (x < priv->synpara.left_edge) edge |= LEFT_EDGE;

    if (y < priv->synpara.top_edge) edge |= TOP_EDGE;
    else if (y > priv->synpara.bottom_edge) edge |= BOTTOM_EDGE;
...
is_inside_active_area(SynapticsPrivate *priv, int x, int y)
{
    ...
    if (priv->finger_state >= FS_TOUCHED) return TRUE;
...
static Bool
is_inside_top_or_bottom_button_area(SynapticsParameters *para,
                                    int                  offset,
                                    int                  x,
                                    int                  y)
{
    ...
    if (!right_valid && !middle_valid) return FALSE;
    ...
    if (top && y < top) inside_area = FALSE;
    else if (bottom && y > bottom) inside_area = FALSE;
...
static Bool
SynapticsGetHwState(InputInfoPtr             pInfo,
                    SynapticsPrivate        *priv,
...
static int
HandleMidButtonEmulation(SynapticsPrivate        *priv,
                         struct SynapticsHwState *hw,
                         CARD32                   now,
                         int                     *delay)
{
    ...
    if (para->emulate_mid_button_time <= 0) return mid;
    ...
    while (!done)
    {
        switch (priv->mid_emu_state)
        {
            ...
            case MBE_OFF:
                ...
                break;
            case MBE_LEFT:
                timeleft = TIME_DIFF(priv->button_delay_millis +
                                         para->emulate_mid_button_time,
                                     now);
                if (timeleft > 0) *delay = MIN(*delay, timeleft);
...
SynapticsDetectFinger(SynapticsPrivate *priv, struct SynapticsHwState *hw)
{
    ...
    if (hw->z < para->finger_low) return FS_UNTOUCHED;

    if (priv->finger_state == FS_BLOCKED) return FS_BLOCKED;

    if (hw->z > para->finger_high && priv->finger_state == FS_UNTOUCHED)
        finger = FS_TOUCHED;
    else finger = priv->finger_state;
    ...
    if (priv->has_mt_palm_detect) return finger;

    if (hw->x == 0 || priv->finger_state == FS_UNTOUCHED) priv->avg_width = 0;
    else priv->avg_width += (hw->fingerWidth - priv->avg_width + 1) / 2;
    ...
    if (finger != FS_UNTOUCHED && priv->finger_state == FS_UNTOUCHED)
    {
        ...
        if (hw->numFingers > 1 || /* more than one finger -> not a palm */
            ((safe_width < 6) &&
             (priv->prev_z <
              para->finger_high)) || /* thin finger, distinct touch -> not a palm */
            ((safe_width < 7) && (priv->prev_z < para->finger_high / 2)))
        { /* thin finger, distinct touch -> not a palm */
            ...
            hw->fingerWidth >
            para->palm_min_width) /* finger width too large -> probably palm */
            finger = FS_UNTOUCHED;
    }
    priv->prev_z = hw->z;
...
static void
SetMovingState(SynapticsPrivate *priv,
               enum MovingState  moving_state,
               CARD32            millis)
{
    DBG(7,
        "SetMovingState - %d -> %d center at %d/%d (millis:%u)\n",
        priv->moving_state,
        moving_state,
        priv->hwState->x,
        priv->hwState->y,
...
static int
HandleTapProcessing(SynapticsPrivate        *priv,
                    struct SynapticsHwState *hw,
                    CARD32                   now,
                    enum FingerState         finger,
                    Bool                     inside_active_area)
{
    ...
    if (para->touchpad_off == TOUCHPAD_OFF || priv->finger_state == FS_BLOCKED)
        return delay;
...
restart:
    ...
    {
        case TS_START:
            if (touch) SetTapState(priv, TS_1, now);
            break;
        case TS_1:
        ...
        case TS_MOVE:
            ...
            break;
        case TS_2A:
            if (touch) SetTapState(priv, TS_3, now);
            else if (is_timeout) SetTapState(priv, TS_SINGLETAP, now);
            break;
        case TS_2B:
            if (touch) SetTapState(priv, TS_3, now);
            else if (is_timeout) SetTapState(priv, TS_SINGLETAP, now);
        ...
        case TS_SINGLETAP:
            if (touch) SetTapState(priv, TS_1, now);
            else if (is_timeout) SetTapState(priv, TS_START, now);
            break;
        case TS_3:
            if (move)
            {
        ...
        case TS_DRAG:
            ...
            if (move) SetMovingState(priv, MS_TOUCHPAD_RELATIVE, now);
            if (release)
            {
                SetMovingState(priv, MS_FALSE, now);
        ...
        case TS_4:
            ...
            if (touch) SetTapState(priv, TS_5, now);
            break;
        case TS_5:
            if (is_timeout || move)
            {
        ...
        case TS_CLICKPAD_MOVE:
            ...
            if (hw->numFingers > 1) hw->numFingers--;
            SetMovingState(priv, MS_TOUCHPAD_RELATIVE, now);
            if (!press)
            {
                SetMovingState(priv, MS_FALSE, now);
...
#define HIST(a)                                                           \
    (priv->move_hist[((priv->hist_index - (a) + SYNAPTICS_MOVE_HISTORY) % \
                      SYNAPTICS_MOVE_HISTORY)])
...
static void
get_delta(SynapticsPrivate              *priv,
 ...
 */
static int
ComputeDeltas(SynapticsPrivate              *priv,
              const struct SynapticsHwState *hw,
              enum EdgeType                  edge,
              int                           *dxP,
              int                           *dyP,
              Bool                           inside_area)
{
    ...
    if (priv->count_packet_finger <= 1) goto out; /* skip the lot */
 ...
 */
static void
start_coasting(SynapticsPrivate        *priv,
               struct SynapticsHwState *hw,
               Bool                     vert,
               Bool                     horiz,
               Bool                     circ)
{
    ...
    if ((priv->scroll.packets_this_scroll > 3) && (para->coasting_speed > 0.0))
    {
        ...
        if (vert && !circ)
        {
            ...
            if (pkt_time > 0)
            {
                double scrolls_per_sec =
                    (dy / abs(para->scroll_dist_vert)) / pkt_time;
        ...
        if (horiz && !circ)
        {
            ...
            {
                double scrolls_per_sec =
                    (dx / abs(para->scroll_dist_vert)) / pkt_time;
        ...
        if (circ)
        {
            ...
            {
                double scrolls_per_sec =
                    (da / para->scroll_dist_circ) / pkt_time;
...
static int
HandleScrolling(SynapticsPrivate        *priv,
                struct SynapticsHwState *hw,
                enum EdgeType            edge,
                Bool                     finger)
{
    ...
    if (!priv->circ_scroll_on)
    {
        ...
        if (finger && priv->finger_state == FS_UNTOUCHED)
        {
                ...
                !priv->horiz_scroll_twofinger_on)
            {
                ...
                if ((para->scroll_edge_horiz) &&
                    (para->scroll_dist_horiz != 0) && (edge & BOTTOM_EDGE))
                {
    ...
    {
        Bool oldv = priv->vert_scroll_twofinger_on ||
        ...
        oldv = oldv &&
               !(priv->vert_scroll_twofinger_on || priv->vert_scroll_edge_on ||
                 (priv->circ_scroll_on && priv->circ_scroll_vert));
    ...
    if (priv->circ_scroll_on)
    {
        ...
        if (delta >= 0.005 && diff != 0.0)
        {
            ...
            else priv->scroll.delta_x -= diff / delta * para->scroll_dist_horiz;
            priv->scroll.last_a = angle(priv, hw->x, hw->y);
        }
    }
    ...
    if (priv->scroll.coast_speed_y)
    {
        ...
        priv->scroll.delta_y +=
            priv->scroll.coast_speed_y * dtime * abs(para->scroll_dist_vert);
        delay = MIN(delay, POLL_MS);
    ...
    if (priv->scroll.coast_speed_x)
    {
        ...
        priv->scroll.delta_x +=
            priv->scroll.coast_speed_x * dtime * abs(para->scroll_dist_horiz);
        delay = MIN(delay, POLL_MS);
...
clickpad_guess_clickfingers(SynapticsPrivate *priv, struct SynapticsHwState *hw)
{
     ...
     */
    if (hw->numFingers >= 3 && hw->num_mt_mask < 3) nfingers = 3;
...
handle_clickfinger(SynapticsPrivate *priv, struct SynapticsHwState *hw)
{
    ...
    if (para->clickpad) nfingers = clickpad_guess_clickfingers(priv, hw);
                                ...
                                struct SynapticsHwState *hw)
{
    ...
    if (!para->updown_button_scrolling)
    {
        ...
        { /* up button generates double click */
            if (!priv->prev_up) double_click = TRUE;
        }
        priv->prev_up = hw->up;
    ...
    {
        if (hw->multi[2] || hw->multi[3]) hw->middle = TRUE;
...
static void
update_hw_button_state(const InputInfoPtr       pInfo,
                       struct SynapticsHwState *hw,
                       CARD32                   now,
                       int                     *delay)
{
    ...
    if (priv->repeatButtons)
    {
        ...
        if (timeleft <= 0)
        {
            ...
            while (change)
            {
                ...
                change &= ~(1 << (id - 1));
                if (id == 4) priv->scroll.delta_y -= para->scroll_dist_vert;
                else if (id == 5)
...
UpdateTouchState(InputInfoPtr pInfo, struct SynapticsHwState *hw)
{
    ...
    for (int i = 0; i < hw->num_mt_mask; i++)
    {
        if (hw->slot_state[i] == SLOTSTATE_OPEN)
        {
            ...
            {
                ErrorFSigSafe("BUG: synaptics: more active touches (%d) than "
                              "slots (%d)\n",
                              priv->num_active_touches,
                              priv->num_slots);
                xorg_backtrace();
        ...
        else if (hw->slot_state[i] == SLOTSTATE_CLOSE)
        {
            ...
            {
                if (priv->open_slots[j] == i) found = TRUE;

                if (found) priv->open_slots[j] = priv->open_slots[j + 1];
            }

            if (priv->num_active_touches == 0)
            {
                ErrorFSigSafe("BUG: synaptics: no active touches where there "
                              "should be at least one\n");
                xorg_backtrace();
            }

            if (priv->num_active_touches > 0) priv->num_active_touches--;
        }
    }
 ...
 */
static int
HandleState(InputInfoPtr             pInfo,
            struct SynapticsHwState *hw,
            CARD32                   now,
            Bool                     from_timer)
{
    ...
    ignore_motion =
        para->touchpad_off == TOUCHPAD_OFF ||
    ...
    if (priv->clickpad_click_millis)
    {
        if (TIME_DIFF(priv->clickpad_click_millis +
                      ...
                      now) > 0)
            ignore_motion = TRUE;
        else priv->clickpad_click_millis = 0;
    }
     ...
     */
    if (!inside_active_area) reset_hw_state(hw);

    /* no edge or finger detection outside of area */
    if (inside_active_area)
    {
        edge = edge_detection(priv, hw->x, hw->y);
        if (!from_timer) finger = SynapticsDetectFinger(priv, hw);
        else finger = priv->finger_state;
    }
    ...
    buttons =
        ((hw->left ? 0x01 : 0) | (hw->middle ? 0x02 : 0) |
         (hw->right ? 0x04 : 0) | (hw->up ? 0x08 : 0) | (hw->down ? 0x10 : 0) |
    ...
    if (inside_active_area) store_history(priv, hw->x, hw->y, hw->millis);
