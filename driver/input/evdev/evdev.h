...
#  define EVDEV_RELATIVE_MODE \
      (1 << 11) /* Force relative events for devices with absolute axes */
...
typedef struct
{
    ...
    ValuatorMask
        *old_vals; /* old absolute values for calculating relative motion */
...
void EvdevQueueTouchEvent(InputInfoPtr  pInfo,
                          unsigned int  touch,
                          ValuatorMask *mask,
                          uint16_t      type);
void
