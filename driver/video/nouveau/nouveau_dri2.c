...
nouveau_dri2_create_buffer2(ScreenPtr    pScreen,
                            DrawablePtr  pDraw,
                            ...
                            unsigned int format)
{
    ...
    if (!nvbuf) return NULL;

    if (attachment == DRI2BufferFrontLeft)
    {
        ppix = get_drawable_pixmap(pDraw);
        if (pScreen != ppix->drawable.pScreen) ppix = NULL;
    ...
    else
    {
        ...
        else usage_hint |= NOUVEAU_CREATE_PIXMAP_SCANOUT;
...
static DRI2BufferPtr
nouveau_dri2_create_buffer(DrawablePtr  pDraw,
                           unsigned int attachment,
                           unsigned int format)
{
    return nouveau_dri2_create_buffer2(pDraw->pScreen,
                                       pDraw,
                                       attachment,
                                       format);
}
...
static void
nouveau_dri2_destroy_buffer2(ScreenPtr     pScreen,
                             DrawablePtr   pDraw,
                             DRI2BufferPtr buf)
{
    struct nouveau_dri2_buffer *nvbuf;
    ...
    if (nvbuf->ppix) dixDestroyPixmap(nvbuf->ppix, 0);
    free(nvbuf);
}
...
static void
nouveau_dri2_copy_region2(ScreenPtr     pScreen,
                          DrawablePtr   pDraw,
                          RegionPtr     pRegion,
                          ...
                          DRI2BufferPtr pSrcBuffer)
{
...
#endif
    if (src->base.attachment == DRI2BufferFrontLeft) src_draw = pDraw;
...
#endif
            dst_draw = pDraw;
        if (dst_draw != pDraw) translate = TRUE;
    }
        ...
        REGION_NUM_RECTS(pRegion) == 1)
    {
            ...
            extents->x2 == pDraw->width && extents->y2 == pDraw->height)
        {
            ...
            if (bo) nouveau_bo_wait(bo, NOUVEAU_BO_RD, pNv->client);
        }
    }

    pGC->ops->CopyArea(src_draw,
                       dst_draw,
                       pGC,
                       0,
                       0,
...
static void
nouveau_dri2_copy_region(DrawablePtr   pDraw,
                         RegionPtr     pRegion,
                         DRI2BufferPtr pDstBuffer,
                         DRI2BufferPtr pSrcBuffer)
{
    return nouveau_dri2_copy_region2(pDraw->pScreen,
                                     pDraw,
                                     pRegion,
                                     pDstBuffer,
                                     pSrcBuffer);
...
update_front(DrawablePtr draw, DRI2BufferPtr front)
{
    ...
    if (draw->type == DRAWABLE_PIXMAP) pixmap = (PixmapPtr)draw;
    else pixmap = (*draw->pScreen->GetWindowPixmap)((WindowPtr)draw);
    ...
    if (!pixmap_bo) r = -1;
    else r = nouveau_bo_name_get(pixmap_bo, &front->name);
...
can_exchange(DrawablePtr draw, PixmapPtr dst_pix, PixmapPtr src_pix)
{
    ...
    if (!xf86_config->num_crtc) return FALSE;
    ...
    for (i = 0; i < xf86_config->num_crtc; i++)
    {
        ...
        {
            if (crtc->rotatedData) return FALSE;
    ...
    return ((DRI2CanFlip(draw) && pNv->has_pageflip)) &&
           ...
           dst_pix->devKind == src_pix->devKind && active_crtc_count;
}
...
can_sync_to_vblank(DrawablePtr draw)
{
    ...
    return pNv->glx_vblank && nv_window_belongs_to_crtc(scrn,
                                                        draw->x,
                                                        draw->y,
                                                        draw->width,
                                                        draw->height);
...
nouveau_dri2_swap_limit_validate(DrawablePtr draw, int swap_limit)
{
    ...
    if ((swap_limit < 1) || (swap_limit > pNv->max_swap_limit)) return FALSE;
...
static void
nouveau_dri2_flip_event_handler(unsigned int frame,
                                unsigned int tv_sec,
...
nouveau_dri2_flip_handler(void *priv, uint64_t name, uint64_t ust, uint32_t msc)
{
    ...
    flipdata->flip_count--;
    if (flipdata->flip_count > 0) return;
...
static Bool
dri2_page_flip(DrawablePtr draw,
               PixmapPtr   back,
               void       *priv,
               xf86CrtcPtr ref_crtc)
{
    ...
    for (i = 0; i < config->num_crtc; i++)
    {
        ...
        flipcarrier = drmmode_event_queue(scrn,
                                          ++dri2_sequence,
                                          sizeof(*flipcarrier),
        ...
        ret = drmModePageFlip(pNv->dev->fd,
                              head,
                              next_fb,
...
static void nouveau_dri2_finish_swap(DrawablePtr                       draw,
                                     unsigned int                      frame,
                                     unsigned int                      tv_sec,
                                     unsigned int                      tv_usec,
                                     struct nouveau_dri2_vblank_state *s);
...
static void
nouveau_dri2_vblank_handler(void    *priv,
                            uint64_t name,
                            uint64_t ust,
                            uint32_t frame)
{
    ...
    ret =
        dixLookupDrawable(&draw, s->draw, serverClient, M_ANY, DixWriteAccess);
    if (ret)
    {
        free(s);
        return;
...
static int
nouveau_wait_vblank(DrawablePtr draw,
                    int         type,
                    CARD64      msc,
                    CARD64     *pmsc,
                    CARD64     *pust,
                    void       *data)
{
    ...
    {
        event = drmmode_event_queue(scrn,
                                    ++dri2_sequence,
                                    sizeof(*event),
    ...
    if (head == 1) type |= DRM_VBLANK_SECONDARY;
    else if (head > 1)
#ifdef DRM_VBLANK_HIGH_CRTC_SHIFT
        type |=
            (head << DRM_VBLANK_HIGH_CRTC_SHIFT) & DRM_VBLANK_HIGH_CRTC_MASK;
#else
        xf86DrvMsg(scrn->scrnIndex,
                   X_WARNING,
    ...
    if (ret)
    {
        xf86DrvMsg(scrn->scrnIndex,
                   ...
                   strerror(errno));
        if (event) drmmode_event_abort(scrn, dri2_sequence--, false);
        return ret;
    }

    if (pmsc) *pmsc = vbl.reply.sequence;
    if (pust)
        *pust = (CARD64)vbl.reply.tval_sec * 1000000 + vbl.reply.tval_usec;
    return 0;
}

static void
nouveau_dri2_finish_swap(DrawablePtr                       draw,
                         unsigned int                      frame,
                         unsigned int                      tv_sec,
                         unsigned int                      tv_usec,
                         struct nouveau_dri2_vblank_state *s)
{
    ...
    ref_crtc = nouveau_pick_best_crtc(scrn,
                                      draw->x,
                                      draw->y,
                                      draw->width,
                                      draw->height);
         ...
         (!pNv->has_async_pageflip && nouveau_exa_pixmap_is_onscreen(dst_pix))))
    {
        ...
        nouveau_pushbuf_refn(
            push,
            &(struct nouveau_pushbuf_refn){ src_bo,
        ...
        else NV11SyncToVBlank(dst_pix, REGION_EXTENTS(0, &reg));
    ...
    if (will_exchange)
    {
        ...
        if ((type == DRI2_FLIP_COMPLETE) && !violate_oml(draw)) return;
    }
    else
    {
        ...
        nouveau_pushbuf_refn(
            push,
            &(struct nouveau_pushbuf_refn){ dst_bo,
                                            NOUVEAU_BO_VRAM | NOUVEAU_BO_RD },
            1);
        ...
        if (can_sync_to_vblank(draw) && !violate_oml(draw))
        {
			 ...
			 */
            s->action = BLIT;
            ret       = nouveau_wait_vblank(draw,
                                      DRM_VBLANK_EVENT | DRM_VBLANK_RELATIVE,
                                      1,
                                      NULL,
                                      NULL,
                                      s);
...
static Bool
nouveau_dri2_schedule_swap(ClientPtr        client,
                           DrawablePtr      draw,
                           DRI2BufferPtr    dst,
                           DRI2BufferPtr    src,
                           CARD64          *target_msc,
                           ...
                           void            *data)
{
    ...
    *s = (struct nouveau_dri2_vblank_state){ SWAP, client, draw->id, dst,
                                             src,  func,   data,     0 };
    ...
    if (can_sync_to_vblank(draw))
    {
        ...
        *target_msc &= 0xffffffff;
        ...
        if (current_msc >= *target_msc && divisor)
            *target_msc =
                current_msc + divisor - (current_msc - remainder) % divisor;

        /* Avoid underflow of unsigned value below */
        if (*target_msc == 0) *target_msc = 1;
...
static Bool
nouveau_dri2_schedule_wait(ClientPtr   client,
                           DrawablePtr draw,
                           CARD64      target_msc,
                           CARD64      divisor,
                           CARD64      remainder)
...
nouveau_dri2_init(ScreenPtr pScreen)
{
    ...
    if (pNv->AccelMethod != EXA) return FALSE;

    if (pNv->Architecture >= NV_ARCH_30) dri2.driverNames = drivernames[0];
    else dri2.driverNames = drivernames[1];
    dri2.numDrivers = 2;
...
nouveau_dri2_fini(ScreenPtr pScreen)
{
    ScrnInfoPtr pScrn = xf86ScreenToScrn(pScreen);
    NVPtr       pNv   = NVPTR(pScrn);
    if (pNv->AccelMethod == EXA) DRI2CloseScreen(pScreen);
}

#ifdef DRI3
static int
is_render_node(int fd)
{
    struct stat st;
...
#  endif
    if (fd < 0) fd = open(pNv->render_node, O_RDWR);
...
static PixmapPtr
nouveau_dri3_pixmap_from_fd(ScreenPtr screen,
                            int       fd,
                            CARD16    width,
                            CARD16    height,
                            CARD16    stride,
                            ...
                            CARD8     bpp)
{
    ...
    if (depth < 8 || depth > 32) return NULL;

    pixmap = screen->CreatePixmap(screen, 0, 0, depth, 0);
    if (!pixmap) return NULL;

    if (pixmap->drawable.bitsPerPixel % 8) goto free_pixmap;

    if (!screen->ModifyPixmapHeader(pixmap, width, height, 0, 0, stride, NULL))
        goto free_pixmap;

    if (nouveau_bo_prime_handle_ref(pNv->dev, fd, &bo)) goto free_pixmap;
...
static int
nouveau_dri3_fd_from_pixmap(ScreenPtr screen,
                            PixmapPtr pixmap,
                            CARD16   *stride,
                            CARD32   *size)
{
    ...
    if (!bo || nouveau_bo_set_prime(bo, &fd) < 0) return -EINVAL;
...
nouveau_dri3_screen_init(ScreenPtr screen)
{
    ...
    if (is_render_node(pNv->dev->fd)) return TRUE;
    ...
    if (buf)
    {
        ...
        {
            xf86DrvMsg(pScrn->scrnIndex, X_INFO, "DRI3 on EXA enabled\n");
            return TRUE;
        }
        else
        {
