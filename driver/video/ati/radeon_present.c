...
radeon_present_get_crtc(WindowPtr window)
{
    ...
    if (crtc) randr_crtc = crtc->randr_crtc;
...
radeon_present_flush_drm_events(ScreenPtr screen)
{
    ...
    return radeon_drm_handle_event(pRADEONEnt->fd, &drmmode->event_context) >=
           0;
}
 ...
 */
static void
radeon_present_vblank_handler(xf86CrtcPtr  crtc,
                              unsigned int msc,
                              uint64_t     usec,
                              void        *data)
{
 ...
 */
static Bool
radeon_present_check_flip(RRCrtcPtr crtc,
                          WindowPtr window,
                          PixmapPtr pixmap,
                          Bool      sync_flip)
{
...
#  endif
    ...
    {
#  ifdef USE_GLAMOR
        if (!priv) priv = radeon_get_pixmap_private(pixmap);
 ...
 */
static void
radeon_present_flip_event(xf86CrtcPtr crtc,
                          uint32_t    msc,
                          uint64_t    ust,
                          void       *pageflip_data)
{
 ...
 */
static Bool
radeon_present_flip(RRCrtcPtr crtc,
                    uint64_t  event_id,
                    uint64_t  target_msc,
                    PixmapPtr pixmap,
                    Bool      sync_flip)
...
radeon_present_unflip(ScreenPtr screen, uint64_t event_id)
{
    ...
    if (!radeon_present_check_unflip(scrn)) goto modeset;
