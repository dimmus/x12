                      ...
                      int         y2)
{
    ...
    for (cd = 0; cd < (consider_disabled ? 2 : 1); cd++)
    {
        for (c = 0; c < xf86_config->num_crtc; c++)
        {
            ...
            if (!cd && !radeon_crtc_is_enabled(crtc)) continue;
...
void
RADEONInitVideo(ScreenPtr pScreen)
{
    ...
    if (info->ChipFamily == CHIP_FAMILY_RV100 && !pRADEONEnt->HasCRTC2) return;
    ...
    if (texturedAdaptor)
    {
        XF86MCAdaptorPtr xvmcAdaptor =
            RADEONCreateAdaptorXvMC(pScreen, (char *)texturedAdaptor->name);
        if (xvmcAdaptor)
        {
                     ...
                     unsigned int   w)
{
    ...
    for (j = 0; j < h; j++)
    {
        ...
        while (i > 4)
        {
            dst[0] = cpu_to_le32(s1[0] | (s1[1] << 16) | (s3[0] << 8) |
                                 (s2[0] << 24));
            dst[1] = cpu_to_le32(s1[2] | (s1[3] << 16) | (s3[1] << 8) |
                                 (s2[1] << 24));
            dst[2] = cpu_to_le32(s1[4] | (s1[5] << 16) | (s3[2] << 8) |
                                 (s2[2] << 24));
            dst[3] = cpu_to_le32(s1[6] | (s1[7] << 16) | (s3[3] << 8) |
                                 (s2[3] << 24));
            dst += 4;
            s2 += 4;
            s3 += 4;
