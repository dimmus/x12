...
static int
open_master_node(ScreenPtr screen, int *out)
{
...
radeon_dri3_open(ScreenPtr screen, RRProviderPtr provider, int *out)
{
    ...
    if (pRADEONEnt->render_node) ret = open_render_node(screen, out);

    if (ret != Success) ret = open_master_node(screen, out);
                        ...
                        int          *out)
{
    ...
    if (!is_ssh) return radeon_dri3_open(screen, provider, out);
...
static PixmapPtr
radeon_dri3_pixmap_from_fd(ScreenPtr screen,
                           int       fd,
                           CARD16    width,
                           CARD16    height,
                           CARD16    stride,
...
#  endif
    ...
    pixmap =
        screen->CreatePixmap(screen, 0, 0, depth, RADEON_CREATE_PIXMAP_DRI2);
    if (!pixmap) return NULL;

    if (!screen
             ->ModifyPixmapHeader(pixmap, width, height, 0, bpp, stride, NULL))
        goto free_pixmap;
...
static int
radeon_dri3_fd_from_pixmap(ScreenPtr screen,
                           PixmapPtr pixmap,
                           CARD16   *stride,
                           CARD32   *size)
{
...
#  ifdef USE_GLAMOR
    ...
    if (info->use_glamor)
    {
		 ...
		 */
        if (ret >= 0) radeon_cs_flush_indirect(scrn);
...
#  endif
    ...
    if (pixmap->devKind > UINT16_MAX) return -1;

    if (radeon_gem_prime_share_bo(bo->bo.radeon, &fd) < 0) return -1;
