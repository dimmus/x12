...
#define error_get() setjmp(error_handler[have_error_handler++])

#define error_put() have_error_handler--

static int (*saved_io_error)(Display *dpy);

static int
io_error(Display *dpy)
{
    if (have_error_handler) longjmp(error_handler[--have_error_handler], 0);
             ...
             int           nattachments)
{
    ...
    seq[0] = xcb_dri2_swap_buffers_unchecked(c, win, 0, 0, 0, divisor, 0, 0)
                 .sequence;
...
has_composite(Display *dpy)
{
    ...
    if (dpy == NULL) dummy = dpy = XOpenDisplay(NULL);

    if (XCompositeQueryExtension(dpy, &event, &error))
        XCompositeQueryVersion(dpy, &major, &minor);

    if (dummy) XCloseDisplay(dummy);

    return major > 0 || minor >= 4;
}

static void
race_window(Display      *dpy,
            int           width,
            int           height,
            unsigned int *attachments,
            int           nattachments,
            ...
            const char   *name)
{
    ...
    if (flags & COMPOSITE && !has_composite(dpy)) return;
    ...
    {
        loop = 256 >> ffs(divisors[n]);
        printf("xcb_dri2_swap_buffers(divisor=%d), loops=%d",
               divisors[n],
               loop);
        fflush(stdout);
        do
           ...
           const char   *name)
{
    ...
    if (flags & COMPOSITE && !has_composite(NULL)) return;
...
main(void)
{
    ...
    race_window(dpy,
                ...
                2,
                COMPOSITE,
                "composite fullscreen (with front)");
