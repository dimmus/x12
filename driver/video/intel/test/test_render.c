...
const char *
test_target_name(enum target target)
{
    switch (target)
    {
        default:
...
void
test_target_create_render(struct test_display *dpy,
                          enum target          target,
                          struct test_target  *tt)
{
    ...
    switch (target)
    {
        ...
        case PIXMAP:
            tt->format =
                XRenderFindStandardFormat(dpy->dpy, PictStandardARGB32);
            tt->draw  = XCreatePixmap(dpy->dpy,
                                     tt->draw,
                                     dpy->width,
                                     dpy->height,
                                     tt->format->depth);
    ...
    tt->picture = XRenderCreatePicture(dpy->dpy, tt->draw, tt->format, 0, NULL);
...
void
test_target_destroy_render(struct test_display *dpy, struct test_target *tt)
{
