...
static void
fill_rect(struct test_display *t,
          Picture              p,
          ...
          uint8_t              alpha)
{
    ...
    if (use_window) XDestroyWindow(t->dpy, tmp);
    else XFreePixmap(t->dpy, tmp);
}

static void
pixel_tests(struct test *t,
            int          reps,
            int          sets,
            enum target  target,
            int          use_window)
...
static void
set_mask(struct test_display *t, struct test_target *tt, enum mask mask)
{
           ...
           int          use_window)
{
    ...
    for (s = 0; s < sets; s++)
    {
        for (r = 0; r < reps; r++)
        {
            ...
            while (((tx + w > x && tx < x + w) && (ty + h > y && ty < y + h)) &&
                   --try);
...
static void
fill_traps(struct test_display *t,
           Picture              p,
           XRenderPictFormat   *format,
           uint8_t              op,
           XTrapezoid          *traps,
