...
int
pixel_difference(uint32_t a, uint32_t b)
{
    int max = 0;
    int i;
            ...
            int           h)
{
    ...
    for (j = y - 2; j <= y + 2; j++)
    {
        ...
        for (i = x - 2; i <= x + 2; i++)
        {
            ...
            len += sprintf(buf + len,
                           "%08x ",
                           *(uint32_t *)(out->data + j * out->bytes_per_line +
        ...
        for (i = x - 2; i <= x + 2; i++)
        {
            ...
            len += sprintf(buf + len,
                           "%08x ",
                           *(uint32_t *)(ref->data + j * out->bytes_per_line +
                                         i * out->bits_per_pixel / 8));
        }
...
static void
test_compare_fallback(struct test       *t,
                      Drawable           out_draw,
                      XRenderPictFormat *out_format,
                      Drawable           ref_draw,
                      XRenderPictFormat *ref_format,
                      int                x,
                      int                y,
                      int                w,
                      int                h)
{
    ...
    out_image = XGetImage(t->out.dpy, out_draw, x, y, w, h, AllPlanes, ZPixmap);
    out       = out_image->data;

    ref_image = XGetImage(t->ref.dpy, ref_draw, x, y, w, h, AllPlanes, ZPixmap);
    ref       = ref_image->data;
    ...
    for (j = 0; j < h; j++)
    {
        for (i = 0; i < w; i++)
        {
            ...
            {
                show_pixels(buf, out_image, ref_image, i, j, w, h);
                die("discrepancy found at (%d+%d, %d+%d): found %08x, expected "
                    "%08x (delta: %d)\n%s",
                    x,
                    i,
                    y,
...
static void
save_image(XImage *image, const char *filename)
{
    FILE       *file;
    png_struct *png  = NULL;
    png_info   *info = NULL;
