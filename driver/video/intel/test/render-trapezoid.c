...
mask_format(Display *dpy, enum mask mask)
{
    switch (mask)
    {
        default:
...
static const char *
trapezoid_name(enum trapezoid trapezoid)
{
    switch (trapezoid)
    {
        default:
...
static void
fill_rect(struct test_display *dpy,
          Picture              p,
          uint8_t              op,
          int                  x,
          int                  y,
...
static void
pixel_tests(struct test *t, int reps, int sets, enum target target)
{
...
static void
area_tests(struct test *t, int reps, int sets, enum target target)
{
    struct test_target tt;
    XImage             image;
    ...
    for (s = 0; s < sets; s++)
    {
        for (r = 0; r < reps; r++)
        {
            ...
            if (x < 0) w += x, x = 0;
            if (y < 0) h += y, y = 0;
            if (x >= tt.width || y >= tt.height) continue;

            if (x + w > tt.width) w = tt.width - x;
            if (y + h > tt.height) h = tt.height - y;
            if (w <= 0 || h <= 0) continue;
        ...
        for (y = 0; y < tt.height; y++)
        {
            for (x = 0; x < tt.width; x++)
            {
                ...
                if (!pixel_equal(image.depth, result, cells[y * tt.width + x]))
                {
                    ...
                    die("failed to set pixel (%d,%d) to %08x [%08x], found "
                        "%08x [%08x] instead\n",
                        x,
                        y,
                        cells[y * tt.width + x] & mask,
...
static void
random_trapezoid(XTrapezoid    *trap,
    ...
    for (s = 0; s < sets; s++)
    {
        for (r = 0; r < reps; r++)
        {
            ...
            for (n = 0; n < num_traps; n++)
                random_trapezoid(&traps[n], 0, 0, 0, out.width, out.height);
            ...
            src = XRenderCreateSolidFill(t->out.dpy, &render_color);
            ...
            src = XRenderCreateSolidFill(t->ref.dpy, &render_color);
...
static void
set_edge(Display *dpy, Picture p, enum edge edge)
{
...
static void
edge_test(struct test *t, enum mask mask, enum edge edge, enum target target)
{
    struct test_target out, ref;
