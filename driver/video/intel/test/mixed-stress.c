...
static void
_render_copy(struct test_target *tt,
             int                 x,
             int                 y,
             int                 w,
             int                 h,
             ...
             uint8_t             alpha)
{
    ...
    tmp = XCreatePixmap(tt->dpy->dpy,
                        DefaultRootWindow(tt->dpy->dpy),
                        10 + w,
                        20 + h,
                        tt->format->depth);
...
static void
render_copy(struct test_target *out, struct test_target *ref)
{
...
static void
fill_rect(struct test_target *tt,
          int                 alu,
          int                 color,
          int                 x,
          int                 y,
...
{
    fill_rect(tt, GXcopy, 0, 0, 0, tt->width, tt->height);
}

static void
basic_fill(struct test_target *out, struct test_target *ref)
{
    int x     = rand() % (2 * out->width) - out->width;
    int y     = rand() % (2 * out->height) - out->height;
    int w     = rand() % (2 * out->width);
...
static void
basic_copy(struct test_target *out, struct test_target *ref)
{
...
static void
_put(struct test_target *tt, int x, int y, int w, int h, int color, int alu)
{
    XImage    image;
    XGCValues val;
    ...
    {
        XPutImage(tt->dpy->dpy, tt->draw, tt->gc, &image, 0, 0, x, y, w, h);
    }
}

static void
basic_put(struct test_target *out, struct test_target *ref)
{
    int x     = rand() % (2 * out->width) - out->width;
    int y     = rand() % (2 * out->height) - out->height;
...
static void
rect_tests(struct test *test, int iterations, enum target target)
{
    ...
    printf("Running mixed ops stress against %s: ", test_target_name(target));
    fflush(stdout);
