...
static unsigned char bitmap4x4[] = { 0x03, 0x06, 0x0c, 0x09 };
          ...
          uint32_t            bg)
{
    ...
    {
        val.stipple =
            XCreateBitmapFromData(dpy, t->draw, (char *)bitmap4x4, 4, 4);
    }
    else
    {
    ...
    gc = XCreateGC(dpy,
                   ...
                   GCFillStyle | GCTileStipXOrigin | GCTileStipYOrigin |
                       GCStipple | GCForeground | GCBackground | GCFunction,
                   &val);
...
simple_clip_tests(struct test *t, int reps, int sets, enum target target)
{
    ...
    printf("Testing simple clipped stippled fills (%s): ",
...
complex_clip_tests(struct test *t, int reps, int sets, enum target target)
{
    ...
    printf("Testing complex clipped stippled fills (%s): ",
           test_target_name(target));
    fflush(stdout);
