...
#define die_unless(expr)                                      \
    do                                                        \
    {                                                         \
        if (!(expr)) die("verification failed: %s\n", #expr); \
    }                                                         \
    while (0)
...
static inline int
pixel_equal(int depth, uint32_t a, uint32_t b)
{
    if (depth != 32)
    {
...
void test_init_image(XImage            *ximage,
                     XShmSegmentInfo   *shm,
                     XRenderPictFormat *format,
                     int                width,
                     int                height);
...
depth_mask(int depth)
{
    if (depth == 32) return 0xffffffff;
    else return (1 << depth) - 1;
}
...
static inline uint32_t
xrender_color(const XRenderColor *c)
{
