...
static Window
get_root(struct test_display *t)
{
    ...
    w                      = XCreateWindow(t->dpy,
                      DefaultRootWindow(t->dpy),
                      0,
                      0,
                      t->width,
                      t->height,
...
static Display *
out_display(int argc, char **argv)
{
    Display    *dpy;
    ...
    if (name == NULL) name = getenv("DISPLAY");
    if (name == NULL) name = ":0"; /* useful default */

    dpy = XOpenDisplay(name);
    if (dpy == NULL) die("unable to open out display %s\n", name);
...
static Display *
ref_display(int width, int height, int depth)
{
    Display    *dpy;
    char        buf[160];
    const char *name;
    ...
    {
        dpy = XOpenDisplay(name);
        if (dpy == NULL) die("unable to open reference display %s\n", name);
    ...
    snprintf(buf,
             ...
             height,
             depth);
    if (system(buf)) die("unable to spawn '%s' for reference display\n", buf);
    ...
    if (dpy == NULL) die("unable to open reference display\n");

    return dpy;
}

static void
shm_setup(struct test_display *d)
{
    int major, minor;
    int size;

    XShmQueryVersion(d->dpy, &major, &minor, &d->has_shm_pixmaps);
    if (major == 0 && minor == 0) die("XSHM not supported\n");
...
void
test_timer_start(struct test_display *t, struct timespec *tv)
{
    clock_gettime(CLOCK_MONOTONIC, tv);
}

double
test_timer_stop(struct test_display *t, struct timespec *tv)
{
    XImage         *image;
    struct timespec now;
