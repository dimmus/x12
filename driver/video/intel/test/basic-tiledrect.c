...
reset_cache(void)
{
    ...
    for (n = 0; n < sizeof(bitmaps) / sizeof(bitmaps[0]); n++)
    {
        for (m = 0; m < 2; m++)
        {
            ...
            {
                XFreePixmap(bitmaps[n].cached[m].dpy,
                            bitmaps[n].cached[m].pixmap);
...
static void
fill_rect(struct test_target *t,
          uint8_t             alu,
          XRectangle         *clip,
          int                 nclip,
          uint8_t             tile,
          ...
          uint32_t            bg)
{
    ...
    struct bitmap *b =
        &bitmaps[(tile >> 1) % (sizeof(bitmaps) / sizeof(bitmaps[0]))];
    XGCValues val;
    GC        gc;
    int       n;
    ...
    if (tile & 1)
    {
        ...
        if (val.tile == 0)
        {
            val.tile = XCreatePixmapFromBitmapData(dpy,
                                                   t->draw,
                                                   (char *)data,
                                                   b->width,
                                                   b->height,
    ...
    gc = XCreateGC(dpy,
                   ...
                   GCFillStyle | GCTileStipXOrigin | GCTileStipYOrigin |
                       GCTile | GCFunction,
                   &val);
    if (nclip) XSetClipRectangles(dpy, gc, 0, 0, clip, nclip, Unsorted);
    XFillRectangle(dpy, t->draw, gc, x, y, w, h);
    XFreeGC(dpy, gc);
    if ((tile & 1) == 0) XFreePixmap(dpy, val.tile);
}

static void
clear(struct test_target *tt)
{
    XRenderColor render_color = { 0 };
    XRenderFillRectangle(tt->dpy->dpy,
                         PictOpClear,
                         tt->picture,
                         &render_color,
                         0,
...
static void
small_tests(struct test *t, int reps, int sets, enum target target)
{
    ...
    for (s = 0; s < sets; s++)
    {
        for (r = 0; r < reps; r++)
        {
            ...
            fill_rect(&out, alu, NULL, 0, tile, tx, ty, x, y, w, h, fg, bg);
            fill_rect(&ref, alu, NULL, 0, tile, tx, ty, x, y, w, h, fg, bg);

            fill_rect(&out, alu, NULL, 0, tile, tx, ty, x, y, h, w, fg, bg);
            fill_rect(&ref, alu, NULL, 0, tile, tx, ty, x, y, h, w, fg, bg);
        }
...
static void
unclipped_tests(struct test *t, int reps, int sets, enum target target)
{
    struct test_target out, ref;
    int                r, s;
...
static void
simple_clip_tests(struct test *t, int reps, int sets, enum target target)
{
    struct test_target out, ref;
    int                r, s;

    printf("Testing simple clipped tiled fills (%s): ",
...
static void
complex_clip_tests(struct test *t, int reps, int sets, enum target target)
{
    struct test_target out, ref;
    XRectangle        *clip;
    int                nclip, r, s;

    printf("Testing complex clipped tiled fills (%s): ",
           test_target_name(target));
    fflush(stdout);
    ...
    for (s = 0; s < sets; s++)
    {
        ...
        for (r = 0; r < reps; r++)
        {
            ...
            fill_rect(&out, alu, clip, nclip, tile, tx, ty, x, y, w, h, fg, bg);
            fill_rect(&ref, alu, clip, nclip, tile, tx, ty, x, y, w, h, fg, bg);
        }
