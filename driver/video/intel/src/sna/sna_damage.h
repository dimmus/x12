...
_sna_damage_all(struct sna_damage *damage, int width, int height)
{
...
static inline void
sna_damage_all(struct sna_damage **damage, PixmapPtr pixmap)
{
...
static inline void
sna_damage_combine(struct sna_damage **l, struct sna_damage *r, int dx, int dy)
{
...
static inline void
sna_damage_add(struct sna_damage **damage, RegionPtr region)
{
...
static inline bool
sna_damage_add_to_pixmap(struct sna_damage **damage,
                         RegionPtr           region,
                         PixmapPtr           pixmap)
{
    assert(!DAMAGE_IS_ALL(*damage));
...
static inline void
sna_damage_add_box(struct sna_damage **damage, const BoxRec *box)
{
    assert(!DAMAGE_IS_ALL(*damage));
...
static inline void
sna_damage_add_boxes(struct sna_damage **damage,
                     const BoxRec       *box,
                     int                 n,
                     int16_t             dx,
                     int16_t             dy)
...
static inline void
sna_damage_add_rectangles(struct sna_damage **damage,
                          const xRectangle   *r,
                          int                 n,
                          int16_t             dx,
                          int16_t             dy)
...
static inline void
sna_damage_add_points(struct sna_damage **damage,
                      const DDXPointRec  *p,
                      int                 n,
                      int16_t             dx,
                      int16_t             dy)
...
static inline bool
sna_damage_is_all(struct sna_damage **_damage, int width, int height)
{
...
static inline void
sna_damage_subtract(struct sna_damage **damage, RegionPtr region)
{
...
static inline void
sna_damage_subtract_box(struct sna_damage **damage, const BoxRec *box)
{
...
static inline void
sna_damage_subtract_boxes(struct sna_damage **damage,
                          const BoxRec       *box,
                          int                 n,
                          int                 dx,
                          int                 dy)
{
    *damage = _sna_damage_subtract_boxes(DAMAGE_PTR(*damage), box, n, dx, dy);
    assert(*damage == NULL || (*damage)->mode != DAMAGE_ALL);
}
...
static inline bool
sna_damage_intersect(struct sna_damage *damage,
                     RegionPtr          region,
                     RegionPtr          result)
{
    assert(damage);
...
static inline bool
sna_damage_overlaps_box(const struct sna_damage *damage, const BoxRec *box)
{
    if (box->x2 <= damage->extents.x1 || box->x1 >= damage->extents.x2)
        return false;

    if (box->y2 <= damage->extents.y1 || box->y1 >= damage->extents.y2)
        return false;

    return true;
}

int _sna_damage_contains_box(struct sna_damage **damage, const BoxRec *box);

static inline int
sna_damage_contains_box(struct sna_damage **damage, const BoxRec *box)
{
    if (DAMAGE_IS_ALL(*damage)) return PIXMAN_REGION_IN;
    if (*damage == NULL) return PIXMAN_REGION_OUT;

    return _sna_damage_contains_box(damage, box);
}

static inline int
sna_damage_contains_box__offset(struct sna_damage **damage,
                                const BoxRec       *box,
                                int                 dx,
                                int                 dy)
{
...
sna_damage_get_boxes(struct sna_damage *damage, const BoxRec **boxes)
{
    ...
    else return _sna_damage_get_boxes(damage, boxes);
}

struct sna_damage *_sna_damage_reduce(struct sna_damage *damage);

static inline void
sna_damage_reduce(struct sna_damage **damage)
{
...
static inline void
sna_damage_reduce_all(struct sna_damage **_damage, PixmapPtr pixmap)
{
    struct sna_damage *damage = *_damage;

    if (damage == NULL || DAMAGE_IS_ALL(damage)) return;
    ...
    {
        if (damage->extents.x1 <= 0 && damage->extents.y1 <= 0 &&
    ...
    else *_damage = _sna_damage_reduce(damage);
}

void __sna_damage_destroy(struct sna_damage *damage);

static inline void
sna_damage_destroy(struct sna_damage **damage)
{
    if (*damage == NULL) return;

    if (DAMAGE_PTR(*damage)) __sna_damage_destroy(DAMAGE_PTR(*damage));
    *damage = NULL;
}
