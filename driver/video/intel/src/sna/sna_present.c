...
static struct sna_present_event *
info_alloc(struct sna *sna)
{
...
static void
info_free(struct sna_present_event *info)
{
    struct sna *sna = info->sna;

    if (sna->present.freed_info) free(sna->present.freed_info);

    sna->present.freed_info = info;
}

static inline bool
msc_before(uint64_t msc, uint64_t target)
{
...
unmask_crtc(xf86CrtcPtr crtc)
{
    return (xf86CrtcPtr)((uintptr_t)crtc & ~1);
}

static inline xf86CrtcPtr
mark_crtc(xf86CrtcPtr crtc)
{
    return (xf86CrtcPtr)((uintptr_t)crtc | 1);
}

static inline bool
has_vblank(xf86CrtcPtr crtc)
{
    return (uintptr_t)crtc & 1;
}

static inline int
crtc_index_from_crtc(RRCrtcPtr crtc)
{
...
static inline int
sna_wait_vblank(struct sna *sna, union drm_wait_vblank *vbl, int index)
{
...
gettime_ust64(void)
{
    ...
    if (clock_gettime(CLOCK_MONOTONIC, &tv)) return GetTimeInMicros();

    return ust64(tv.tv_sec, tv.tv_nsec / 1000);
}

static void
vblank_complete(struct sna_present_event *info, uint64_t ust, uint64_t msc)
{
    ...
    do
    {
        ...
        if (msc_before(msc, info->target_msc))
        {
            ...
            if (sna_present_queue(info, msc)) return;
        }

        DBG(("%s: %d events complete\n", __FUNCTION__, info->n_event_id));
        for (n = 0; n < info->n_event_id; n++)
        {
            DBG(("%s: certc=%d tv=%d.%06d msc=%lld (target=%lld), event=%lld "
                 "complete%s\n",
                 __FUNCTION__,
                 ...
                 info->target_msc && msc == (uint32_t)info->target_msc
                     ? ""
                     : ": MISS"));
            present_event_notify(info->event_id[n], ust, msc);
        }
        if (info->n_event_id > 1) free(info->event_id);
...
static uint32_t
msc_to_delay(xf86CrtcPtr crtc, uint64_t target)
{
    ...
    assert(delay >= 0);
    if (delay > 1)
    { /* try to use the hw vblank for the last frame */
        delay--;
        subframe = 0;
    }
    else
    ...
    if (subframe < delay) delay -= subframe;
    else delay = 0;
...
static void
add_to_crtc_vblank(struct sna_present_event *info, int delta)
{
    info->active = true;
...
static CARD32
sna_fake_vblank_handler(OsTimerPtr timer, CARD32 now, void *data)
{
    ...
    if (sna_wait_vblank(info->sna, &vbl, sna_crtc_index(info->crtc)) == 0)
    {
        ...
        if (msc_before(msc, info->target_msc))
        {
            ...
            if (delta <= 2)
            {
                ...
                if (sna_wait_vblank(info->sna,
                                    &vbl,
                                    sna_crtc_index(info->crtc)) == 0)
                {
            ...
            vbl.request.sequence = info->target_msc;
            if (sna_wait_vblank(info->sna, &vbl, sna_crtc_index(info->crtc)) ==
                0)
            {
                ust = ust64(vbl.reply.tval_sec, vbl.reply.tval_usec);
                msc = sna_crtc_record_vblank(info->crtc, &vbl);
...
fixup:
        ...
        DBG(("%s: event=%lld, CRTC OFF, target msc=%lld, was %lld (off)\n",
             ...
             (long long)info->event_id[0],
             (long long)info->target_msc,
             (long long)sna_crtc_last_swap(info->crtc)->msc));
    }
...
static bool
sna_fake_vblank(struct sna_present_event *info)
{
...
static bool
sna_present_queue(struct sna_present_event *info, uint64_t last_msc)
{
    union drm_wait_vblank vbl;
        ...
        sna_wait_vblank(info->sna, &vbl, sna_crtc_index(info->crtc)))
    {
        DBG(("%s: vblank enqueue failed, faking delta=%d\n",
             __FUNCTION__,
             delta));
        if (!sna_fake_vblank(info)) return false;
    }
    else
    {
        add_to_crtc_vblank(info, delta);
...
sna_present_get_crtc(WindowPtr window)
{
    ...
    if (crtc) return crtc->randr_crtc;

    return NULL;
}

static void
add_keepalive(struct sna *sna, xf86CrtcPtr crtc, uint64_t msc)
{
    ...
    list_for_each_entry(tmp, q, link)
    {
        ...
        if ((int64_t)(tmp->target_msc - msc) > 0) break;
    }
...
sna_present_queue_vblank(RRCrtcPtr crtc, uint64_t event_id, uint64_t msc)
{
    ...
    if (!sna_crtc_is_on(crtc->devPrivate)) return BadAlloc;
    ...
    {
        DBG(("%s: crtc=%d tv=%d.%06d msc=%lld (target=%lld), event=%lld "
             "complete\n",
             __FUNCTION__,
             sna_crtc_index(crtc->devPrivate),
             swap->tv_sec,
    ...
    if (warn_unless(msc - swap->msc < 1ull << 31)) return BadValue;
    ...
    list_for_each_entry(tmp, q, link)
    {
        if (tmp->target_msc == msc)
        {
            ...
            if (tmp->n_event_id && is_power_of_two(tmp->n_event_id))
            {
                events = malloc(2 * sizeof(uint64_t) * tmp->n_event_id);
                if (events == NULL) return BadAlloc;

                memcpy(events,
                       tmp->event_id,
                       tmp->n_event_id * sizeof(uint64_t));
                if (tmp->n_event_id != 1) free(tmp->event_id);
                tmp->event_id = events;
            }
        ...
        if ((int64_t)(tmp->target_msc - msc) > 0)
        {
            DBG(("%s: previous target_msc=%lld invalid for coalescing\n",
                 __FUNCTION__,
                 (long long)tmp->target_msc));
            break;
        }
    }

    info = info_alloc(sna);
    if (info == NULL) return BadAlloc;
...
static bool
check_flip__crtc(struct sna *sna, RRCrtcPtr crtc)
{
...
present_flip_handler(struct drm_event_vblank *event, void *data)
{
    ...
    if (info->sna->present.unflip)
    {
        DBG(("%s: executing queued unflip (event=%lld)\n",
             __FUNCTION__,
             (long long)info->sna->present.unflip));
...
get_flip_bo(PixmapPtr pixmap)
{
    ...
    priv = sna_pixmap_move_to_gpu(pixmap,
                                  MOVE_READ | __MOVE_SCANOUT | __MOVE_FORCE);
    if (priv == NULL)
    {
    ...
    if (priv->gpu_bo->scanout) return priv->gpu_bo;
    ...
    if (sna->kgem.has_llc && !wedged(sna) && !priv->pinned)
    {
        ...
        if ((sna->flags & SNA_LINEAR_FB) == 0) tiling = I915_TILING_X;
        ...
        if (bo)
        {
            ...
            else kgem_bo_destroy(&sna->kgem, bo);
        }
    }

    if (sna->flags & SNA_LINEAR_FB && priv->gpu_bo->tiling &&
    ...
    if (priv->gpu_bo->tiling == I915_TILING_Y && !sna->kgem.can_scanout_y &&
                 ...
                 Bool      sync_flip)
{
    ...
    if (sna->flags & SNA_TEAR_FREE)
    {
        DBG(("%s: disabling TearFree (was %s) in favour of Present flips\n",
             __FUNCTION__,
             sna->mode.shadow_enabled ? "enabled" : "disabled"));
    ...
    if (sna->mode.flip_active)
    {
        ...
        while (poll(&pfd, 1, 0) == 1)
            sna_mode_wakeup(sna);
        if (sna->mode.flip_active) return FALSE;
    }
    ...
    if (sync_flip) return flip(sna, crtc, event_id, target_msc, bo);
    else return flip__async(sna, crtc, event_id, target_msc, bo);
}
...
notify:
    ...
    if (!sna_needs_page_flip(sna, bo)) goto notify;
    ...
    if (sna->flags & SNA_TEAR_FREE)
    {
        DBG(("%s: %s TearFree after Present flips\n",
             __FUNCTION__,
             sna->mode.shadow_damage != NULL ? "enabling" : "disabling"));
...
reset_mode:
    ...
    if (sna->flags & SNA_HAS_ASYNC_FLIP)
    {
        ...
        if (flip__async(sna, NULL, event_id, 0, bo)) return;
    }

    if (!flip(sna, NULL, event_id, 0, bo)) goto reset_mode;
}
...
sna_present_cancel_flip(struct sna *sna)
{
    if (sna->present.unflip)
    {
        ...
        swap = sna_crtc_last_swap(sna_primary_crtc(sna));
        present_event_notify(sna->present.unflip, swap_ust(swap), swap->msc);
...
bool
sna_present_open(struct sna *sna, ScreenPtr screen)
{
...
sna_present_update(struct sna *sna)
{
    ...
    else present_info.capabilities &= ~PresentCapabilityAsync;
...
void
sna_present_close(struct sna *sna, ScreenPtr screen)
{
    DBG(("%s()\n", __FUNCTION__));
}
