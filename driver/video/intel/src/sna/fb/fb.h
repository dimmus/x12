...
#define FbRotStipLeft(x, n) \
    FbStipLeft(x, n) | (n ? FbStipRight(x, FB_STIP_UNIT - n) : 0)
#define FbRotStipRight(x, n) \
...
#define FbLeftStipMask(x)                                                    \
    (((x) & FB_STIP_MASK) ? FbStipRight(FB_STIP_ALLONES, (x) & FB_STIP_MASK) \
                          : 0)
#define FbRightStipMask(x)                                                 \
    (((FB_STIP_UNIT - (x)) & FB_STIP_MASK)                                 \
         ? FbScrLeft(FB_STIP_ALLONES, (FB_STIP_UNIT - (x)) & FB_STIP_MASK) \
         : 0)
...
#define FbStipMask(x, w)                                \
    (FbStipRight(FB_STIP_ALLONES, (x) & FB_STIP_MASK) & \
...
#define FbStorePart(dst, off, t, xor) \
    (WRITE(FbPtrOffset(dst, off, t), FbSelectPart(xor, off, t)))
#ifndef FbSelectPart
...
#define FbLaneCase2(n, a, o)                                         \
    ...
    {                                                                \
        FbLaneCase1((n) & 1, a, o) FbLaneCase1((n) >> 1, a, (o) + 1) \
    }
...
#define FbLaneCase4(n, a, o)                                         \
    ...
    {                                                                \
        FbLaneCase2((n) & 3, a, o) FbLaneCase2((n) >> 2, a, (o) + 2) \
    }
...
fbGetWindowPixmap(WindowPtr window)
{
...
#define fbGetPixmapBitsData(pixmap, pointer, stride, bpp)      \
    {                                                          \
        ...
        (stride)  = ((int)(pixmap)->devKind) / sizeof(FbBits); \
        (void)(stride);                                        \
...
#define fbGetPixmapStipData(pixmap, pointer, stride, bpp)      \
    {                                                          \
        ...
        (stride)  = ((int)(pixmap)->devKind) / sizeof(FbStip); \
        (void)(stride);                                        \
        (bpp) = (pixmap)->drawable.bitsPerPixel;               \
        (void)(bpp);                                           \
    }
...
#define fbDrawableEnabled(drawable)      \
    ((drawable)->type == DRAWABLE_PIXMAP \
         ? TRUE                          \
         : fbWindowEnabled((WindowPtr)drawable))
...
extern void fbPolyArc(DrawablePtr drawable, GCPtr gc, int narcs, xArc *parcs);

extern void fbBlt(FbBits  *src,
                  FbStride srcStride,
                  int      srcX,
                  FbBits  *dst,
                  FbStride dstStride,
...
extern void fbBltOne(FbStip  *src,
                     FbStride srcStride,
                     int      srcX,
                     FbBits  *dst,
                     FbStride dstStride,
...
extern void fbBltPlane(FbBits  *src,
                       FbStride srcStride,
                       int      srcX,
                       int      srcBpp,
                       FbStip  *dst,
...
extern void fbCopyNtoN(DrawablePtr src,
                       DrawablePtr dst,
                       GCPtr       gc,
                       BoxPtr      pbox,
                       int         nbox,
...
extern void fbCopy1toN(DrawablePtr src,
                       DrawablePtr dst,
                       GCPtr       gc,
                       BoxPtr      pbox,
                       int         nbox,
...
extern void fbCopyNto1(DrawablePtr src,
                       DrawablePtr dst,
                       GCPtr       gc,
                       BoxPtr      pbox,
                       int         nbox,
...
extern RegionPtr fbCopyArea(DrawablePtr src,
                            DrawablePtr dst,
                            GCPtr       gc,
                            int         sx,
                            int         sy,
...
extern RegionPtr fbCopyPlane(DrawablePtr   src,
                             DrawablePtr   dst,
                             GCPtr         gc,
                             int           sx,
                             int           sy,
...
extern void fbSolidBoxClipped(DrawablePtr drawable,
                              GCPtr       gc,
                              int         x1,
                              int         y1,
                              int         x2,
...
extern void fbFillSpans(DrawablePtr drawable,
                        GCPtr       gc,
                        int         n,
                        DDXPointPtr pt,
                        int        *width,
...
extern void fbValidateGC(GCPtr gc, unsigned long changes, DrawablePtr drawable);
...
extern void fbPolyGlyphBlt(DrawablePtr  drawable,
                           GCPtr        gc,
                           int          x,
                           int          y,
                           unsigned int n,
...
extern void fbImageGlyphBlt(DrawablePtr  drawable,
                            GCPtr        gc,
                            int          x,
                            int          y,
                            unsigned int n,
...
extern void fbPutImage(DrawablePtr drawable,
                       GCPtr       gc,
                       int         depth,
                       int         x,
                       int         y,
...
extern void fbPutXYImage(DrawablePtr drawable,
                         GCPtr       gc,
                         FbBits      fg,
                         FbBits      bg,
                         FbBits      pm,
...
extern void fbGetImage(DrawablePtr   drawable,
                       int           x,
                       int           y,
                       int           w,
                       int           h,
...
extern void fbFixCoordModePrevious(int n, DDXPointPtr pt);

extern void fbPolySegment(DrawablePtr drawable, GCPtr gc, int n, xSegment *seg);

extern RegionPtr fbBitmapToRegion(PixmapPtr pixmap);

extern void fbPolyPoint(DrawablePtr drawable,
                        GCPtr       gc,
                        int         mode,
                        int         n,
                        xPoint     *pt,
...
extern void fbPushImage(DrawablePtr drawable,
                        GCPtr       gc,
                        FbStip     *src,
                        FbStride    srcStride,
                        int         srcX,
...
extern void fbPushPixels(GCPtr       gc,
                         PixmapPtr   pBitmap,
...
extern void fbSetSpans(DrawablePtr drawable,
                       GCPtr       gc,
                       char       *src,
                       DDXPointPtr pt,
                       int        *width,
...
extern void fbSegment(DrawablePtr drawable,
                      GCPtr       gc,
                      int         xa,
                      int         ya,
                      int         xb,
...
extern void fbSegment1(DrawablePtr   drawable,
                       GCPtr         gc,
                       const BoxRec *clip,
                       int           xa,
                       int           ya,
...
extern void fbTransparentSpan(FbBits *dst, FbBits stip, FbBits fgxor, int n);

extern void fbStipple(FbBits  *dst,
                      FbStride dstStride,
                      int      dstX,
                      int      dstBpp,
                      int      width,
