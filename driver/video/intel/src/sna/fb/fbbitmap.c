...
region_grow(RegionPtr region)
{
    ...
    if (!region->data)
    {
        ...
        if (!region->data) return RegionBreak(region);
    ...
    else
    {
        ...
        if (!data) return RegionBreak(region);
        region->data = data;
    }
    region->data->size = n;
    return TRUE;
...
static inline void
add(RegionPtr region, int16_t x1, int16_t y1, int16_t x2, int16_t y2)
{
    BoxPtr r;

    if (region->data->numRects == region->data->size && !region_grow(region))
        return;
    ...
    DBG(("%s[%ld/%ld]: (%d, %d), (%d, %d)\n",
         __FUNCTION__,
         (long)region->data->numRects,
         (long)region->data->size,
         x1,
         y1,
         x2,
         y2));

    if (x1 < region->extents.x1) region->extents.x1 = x1;
    if (x2 > region->extents.x2) region->extents.x2 = x2;
}
...
fbBitmapToRegion(PixmapPtr pixmap)
{
    ...
    while (y2 < pixmap->drawable.height)
    {
        ...
        while (y2 < pixmap->drawable.height &&
               ...
               (maskw == 0 || (bits[width >> FB_SHIFT] & maskw) ==
                                  (line[width >> FB_SHIFT] & maskw)))
            line += stride, y2++;

        if (READ(bits) & MASK_0) x1 = 0;
        else x1 = -1;
        ...
        if (x1 >= 0) add(region, x1, y1, width, y2);
    }
    ...
    else region->extents.x1 = region->extents.x2 = 0;
