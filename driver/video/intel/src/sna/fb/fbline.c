...
fbPolyLine(DrawablePtr drawable, GCPtr gc, int mode, int n, DDXPointPtr pt)
{
    ...
    DBG((
        "%s x %d, width=%d, fill=%d [solid? %d], line=%d [solid? %d], bpp=%d\n",
        __FUNCTION__,
        n,
        gc->lineWidth,
        gc->fillStyle,
    ...
    {
        if (gc->lineStyle != LineSolid) raster = miWideDash;
        else raster = miWideLine;
    }
