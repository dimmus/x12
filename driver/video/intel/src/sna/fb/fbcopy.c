...
void
fbCopyNtoN(DrawablePtr src_drawable,
           ...
           void       *closure)
{
    ...
    do
    {
        fbBlt(src + box->y1 * srcStride,
              srcStride,
              (box->x1 + srcXoff) * srcBpp,
              dst + box->y1 * dstStride,
              dstStride,
...
void
fbCopy1toN(DrawablePtr src_drawable,
           DrawablePtr dst_drawable,
           GCPtr       gc,
           BoxPtr      box,
           int         nbox,
    ...
    while (nbox--)
    {
        if (dstBpp == 1)
        {
            fbBlt(src + (box->y1 + dy + srcYoff) * srcStride,
                  ...
                  (box->x2 - box->x1) * dstBpp,
                  (box->y2 - box->y1),
                  FbOpaqueStipple1Rop(gc->alu, gc->fgPixel, gc->bgPixel),
                  pgc->pm,
                  dstBpp,
                  reverse,
                  upsidedown);
...
void
fbCopyNto1(DrawablePtr src_drawable,
           DrawablePtr dst_drawable,
           GCPtr       gc,
           BoxPtr      box,
           int         nbox,
