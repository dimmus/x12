...
_fbPutZImage(DrawablePtr drawable, GCPtr gc, const BoxRec *b, void *_data)
{
    ...
    fbBltStip(data->src + (b->y1 - data->y0) * data->src_stride,
              data->src_stride,
              (b->x1 - data->x0) * bpp,
...
static void
fbPutZImage(DrawablePtr drawable,
            GCPtr       gc,
            int         x,
            int         y,
            int         width,
...
void
fbPutXYImage(DrawablePtr drawable,
             GCPtr       gc,
             FbBits      fg,
             FbBits      bg,
             FbBits      pm,
             ...
             int         srcX)
{
    ...
    {
        if (opaque) data.alu = FbOpaqueStipple1Rop(alu, fg, bg);
        else data.alu = FbStipple1Rop(alu, fg);
        data.pm = pm;
...
void
fbPutImage(DrawablePtr drawable,
           GCPtr       gc,
           int         depth,
           int         x,
           int         y,
           ...
           char       *image)
{
    ...
    switch (format)
    {
        ...
        case ZPixmap:
            ...
            fbPutZImage(drawable, gc, x, y, w, h, src, srcStride);
    }
}
