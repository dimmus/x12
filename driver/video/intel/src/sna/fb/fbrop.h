...
#define FbDestInvarientRop(alu, pm) \
    ((pm) == FB_ALLONES && (((alu) >> 1 & 5) == ((alu) & 5)))
...
#define fbXorT(rop, fg, pm, t)                  \
    ((((fg) & fbFillFromBit((rop) >> 1, t)) |   \
      (~(fg) & fbFillFromBit((rop) >> 3, t))) & \
     (pm))

#define fbAndT(rop, fg, pm, t)                               \
    ((((fg) & fbFillFromBit(rop ^ (rop >> 1), t)) |          \
...
#define FbOpaqueStipple1Rop(alu, fg, bg) \
    (FbStipple1RopPick(alu, fg) | (FbStipple1RopPick(alu, bg) << 2))
