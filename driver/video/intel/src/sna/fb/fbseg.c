...
#define FbDashDeclare unsigned char *__dash, *__firstDash, *__lastDash
...
#define FbDashInit(gc, pgc, dashOffset, dashlen, even)        \
    {                                                         \
        ...
        {                                                     \
            (dashOffset) -= (dashlen);                        \
            (even) = 1 - (even);                              \
            if (++__dash == __lastDash) __dash = __firstDash; \
        }                                                     \
        (dashlen) -= (dashOffset);                            \
    }
...
#define fbBresShiftMask(mask, dir, bpp) \
    ((bpp == FB_STIP_UNIT)              \
         ? 0                            \
         : ((dir < 0) ? FbStipLeft(mask, bpp) : FbStipRight(mask, bpp)))
...
static void
fbBresSolid(DrawablePtr drawable,
            GCPtr       gc,
            int         dashOffset,
            int         sdx,
            int         sdy,
            ...
            int         len)
{
    ...
    if (sdx < 0) mask0 = FbStipRight(mask0, FB_STIP_UNIT - bpp);
    if (sdy < 0) stride = -stride;
    if (axis == X_AXIS)
    {
        bits = 0;
        while (len--)
        ...
        if (bits) WRITE(dst, FbDoMaskRRop(READ(dst), and, xor, bits));
    }
    else
    {
        while (len--)
...
static void
fbBresDash(DrawablePtr drawable,
           GCPtr       gc,
           int         dashOffset,
           int         sdx,
           int         sdy,
           ...
           int         len)
{
    ...
    while (len--)
    {
        if (even) WRITE(dst, FbDoMaskRRop(READ(dst), and, xor, mask));
        else if (doOdd) WRITE(dst, FbDoMaskRRop(READ(dst), bgand, bgxor, mask));
        if (axis == X_AXIS)
        {
            mask = fbBresShiftMask(mask, sdx, bpp);
            if (!mask)
...
static void
fbBresFill(DrawablePtr drawable,
           GCPtr       gc,
           int         dashOffset,
           int         sdx,
           int         sdy,
               ...
               int         len)
{
    ...
    doBg =
        doOdd && (gc->fillStyle == FillSolid || gc->fillStyle == FillStippled);
    ...
    while (len--)
    {
        if (even || doOdd)
        {
            if (doBg)
            {
                if (even) fbSetFg(drawable, gc, fg);
                else fbSetFg(drawable, gc, bg);
            }
            fbFill(drawable, gc, x1, y1, 1, 1);
        }
        if (axis == X_AXIS)
    ...
    if (doBg) fbSetFg(drawable, gc, fg);
}
...
_fbSegment(DrawablePtr drawable, GCPtr gc, const BoxRec *b, void *_data)
{
    ...
    *data->dashOffset = dashoff + len;
    ...
    else
    {
        ...
        if (axis == X_AXIS) len = abs(new_x2 - new_x1);
        else len = abs(new_y2 - new_y1);
        if (clip2 != 0 || data->drawLast) len++;
        if (len)
        {
...
void
fbSegment(DrawablePtr drawable,
          GCPtr       gc,
          int         x1,
          int         y1,
          int         x2,
...
void
fbSegment1(DrawablePtr   drawable,
           GCPtr         gc,
           const BoxRec *b,
           int           x1,
           int           y1,
