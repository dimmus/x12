        ...
        FbBits xor)
{
    ...
    while (height--)
    {
        ...
        if (endmask) FbDoRightMaskByteRRop(dst, endbyte, endmask, and, xor);
        dst += dstStride;
    }
}
...
fbFill(DrawablePtr drawable, GCPtr gc, int x, int y, int width, int height)
{
    ...
    {
        case FillSolid:
            if (pgc->and || !pixman_fill((uint32_t *)dst,
                                         dstStride,
                                         dstBpp,
                                         x + dstXoff,
                                         y + dstYoff,
        ...
        case FillOpaqueStippled:
            {
                ...
                if (dstBpp == 1)
                {
                    ...
                    if (gc->fillStyle == FillStippled)
                        ...
                        alu = FbOpaqueStipple1Rop(gc->alu,
                                                  gc->fgPixel,
                                                  gc->bgPixel);
                    fbGetDrawable(&pStip->drawable,
                                  stip,
                                  stipStride,
                                  stipBpp,
                                  stipXoff,
                ...
                else
                {
                    ...
                    fbGetStipDrawable(&pStip->drawable,
                                      stip,
                                      stipStride,
                                      stipBpp,
                                      stipXoff,
...
_fbSolidBox(DrawablePtr drawable, GCPtr gc, const BoxRec *b, void *_data)
{
    ...
    if (and|| !pixman_fill((uint32_t *)dst,
                           stride,
                           bpp,
                           b->x1 + dx,
                           b->y1 + dy,
...
void
fbSolidBoxClipped(DrawablePtr drawable,
                  GCPtr       gc,
                  int         x1,
                  int         y1,
                  int         x2,
