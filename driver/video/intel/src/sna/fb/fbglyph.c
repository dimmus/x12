...
fbGlyphIn(GCPtr gc, int x, int y, int width, int height)
{
	 ...
	 */
    if (x < (int)extents->x1 || (int)extents->x2 < x + width) return FALSE;
    if (y < (int)extents->y1 || (int)extents->y2 < y + height) return FALSE;
...
void
fbPolyGlyphBlt(DrawablePtr  drawable,
               GCPtr        gc,
               int          x,
               int          y,
               unsigned int nglyph,
               ...
               pointer      glyphs)
{
    ...
    while (nglyph--)
    {
        ...
        if (gWidth && gHeight)
        {
            ...
            {
                fbGetDrawable(drawable,
                              dst,
                              dstStride,
                              dstBpp,
                              dstXoff,
                              dstYoff);
                raster(dst + (gy + dstYoff) * dstStride,
                       dstStride,
                       dstBpp,
                       (FbStip *)pglyph,
                       pgc->xor
...
void
fbImageGlyphBlt(DrawablePtr  drawable,
                GCPtr        gc,
                int          x,
                int          y,
                unsigned int nglyph,
