...
POLYLINE(DrawablePtr drawable, GCPtr gc, int mode, int n_0, DDXPointPtr pt_0)
{
    ...
    if (mode == CoordModePrevious) fbFixCoordModePrevious(n_0, pt_0);
        ...
        while (n--)
        {
            ...
            if (isClipped(u.pt32[0], ul, lr) | isClipped(u.pt32[1], ul, lr))
            {
                ...
                fbSegment1(drawable,
                           gc,
                           clip,
                           intToX(u.pt32[0]) + xoff,
                           intToY(u.pt32[0]) + yoff,
                           intToX(u.pt32[1]) + xoff,
                           intToY(u.pt32[1]) + yoff,
                           !capNotLast,
                           &dashoffset);
            }
            else
            {
                CalcLineDeltas(intToX(u.pt32[0]),
                               intToY(u.pt32[0]),
                               intToX(u.pt32[1]),
                               intToY(u.pt32[1]),
                               len,
                ...
                if (e1 == 0 && len > 3)
                {
                    ...
                    dstLine =
                        dst + (intToY(u.pt32[0]) + yoff + dstYoff) * dstStride;
                    ...
                    {
                        WRITE(dstLine,
                              FbDoMaskRRop(READ(dstLine), and, xor, startmask));
                        dstLine++;
                    }
                    if (!and)
                        while (nmiddle--)
                    ...
                    if (endmask)
                        WRITE(dstLine,
                              FbDoMaskRRop(READ(dstLine), and, xor, endmask));
                }
                else
                {
                    bits = bitsBase + intToY(u.pt32[0]) * bitsStride +
                           intToX(u.pt32[0]);
                    if (len < e1)
                    {
                        e3  = len;
