...
static void
fbPushPattern(DrawablePtr drawable,
              GCPtr       gc,
              FbStip     *src,
              FbStride    srcStride,
              int         srcX,
...
static void
fbPushFill(DrawablePtr drawable,
           GCPtr       gc,
           FbStip     *src,
           FbStride    srcStride,
           int         srcX,
           ...
           int         height)
{
    ...
    if (gc->fillStyle == FillSolid)
    {
        ...
        if (dstBpp == 1)
        {
            fbBltStip(src,
                      ...
                      dstWidth,
                      height,
                      FbStipple1Rop(gc->alu, gc->fgPixel),
                      pgc->pm,
                      dstBpp);
        }
        else
    ...
    else fbPushPattern(drawable, gc, src, srcStride, srcX, x, y, width, height);
}
...
_fbPushImage(DrawablePtr drawable, GCPtr gc, const BoxRec *b, void *_data)
{
    ...
    fbPushFill(drawable,
               gc,
               data->src + (b->y1 - data->y0) * data->stride,
               data->stride,
               b->x1 - data->x0,
               b->x1,
               b->y1,
...
void
fbPushImage(DrawablePtr drawable,
            GCPtr       gc,
            FbStip     *src,
            FbStride    stride,
            int         dx,
             ...
             int         yOrg)
{
    ...
    fbGetStipDrawable(&bitmap->drawable,
                      stip,
                      stipStride,
                      stipBpp,
                      stipXoff,
