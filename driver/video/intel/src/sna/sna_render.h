...
{
    fastcall void (*blt)(struct sna                            *sna,
    ...
    void (*boxes)(struct sna                    *sna,
                  ...
                  const BoxRec                  *box,
                  int                            nbox);
    void (*thread_boxes)(struct sna                    *sna,
                         const struct sna_composite_op *op,
                         const BoxRec                  *box,
                         int                            nbox);
...
#define COMPOSITE_FALLBACK 0x80000000
    ...
    bool (*check_composite_spans)(struct sna *sna,
                                  uint8_t     op,
                                  PicturePtr  dst,
                                  PicturePtr  src,
                                  int16_t     w,
                                  int16_t     h,
                                  unsigned    flags);
    bool (*composite_spans)(struct sna                    *sna,
                            uint8_t                        op,
                            PicturePtr                     dst,
                            PicturePtr                     src,
                            int16_t                        src_x,
...
#define FILL_SPANS  0x4
    ...
    bool (*copy_boxes)(struct sna        *sna,
                       uint8_t            alu,
...
struct kgem_bo *sna_render_get_solid(struct sna *sna, uint32_t color);

void sna_render_flush_solid(struct sna *sna);

struct kgem_bo *sna_render_get_gradient(struct sna *sna, PictGradient *pattern);

bool sna_gradient_is_opaque(const PictGradient *gradient);
...
bool sna_tiling_copy_boxes(struct sna        *sna,
                           uint8_t            alu,
                           const DrawableRec *src,
                           struct kgem_bo    *src_bo,
                           int16_t            src_dx,
...
bool sna_tiling_blt_copy_boxes(struct sna     *sna,
                               uint8_t         alu,
                               struct kgem_bo *src_bo,
                               int16_t         src_dx,
                               int16_t         src_dy,
...
bool sna_blt_fill(struct sna         *sna,
                  uint8_t             alu,
                  struct kgem_bo     *bo,
                  int                 bpp,
                  uint32_t            pixel,
...
bool sna_blt_copy(struct sna         *sna,
                  uint8_t             alu,
                  struct kgem_bo     *src,
                  struct kgem_bo     *dst,
                  int                 bpp,
...
bool sna_blt_fill_boxes(struct sna     *sna,
                        uint8_t         alu,
                        struct kgem_bo *bo,
                        int             bpp,
                        uint32_t        pixel,
...
bool sna_blt_copy_boxes(struct sna     *sna,
                        uint8_t         alu,
                        struct kgem_bo *src_bo,
                        int16_t         src_dx,
                        int16_t         src_dy,
                        ...
                        const BoxRec   *box,
                        int             n);
bool sna_blt_copy_boxes__with_alpha(struct sna     *sna,
                                    uint8_t         alu,
                                    struct kgem_bo *src_bo,
                                    int16_t         src_dx,
                                    int16_t         src_dy,
                                    ...
                                    const BoxRec   *box,
                                    int             nbox);
bool sna_blt_copy_boxes_fallback(struct sna        *sna,
                                 uint8_t            alu,
                                 const DrawableRec *src,
                                 struct kgem_bo    *src_bo,
                                 int16_t            src_dx,
...
bool memcpy_copy_boxes(struct sna        *sna,
                       uint8_t            op,
                        ...
                        uint32_t  format)
{
    switch (format)
    {
        ...
        /* fall through */
        case PICT_a8r8g8b8:
            *pixel = ((alpha >> 8 << 24) | (red >> 8 << 16) | (green & 0xff00) |
                      (blue >> 8));
            return TRUE;
        case PICT_a8:
            *pixel = alpha >> 8;
...
struct kgem_bo *__sna_render_pixmap_bo(struct sna   *sna,
                                       PixmapPtr     pixmap,
                                       const BoxRec *box,
                                       bool          blt);

int sna_render_pixmap_bo(struct sna                   *sna,
...
bool sna_render_pixmap_partial(struct sna                   *sna,
...
int sna_render_picture_extract(struct sna                   *sna,
                               PicturePtr                    picture,
                               struct sna_composite_channel *channel,
                               int16_t                       x,
                               int16_t                       y,
...
int sna_render_picture_fixup(struct sna                   *sna,
                             PicturePtr                    picture,
                             struct sna_composite_channel *channel,
                             int16_t                       x,
                             int16_t                       y,
...
int sna_render_picture_convert(struct sna                   *sna,
                               PicturePtr                    picture,
                               struct sna_composite_channel *channel,
                               PixmapPtr                     pixmap,
                               int16_t                       x,
...
static inline void
sna_render_composite_redirect_init(struct sna_composite_op *op)
{
...
bool sna_render_composite_redirect(struct sna              *sna,
                                   struct sna_composite_op *op,
                                   int                      x,
                                   int                      y,
                                   int                      width,
...
void sna_render_composite_redirect_done(struct sna                    *sna,
                                        const struct sna_composite_op *op);

bool sna_render_copy_boxes__overlap(struct sna        *sna,
                                    uint8_t            alu,
                                    const DrawableRec *draw,
                                    struct kgem_bo    *bo,
                                    int16_t            src_dx,
...
bool sna_composite_mask_is_opaque(PicturePtr mask);

void sna_vertex_init(struct sna *sna);

static inline void
sna_vertex_lock(struct sna_render *r)
{
    pthread_mutex_lock(&r->lock);
}

static inline void
sna_vertex_acquire__locked(struct sna_render *r)
{
    r->active++;
}

static inline void
sna_vertex_unlock(struct sna_render *r)
{
    pthread_mutex_unlock(&r->lock);
}

static inline void
sna_vertex_release__locked(struct sna_render *r)
{
    assert(r->active > 0);
    if (--r->active == 0) pthread_cond_signal(&r->wait);
}

static inline bool
sna_vertex_wait__locked(struct sna_render *r)
{
