...
static inline struct sna_glyph *
sna_glyph(GlyphPtr glyph)
{
    return __get_private(glyph, sna_glyph_key);
}

static inline struct sna_glyph *
sna_glyph0(GlyphPtr glyph)
{
...
void
sna_glyphs_close(struct sna *sna)
{
    ...
    for (i = 0; i < ARRAY_SIZE(render->glyph); i++)
    {
        ...
        if (cache->picture) FreePicture(cache->picture, 0);
 ...
 */
bool
sna_glyphs_create(struct sna *sna)
{
    ...
    if (__global_glyph_cache == NULL)
    {
        ...
        if (__global_glyph_cache == NULL) goto bail;
    }
#endif

    sna->render.white_image = pixman_image_create_solid_fill(&white);
    if (sna->render.white_image == NULL) goto bail;
    ...
    for (i = 0; i < ARRAY_SIZE(formats); i++)
    {
        ...
        cache->glyphs = calloc(sizeof(struct sna_glyph *), GLYPH_CACHE_SIZE);
        if (!cache->glyphs) goto bail;
    ...
    if (sna->render.white_picture == NULL) goto bail;
...
static void
glyph_extents(int nlist, GlyphListPtr list, GlyphPtr *glyphs, BoxPtr extents)
{
    int x1, x2, y1, y2;
    int x, y;
...
#if HAS_DEBUG_FULL
static int
glyph_count(int nlist, GlyphListPtr list)
{
    int count = 0;
    while (nlist--)
    {
...
static int
glyph_cache(ScreenPtr screen, struct sna_render *render, GlyphPtr glyph)
{
    PicturePtr              glyph_picture;
    ...
    if (NO_GLYPH_CACHE || glyph->info.width > GLYPH_MAX_SIZE ||
    ...
    for (size = GLYPH_MIN_SIZE; size <= GLYPH_MAX_SIZE; size *= 2)
        if (glyph->info.width <= size && glyph->info.height <= size) break;
    ...
    DBG(("%s(%d): adding glyph to cache %d, pos %d\n",
         __FUNCTION__,
         screen->myNum,
         PICT_FORMAT_RGB(glyph_picture->format) != 0,
         pos));
    cache->glyphs[pos] = p;
    ...
    s      = pos /
        ((GLYPH_MAX_SIZE / GLYPH_MIN_SIZE) * (GLYPH_MAX_SIZE / GLYPH_MIN_SIZE));
    p->coordinate.x =
    ...
    {
        if (pos & 1) p->coordinate.x += s;
        if (pos & 2) p->coordinate.y += s;
        pos >>= 2;
    }

    glyph_cache_upload(cache,
                       glyph,
                       glyph_picture,
                       p->coordinate.x,
                       p->coordinate.y);
...
static void
apply_damage(struct sna_composite_op               *op,
...
static void
apply_damage_clipped_to_dst(struct sna_composite_op               *op,
                            const struct sna_composite_rectangles *r,
                            DrawablePtr                            dst)
{
    BoxRec box;
    ...
    if (box.x1 < dst->x) box.x1 = dst->x;

    if (box.x2 > op->dst.width) box.x2 = op->dst.width;

    if (box.y1 < dst->y) box.y1 = dst->y;

    if (box.y2 > op->dst.height) box.y2 = op->dst.height;
...
static inline bool
region_matches_pixmap(const RegionRec *r, PixmapPtr pixmap)
{
...
static inline bool
clipped_glyphs(PicturePtr dst, int nlist, GlyphListPtr list, GlyphPtr *glyphs)
{
    BoxRec box;
    ...
    {
        DBG(("%s: no, clip region (%d, %d), (%d, %d) matches drawable "
             "pixmap=%ld size=%dx%d\n",
             __FUNCTION__,
    ...
    DBG(("%s? %d glyph in %d lists extents (%d, %d), (%d, %d), region (%d, "
         "%d), (%d, %d): %s\n",
         __FUNCTION__,
         glyph_count(nlist, list),
         nlist,
         box.x1,
         box.y1,
         box.x2,
         ...
         dst->pCompositeClip->extents.x2,
         dst->pCompositeClip->extents.y2,
         pixman_region_contains_rectangle(dst->pCompositeClip, &box) !=
                 PIXMAN_REGION_IN
             ? "yes"
             : "no"));

    return pixman_region_contains_rectangle(dst->pCompositeClip, &box) !=
           PIXMAN_REGION_IN;
}
              ...
              GlyphPtr    *glyphs)
{
    ...
    if (NO_GLYPHS_TO_DST) return false;
    ...
    while (nlist--)
    {
        ...
        while (n--)
        {
            ...
            {
                if (unlikely(!glyph_valid(glyph))) goto next_glyph;
...
next_glyph:
    ...
    if (glyph_atlas != NO_ATLAS) tmp.done(sna, &tmp);
    ...
    if (clipped_glyphs(dst, nlist, list, glyphs))
    {
        ...
        while (nlist--)
        {
            ...
            while (n--)
            {
                ...
                if (unlikely(p->atlas != glyph_atlas))
                {
                    if (unlikely(!glyph_valid(glyph))) goto next_glyph_N;
...
next_glyph_N:
        ...
        while (nlist--)
        {
            ...
            while (n--)
            {
                ...
                if (unlikely(p->atlas != glyph_atlas))
                {
                    if (unlikely(!glyph_valid(glyph))) goto next_glyph_0;
            ...
            GlyphPtr    *glyphs)
{
    ...
    if (NO_GLYPHS_SLOW) return false;
    ...
    while (nlist--)
    {
        ...
        while (n--)
        {
            ...
            if (!box_intersect(&box, &dst->pCompositeClip->extents))
                goto next_glyph;
            ...
            if (unlikely(p->atlas == NULL))
            {
                ...
                if (!glyph_cache(screen, &sna->render, glyph)) goto next_glyph;
            }
            ...
            do
            {
                ...
                DBG(("%s: glyph=(%d, %d), (%d, %d), clip=(%d, %d), (%d, %d)\n",
                     ...
                     rects->x2,
                     rects->y2));
                if (rects->y1 >= y2) break;

                if (r.dst.x < rects->x1) r.dst.x = rects->x1;
                if (x2 > rects->x2) x2 = rects->x2;

                if (r.dst.y < rects->y1) r.dst.y = rects->y1;
                if (y2 > rects->y2) y2 = rects->y2;
...
sna_glyph_get_image(GlyphPtr g, ScreenPtr s)
{
    ...
    if (image == NULL) image = __sna_glyph_get_image(g, s);

    return image;
}

static inline bool
use_small_mask(struct sna *sna, int16_t width, int16_t height, int depth)
{
    if (depth < 8) return true;

    if (FORCE_SMALL_MASK) return FORCE_SMALL_MASK > 0;

    if (depth * width * height < 8 * 4096) return true;
                ...
                GlyphPtr     *glyphs)
{
    ...
    if (NO_GLYPHS_VIA_MASK) return false;
    ...
    glyph_extents(nlist, list, glyphs, &box);
    if (box.x2 <= box.x1 || box.y2 <= box.y1) return true;
...
use_small_mask:
        DBG(("%s: small mask [format=%lx, depth=%d, size=%d], rendering glyphs "
             "to upload buffer\n",
             __FUNCTION__,
        ...
        mask_image = pixman_image_create_bits(
            pixmap->drawable.bitsPerPixel << 24 | format->format,
            width,
            height,
            pixmap->devPrivate.ptr,
            pixmap->devKind);
        if (mask_image == NULL) goto err_pixmap;
        ...
        if (__global_glyph_cache)
        {
            ...
            if (count > N_STACK_GLYPHS)
            {
                ...
                if (pglyphs == NULL) goto err_pixmap;
            }
            ...
            do
            {
                ...
                while (n--)
                {
                    ...
                    if (!glyph_valid(g)) goto next_pglyph;

                    ptr = pixman_glyph_cache_lookup(__global_glyph_cache,
                                                    g,
                                                    NULL);
                    if (ptr == NULL)
                    {
                        ...
                        if (glyph_image == NULL) goto next_pglyph;

                        DBG(("%s: inserting glyph %p into pixman cache\n",
                             __FUNCTION__,
                             g));
                        ptr = pixman_glyph_cache_insert(__global_glyph_cache,
                                                        g,
                                                        NULL,
                                                        g->info.x,
                                                        g->info.y,
                                                        glyph_image);
                        if (ptr == NULL) goto next_pglyph;
                    }
...
next_pglyph:
            ...
            if (pglyphs != stack_glyphs) free(pglyphs);
        }
        else
#endif
            do
            {
                ...
                while (n--)
                {
                    ...
                    if (!glyph_valid(g)) goto next_image;
				 ...
				 */
                    xi = x - g->info.x;
                    yi = y - g->info.y;
                    if (xi >= width || yi >= height) goto next_image;
                    if (xi + g->info.width <= 0 || yi + g->info.height <= 0)
                        goto next_image;

                    glyph_image =
                        sna_glyph_get_image(g, dst->pDrawable->pScreen);
                    if (glyph_image == NULL) goto next_image;
                    ...
                    {
                        assert(pixman_image_get_format(glyph_image) ==
...
next_image:
    ...
    else
    {
        ...
        ValidatePicture(mask);
        if (!clear_pixmap(sna, pixmap)) goto err_mask;
        ...
        do
        {
            ...
            while (n--)
            {
                ...
                if (unlikely(p->atlas != glyph_atlas))
                {
                    ...
                    if (!ok)
                    {
                        DBG(("%s: fallback -- can not handle PictOpAdd of "
                             "glyph onto mask!\n",
                             __FUNCTION__));
                        goto err_mask;
                    }
                ...
                DBG(("%s: blt glyph origin (%d, %d), offset (%d, %d), src (%d, "
                     "%d), size (%d, %d)\n",
                     __FUNCTION__,
                     x,
                     y,
...
glyphs_format(int nlist, GlyphListPtr list, GlyphPtr *glyphs)
{
    ...
    if (nlist > ARRAY_SIZE(stack_extents))
    {
        ...
        if (list_extents == NULL) return NULL;
    }
    ...
    while (nlist--)
    {
        ...
        while (n--)
        {
            ...
            if (!glyph_valid(glyph)) goto skip_glyph;
            ...
            else
            {
                ...
                {
                    DBG(("%s: overlapping glyph inside line, current bbox (%d, "
                         "%d), (%d, %d), glyph (%d, %d), (%d, %d)\n",
                         __FUNCTION__,
                         extents.x1,
                         extents.y1,
                ...
                if (x1 < extents.x1) extents.x1 = x1;
                if (x2 > extents.x2) extents.x2 = x2;
                if (y1 < extents.y1) extents.y1 = y1;
                if (y2 > extents.y2) extents.y2 = y2;
            }
skip_glyph:
            x += glyph->info.xOff;
            y += glyph->info.yOff;
        ...
        if (!first)
        {
            for (j = 0; j < i; j++)
            {
                ...
                {
                    DBG(("%s: overlapping lines, current bbox (%d, %d), (%d, "
                         "%d), previous line (%d, %d), (%d, %d)\n",
                         __FUNCTION__,
                         extents.x1,
                         extents.y1,
                         extents.x2,
    ...
    DBG(("%s: format=%08d, depth=%d\n",
         ...
         format->depth));
out:
    if (list_extents != stack_extents) free(list_extents);
    return format;
}

static bool
can_discard_mask(uint8_t       op,
                 PicturePtr    src,
                 PictFormatPtr mask,
                 int           nlist,
                 GlyphListPtr  list,
                 GlyphPtr     *glyphs)
{
    ...
    if (DISCARD_MASK) return DISCARD_MASK > 0;

    DBG(("%s: nlist=%d, mask=%08x, depth %d, op=%d (bounded? %d)\n",
         __FUNCTION__,
         nlist,
         mask ? (unsigned)mask->format : 0,
         mask ? mask->depth : 0,
         op,
         op_is_bounded(op)));
    ...
    {
        if (mask == list->format) return true;
    ...
    if (g == NULL)
    {
        while (nlist--)
        {
            if (list->format->depth != 1) return false;

            list++;
        }

        if (!sna_picture_is_solid(src, &color)) return false;
    ...
    {
skip:
        if (mask->format == g->format) return true;

        if (mask->format == alphaless(g->format)) return true;
...
static uint32_t
pixman_format(PictFormatPtr short_format)
{
    uint32_t bpp;
                ...
                GlyphPtr     *glyphs)
{
    ...
    if (!sna_drawable_move_region_to_cpu(dst->pDrawable,
        ...
        return;
    if (dst->alphaMap && !sna_drawable_move_to_cpu(dst->alphaMap->pDrawable,
                                                   MOVE_READ | MOVE_WRITE))
        return;

    if (src->pDrawable)
    {
        if (!sna_drawable_move_to_cpu(src->pDrawable, MOVE_READ)) return;
...
next:
        ...
        if (src_image == NULL) goto out;

        dst_image = image_from_pict(dst, TRUE, &dst_dx, &dst_dy);
        if (dst_image == NULL) goto out_free_src;
        ...
        if (sigtrap_get() == 0)
        {
            if (mask_format)
            {
                pixman_composite_glyphs(
                    ...
                    region.extents.x1,
                    region.extents.y1,
                    region.extents.x1 + dst_dx,
                    region.extents.y1 + dst_dy,
            ...
            {
                pixman_composite_glyphs_no_mask(op,
                                                src_image,
                                                dst_image,
                                                src_x + src_dx - dst_x,
                                                src_y + src_dy - dst_y,
...
out:
...
#endif
    {
        ...
        if (dst_image == NULL) goto cleanup_region;
        ...
        src_image = image_from_pict(src, FALSE, &src_dx, &src_dy);
        if (src_image == NULL) goto cleanup_dst;
        ...
        if (mask_format)
        {
            DBG(("%s: create mask (%d, %d)x(%d,%d) + (%d,%d) + (%d,%d), "
                 "depth=%d, format=%lx [%lx], ca? %d\n",
                 __FUNCTION__,
                 region.extents.x1,
                 region.extents.y1,
                 region.extents.x2 - region.extents.x1,
            ...
            mask_image =
                pixman_image_create_bits(pixman_format(mask_format),
                                         ...
                                         NULL,
                                         0);
            if (mask_image == NULL) goto cleanup_src;
        ...
        if (sigtrap_get() == 0)
        {
            do
            {
                ...
                while (n--)
                {
                    ...
                    if (!glyph_valid(g)) goto next_glyph;

                    glyph_image = sna_glyph_get_image(g, screen);
                    if (glyph_image == NULL) goto next_glyph;
                    ...
                    else
                    {
                        ...
                        DBG(("%s: glyph to dst (%d, %d)x(%d, %d)/[(%d, "
                             "%d)x(%d, %d)], src (%d, %d) [op=%d]\n",
                             __FUNCTION__,
                             xi,
                             yi,
                             g->info.width,
...
next_glyph:
        ...
        if (mask_format)
        {
            DBG(("%s: glyph mask composite src=(%d+%d,%d+%d) dst=(%d, %d)x(%d, "
                 "%d)\n",
                 __FUNCTION__,
                 src_x,
                 region.extents.x1,
           ...
           GlyphPtr     *glyphs)
{
    ...
    if (RegionNil(dst->pCompositeClip)) return;
    ...
    if (FORCE_GLYPHS_TO_DST || mask == NULL ||
    ...
    {
        if (glyphs_slow(sna, op, src, dst, src_x, src_y, nlist, list, glyphs))
            return;
    }
                 ...
                 GlyphPtr     *glyphs)
{
    ...
    DBG(("%s: small mask [format=%lx, depth=%d, size=%d], rendering glyphs to "
         "upload buffer\n",
         __FUNCTION__,
         (unsigned long)format->format,
         format->depth,
    ...
    mask_image = pixman_image_create_bits(pixmap->drawable.bitsPerPixel << 24 |
                                              format->format,
                                          width,
                                          height,
                                          pixmap->devPrivate.ptr,
...
sna_glyph_unrealize(ScreenPtr screen, GlyphPtr glyph)
{
    ...
    if (p->image)
    {
        ...
        if (__global_glyph_cache)
        {
            DBG(("%s: removing glyph %p from pixman cache\n",
                 __FUNCTION__,
                 glyph));
            pixman_glyph_cache_remove(__global_glyph_cache, glyph, NULL);
        }
#endif
    ...
    assert(__global_glyph_cache == NULL ||
           pixman_glyph_cache_lookup(__global_glyph_cache, glyph, NULL) ==
               NULL);
#endif
}
