...
static inline bool
need_tiling(struct sna *sna, int16_t width, int16_t height)
{
...
static inline bool
need_redirect(struct sna *sna, PixmapPtr dst)
{
...
vertex_space(struct sna *sna)
{
    return sna->render.vertex_size - sna->render.vertex_used;
}

static force_inline void
vertex_emit(struct sna *sna, float v)
{
...
static force_inline void
batch_emit(struct sna *sna, uint32_t dword)
{
...
static force_inline void
batch_emit_aligned(struct sna *sna, uint32_t dword, unsigned align)
{
    assert(sna->kgem.mode != KGEM_NONE);
    assert(sna->kgem.nbatch + KGEM_BATCH_RESERVED < sna->kgem.surface);
...
static force_inline void
batch_emit64(struct sna *sna, uint64_t qword)
{
    assert(sna->kgem.mode != KGEM_NONE);
...
static force_inline void
batch_emit_float(struct sna *sna, float f)
{
    union {
        uint32_t dw;
        float    f;
...
is_gpu(struct sna *sna, DrawablePtr drawable, unsigned prefer)
{
    ...
    if (priv == NULL || priv->clear || priv->cpu) return false;
...
is_gpu_dst(struct sna_pixmap *priv)
{
    ...
    if (too_small(priv)) return false;
...
unattached(DrawablePtr drawable)
{
    struct sna_pixmap *priv = sna_pixmap_from_drawable(drawable);
    return priv == NULL ||
           (priv->gpu_damage == NULL && priv->cpu_damage && !priv->cpu_bo);
}
...
static inline bool
sna_blt_compare_depth(const DrawableRec *src, const DrawableRec *dst)
{
    if (src->depth == dst->depth) return true;

    /* Also allow for the alpha to be discarded on a copy */
    if (src->bitsPerPixel != dst->bitsPerPixel) return false;

    if (dst->depth == 24 && src->depth == 32) return true;
                         ...
                         int                      height)
{
    ...
    if (op->damage == NULL || *op->damage == NULL) return;
...
color_convert(uint32_t pixel, uint32_t src_format, uint32_t dst_format)
{
...
static inline bool
dst_use_cpu(PixmapPtr pixmap)
{
    struct sna_pixmap *priv = sna_pixmap(pixmap);
    if (priv == NULL || priv->shm) return true;

    return priv->cpu_damage && priv->cpu;
}

static inline bool
dst_is_cpu(PixmapPtr pixmap)
{
    struct sna_pixmap *priv = sna_pixmap(pixmap);
...
overlaps(struct sna     *sna,
         ...
         BoxRec         *extents)
{
    if (src_bo != dst_bo) return false;

    if (flags & COPY_NO_OVERLAP) return false;
...
static inline long
get_picture_id(PicturePtr picture)
{
    return picture && picture->pDrawable
               ? get_drawable_pixmap(picture->pDrawable)->drawable.serialNumber
               : 0;
}
