...
static void
brw_wm_xy(struct brw_compile *p, int dw)
{
    ...
    brw_ADD(p,
            brw_vec8_grf(Y16, 0),
            vec8(y_uw),
            brw_negate(__suboffset(r1, 1)));
}

static void
brw_wm_affine_st(struct brw_compile *p, int dw, int channel, int msg)
{
    int uv;
...
static int
brw_wm_sample(struct brw_compile *p, int dw, int channel, int msg, int result)
{
    struct brw_reg src0;
    bool           header;
    int            len;
...
static int
brw_wm_sample__alpha(struct brw_compile *p,
                     int                 dw,
                     int                 channel,
                     int                 msg,
                     int                 result)
{
    ...
    if (p->gen >= 060) src0 = brw_message_reg(msg);
    else src0 = brw_vec8_grf(0, 0);
...
static int
brw_wm_affine(struct brw_compile *p, int dw, int channel, int msg, int result)
{
...
static int
brw_wm_affine__alpha(struct brw_compile *p,
                     int                 dw,
                     int                 channel,
                     int                 msg,
                     int                 result)
...
static void
brw_fb_write(struct brw_compile *p, int dw)
{
...
static void
brw_wm_write(struct brw_compile *p, int dw, int src)
{
    int n;
    ...
    for (n = 0; n < 4; n++)
    {
        ...
        {
            brw_set_compression_control(p, BRW_COMPRESSION_NONE);
            brw_MOV(p, brw_message_reg(2 + n), brw_vec8_grf(src + 2 * n, 0));
...
static void
brw_wm_write__mask(struct brw_compile *p, int dw, int src, int mask)
{
    int n;
...
static void
brw_wm_write__opacity(struct brw_compile *p, int dw, int src, int mask)
{
    int n;
...
static void
brw_wm_write__mask_ca(struct brw_compile *p, int dw, int src, int mask)
{
    int n;
...
{
    if (p->gen < 060) brw_wm_xy(p, dispatch);
...
static void
brw_wm_projective_st(struct brw_compile *p, int dw, int channel, int msg)
{
    int uv;
    ...
    if (p->gen >= 060)
    {
        ...
        else brw_math_invert(p, brw_vec8_grf(30, 0), brw_vec8_grf(30, 0));
...
static int
brw_wm_projective(struct brw_compile *p,
                  int                 dw,
                  int                 channel,
                  int                 msg,
                  int                 result)
...
static int
brw_wm_projective__alpha(struct brw_compile *p,
                         int                 dw,
                         int                 channel,
                         int                 msg,
                         int                 result)
