...
static const char *conditional_modifier[16] = {
    [BRW_CONDITIONAL_NONE] = "",  [BRW_CONDITIONAL_Z] = ".e",
    [BRW_CONDITIONAL_NZ] = ".ne", [BRW_CONDITIONAL_G] = ".g",
    [BRW_CONDITIONAL_GE] = ".ge", [BRW_CONDITIONAL_L] = ".l",
    [BRW_CONDITIONAL_LE] = ".le", [BRW_CONDITIONAL_R] = ".r",
    [BRW_CONDITIONAL_O] = ".o",   [BRW_CONDITIONAL_U] = ".u",
};
...
static const char
    *horiz_stride[4] = { [0] = "0", [1] = "1", [2] = "2", [3] = "4" };
...
static const char *debug_ctrl[2] = { [0] = "", [1] = ".breakpoint" };

static const char *saturate[2] = { [0] = "", [1] = ".sat" };

static const char *accwr[2] = { [0] = "", [1] = "AccWrEnable" };

static const char *wectrl[2] = { [0] = "WE_normal", [1] = "WE_all" };

static const char *exec_size[8] = {
    [0] = "1", [1] = "2", [2] = "4", [3] = "8", [4] = "16", [5] = "32"
};

static const char *pred_inv[2] = { [0] = "+", [1] = "-" };
...
static const char *pred_ctrl_align1[16] = {
    [1] = "",       [2] = ".anyv",    [3] = ".allv",    [4] = ".any2h",
    [5] = ".all2h", [6] = ".any4h",   [7] = ".all4h",   [8] = ".any8h",
    [9] = ".all8h", [10] = ".any16h", [11] = ".all16h",
};

static const char *thread_ctrl[4] = { [0] = "", [2] = "switch" };
...
static const char *writemask[16] = {
    [0x0] = ".",   [0x1] = ".x",   [0x2] = ".y",   [0x3] = ".xy",
    [0x4] = ".z",  [0x5] = ".xz",  [0x6] = ".yz",  [0x7] = ".xyz",
    [0x8] = ".w",  [0x9] = ".xw",  [0xa] = ".yw",  [0xb] = ".xyw",
    [0xc] = ".zw", [0xd] = ".xzw", [0xe] = ".yzw", [0xf] = "",
};

static const char *end_of_thread[2] = { [0] = "", [1] = "EOT" };

static const char *target_function[16] = { [BRW_SFID_NULL]    = "null",
                                           [BRW_SFID_MATH]    = "math",
...
static const char *math_saturate[2] = { [0] = "", [1] = "sat" };

static const char *math_signed[2] = { [0] = "", [1] = "signed" };

static const char *math_scalar[2] = { [0] = "", [1] = "scalar" };

static const char *math_precision[2] = { [0] = "", [1] = "partial_precision" };
...
static const char *urb_allocate[2] = { [0] = "", [1] = "allocate" };

static const char *urb_used[2] = { [0] = "", [1] = "used" };

static const char *urb_complete[2] = { [0] = "", [1] = "complete" };

static const char
    *sampler_target_format[4] = { [0] = "F", [2] = "UD", [3] = "D" };

static int column;

static int
string(FILE *file, const char *str)
{
    fputs(str, file);
    column += strlen(str);
    return 0;
...
reg(FILE *file, unsigned _reg_file, unsigned _reg_nr)
{
    ...
    if (_reg_file == BRW_MESSAGE_REGISTER_FILE) _reg_nr &= ~(1 << 7);
...
static void
dest(FILE *file, const struct brw_instruction *inst)
{
    ...
    else
    {
        if (inst->bits1.da16.dest_address_mode == BRW_ADDRESS_DIRECT)
        {
            ...
            if (inst->bits1.da16.dest_subreg_nr)
                format(file,
                       ".%d",
                       inst->bits1.da16.dest_subreg_nr /
                           reg_type_size[inst->bits1.da16.dest_reg_type]);
            string(file, "<1>");
            control(file,
                    "writemask",
                    writemask,
                    inst->bits1.da16.dest_writemask,
                    NULL);
            control(file,
                    "dest reg encoding",
                    reg_encoding,
                    inst->bits1.da16.dest_reg_type,
                    NULL);
        }
        else
        {
        ...
        unsigned _negate)
{
    ...
    if (reg(file, _reg_file, reg_num)) return;

    if (sub_reg_num)
        format(file,
               ".%d",
               sub_reg_num /
                   reg_type_size[type]); /* use formal style like spec */
        ...
        unsigned _vert_stride)
{
    ...
    string(file, "g[a0");
    if (_addr_subreg_nr) format(file, ".%d", _addr_subreg_nr);
    if (_addr_imm) format(file, " %d", _addr_imm);
    string(file, "]");
    src_align1_region(file, _vert_stride, _width, _horiz_stride);
    control(file, "src reg encoding", reg_encoding, type, NULL);
}
         ...
         unsigned swz_w)
{
    ...
    if (reg(file, _reg_file, _reg_nr)) return;
	 ...
	 */
    if (swz_x == BRW_CHANNEL_X && swz_y == BRW_CHANNEL_Y &&
...
static void
imm(FILE *file, unsigned type, const struct brw_instruction *inst)
{
    switch (type)
    {
...
static void
src0(FILE *file, const struct brw_instruction *inst)
{
...
static void
src1(FILE *file, const struct brw_instruction *inst)
{
...
static int
qtr_ctrl(FILE *file, const struct brw_instruction *inst)
{
...
void
brw_disasm(FILE *file, const struct brw_instruction *inst, int gen)
{
    int space = 0;
    ...
    if (inst->header.opcode == BRW_OPCODE_MATH)
    {
        ...
        control(file,
                "function",
                math_function,
                inst->header.destreg__conditionalmod,
                NULL);
    }
    ...
    else if (gen >= 060 && (inst->header.opcode == BRW_OPCODE_IF ||
        ...
        inst->header.opcode == BRW_OPCODE_SENDC)
    {
        ...
        if (gen >= 060) target = inst->header.destreg__conditionalmod;
        else if (gen >= 050) target = inst->bits2.send_gen5.sfid;
        else target = inst->bits3.generic.msg_target;
        ...
        if (gen >= 060)
        {
            control(file,
                    "target function",
                    target_function_gen6,
                    target,
                    &space);
        }
        else
        {
            control(file, "target function", target_function, target, &space);
        }
        ...
        switch (target)
        {
            case BRW_SFID_MATH:
                control(file,
                        "math function",
                        math_function,
                        inst->bits3.math.function,
                        &space);
                control(file,
                        "math saturate",
                        math_saturate,
                        inst->bits3.math.saturate,
                        &space);
                control(file,
                        "math signed",
                        math_signed,
                        inst->bits3.math.int_type,
                        &space);
                control(file,
                        "math scalar",
                        math_scalar,
                        inst->bits3.math.data_type,
                        &space);
                control(file,
                        "math precision",
                        math_precision,
                        inst->bits3.math.precision,
                        &space);
                break;
            case BRW_SFID_SAMPLER:
                ...
                else
                {
                    ...
                    control(file,
                            ...
                            inst->bits3.sampler.return_format,
                            NULL);
                    string(file, ")");
                }
                break;
            ...
            case BRW_SFID_URB:
                ...
                if (gen >= 050)
                {
                    control(file,
                            "urb opcode",
                            urb_opcode,
                            inst->bits3.urb_gen5.opcode,
                            &space);
                }
                control(file,
                        "urb swizzle",
                        urb_swizzle,
                        inst->bits3.urb.swizzle_control,
                        &space);
                control(file,
                        "urb allocate",
                        urb_allocate,
                        inst->bits3.urb.allocate,
                        &space);
                control(file,
                        "urb used",
                        urb_used,
        ...
        {
            format(file, "mlen %d", inst->bits3.generic.msg_length);
            format(file, " rlen %d", inst->bits3.generic.response_length);
        }
    }
    pad(file, 64);
    if (inst->header.opcode != BRW_OPCODE_NOP)
    {
        ...
        control(file,
                "access mode",
                access_mode,
                inst->header.access_mode,
                &space);
        if (gen >= 060)
            control(file,
                    "write enable control",
                    wectrl,
                    inst->header.mask_control,
                    &space);
        else
            control(file,
                    "mask control",
        ...
        control(file,
                "dependency control",
                dep_ctrl,
                inst->header.dependency_control,
                &space);

        if (gen >= 060) qtr_ctrl(file, inst);
        else
        {
            ...
            else
            {
                control(file,
                        "compression control",
                        compr_ctrl,
                        inst->header.compression_control,
                        &space);
            }
        }
        ...
        control(file,
                "thread control",
                thread_ctrl,
                inst->header.thread_control,
                &space);
        if (gen >= 060)
            control(file,
                    "acc write control",
                    accwr,
                    inst->header.acc_wr_control,
                    &space);
        if (inst->header.opcode == BRW_OPCODE_SEND ||
            inst->header.opcode == BRW_OPCODE_SENDC)
            control(file,
                    "end of thread",
                    end_of_thread,
                    inst->bits3.generic.end_of_thread,
                    &space);
        if (space) string(file, " ");
        string(file, "}");
    }
