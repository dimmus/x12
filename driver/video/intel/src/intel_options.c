...
OptionInfoPtr
intel_options_get(ScrnInfoPtr scrn)
{
    OptionInfoPtr options;

    xf86CollectOptions(scrn, NULL);
    if (!(options = malloc(sizeof(intel_options)))) return NULL;
...
Bool
intel_option_cast_to_bool(OptionInfoPtr options, int id, Bool val)
{
...
unsigned
intel_option_cast_to_unsigned(OptionInfoPtr options, int id, unsigned val)
{
#if XORG_VERSION_CURRENT >= XORG_VERSION_NUMERIC(1, 7, 99, 901, 0)
    ...
    if (str == NULL || *str == '\0') return val;

    if (namecmp(str, "on") == 0) return val;
    if (namecmp(str, "true") == 0) return val;
    if (namecmp(str, "yes") == 0) return val;

    if (namecmp(str, "0") == 0) return 0;
