...
uxa_unrealize_glyph_caches(ScreenPtr pScreen)
{
    ...
    if (!uxa_screen->glyph_cache_initialized) return;
...
uxa_realize_glyph_caches(ScreenPtr pScreen)
{
    ...
    if (uxa_screen->glyph_cache_initialized) return TRUE;
...
uxa_glyphs_init(ScreenPtr pScreen)
{
    ...
    if (uxa_get_screen(pScreen)->force_fallback) return TRUE;
...
void
uxa_glyph_unrealize(ScreenPtr screen, GlyphPtr glyph)
{
                 ...
                 GlyphPtr     *glyphs)
{
    ...
    if (white) FreePicture(white, 0);

    if (maskFormat)
    {
        x = extents.x1;
        y = extents.y1;
        CompositePicture(mask_op,
                         mask_src,
                         mask,
                         mask_dst,
                         xSrc + x - xDst,
                    ...
                    GlyphPtr     *glyphs)
{
    ...
    if (white) white_pixmap = uxa_get_drawable_pixmap(white->pDrawable);
    if (!white_pixmap)
    {
        if (white) FreePicture(white, 0);
        screen->DestroyPixmap(pixmap);
    ...
    while (nlist--)
    {
        ...
        while (n--)
        {
            ...
            if (this_atlas != glyph_atlas)
            {
                ...
                glyph_pixmap = uxa_get_drawable_pixmap(this_atlas->pDrawable);
                  ...
                  GlyphPtr    *glyphs)
{
    ...
    while (nlist--)
    {
        ...
        while (n--)
        {
            ...
            {
                glyph_atlas =
                    uxa_glyph_cache(screen, glyph, &glyph_x, &glyph_y);
                if (glyph_atlas == NULL)
                {
                    /* no cache for this glyph */
...
is_solid(PicturePtr picture)
{
    ...
    {
        return (picture->repeat && picture->pDrawable->width == 1 &&
           ...
           GlyphPtr     *glyphs)
{
    ...
    if (!uxa_screen->info->prepare_composite || uxa_screen->force_fallback ||
...
fallback:
    ...
    if (!maskFormat)
    {
        if (uxa_glyphs_to_dst(op, pSrc, pDst, xSrc, ySrc, nlist, list, glyphs))
            goto fallback;
    }
    else
    {
