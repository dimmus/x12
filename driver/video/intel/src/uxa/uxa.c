 ...
 */
void
uxa_finish_access(DrawablePtr pDrawable, uxa_access_t access)
{
    ScreenPtr     pScreen    = pDrawable->pScreen;
    uxa_screen_t *uxa_screen = uxa_get_screen(pScreen);
    PixmapPtr     pPixmap;
...
{
    RegionPtr ret;
    if (!uxa_prepare_access(&pPix->drawable, UXA_ACCESS_RO)) return NULL;
...
uxa_close_screen(CLOSE_SCREEN_ARGS_DECL)
{
    ...
    screen->ChangeWindowAttributes = uxa_screen->SavedChangeWindowAttributes;
