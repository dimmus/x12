                 ...
                 int         src_stride)
{
    ...
    if (uxa_screen->force_fallback) return FALSE;
         ...
         pbox++)
    {
        ...
        if (!ok)
        {
            ...
            if (!uxa_prepare_access(pDrawable, UXA_ACCESS_RW)) return FALSE;
                ...
                void       *closure)
{
    ...
    if (!uxa_pixmap_is_offscreen(pDstPixmap))
    {
        ...
        if (!uxa_pixmap_is_offscreen(pSrcPixmap)) goto fallback;

        if (!uxa_screen->info->get_image) goto fallback;
        ...
        while (nbox--)
        {
            if (!uxa_screen->info->get_image(
                    ...
                    pbox->x2 - pbox->x1,
                    pbox->y2 - pbox->y1,
                    (char *)dst + (pbox->y1 + dst_off_y) * stride +
...
static BoxRec
box_from_seg(const xSegment *seg, GCPtr gc)
{
    BoxRec b;
...
static void
uxa_set_spans(DrawablePtr pDrawable,
              GCPtr       gc,
              char       *src,
              DDXPointPtr points,
              int        *widths,
...
static RegionPtr
uxa_copy_plane(DrawablePtr   pSrc,
               DrawablePtr   pDst,
               GCPtr         pGC,
               int           srcx,
               int           srcy,
...
{
    return uxa_check_copy_plane(pSrc,
                                pDst,
                                pGC,
                                srcx,
                                srcy,
...
static void
uxa_image_glyph_blt(DrawablePtr  pDrawable,
                    GCPtr        pGC,
                    int          x,
                    int          y,
                    unsigned int nglyph,
...
static void
uxa_poly_glyph_blt(DrawablePtr  pDrawable,
                   GCPtr        pGC,
                   int          x,
                   int          y,
                   unsigned int nglyph,
...
const GCOps uxa_ops = {
    uxa_fill_spans,     uxa_set_spans,    uxa_put_image,
    uxa_copy_area,      uxa_copy_plane,   uxa_poly_point,
    uxa_poly_lines,     uxa_poly_segment, miPolyRectangle,
    uxa_check_poly_arc, miFillPolygon,    uxa_poly_fill_rect,
    miPolyFillArc,      miPolyText8,      miPolyText16,
    miImageText8,       miImageText16,    uxa_image_glyph_blt,
    uxa_poly_glyph_blt, uxa_push_pixels,
};
                      ...
                      CARD32      alu)
{
    ...
    while (nbox--)
    {
        uxa_screen->info->solid(pixmap, pBox->x1, pBox->y1, pBox->x2, pBox->y2);
        pBox++;
    }
