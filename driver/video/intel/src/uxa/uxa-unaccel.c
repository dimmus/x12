...
uxa_finish_access_gc(GCPtr pGC)
{
    ...
    if (pGC->stipple) uxa_finish_access(&pGC->stipple->drawable, UXA_ACCESS_RO);
}

Bool
uxa_picture_prepare_access(PicturePtr picture, int mode)
{
    if (picture->pDrawable == NULL) return TRUE;

    if (!uxa_prepare_access(picture->pDrawable, mode)) return FALSE;
...
void
uxa_picture_finish_access(PicturePtr picture, int mode)
{
    if (picture->pDrawable == NULL) return;
...
void
uxa_check_fill_spans(DrawablePtr pDrawable,
                     GCPtr       pGC,
                     int         nspans,
                     DDXPointPtr ppt,
                     int        *pwidth,
...
void
uxa_check_set_spans(DrawablePtr pDrawable,
                    GCPtr       pGC,
                    char       *psrc,
                    DDXPointPtr ppt,
                    int        *pwidth,
...
void
uxa_check_put_image(DrawablePtr pDrawable,
                    GCPtr       pGC,
                    int         depth,
                    int         x,
                    int         y,
...
RegionPtr
uxa_check_copy_area(DrawablePtr pSrc,
                    DrawablePtr pDst,
                    GCPtr       pGC,
                    int         srcx,
                    int         srcy,
...
RegionPtr
uxa_check_copy_plane(DrawablePtr   pSrc,
                     DrawablePtr   pDst,
                     GCPtr         pGC,
                     int           srcx,
                     int           srcy,
...
void
uxa_check_poly_point(DrawablePtr pDrawable,
                     GCPtr       pGC,
                     int         mode,
                     int         npt,
                     DDXPointPtr pptInit)
...
void
uxa_check_poly_lines(DrawablePtr pDrawable,
                     GCPtr       pGC,
                     int         mode,
                     int         npt,
                     DDXPointPtr ppt)
...
void
uxa_check_poly_segment(DrawablePtr pDrawable,
                       GCPtr       pGC,
                       int         nsegInit,
                       xSegment   *pSegInit)
{
...
void
uxa_check_poly_fill_rect(DrawablePtr pDrawable,
                         GCPtr       pGC,
                         int         nrect,
                         xRectangle *prect)
{
...
void
uxa_check_image_glyph_blt(DrawablePtr  pDrawable,
                          GCPtr        pGC,
                          int          x,
                          int          y,
                          unsigned int nglyph,
...
void
uxa_check_poly_glyph_blt(DrawablePtr  pDrawable,
                         GCPtr        pGC,
                         int          x,
                         int          y,
                         unsigned int nglyph,
...
void
uxa_check_push_pixels(GCPtr       pGC,
                      PixmapPtr   pBitmap,
        ...
        if (uxa_picture_prepare_access(pSrc, UXA_ACCESS_RO))
        {
            if (!pMask || uxa_picture_prepare_access(pMask, UXA_ACCESS_RO))
            {
                fbComposite(op,
                            ...
                            width,
                            height);
                if (pMask) uxa_picture_finish_access(pMask, UXA_ACCESS_RO);
            }
