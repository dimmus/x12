...
static inline struct intel_uxa_pixmap *
intel_uxa_get_pixmap_private(PixmapPtr pixmap)
{
#if HAS_DEVPRIVATEKEYREC
...
static inline Bool
intel_uxa_pixmap_is_busy(struct intel_uxa_pixmap *priv)
{
    if (priv->busy == -1) priv->busy = drm_intel_bo_busy(priv->bo);
    return priv->busy;
}

static inline void
intel_uxa_set_pixmap_private(PixmapPtr pixmap, struct intel_uxa_pixmap *intel)
{
    dixSetPrivate(&pixmap->devPrivates, &uxa_pixmap_index, intel);
}

static inline Bool
intel_uxa_pixmap_is_dirty(PixmapPtr pixmap)
{
    return pixmap && intel_uxa_get_pixmap_private(pixmap)->dirty;
}

static inline Bool
intel_uxa_pixmap_tiled(PixmapPtr pixmap)
{
...
static inline Bool
intel_uxa_pixmap_is_offscreen(PixmapPtr pixmap)
{
...
Bool i830_prepare_composite(int        op,
                            PicturePtr sourcec,
                            PicturePtr mask,
                            PicturePtr dest,
...
void i830_composite(PixmapPtr dest,
                    int       srcX,
                    int       srcY,
                    int       maskX,
                    int       maskY,
...
Bool i915_prepare_composite(int        op,
                            PicturePtr sourcec,
                            PicturePtr mask,
                            PicturePtr dest,
                            PixmapPtr  sourcecPixmap,
                            PixmapPtr  maskPixmap,
                            PixmapPtr  destPixmap);
void i915_composite(PixmapPtr dest,
                    int       srcX,
                    int       srcY,
                    int       maskX,
                    int       maskY,
...
Bool         i965_prepare_composite(int        op,
                                    PicturePtr sourcec,
                                    PicturePtr mask,
                                    PicturePtr dest,
                                    PixmapPtr  sourcecPixmap,
                                    PixmapPtr  maskPixmap,
                                    PixmapPtr  destPixmap);
void         i965_composite(PixmapPtr dest,
                            int       srcX,
                            int       srcY,
                            int       maskX,
                            int       maskY,
...
void gen7_upload_binding_table(intel_screen_private *intel,
                               uint32_t              ps_binding_table_offset);
...
Bool intel_uxa_get_transformed_coordinates(int              x,
                                           int              y,
                                           PictTransformPtr transform,
                                           float           *x_out,
                                           float           *y_out);

Bool intel_uxa_get_transformed_coordinates_3d(int              x,
                                              int              y,
                                              PictTransformPtr transform,
                                              float           *x_out,
                                              float           *y_out,
...
static inline void intel_uxa_debug_fallback(ScrnInfoPtr scrn,
                                            const char *format,
...
intel_uxa_emit_reloc(drm_intel_bo *bo,
                     uint32_t      offset,
...
intel_uxa_bo_alloc_for_data(intel_screen_private *intel,
                            const void           *data,
                            unsigned int          size,
                            const char           *name)
{
...
Bool intel_uxa_get_aperture_space(ScrnInfoPtr    scrn,
