                     ...
                     INT16     *out_y)
{
    ...
    if (!transform_is_integer_translation(pSrc->transform, &tx, &ty) ||
        ...
        depth == 1 || pSrc->filter == PictFilterConvolution)
    {
              ...
              CARD16     height)
{
    ...
    if (uxa_screen->force_fallback) goto fallback;
              ...
              xTriangle    *tris)
{
    ...
    if (uxa_screen->force_fallback)
    {
        uxa_check_triangles(op, pSrc, pDst, maskFormat, xSrc, ySrc, ntri, tris);
        return;
    }
