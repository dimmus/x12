...
static Bool
amdgpu_glamor_prepare_access_cpu(ScrnInfoPtr           scrn,
                                 AMDGPUInfoPtr         info,
                                 PixmapPtr             pixmap,
                                 ...
                                 Bool                  need_sync)
{
    ...
    if (need_sync) amdgpu_glamor_flush(scrn);
    ...
    else if (need_sync)
    {
        ...
        info->glamor
            .SavedGetImage(&pixmap->drawable, 0, 0, 1, 1, ZPixmap, ~0, pixel);
        info->gpu_synced = info->gpu_flushed;
    }
...
static Bool
amdgpu_glamor_prepare_access_cpu_ro(ScrnInfoPtr           scrn,
                                    PixmapPtr             pixmap,
                                    struct amdgpu_pixmap *priv)
{
    AMDGPUInfoPtr info;
    ...
    return amdgpu_glamor_prepare_access_cpu(scrn,
                                            AMDGPUPTR(scrn),
                                            pixmap,
                                            priv,
                                            need_sync);
...
static Bool
amdgpu_glamor_prepare_access_cpu_rw(ScrnInfoPtr           scrn,
                                    PixmapPtr             pixmap,
                                    struct amdgpu_pixmap *priv)
{
    AMDGPUInfoPtr info;
...
amdgpu_glamor_prepare_access_gc(ScrnInfoPtr scrn, GCPtr pGC)
{
    ...
    if (pGC->fillStyle == FillTiled)
    {
        ...
        if (!amdgpu_glamor_prepare_access_cpu_ro(scrn, pGC->tile.pixmap, priv))
        {
            if (pGC->stipple) amdgpu_glamor_finish_access_cpu(pGC->stipple);
            return FALSE;
        }
    }
    return TRUE;
                                            ...
                                            PicturePtr  picture)
{
    ...
    if (!amdgpu_glamor_prepare_access_cpu_ro(scrn, pixmap, priv)) return FALSE;
                                            ...
                                            PicturePtr  picture)
{
    ...
    pixmap = get_drawable_pixmap(picture->pDrawable);
    priv   = amdgpu_get_pixmap_private(pixmap);
    if (!amdgpu_glamor_prepare_access_cpu_rw(scrn, pixmap, priv)) return FALSE;
...
static void
amdgpu_glamor_fill_spans(DrawablePtr pDrawable,
                         GCPtr       pGC,
                         int         nspans,
                         DDXPointPtr ppt,
                         int        *pwidth,
...
static void
amdgpu_glamor_set_spans(DrawablePtr pDrawable,
                        GCPtr       pGC,
                        char       *psrc,
                        DDXPointPtr ppt,
                        int        *pwidth,
...
static void
amdgpu_glamor_put_image(DrawablePtr pDrawable,
                        GCPtr       pGC,
                        int         depth,
                        int         x,
                        int         y,
...
static RegionPtr
amdgpu_glamor_copy_plane(DrawablePtr   pSrc,
                         DrawablePtr   pDst,
                         GCPtr         pGC,
                         int           srcx,
                         int           srcy,
...
static RegionPtr
amdgpu_glamor_copy_plane_nodstbo(DrawablePtr   pSrc,
                                 DrawablePtr   pDst,
                                 GCPtr         pGC,
                                 int           srcx,
                                 int           srcy,
...
static void
amdgpu_glamor_poly_point(DrawablePtr pDrawable,
                         GCPtr       pGC,
                         int         mode,
                         int         npt,
                         DDXPointPtr pptInit)
...
static void
amdgpu_glamor_poly_lines(DrawablePtr pDrawable,
                         GCPtr       pGC,
                         int         mode,
                         int         npt,
                         DDXPointPtr ppt)
...
static void
amdgpu_glamor_poly_segment(DrawablePtr pDrawable,
                           GCPtr       pGC,
                           int         nsegInit,
                           xSegment   *pSegInit)
{
...
static void
amdgpu_glamor_poly_fill_rect(DrawablePtr pDrawable,
                             GCPtr       pGC,
                             int         nrect,
                             xRectangle *prect)
{
...
static void
amdgpu_glamor_image_glyph_blt(DrawablePtr  pDrawable,
                              GCPtr        pGC,
                              int          x,
                              int          y,
                              unsigned int nglyph,
...
static void
amdgpu_glamor_poly_glyph_blt(DrawablePtr  pDrawable,
                             GCPtr        pGC,
                             int          x,
                             int          y,
                             unsigned int nglyph,
...
static void
amdgpu_glamor_push_pixels(GCPtr       pGC,
                          PixmapPtr   pBitmap,
...
static void
amdgpu_glamor_push_pixels_nodstbo(GCPtr       pGC,
                                  PixmapPtr   pBitmap,
                                  DrawablePtr pDrawable,
                                  int         w,
                                  int         h,
...
static RegionPtr
amdgpu_glamor_copy_area(DrawablePtr pSrcDrawable,
                        ...
                        int         dsty)
{
    ...
    {
        if (!amdgpu_glamor_prepare_access_gpu(dst_priv)) goto fallback;
        if (src_priv != dst_priv && !amdgpu_glamor_prepare_access_gpu(src_priv))
            goto fallback;
...
fallback:
    if (amdgpu_glamor_prepare_access_cpu_rw(scrn, dst_pixmap, dst_priv))
    {
        if (pSrcDrawable == pDstDrawable ||
            amdgpu_glamor_prepare_access_cpu_ro(scrn, src_pixmap, src_priv))
        {
                                ...
                                int         dsty)
{
    ...
    if (src_pixmap != dst_pixmap)
    {
        ...
        if (!amdgpu_glamor_prepare_access_cpu_ro(scrn, src_pixmap, src_priv))
            return ret;
    }
    ...
    if (src_pixmap != dst_pixmap) amdgpu_glamor_finish_access_cpu(src_pixmap);
 ...
 */
static void
amdgpu_glamor_validate_gc(GCPtr         pGC,
                          ...
                          DrawablePtr   pDrawable)
{
    ...
    else pGC->ops = &amdgpu_glamor_nodstbo_ops;
}

static GCFuncs glamorGCFuncs = { amdgpu_glamor_validate_gc,
                                 miChangeGC,
                                 miCopyGC,
                                 miDestroyGC,
                                 miChangeClip,
...
amdgpu_glamor_bitmap_to_region(PixmapPtr pPix)
{
    ...
    if (!amdgpu_glamor_prepare_access_cpu_ro(scrn, pPix, priv)) return NULL;
...
static void
amdgpu_glamor_copy_window(WindowPtr   pWin,
...
static void
amdgpu_glamor_get_image(DrawablePtr   pDrawable,
                        int           x,
                        int           y,
                        int           w,
                        int           h,
...
static void
amdgpu_glamor_get_spans(DrawablePtr pDrawable,
                        int         wMax,
                        DDXPointPtr ppt,
                        int        *pwidth,
                        int         nspans,
                        ...
                        CARD16     height)
{
                             ...
                             amdgpu_glamor_prepare_access_gpu(src_priv)))
    {
             ...
             amdgpu_glamor_prepare_access_gpu(mask_priv)))
        {
            ...
            if (mask_priv) amdgpu_glamor_finish_access_gpu_ro(info, mask_priv);
        }

        if (src_priv) amdgpu_glamor_finish_access_gpu_ro(info, src_priv);
    }
    amdgpu_glamor_finish_access_gpu_rw(info, dst_priv);
        ...
        if (amdgpu_glamor_picture_prepare_access_cpu_ro(scrn, pSrc))
        {
                ...
                amdgpu_glamor_picture_prepare_access_cpu_ro(scrn, pMask))
            {
                fbComposite(op,
                            ...
                            width,
                            height);
                if (pMask) amdgpu_glamor_picture_finish_access_cpu(pMask);
            }
...
static void
amdgpu_glamor_trapezoids(CARD8         op,
                         PicturePtr    src,
                         PicturePtr    dst,
                         PictFormatPtr maskFormat,
                         INT16         xSrc,
...
static void
amdgpu_glamor_triangles(CARD8         op,
                        PicturePtr    src,
                        PicturePtr    dst,
                        PictFormatPtr maskFormat,
                        INT16         xSrc,
