...
static void
amdgpu_box_intersect(BoxPtr dest, BoxPtr a, BoxPtr b)
{
...
static void
amdgpu_crtc_box(RRCrtcPtr crtc, BoxPtr crtc_box)
{
    if (crtc->mode)
    {
        crtc_box->x1 = crtc->x;
...
static Bool
amdgpu_crtc_on(RRCrtcPtr crtc, Bool crtc_is_xf86_hint)
{
    if (!crtc)
    {
        return FALSE;
 ...
 */
static RRCrtcPtr
amdgpu_crtc_covering_box(ScreenPtr pScreen,
                         BoxPtr    box,
                         Bool      screen_is_xf86_hint)
{
    ...
    for (c = 0; c < pScrPriv->numCrtcs; c++)
    {
        ...
        if (!amdgpu_crtc_on(crtc, screen_is_xf86_hint)) continue;
...
void
AMDGPUInitVideo(ScreenPtr pScreen)
{
    ScrnInfoPtr          pScrn = xf86ScreenToScrn(pScreen);
