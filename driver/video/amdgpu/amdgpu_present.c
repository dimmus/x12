...
amdgpu_present_get_ust_msc(RRCrtcPtr crtc, CARD64 *ust, CARD64 *msc)
{
    ...
    if (drmmode_crtc->dpms_mode != DPMSModeOn) return BadAlloc;
...
amdgpu_present_flush_drm_events(ScreenPtr screen)
{
    ...
    return amdgpu_drm_handle_event(pAMDGPUEnt->fd, &drmmode->event_context) >=
           0;
}
 ...
 */
static void
amdgpu_present_vblank_handler(xf86CrtcPtr  crtc,
                              unsigned int msc,
                              uint64_t     usec,
                              void        *data)
{
...
amdgpu_present_check_unflip(ScrnInfoPtr scrn)
{
    ...
    for (i = 0, num_crtcs_on = 0; i < config->num_crtc; i++)
    {
        ...
        if (drmmode_crtc_can_flip(crtc))
        {
            ...
            if (drmmode_crtc->flip_pending) return FALSE;

            if (!drmmode_crtc->tear_free) num_crtcs_on++;
        }
    }
 ...
 */
static Bool
amdgpu_present_check_flip(RRCrtcPtr crtc,
                          WindowPtr window,
                          PixmapPtr pixmap,
                          Bool      sync_flip)
{
    ...
    if (!info->allowPageFlip) return FALSE;

    if (info->sprites_visible > 0) return FALSE;

    if (info->drmmode.dri2_flipping) return FALSE;

    if (priv && priv->fb_failed) return FALSE;

    if (!amdgpu_pixmap_get_fb(pixmap))
    {
        if (!priv) priv = amdgpu_get_pixmap_private(pixmap);
    ...
    if (info->dri2.pKernelDRMVersion->version_minor < (dc_enabled ? 31 : 34))
    {
		 ...
		 */
        if (pixmap->devKind != screen_pixmap->devKind) return FALSE;
    }
    ...
    for (i = 0, num_crtcs_on = 0; i < config->num_crtc; i++)
    {
        ...
        else if (config->crtc[i] == crtc->devPrivate) return FALSE;
    }

    if (num_crtcs_on == 0) return FALSE;
 ...
 */
static void
amdgpu_present_flip_event(xf86CrtcPtr crtc,
                          uint32_t    msc,
                          uint64_t    ust,
                          void       *pageflip_data)
{
    ...
    if (event->unflip) info->drmmode.present_flipping = FALSE;
 ...
 */
static Bool
amdgpu_present_flip(RRCrtcPtr crtc,
                    uint64_t  event_id,
                    uint64_t  target_msc,
                    PixmapPtr pixmap,
                    Bool      sync_flip)
{
    ...
    else info->drmmode.present_flipping = TRUE;
...
amdgpu_present_unflip(ScreenPtr screen, uint64_t event_id)
{
    ...
    if (!amdgpu_present_check_unflip(scrn)) goto modeset;
...
modeset:
    ...
    for (i = 0; i < config->num_crtc; i++)
    {
        ...
        if (!crtc->enabled || drmmode_crtc->tear_free) continue;
        ...
        if (drmmode_crtc->dpms_mode == DPMSModeOn)
            crtc->funcs->set_mode_major(crtc,
                                        ...
                                        crtc->x,
                                        crtc->y);
        else drmmode_crtc->need_modeset = TRUE;
    }
...
amdgpu_present_screen_init(ScreenPtr screen)
{
    ...
    {
        xf86DrvMsg(
            xf86ScreenToScrn(screen)->scrnIndex,
            X_WARNING,
