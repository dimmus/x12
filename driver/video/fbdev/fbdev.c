...
#define TRACE_ENTER(str)                                            \
    do                                                              \
    {                                                               \
        if (debug) ErrorF("fbdev: " str " %d\n", pScrn->scrnIndex); \
    }                                                               \
    while (0)
#define TRACE_EXIT(str)                             \
    do                                              \
    {                                               \
        if (debug) ErrorF("fbdev: " str " done\n"); \
    }                                               \
    while (0)
#define TRACE(str)                                   \
    do                                               \
    {                                                \
        if (debug) ErrorF("fbdev trace: " str "\n"); \
    }                                                \
    while (0)
...
#ifdef XSERVER_LIBPCIACCESS
static Bool FBDevPciProbe(DriverPtr          drv,
                          int                entity_num,
...
_X_EXPORT DriverRec FBDEV = {
    FBDEV_VERSION,      FBDEV_DRIVER_NAME,
#if 0
	"driver for linux framebuffer devices",
#endif
    FBDevIdentify,      FBDevProbe,        FBDevAvailableOptions, NULL, 0,
    FBDevDriverFunc,

#ifdef XSERVER_LIBPCIACCESS
    fbdev_device_match, FBDevPciProbe
#endif
};
...
static XF86ModuleVersionInfo FBDevVersRec = {
    ...
    FBDEV_PACKAGE_VERSION_MAJOR,
    FBDEV_PACKAGE_VERSION_MINOR,
    FBDEV_PACKAGE_VERSION_PATCHLEVEL,
    ABI_CLASS_VIDEODRV,
...
FBDevProbe(DriverPtr drv, int flags)
{
    ...
    for (i = 0; i < numDevSections; i++)
    {
        ...
        {
#ifndef XSERVER_LIBPCIACCESS
            if (xf86ParsePciBusString(devSections[i]->busID,
                                      &bus,
                                      &device,
                                      &func))
            {
                if (!xf86CheckPciSlot(bus, device, func)) continue;
                isPci = TRUE;
            }
#endif
        }
        if (fbdevHWProbe(NULL, dev, NULL))
        {
            ...
            else
            {
                ...
                pScrn  = xf86ConfigFbEntity(pScrn,
                                           0,
                                           entity,
                                           NULL,
                                           NULL,
...
FBDevPreInit(ScrnInfoPtr pScrn, int flags)
{
    ...
    if (default_depth == 8) do
        ...
        /* trust the command line */
            if (xf86FbBpp > 0 || xf86Depth > 0) break;
    ...
    if (!(fPtr->Options = malloc(sizeof(FBDevOptions)))) return FALSE;
    memcpy(fPtr->Options, FBDevOptions, sizeof(FBDevOptions));
    xf86ProcessOptions(pScrn->scrnIndex,
                       fPtr->pEnt->device->options,
                       fPtr->Options);

    /* use shadow framebuffer by default */
    fPtr->shadowFB =
        xf86ReturnOptValBool(fPtr->Options, OPTION_SHADOW_FB, TRUE);
    ...
    if (NULL == pScrn->modes) fbdevHWUseBuildinMode(pScrn);
...
FBDevCreateScreenResources(ScreenPtr pScreen)
{
    ...
    if (fPtr->shadow24) update = fbdevUpdate32to24;
    else if (fPtr->rotate) update = fbdevUpdateRotatePacked;
    else update = fbdevUpdatePacked;
...
FBDevScreenInit(ScreenPtr pScreen, int argc, char **argv)
{
    ...
    if (!fPtr->rotate) FBDevDGAInit(pScrn, pScreen);
    else
    {
        xf86DrvMsg(pScrn->scrnIndex,
                   X_INFO,
                  ...
                  void     *closure)
{
    ...
    if (fPtr->lineLength) *size = fPtr->lineLength;
    else *size = fPtr->lineLength = fbdevHWGetLineLength(pScrn);
 ...
 * DGA stuff
 ***********************************************************************/
static Bool FBDevDGAOpenFramebuffer(ScrnInfoPtr     pScrn,
                                    char          **DeviceName,
...
static Bool
FBDevDGAOpenFramebuffer(ScrnInfoPtr     pScrn,
                        char          **DeviceName,
                        unsigned char **ApertureBase,
                        int            *ApertureSize,
                        int            *ApertureOffset,
...
FBDevDGASetMode(ScrnInfoPtr pScrn, DGAModePtr pDGAMode)
{
    ...
    if (!(*pScrn->SwitchMode)(pScrn, pMode)) return FALSE;
...
FBDevDGAAddModes(ScrnInfoPtr pScrn)
{
    ...
    {
        pDGAMode =
            realloc(fPtr->pDGAMode, (fPtr->nDGAMode + 1) * sizeof(DGAModeRec));
        if (!pDGAMode) break;
        ...
        if (fPtr->lineLength) pDGAMode->bytesPerScanline = fPtr->lineLength;
        else
            pDGAMode->bytesPerScanline = fPtr->lineLength =
                fbdevHWGetLineLength(pScrn);
...
FBDevDGAInit(ScrnInfoPtr pScrn, ScreenPtr pScreen)
{
    ...
    if (pScrn->depth < 8) return FALSE;

    if (!fPtr->nDGAMode) FBDevDGAAddModes(pScrn);

    return (
        DGAInit(pScreen, &FBDevDGAFunctions, fPtr->pDGAMode, fPtr->nDGAMode));
#else
    return TRUE;
#endif
}
