...
#define BOX_NOT_EMPTY(box) (((box.x2 - box.x1) > 0) && ((box.y2 - box.y1) > 0))

#define checkGCDamage(g) \
    ((!g->pCompositeClip || REGION_NOTEMPTY(d->pScreen, g->pCompositeClip)))
...
static void
trim_region(RegionPtr pRegion, DrawablePtr pDrawable, int subWindowMode)
{
    RegionRec pixClip;
    int       draw_x = 0;
    int       draw_y = 0;
    ...
    if (!REGION_NOTEMPTY(pScreen, pRegion)) return;
     ...
     */
    if (draw_x || draw_y) REGION_TRANSLATE(pScreen, pRegion, -draw_x, -draw_y);
                     ...
                     int         dsty)
{
    if (checkGCDamage(pGC))
    {
        ...
        TRIM_BOX(box, pGC);
        if (BOX_NOT_EMPTY(box)) add_box(region, &box, pDst, pGC->subWindowMode);
    }
}
                      ...
                      DDXPointPtr ppt)
{
    if (npt && checkGCDamage(pGC))
    {
        ...
        if (nptTmp > 1)
        {
            if (pGC->joinStyle == JoinMiter) extra = 6 * pGC->lineWidth;
            else if (pGC->capStyle == CapProjecting) extra = pGC->lineWidth;
        }
                        ...
                        xSegment   *pSeg)
{
    if (nSeg && checkGCDamage(pGC))
    {
        ...
        if (pGC->capStyle != CapProjecting) extra >>= 1;
                    ...
                    xArc       *pArcs)
{
    if (nArcs && checkGCDamage(pGC))
    {
        ...
        {
            pArcsTmp++;
            if (box.x1 > pArcsTmp->x) box.x1 = pArcsTmp->x;
            if (box.x2 < (pArcsTmp->x + pArcsTmp->width))
                box.x2 = pArcsTmp->x + pArcsTmp->width;
            if (box.y1 > pArcsTmp->y) box.y1 = pArcsTmp->y;
                 ...
                 int          subWindowMode)
{
    ...
    if (imageblt)
    {
        ...
        if (extents.overallLeft > 0) extents.overallLeft = 0;
                        ...
                        unsigned short *chars)
{
    if (checkGCDamage(pGC))
        x = uxa_damage_text(region,
                            ...
                            (unsigned long)count,
                            (char *)chars,
                            FONTLASTROW(pGC->font) == 0 ? Linear16Bit
                                                        : TwoD16Bit,
                            TT_POLY16);
