...
static void
uxa_fill_spans(DrawablePtr pDrawable,
               GCPtr       pGC,
               int         n,
               DDXPointPtr ppt,
               int        *pwidth,
               int         fSorted)
{
    ...
    if (pGC->fillStyle != FillSolid) goto fallback;

    dst_pixmap = uxa_get_offscreen_pixmap(pDrawable, &off_x, &off_y);
    if (!dst_pixmap) goto fallback;

    if (pGC->alu != GXcopy || pGC->planemask != FB_ALLONES) goto solid;
        ...
        !uxa_screen->info->check_composite_texture(screen, src))
    {
        ...
        src = solid;
        src_pixmap =
            uxa_get_offscreen_pixmap(src->pDrawable, &src_off_x, &src_off_y);
        if (!src_pixmap)
        {
            FreePicture(src, 0);
            FreePicture(dst, 0);
    ...
    if (!uxa_screen->info->prepare_composite(PictOpSrc,
                                             src,
                                             NULL,
                                             dst,
                                             src_pixmap,
    ...
    while (n--)
    {
        ...
        if (fullY1 < extentY1 || extentY2 <= fullY1) continue;

        if (fullX1 < extentX1) fullX1 = extentX1;

        if (fullX2 > extentX2) fullX2 = extentX2;
        ...
        else
        {
            ...
            while (nbox--)
            {
                if (pbox->y1 > fullY1) break;

                if (pbox->y1 <= fullY1)
                {
                    partX1 = pbox->x1;
                    if (partX1 < fullX1) partX1 = fullX1;

                    partX2 = pbox->x2;
                    if (partX2 > fullX2) partX2 = fullX2;
...
static Bool
uxa_do_put_image(DrawablePtr pDrawable,
                 GCPtr       pGC,
                 int         depth,
                 int         x,
                 int         y,
                 ...
                 int         src_stride)
{
    ...
    if (format != ZPixmap || bpp < 8) return FALSE;

    if (uxa_screen->swappedOut || uxa_screen->force_fallback) return FALSE;

    if (!uxa_screen->info->put_image) return FALSE;
    ...
    for (nbox = REGION_NUM_RECTS(pClip), pbox = REGION_RECTS(pClip); nbox--;
         pbox++)
    {
        int   x1 = x;
        int   y1 = y;
        ...
        ok  = uxa_screen->info->put_image(pPix,
                                         x1 + xoff,
                                         y1 + yoff,
                                         x2 - x1,
                                         y2 - y1,
...
static void
uxa_put_image(DrawablePtr pDrawable,
              GCPtr       pGC,
              int         depth,
              int         x,
              int         y,
...
static inline Bool
uxa_copy_n_to_n_two_dir(DrawablePtr pSrcDrawable,
                        ...
                        int         dy)
{
    ...
    if (!pSrcPixmap || !pDstPixmap) return FALSE;
    ...
    for (; nbox; pbox++, nbox--)
    {
        if (dx >= 0 && (src_off_y + pbox->y1 + dy) != pbox->y1)
        {
            ...
            if (dirsetup != -1)
            {
                if (dirsetup != 0) uxa_screen->info->done_copy(pDstPixmap);
                dirsetup = -1;
                ...
                void       *closure)
{
    ...
    if (!pSrcPixmap || !pDstPixmap) goto fallback;

    if (uxa_screen->info->check_copy &&
        !uxa_screen->info->check_copy(pSrcPixmap,
                                      pDstPixmap,
                                      pGC ? pGC->alu : GXcopy,
    ...
    if ((uxa_screen->info->flags & UXA_TWO_BITBLT_DIRECTIONS) &&
        reverse != upsidedown)
    {
        if (uxa_copy_n_to_n_two_dir(pSrcDrawable,
                                    pDstDrawable,
                                    pGC,
                                    pbox,
                                    nbox,
    ...
    if (!uxa_pixmap_is_offscreen(pDstPixmap)) goto fallback;
    ...
    else
    {
        ...
        if (!uxa_screen->info->put_image) goto fallback;

        /* Don't bother with under 8bpp, XYPixmaps. */
        bpp = pSrcPixmap->drawable.bitsPerPixel;
        if (bpp != pDstDrawable->bitsPerPixel || bpp < 8) goto fallback;

        /* Only accelerate copies: no rop or planemask. */
        if (pGC && (!UXA_PM_IS_SOLID(pSrcDrawable, pGC->planemask) ||
                    pGC->alu != GXcopy))
            goto fallback;
        ...
        while (nbox--)
        {
            if (!uxa_screen->info->put_image(
                    ...
                    pbox->x2 - pbox->x1,
                    pbox->y2 - pbox->y1,
                    (char *)src + (pbox->y1 + dy + src_off_y) * stride +
...
RegionPtr
uxa_copy_area(DrawablePtr pSrcDrawable,
              DrawablePtr pDstDrawable,
              GCPtr       pGC,
              int         srcx,
              int         srcy,
              ...
              int         dsty)
{
    ...
    {
        return uxa_check_copy_area(pSrcDrawable,
                                   pDstDrawable,
                                   pGC,
                                   srcx,
                                   srcy,
...
static void
uxa_poly_point(DrawablePtr pDrawable,
               GCPtr       pGC,
               int         mode,
               int         npt,
               DDXPointPtr ppt)
 ...
 */
static void
uxa_poly_lines(DrawablePtr pDrawable,
               GCPtr       pGC,
               int         mode,
               int         npt,
               DDXPointPtr ppt)
...
uxa_poly_segment(DrawablePtr pDrawable, GCPtr pGC, int nseg, xSegment *pSeg)
{
    ...
    for (i = 0; i < nseg; i++)
    {
        ...
        {
            if (prect[i].width == 1) prect[i].height--;
            else prect[i].width--;
        }
    }
...
static Bool uxa_fill_region_solid(DrawablePtr pDrawable,
                                  RegionPtr   pRegion,
                                  Pixel       pixel,
                                  CARD32      planemask,
                                  CARD32      alu);
                   ...
                   xRectangle *prect)
{
    ...
    if (!REGION_NUM_RECTS(pReg)) goto out;

    if (uxa_screen->swappedOut || uxa_screen->force_fallback) goto fallback;

    pPixmap = uxa_get_offscreen_pixmap(pDrawable, &xoff, &yoff);
    if (!pPixmap) goto fallback;
    ...
    if ((pGC->fillStyle == FillSolid || pGC->fillStyle == FillTiled) &&
        (nrect == 1 || pGC->alu == GXcopy || pGC->alu == GXclear ||
         pGC->alu == GXnoop || pGC->alu == GXcopyInverted || pGC->alu == GXset))
    {
        if (((pGC->fillStyle == FillSolid || pGC->tileIsPixel) &&
             uxa_fill_region_solid(
                 pDrawable,
                 pReg,
                 pGC->fillStyle == FillSolid ? pGC->fgPixel : pGC->tile.pixel,
                 pGC->planemask,
                 pGC->alu)) ||
            (pGC->fillStyle == FillTiled && !pGC->tileIsPixel &&
             uxa_fill_region_tiled(pDrawable,
                                   pReg,
                                   pGC->tile.pixmap,
                                   &pGC->patOrg,
                                   pGC->planemask,
...
fallback:
    ...
    while (nrect--)
    {
        ...
        if (fullY1 < extentY1) fullY1 = extentY1;

        if (fullX2 > extentX2) fullX2 = extentX2;

        if (fullY2 > extentY2) fullY2 = extentY2;

        if ((fullX1 >= fullX2) || (fullY1 >= fullY2)) continue;
        ...
        else
        {
            ...
            while (n--)
            {
                ...
                if (partX1 < fullX1) partX1 = fullX1;
                partY1 = pbox->y1;
                if (partY1 < fullY1) partY1 = fullY1;
                partX2 = pbox->x2;
                if (partX2 > fullX2) partX2 = fullX2;
                partY2 = pbox->y2;
                if (partY2 > fullY2) partY2 = fullY2;
...
try_solid:
    ...
    else
    {
        ...
        if (!uxa_screen->info->check_composite(PictOpSrc,
                                               src,
                                               NULL,
                                               dst,
                      ...
                      CARD32      alu)
{
    ...
    if (tileWidth == 1 && tileHeight == 1)
        return uxa_fill_region_solid(pDrawable,
                                     pRegion,
    ...
    pPixmap = uxa_get_offscreen_pixmap(pDrawable, &xoff, &yoff);
    if (!pPixmap || !uxa_pixmap_is_offscreen(pTile)) goto out;
    ...
    {
        if (xoff || yoff) REGION_TRANSLATE(pScreen, pRegion, xoff, yoff);
        ...
        for (i = 0; i < nbox; i++)
        {
            ...
            if (alu == GXcopy) height = min(height, tileHeight);
            ...
            while (height > 0)
            {
                ...
                if (alu == GXcopy) width = min(width, tileWidth);
                ...
                while (width > 0)
                {
                    ...
                    (*uxa_screen->info
                          ->copy)(pPixmap, tileX, tileY, dstX, dstY, w, h);
                    dstX += w;
                    tileX = 0;
                }
                dstY += h;
        ...
        else
        {
            ...
            if (more_copy == FALSE) ret = TRUE;

            if (more_copy &&
                (*uxa_screen->info
                      ->prepare_copy)(pPixmap, pPixmap, 1, 1, alu, planemask))
            {
                for (i = 0; i < nbox; i++)
                {
                    int dstX   = pBox[i].x1 + tileWidth;
 ...
 */
void
uxa_get_image(DrawablePtr   pDrawable,
              int           x,
              int           y,
              int           w,
              int           h,
              ...
              char         *d)
{
    ...
    if (pPix == NULL || uxa_screen->info->get_image == NULL) goto fallback;
	 ...
	 * think about <8bpp.
	 */
    if (pDrawable->bitsPerPixel < 8) goto fallback;

    ok = uxa_screen->info->get_image(pPix,
                                     pDrawable->x + x + xoff,
                                     pDrawable->y + y + yoff,
                                     w,
                                     h,
