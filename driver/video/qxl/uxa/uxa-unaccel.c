...
uxa_prepare_access_gc(GCPtr pGC)
{
    ...
    if (pGC->fillStyle == FillTiled)
        if (!uxa_prepare_access(&pGC->tile.pixmap->drawable,
                                ...
                                UXA_ACCESS_RO))
        {
            if (pGC->stipple) uxa_finish_access(&pGC->stipple->drawable);
            return FALSE;
        }
    return TRUE;
}
                          ...
                          pointer      pglyphBase)
{
    ...
    REGION_INIT(screen, &region, (BoxPtr)NULL, 0);
    uxa_damage_image_glyph_blt(&region,
                               pDrawable,
                               pGC,
                               x,
                               y,
                         ...
                         pointer      pglyphBase)
{
    ...
    REGION_INIT(screen, &region, (BoxPtr)NULL, 0);
    uxa_damage_poly_glyph_blt(&region,
                              pDrawable,
                              pGC,
                              x,
                              y,
                    ...
                    CARD16     height)
{
    ...
    if (uxa_prepare_access(pDst->pDrawable, &region, UXA_ACCESS_RW))
    {
            ...
            uxa_prepare_access(pSrc->pDrawable, NULL, UXA_ACCESS_RO))
        {
            ...
            if (pSrc->pDrawable != NULL) uxa_finish_access(pSrc->pDrawable);
        }
...
uxa_get_pixmap_first_pixel(PixmapPtr pPixmap)
{
    ...
    if (!uxa_prepare_access(&pPixmap->drawable, NULL, UXA_ACCESS_RO)) return 0;
