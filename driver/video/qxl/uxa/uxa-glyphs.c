...
static inline struct uxa_glyph *
uxa_glyph_get_private(GlyphPtr glyph)
{
#if HAS_DEVPRIVATEKEYREC
...
static inline void
uxa_glyph_set_private(GlyphPtr glyph, struct uxa_glyph *priv)
{
...
uxa_glyphs_init(ScreenPtr pScreen)
{
    ...
    if (!dixRegisterPrivateKey(&uxa_glyph_key, PRIVATE_GLYPH, 0)) return FALSE;
#else
    if (!dixRequestPrivate(&uxa_glyph_key, 0)) return FALSE;
#endif

    if (!uxa_realize_glyph_caches(pScreen)) return FALSE;
                             ...
                             int                y)
{
    ...
    uxa_copy_area(&scratch->drawable,
    ...
    if (scratch != pGlyphPixmap) screen->DestroyPixmap(scratch);

    FreeScratchGC(gc);
}

void
uxa_glyph_unrealize(ScreenPtr pScreen, GlyphPtr pGlyph)
{
...
uxa_glyphs_intersect(int nlist, GlyphListPtr list, GlyphPtr *glyphs)
{
    ...
    while (nlist--)
    {
        ...
        while (n--)
        {
            ...
            x1 = x - glyph->info.x;
            if (x1 < MINSHORT) x1 = MINSHORT;
            y1 = y - glyph->info.y;
            if (y1 < MINSHORT) y1 = MINSHORT;
            x2 = x1 + glyph->info.width;
            if (x2 > MAXSHORT) x2 = MAXSHORT;
            y2 = y1 + glyph->info.height;
            if (y2 > MAXSHORT) y2 = MAXSHORT;
            ...
            {
                if (x1 < extents.x2 && x2 > extents.x1 && y1 < extents.y2 &&
                    y2 > extents.y1)
                {
                    return TRUE;
                }
                 ...
                 GlyphPtr     *glyphs)
{
    ...
    if (maskFormat)
    {
        ...
        if (extents.x2 <= extents.x1 || extents.y2 <= extents.y1) return;
        ...
        scratch = GetScratchPixmapHeader(dst->pDrawable->pScreen,
                                         width,
                                         height,
...
uxa_glyph_cache(ScreenPtr screen, GlyphPtr glyph, int *out_x, int *out_y)
{
    ...
    uxa_glyph_cache_t *cache =
        &uxa_screen->glyphCaches[PICT_FORMAT_RGB(glyph_picture->format) != 0];
                  ...
                  BoxPtr       extents)
{
    ...
    else
    {
        ...
        if (pSrc->pDrawable)
        {
            ...
            localSrc  = uxa_acquire_drawable(screen,
                                            pSrc,
                                            src_x + src_off_x,
                                            src_y + src_off_y,
                                            width,
            ...
            src_pixmap = uxa_get_offscreen_pixmap(localSrc->pDrawable,
                                                  &src_off_x,
                                                  &src_off_y);
            if (src_pixmap == NULL)
            {
        ...
        {
            localSrc = uxa_acquire_pattern(screen,
                                           pSrc,
                                           PIXMAN_a8r8g8b8,
                                           0,
                                           0,
    ...
    while (nlist--)
    {
        ...
        while (n--)
        {
            ...
            if (this_atlas != glyph_atlas)
            {
                ...
                if (glyph_atlas) uxa_screen->info->done_composite(dst_pixmap);

                mask_pixmap = uxa_get_drawable_pixmap(this_atlas->pDrawable);
            ...
            else
            {
                ...
                do
                {
                    ...
                    if (x1 < rects->x1) dx = rects->x1 - x1, x1 = rects->x1;
                    if (x2 > rects->x2) x2 = rects->x2;
                    if (y1 < rects->y1) dy = rects->y1 - y1, y1 = rects->y1;
...
static void
uxa_clear_pixmap(ScreenPtr screen, uxa_screen_t *uxa_screen, PixmapPtr pixmap)
{
    ...
    while (nlist--)
    {
        ...
        while (n--)
        {
            ...
            if (this_atlas != glyph_atlas)
            {
                ...
                if (glyph_atlas) uxa_screen->info->done_composite(pixmap);

                src_pixmap = uxa_get_drawable_pixmap(this_atlas->pDrawable);
           ...
           GlyphPtr     *glyphs)
{
    ...
    if (!uxa_screen->info->prepare_composite || uxa_screen->swappedOut ||
        ...
        !uxa_drawable_is_offscreen(pDst->pDrawable) || pDst->alphaMap ||
        pSrc->alphaMap)
    {
fallback:
        uxa_check_glyphs(op,
    ...
    if (!maskFormat)
    {
        ...
        if (NeedsComponent(list[0].format->format))
        {
            ...
            if (!sameFormat || uxa_glyphs_intersect(nlist, list, glyphs))
                maskFormat = NULL;
        }
    }

    if (!maskFormat && uxa_screen->info->check_composite_target &&
            ...
            uxa_get_drawable_pixmap(pDst->pDrawable)))
    {
        ...
        if (extents.x1 < 0) extents.x1 = 0;
        if (extents.y1 < 0) extents.y1 = 0;
