...
static void
uxa_composite_fallback_pict_desc(PicturePtr pict, char *string, int n)
{
    char format[20];
    char size[20];
    char loc;
...
static void
uxa_print_composite_fallback(const char *func,
                             CARD8       op,
                             PicturePtr  pSrc,
                             PicturePtr  pMask,
                             PicturePtr  pDst)
{
    ...
    if (!uxa_screen->fallback_debug) return;

    /* Limit the noise if fallbacks are expected. */
    if (uxa_screen->force_fallback) return;
                          ...
                          CARD16     height)
{
    ...
    if (uxa_screen->info->check_solid &&
        !uxa_screen->info->check_solid(pDst->pDrawable, GXcopy, FB_ALLONES))
        return -1;
    ...
    if (pSrcPix)
    {
        if (!uxa_get_color_for_pixmap(pSrcPix,
                                      pSrc->format,
                                      pDst->format,
                                      &pixel))
        {
    ...
    if (!(*uxa_screen->info->prepare_solid)(pDstPix, GXcopy, FB_ALLONES, pixel))
    {
        REGION_UNINIT(pDst->pDrawable->pScreen, &region);
        return -1;
    }
                              ...
                              int                  height)
{
    ...
    if (format == PIXMAN_a1) format = PIXMAN_a8;

    /* fill alpha if unset */
    if (PIXMAN_FORMAT_A(format) == 0) format = PIXMAN_a8r8g8b8;
    ...
    pPicture = CreatePicture(
        0,
        &pPixmap->drawable,
        PictureMatchFormat(pScreen, PIXMAN_FORMAT_DEPTH(format), format),
        0,
        0,
        serverClient,
        &error);
    (*pScreen->DestroyPixmap)(pPixmap);
    if (!pPicture) return 0;
                              ...
                              pixman_format_code_t format)
{
    ...
    pixmap = GetScratchPixmapHeader(screen,
                                    width,
                                    height,
                                    PIXMAN_FORMAT_DEPTH(format),
...
uxa_create_solid(ScreenPtr screen, uint32_t color)
{
    ...
    pixmap =
        (*screen->CreatePixmap)(screen, 1, 1, 32, UXA_CREATE_PIXMAP_FOR_MAP);
    if (!pixmap) return 0;
...
uxa_solid_clear(ScreenPtr screen)
{
    ...
    if (!uxa_screen->solid_clear)
    {
        ...
        if (!uxa_screen->solid_clear) return 0;
    }
...
uxa_acquire_solid(ScreenPtr screen, SourcePict *source)
{
    ...
    else if (solid->color == 0xff000000)
    {
        if (!uxa_screen->solid_black)
        {
            ...
            if (!uxa_screen->solid_black) return 0;
        }
    ...
    else if (solid->color == 0xffffffff)
    {
        if (!uxa_screen->solid_white)
        {
            ...
            if (!uxa_screen->solid_white) return 0;
        }
    ...
    else uxa_screen->solid_cache_size++;
...
transform_is_integer_translation(PictTransformPtr t, int *tx, int *ty)
{
    ...
    if (xFixedFrac(t->matrix[0][2]) != 0 || xFixedFrac(t->matrix[1][2]) != 0)
        return FALSE;
                     ...
                     INT16     *out_y)
{
    ...
    pGC->ops->CopyArea(pSrc->pDrawable,
                       &pPixmap->drawable,
                       pGC,
                       x + tx,
                       y + ty,
                    ...
                    INT16               *out_y)
{
    ...
    if (src->pDrawable)
    {
        ...
        dst = uxa_acquire_drawable(screen,
                                   src,
                                   x,
                                   y,
                                   width,
    ...
    *out_x = 0;
    *out_y = 0;
    return uxa_acquire_pattern(screen, src, format, x, y, width, height);
}
...
uxa_acquire_source(ScreenPtr  screen,
                   ...
                   INT16     *out_y)
{
    return uxa_acquire_picture(screen,
                               pict,
                               PIXMAN_a8r8g8b8,
                               x,
                               y,
                ...
                xRectangle   *rects)
{
    ...
    if (!pixman_region_not_empty(dst->pCompositeClip)) return;

    if (dst->alphaMap) goto fallback;

    dst_pixmap = uxa_get_offscreen_pixmap(dst->pDrawable, &dst_x, &dst_y);
    if (!dst_pixmap) goto fallback;
...
try_solid:
        if (uxa_screen->info->check_solid &&
            !uxa_screen->info->check_solid(&dst_pixmap->drawable,
                                           GXcopy,
                                           FB_ALLONES))
            goto err_region;
    ...
    else
    {
        ...
        if (!src) goto err_region;

        if (!uxa_screen->info->check_composite(op,
                                               src,
                                               NULL,
                                               dst,
                         ...
                         CARD16     height)
{
    ...
    if (uxa_screen->info->check_composite &&
        !(*uxa_screen->info
               ->check_composite)(op, pSrc, pMask, pDst, width, height))
        return -1;

    if (uxa_screen->info->check_composite_target &&
        !uxa_screen->info->check_composite_target(
            uxa_get_drawable_pixmap(pDst->pDrawable)))
    {
        ...
        gc->ops->CopyArea(pDst->pDrawable,
                          &pixmap->drawable,
                          gc,
                          xDst,
                          yDst,
    ...
    if (!pDstPix)
    {
        if (localDst != pDst) FreePicture(localDst, 0);
        return -1;
    }
    ...
    localSrc = uxa_acquire_source(screen,
                                  pSrc,
                                  xSrc,
                                  ySrc,
                                  width,
    ...
    if (pMask)
    {
        localMask = uxa_acquire_mask(screen,
                                     pMask,
                                     xMask,
                                     yMask,
                                     width,
        ...
        if (!localMask)
        {
            if (localSrc != pSrc) FreePicture(localSrc, 0);
            if (localDst != pDst) FreePicture(localDst, 0);
    ...
    {
        if (localSrc != pSrc) FreePicture(localSrc, 0);
        if (localMask && localMask != pMask) FreePicture(localMask, 0);
        if (localDst != pDst) FreePicture(localDst, 0);
    ...
    if (localDst != pDst)
    {
        ...
        if (gc)
        {
            ...
            gc->ops->CopyArea(localDst->pDrawable,
                              pDst->pDrawable,
                              gc,
                              0,
                              0,
                                        ...
                                        CARD16     height)
{
    ...
    if (uxa_screen->info->check_composite &&
        (!(*uxa_screen->info->check_composite)(PictOpOutReverse,
                                               pSrc,
                                               pMask,
                                               pDst,
                                               width,
                                               height) ||
         !(*uxa_screen->info->check_composite)(PictOpAdd,
                                               pSrc,
                                               pMask,
                                               pDst,
                                               width,
	 ...
	 * factors.
	 */
    uxa_composite(PictOpOutReverse,
                  pSrc,
                  pMask,
                  localDst,
                  xSrc,
...
#  endif
    ...
    {
        if (PICT_FORMAT_A(src->format)) return 0;
...
drawable_contains(DrawablePtr drawable, int x, int y, int w, int h)
{
    ...
    if (x + w > drawable->width) return FALSE;

    if (y + h > drawable->height) return FALSE;
              ...
              CARD16     height)
{
    ...
    if (uxa_screen->swappedOut || uxa_screen->force_fallback) goto fallback;

    if (!uxa_drawable_is_offscreen(pDst->pDrawable)) goto fallback;
    ...
    if (!pMask)
    {
        if (op == PictOpClear)
        {
            ...
            if (clear && uxa_try_driver_solid_fill(clear,
                                                   pDst,
                                                   xSrc,
                                                   ySrc,
                                                   xDst,
        ...
        else if (pSrc->pDrawable->width == 1 && pSrc->pDrawable->height == 1 &&
                 pSrc->repeat &&
                 (op == PictOpSrc ||
                  (op == PictOpOver && !PICT_FORMAT_A(pSrc->format))))
        {
            ret = uxa_try_driver_solid_fill(pSrc,
                                            pDst,
                                            xSrc,
            ...
            if (ret == 1) goto done;
                 ...
                 transform_is_integer_translation(pSrc->transform, &tx, &ty))
        {
            if (!pSrc->repeat && drawable_contains(pSrc->pDrawable,
                                                   xSrc + tx,
                                                   ySrc + ty,
                                                   width,
                                                   height))
                     ...
                     pSrc->pDrawable->type == DRAWABLE_PIXMAP)
            {
                ...
                {
                    ret = uxa_try_driver_composite(op,
                                                   pSrc,
                                                   pMask,
                                                   pDst,
                                                   xSrc,
    ...
    if (pMask && pMask->pDrawable && pMask->repeat &&
        ...
        drawable_contains(pMask->pDrawable,
                          xMask + tx,
                          yMask + ty,
                          width,
                          height))
    ...
    if (uxa_screen->info->prepare_composite)
    {
        ...
        {
            ret = uxa_try_magic_two_pass_composite_helper(op,
                                                          pSrc,
                                                          pMask,
                                                          pDst,
                                                          xSrc,
...
fallback:
    uxa_print_composite_fallback("uxa_composite", op, pSrc, pMask, pDst);

    uxa_check_composite(op,
                        pSrc,
                        pMask,
                        pDst,
                        xSrc,
    ...
    if (pMask) pMask->repeat = saveMaskRepeat;
}
#endif
                         ...
                         CARD16        height)
{
    ...
    if (!pPictFormat)
    {
        ...
        else pPictFormat = PictureMatchFormat(pScreen, 8, PICT_a8);
        if (!pPictFormat) return 0;
    }
               ...
               xTrapezoid   *traps)
{
    ...
    if (maskFormat || direct)
    {
        ...
        if (bounds.y1 >= bounds.y2 || bounds.x1 >= bounds.x2) return;
    }
    ...
    else if (maskFormat)
    {
        ...
        if (!mask)
        {
            if (scratch) FreeScratchPixmapHeader(scratch);
    ...
    else
    {
        ...
        else maskFormat = PictureMatchFormat(screen, 8, PICT_a8);
        for (; ntrap; ntrap--, traps++)
            uxa_trapezoids(op, src, dst, maskFormat, xSrc, ySrc, 1, traps);
    }
}
              ...
              xTriangle    *tris)
{
    ...
    else if (maskFormat)
    {
        ...
        pPicture =
            uxa_create_alpha_picture(pScreen, pDst, maskFormat, width, height);
        if (!pPicture) return;
    ...
    else
    {
        ...
        else maskFormat = PictureMatchFormat(pScreen, 8, PICT_a8);
