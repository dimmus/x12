...
default_abort_func(void *user_data)
{
    for (;;)
        ;
}
...
void
default_print_func(void *user_data, const char *format, ...)
{}
...
void
mspace_set_abort_func(mspace_abort_t f)
{
    abort_func = f;
}

void
mspace_set_print_func(mspace_print_t f)
{
    print_func = f;
}
...
#  define compute_tree_index(S, I)                                    \
      {                                                               \
          ...
          if (X == 0) I = 0;                                          \
          else if (X > 0xFFFF) I = NTREEBINS - 1;                     \
          else                                                        \
          {                                                           \
              unsigned int K;                                         \
              __asm__("bsrl %1,%0\n\t" : "=r"(K) : "rm"(X));          \
...
{
    assert(m->user_data,
           (is_aligned(chunk2mem(p))) || (p->head == FENCEPOST_HEAD));
...
do_check_top_chunk(mstate m, mchunkptr p)
{
    ...
    assert(m->user_data,
           sz == ((sp->base + sp->size) - (char *)p) - TOP_FOOT_SIZE);
...
do_check_tree(mstate m, tchunkptr t)
{
    ...
    { /* traverse through chain of same-sized nodes */
        ...
        {
            assert(m->user_data,
                   head == 0); /* only one node on chain has parent */
            head = u;
            assert(m->user_data, u->parent != u);
            assert(m->user_data,
                   u->parent->child[0] == u || u->parent->child[1] == u ||
                       *((tbinptr *)(u->parent)) == u);
            if (u->child[0] != 0)
            {
            ...
            {
                assert(m->user_data,
                       chunksize(u->child[0]) < chunksize(u->child[1]));
            }
        }
        u = u->fd;
    }
...
do_check_treebin(mstate m, bindex_t i)
{
    ...
    if (t == 0) assert(m->user_data, empty);
...
do_check_smallbin(mstate m, bindex_t i)
{
    ...
    if (p == b) assert(m->user_data, empty);
    if (!empty)
    {
        for (; p != b; p = p->bk)
        {
            ...
            assert(m->user_data,
                   p->bk == b || chunksize(p->bk) == chunksize(p));
...
#define insert_small_chunk(M, P, S)                         \
    {                                                       \
        ...
        if (!smallmap_is_marked(M, I)) mark_smallmap(M, I); \
        else if (RTCHECK(ok_address(M, B->fd))) F = B->fd;  \
        else                                                \
        {                                                   \
    ...
    {                                                          \
        ...
        {                                                      \
            ...
            {                                                  \
                ...
                if (RTCHECK(ok_address(M, RP))) *RP = 0;       \
                else                                           \
                {                                              \
                    CORRUPTION_ERROR_ACTION(M);                \
                }                                              \
...
static mstate
init_user_mstate(char *tbase, size_t tsize, void *user_data)
{
...
mspace_malloc(mspace msp, size_t bytes)
{
    ...
    if (!PREACTION(ms))
    {
        ...
        if (bytes <= MAX_SMALL_REQUEST)
        {
            ...
            nb =
                MAX_SIZE_T; /* Too big to allocate. Force failure (in sys alloc) */
        else
        {
        ...
        if (!PREACTION(fm))
        {
            ...
            if (RTCHECK(ok_address(fm, p) && ok_cinuse(p)))
            {
                ...
                if (RTCHECK(ok_next(p, next) && ok_pinuse(next)))
                {
                    ...
                    else set_free_with_pinuse(p, psize, next);
...
mspace_calloc(mspace msp, size_t n_elements, size_t elem_size)
{
    ...
    if (mem != 0 && calloc_must_clear(mem2chunk(mem))) MEMCLEAR(mem, req);
    return mem;
}
...
{
    if (oldmem == 0) return mspace_malloc(msp, bytes);
...
void
mspace_malloc_stats_return(mspace  msp,
                           size_t *ret_maxfp,
                           size_t *ret_fp,
                           size_t *ret_used)
{
